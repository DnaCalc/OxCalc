...
| W4b candidate overlays | N addressable, layerable, non-publishing candidate contexts with publish/discard semantics | `CORE_ENGINE_CANDIDATE_OVERLAY_HANDLE_SPIKE.md`, CTRO/overlay lineage from W047/W050, first slice `calc-etez`, continuation bead `calc-4ipg` | First substrate, commit, DnaTreeCalc projection, parented copy-layer, candidate-private revision-history, candidate basis-retention pin, candidate structural projection/read, structural candidate rename/move/delete intent, constant/empty candidate add-node, and formula-literal candidate add-node dry-bind slices implemented: OxCalc can open an opaque candidate handle on a retained revision, apply a private node edit, evaluate private candidate values, discard without publishing, commit when the live basis revision still matches, open a child over a parent candidate's private state, expose candidate-private revision graph entries with real transaction ids, keep host-visible candidate basis revisions retained under bounded revision retention while a candidate is open, expose candidate-private node views after private structural edits without rewriting the live workspace view, and dry-bind prospective candidate-added formula nodes against candidate-private structure. DnaTreeCalc projects candidate values, parent handles, private revision history, candidate-private node structure, and closed rename/move/delete/add-by-NodeKey candidate intents separately from published node state. Optimized layering/rebase, candidate add-node inherited-table/template initial content, candidate run/invalidation projection for candidate-only nodes, candidate invalidation-summary richness, and broader candidate overlay GC remain open. |
...
2. Continue W4b candidate-overlay work under bead `calc-4ipg` after the first
   ...
   - optimized parent candidate/layer relationship beyond copy-at-open,
   - candidate run/invalidation projection for candidate-only nodes,
   - candidate add-node inherited-table/template initial content on top of candidate-context dry-bind,
   - richer candidate invalidation-summary projection where candidate edits run with planning/recalc,
   - broader candidate overlay retention/GC pressure beyond basis-revision pins.
