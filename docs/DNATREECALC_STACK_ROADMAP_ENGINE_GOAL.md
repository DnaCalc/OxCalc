...
| W4b candidate overlays | N addressable, layerable, non-publishing candidate contexts with publish/discard semantics | `CORE_ENGINE_CANDIDATE_OVERLAY_HANDLE_SPIKE.md`, CTRO/overlay lineage from W047/W050, first slice `calc-etez`, continuation bead `calc-4ipg` | First substrate, commit, DnaTreeCalc projection, parented layering, conservative same-node and structural-lane conflict policy, lane-aware rebase merging, same-node rename/move structural facet merging, same-parent rename/add namespace merging, same-parent rename/reorder structural facet merging, sibling add/delete structural merging, sibling add/reorder and delete/reorder structural merging, candidate-private revision-history, candidate basis-retention pin, candidate structural projection/read, structural candidate rename/move/delete intent, constant/empty candidate add-node, formula-literal candidate add-node dry-bind, candidate-only run projection, inherited table-column formula candidate add-node, host-owned template-bound candidate add-node initial content, candidate private-edit invalidation-summary, first speculation-budget/GC, and host-retention pin slices implemented: OxCalc can open an opaque candidate handle on a retained revision, apply a private node edit, evaluate private candidate values, discard without publishing, commit when the live basis revision still matches, open a child over a parent candidate's private state, refresh parented candidates from parent-private edits made after the child was opened, reject stale rebase with a typed `CandidateRebaseConflict` report when live and candidate private edits overlap on the same stable node/content facet, move destination or old-parent lane, delete-subtree descendant lane, same-lane structural conflict, competing same-node rename, competing reorder/order edits, or replay-validation namespace collision, allow non-overlapping stale candidates plus candidate structural add/rename/move/reorder/delete edits over compatible live content edits to rebase, including one stale candidate that combines rename, move, and add private structural edits over compatible live content edits, the same-node rename-over-live-move / move-over-live-rename facets, same-parent candidate rename over live sibling add, same-parent rename-over-reorder / reorder-over-rename facets, sibling add-over-delete / delete-over-add facets, and sibling add-over-reorder / reorder-over-add plus delete-over-reorder / reorder-over-delete facets when replay validation succeeds, expose candidate-private revision graph entries with real transaction ids, keep host-visible candidate basis revisions retained under bounded revision retention while a candidate is open, expose ordered structural parent/child ids on node views, expose candidate-private node views after private structural edits without rewriting the live workspace view, dry-bind prospective candidate-added formula nodes against candidate-private structure, attach planning-derived transaction summaries to supported candidate apply-only private edits, compute typed candidate pressure, reap unprotected candidates to a requested budget, and protect explicitly host-pinned candidates from budget reaping. DnaTreeCalc projects candidate values, parent handles, private revision history, candidate-private node structure with engine-owned child order, candidate calculation run facts for candidate-only nodes, retention pin counts, reason-specific pressure counts, and closed rename/move/reorder/delete/add-by-NodeKey/reap/pin/unpin candidate intents separately from published node state; it also reads inherited table-column formula text and minimal built-in template-bound content, then dry-binds prospective candidate adds through OxCalc before mutation. Structural name-collision merge algebra beyond same-node rename/move, same-parent rename/add, same-parent rename/reorder, sibling add/delete, sibling add/reorder, and sibling delete/reorder facets, and scenario/what-if UX remain open. |
...
2. Continue W4b candidate-overlay work under bead `calc-4ipg` after the first
   ...
   - broader structural name-collision merge algebra beyond the first lane-aware,
     same-node rename/move, same-parent rename/add, same-parent rename/reorder, sibling add/delete,
     sibling add/reorder, and sibling delete/reorder facet merge slices,
   - full template definition/edit/instantiate/sync substrate after the host-owned minimal
     template-bound initial-content policy,
