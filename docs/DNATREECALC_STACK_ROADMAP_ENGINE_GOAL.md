...
| W4a revision graph | Retained parent-linked revision DAG and cursor; undo/redo by revision navigation, never inverse replay | `OxCalcTreeContext` retained revision graph plus in-memory `navigate_workspace_revision`; bounded retention policy remains W054-adjacent | Engine substrate in progress: in-memory parent-linked revisions, transaction predecessor/successor ids, and retained-state navigation are implemented. Product undo/history projection, bounded eviction/persistence, and transaction invalidation summaries remain open. |
