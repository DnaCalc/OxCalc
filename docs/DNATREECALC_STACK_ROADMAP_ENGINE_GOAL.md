...
| W4a revision graph | Retained parent-linked revision DAG and cursor; undo/redo by revision navigation, never inverse replay | `OxCalcTreeContext` retained revision graph plus in-memory `navigate_workspace_revision`; bounded retention policy remains W054-adjacent | Engine substrate in progress: in-memory parent-linked revisions, transaction predecessor/successor ids, retained-state navigation, and transaction invalidation summaries are implemented. Product undo/history projection is downstream; bounded eviction/persistence remains open. |
