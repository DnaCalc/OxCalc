...
- Current state: active; formalization direction, replay/pack binding, and measurement/assurance planning exist. `W033` established the OxCalc + OxFml first-pass formalization/spec-evolution packet and post-W033 successor evidence slices. `W034` now has a closure audit and successor packet that records the tranche as bounded evidence, not full formalization. `W035` is the ordered successor for proof hardening, TraceCalc oracle matrix expansion, implementation conformance hardening, Lean assumption discharge, TLA non-routine exploration, continuous assurance, and pack/Stage 2 reassessment. Its residual ledger carries the current OxFml W073 typed-only conditional-formatting input contract as watch/input-contract evidence: aggregate and visualization payloads must use `typed_rule`, while W072 bounded `thresholds` strings are not interpreted for those families. Its TraceCalc oracle-matrix evidence records run `w035-tracecalc-oracle-matrix-001`: 30 scenarios, 17 matrix rows, 15 covered rows, 2 classified uncovered rows, and 0 failed/missing rows. Its implementation-conformance evidence records run `w035-implementation-conformance-hardening-001`: 6 W034 gap dispositions, 5 implementation-work deferrals, 1 spec-evolution deferral, and 0 failed rows. Its Lean evidence now includes checked W035 assumption-discharge and seam proof-map artifacts that distinguish local proof rows, zero explicit axiom rows, OxFml external seam assumptions including W073, OxFunc opaque kernel boundaries, and TLA-owned deferrals. Its TLA evidence now includes W035 scheduler-gate, partition-gap, and multi-reader overlay configs with recorded state counts and no Stage 2 promotion. Its continuous-assurance evidence now includes run `w035-continuous-assurance-gate-001`: 5 source rows, 3 scheduled lanes, 4 differential rows, 0 missing artifacts, 0 unexpected mismatches, and 9 no-promotion reasons. Its pack/Stage 2 reassessment evidence now includes run `w035-pack-stage2-readiness-001`: 10 evidence rows, 10 satisfied inputs, 19 no-promotion blockers, 0 missing artifacts, and `cap.C4.distill_valid` as the highest honest capability.
- Canonical owner: [W006_CORE_FORMALIZATION_AND_GATE_BINDING.md](C:\Work\DnaCalc\OxCalc\docs\worksets\W006_CORE_FORMALIZATION_AND_GATE_BINDING.md) through [W010_EXPERIMENT_REGISTER_AND_MEASUREMENT_SCHEMA_PLANNING.md](C:\Work\DnaCalc\OxCalc\docs\worksets\W010_EXPERIMENT_REGISTER_AND_MEASUREMENT_SCHEMA_PLANNING.md), plus [W031_TREECALC_ASSURANCE_REFRESH_AND_RESIDUAL_PACKETIZATION.md](C:\Work\DnaCalc\OxCalc\docs\worksets\W031_TREECALC_ASSURANCE_REFRESH_AND_RESIDUAL_PACKETIZATION.md), [W033_OXCALC_OXFML_CORE_FORMALIZATION_PASS.md](C:\Work\DnaCalc\OxCalc\docs\worksets\W033_OXCALC_OXFML_CORE_FORMALIZATION_PASS.md), [W034_CORE_FORMALIZATION_DEEPENING_AND_IMPLEMENTATION_VERIFICATION.md](C:\Work\DnaCalc\OxCalc\docs\worksets\W034_CORE_FORMALIZATION_DEEPENING_AND_IMPLEMENTATION_VERIFICATION.md), [W035_CORE_FORMALIZATION_PROOF_AND_ASSURANCE_HARDENING.md](C:\Work\DnaCalc\OxCalc\docs\worksets\W035_CORE_FORMALIZATION_PROOF_AND_ASSURANCE_HARDENING.md), [CORE_ENGINE_OXCALC_OXFML_FORMALIZATION_PASS_PLAN.md](C:\Work\DnaCalc\OxCalc\docs\spec\core-engine\CORE_ENGINE_OXCALC_OXFML_FORMALIZATION_PASS_PLAN.md), and [W034_CLOSURE_AUDIT_AND_SUCCESSOR_PACKET.md](C:\Work\DnaCalc\OxCalc\docs\spec\core-engine\w034-formalization\W034_CLOSURE_AUDIT_AND_SUCCESSOR_PACKET.md).
