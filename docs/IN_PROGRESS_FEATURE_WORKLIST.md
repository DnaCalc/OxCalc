...
- **Remaining gaps**: later downstream rounds may still be needed if stronger coordinator pressure appears around execution-restriction transport, publication/topology consequence breadth, or caller-anchor/address-mode carriage for the first TreeCalc relative-reference subset. Availability/provider-failure and callable-publication remain watch lanes only.
...
- **Status**: in-progress
- **Current floor**: the target and execution line are now defined in `CORE_ENGINE_TREECALC_SEMANTIC_COMPLETION_PLAN.md`, packetized into `W025` through `W031`, and the first local implementation floor now exists in Rust: widened structural snapshots with formula/bind attachment points, relative-reference context, immutable structural edits, a TreeCalc-local formula and reference model, local formula-derived dependency descriptors, a local dependency graph substrate, local invalidation closure with cycle-region representation, a local sequential runtime facade that drives dependency ordering, evaluation, verified-clean handling, local candidate adaptation, candidate/publication over the local formula IR, first local runtime-effect emission for host-sensitive and dynamic-reference families, a first local runtime-effect overlay carrier, a checked-in thirteen-case TreeCalc fixture corpus under `docs/test-fixtures/core-engine/treecalc`, and a first emitted local run root at `docs/test-runs/core-engine/treecalc-local/w025-treecalc-local-baseline` with local oracle, conformance, trace, explain, and post-edit rebind, recalc-only, downstream dependency-chain, post-edit runtime-effect/overlay, mixed publication-then-post-edit overlay, move-triggered, and removal sidecars against fixture-declared expectations.
- **Remaining gaps**: consumed OxFml bind/reference intake, formula-driven dependency graph build from real bind products, evaluator-backed candidate intake, runtime-derived effect closure beyond the first local rejection-sidecar and overlay floor, first TreeCalc corpus and baseline beyond the local pre-oracle local-fixture shape, replay/diff/explain widening beyond the current local sidecars, broader structural-edit and successor-snapshot families beyond the first rename-triggered rebind, recalc-only constant-edit, downstream dependency-chain, post-edit runtime-effect/overlay, mixed publication-then-post-edit overlay, move-triggered rebind, and removal cases, and assurance refresh.
- **Why still open**: the initial local substrate now exists, but the real TreeCalc formula/bind/evaluate pipeline is not realized yet.
