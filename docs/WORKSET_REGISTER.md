...
7. rollout_mode:
   `reopened_blocked_on_second_excel_host_or_user_scope` (child beads `calc-zci1.1` through `calc-zci1.8` are predecessor evidence; prior closure audit at `docs/test-runs/core-engine/w048-closure-audit-001/w048_closure_audit_summary.json` is superseded by `docs/spec/core-engine/w048-cycles/W048_REOPEN_SCOPE_AUDIT_AND_REPAIR_PLAN.md`; reopened child beads through `calc-zci1.20` have run/review evidence; `calc-zci1.16` is cleared by `w048-excel-root-report-002`; `calc-zci1.19` remains blocked on second-version evidence or user scope acceptance).
