...
1. purpose:
   widen W051's first `TreeCalcReferenceCollection::ChildrenV1` carrier pattern into the full TreeCalc reference and structured table-lowering scope. W056 owns the admitted TreeReference variants beyond children, dependency edges, invalidation facts, dynamic rebind, host namespace versioning, caller context identity, table row/column/header/totals dependencies, and correlation to OxFml host-reference handles while preserving OxFml integration through generic host context only. Its active first-host bridge slice includes OxCalc-owned raw formula-text prebind for free-standing `@CHILDREN` / `.*` plus caller-supplied resolved-base `base.@CHILDREN` / `base.*`, producing neutral OxFml source plus the source-preserving `ChildrenV1` carrier without DnaTreeCalc parsing. Its ordered selector slice now has resolved-packet carriers plus public raw query/resolved-collection prebind for the authored `@PRECEDING`, `@FOLLOWING`, `@ANCESTORS`, and recursive `**` spellings, preserving host handles, source spans/tokens, optional base/tail spans, member order, membership/order versions, family-specific host bind shape hints, and sparse-reader behavior. DnaTreeCalc commit `66355f8` activates the first ordered-selector receiving-side corpus slice; remaining path resolution breadth, traversal-bound policy, and full W004/W005 non-table corpus activation stay successor work. Its table lane consumes the generic OxFml table-context packet and `StructuredReferenceBindRecord` projection, including stable row membership/order identities, exact header/totals region refs, selected sections/regions, selected column ids, omitted table names with bound effective table identity validation, and `#This Row` caller context, as context dependency descriptors without parsing structured-reference formula text. The table promotion lane is closed for the declared node-associated TreeCalc table scope through `calc-4vs8.34` through `calc-4vs8.38`: full-product closure map, lifecycle callback/version contract, structured-table `ReferenceLike` function breadth, matched TreeCalc/Excel replay/value-wire intake, and final table-topic promotion audit. Full intended table support has prior closure evidence through the third-pass `calc-4vs8.39` through `calc-4vs8.43` and fourth-pass `calc-4vs8.44` through `calc-4vs8.56` table spines, covering empty data-body packet/reader support, executable structured-table function-family evidence, DnaTreeCalc lifecycle bridge acceptance, namespace/anchor/workspace semantics, whole-system ownership, virtual Excel-anchor identity, generic OxFml packet contract, resolver/namespace versioning, full `ReferenceLike` readers, row-context prepared identity, dependency/invalidation matrix, OxXlPlay oracle construction, OxReplay retained evidence, UDF/VBA/XLL impact, cross-repo coordination, and dynamic table reference rebind/`INDIRECT` semantics. A fifth-pass hardening spine `calc-4vs8.57` through `calc-4vs8.63` is now open to revalidate the architecture against current code/evidence, consolidate abstractions, execute the full lifecycle matrix, close function/UDF and replay/value-wire gaps, reconcile cross-repo rollout beads, and perform a final completion audit before the table claim is carried into parent W056 closure.
2. depends_on:
