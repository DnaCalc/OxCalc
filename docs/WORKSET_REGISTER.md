...
**Two R7-prerequisite gaps discovered during R6 (both filed as beads, both OPEN, both P2):**
- **`calc-5kqg.65`** — cross-sheet reference EVALUATION is unwired for freshly-loaded sheets: a loaded `Sheet2!B1 = Sheet1!A1+10` publishes `#VALUE!` instead of the resolved value, because ingest binds against the display name `Sheet1` while the engine's cross-sheet resolution keys by node-token and the display-name-to-token alias / `WorkbookReferenceCatalog` routing that authored workspaces establish (R3.3/R3.6) is not established on the load path. Cross-sheet EDIT propagation (R4.6) already works; only load-time EVAL resolution is missing. Discovered empirically during R6.5 (`calc-5kqg.62`). Not a blocker for R6.6 (the W011 proof is single-sheet), but a real functional hole for any real multi-sheet `.xlsx` load.
- **`calc-5kqg.66`** — Tier-A authored **collection** projection (merged regions, table overlays, defined names, repeated/shared-formula regions) is not yet re-emitted as `DocumentEvent`s by `project_workbook_model_output`; R6.6 made this deferral a loud typed `OxCalcDocumentError::UnprojectableTierACollections` (verified at `consumer.rs:7502-7508`) rather than a silent drop, so projecting a workbook carrying any of these collections fails cleanly instead of losing them — but the projection itself still needs implementing (needs an A1-range renderer plus an engine-sheet-token-to-upstream-sheet-id inverse using `IngestedDocumentFacts.sheet_stream_ids`).

Together with gap (1) above (iteration settings), `.65` and `.66` are the real-multi-feature-xlsx prerequisites for R7 (DnaTreeCalc W011 resuming against actual multi-sheet, collection-bearing workbooks rather than the two-cell fixture). R7 is otherwise unblocked by this close.
