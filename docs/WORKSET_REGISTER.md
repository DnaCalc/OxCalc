...
6. initial_epic_lanes:
   parent epic `calc-4vs8`; `calc-4vs8.1` full TreeReference inventory and host-reference correlation, `calc-4vs8.3` dependency/reverse-edge widening plus invalidation/dynamic rebind/namespace/caller-context work, `calc-4vs8.12` ordered selector collection carriers, `calc-4vs8.13` raw ordered-selector query/resolved-collection prebind, `calc-4vs8.2`/`calc-4vs8.9` structured table packet and bind-record intake plus row/column/header/totals dependency lowering, `calc-4vs8.21` through `calc-4vs8.26` node-associated table completion spine for table-node snapshot projection, TreeCalc table-path prebind, table reference readers, per-row formula runtime, update/invalidation scenarios, and retained evidence closure, W074/W036 intake and handoff watch, end-to-end scenarios.
7. rollout_mode:
