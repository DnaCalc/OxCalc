...
| `FML-01` | formula source/catalog carriage | `TreeFormulaCatalog`, `TreeFormulaBinding`, opaque OxFml source/carriers | every source binding either projects OxFml bind facts/descriptors or rejects with diagnostics; OxCalc does not own spreadsheet formula semantics | `prepare_oxfml_formula`; session-path cases; upstream-host fixture cases | `replay_evidence`; totality proof planned |
...
| `REF-01` | dependency/evaluator-fact carrier lowering | `TreeReference` carrier variants and `descriptor_kind` | carrier classification is total and preserves graph/fact family | carrier unit tests; descriptor taxonomy; W035 dependency closure row | partial `proved_fragment`/`checked_classifier`; full lowering proof planned |
...
| `EVAL-02` | OxFml runtime candidate | `evaluate_with_oxfml_session`, `invoke_prepared_formula_via_session`, residual gate, commit-bundle validation | accepted OxFml result validates into local success; rejected/residual outcome becomes no-publish failure | session-path direct OxFml cases; upstream-host fixture cases; OxFml seam docs | `replay_evidence`; seam proof remains partial |
