...
| `W037-OBL-009` | Stage 2 deterministic replay and partition promotion criteria | W036 has bounded partition model and scheduler-readiness criteria, but no deterministic replay equivalence | `calc-ubd.6`, `calc-ubd.8` | `calc-ubd.6` defines runnable criteria and observable-result invariance obligations, but keeps promotion blocked because deterministic partition replay and pack governance remain absent |
| `W037-OBL-010` | independent evaluator diversity | W036 classifies 0 fully independent evaluator rows | `calc-ubd.7` | `calc-ubd.7` records fully independent evaluator implementation as an explicit blocked service-readiness criterion; no qualifying independent evaluator rows are added |
| `W037-OBL-011` | operated cross-engine differential service | W036 emits deterministic differential rows without an operated service | `calc-ubd.7` | `calc-ubd.7` emits a file-backed cross-engine service pilot with 9 differential rows and explicitly blocks operated service promotion |
| `W037-OBL-012` | operated continuous assurance and quarantine | W036 emits simulated multi-run history, thresholds, and policy only | `calc-ubd.7` | `calc-ubd.7` emits retained history, semantic-first thresholds, quarantine rules, service-readiness criteria, and alert/runner/service blockers without operated promotion |
| `W037-OBL-013` | pack-grade replay governance and C5 | W036 highest honest capability is `cap.C4.distill_valid` with 22 blockers | `calc-ubd.8` | `calc-ubd.8` reassesses C5 after W037 direct evaluator, conformance, proof/model, service, and Stage 2 criteria gates; highest honest capability remains `cap.C4.distill_valid` with 22 blockers |
...
Post-`calc-ubd.6` note: `w037-stage2-deterministic-replay-criteria-001` records the W037 Stage 2 deterministic replay and partition promotion criteria. It records 7 criteria rows, 3 satisfied rows, 4 blocked rows, a checked Lean promotion predicate, 3 checked Stage 2 TLC configs, explicit observable-result invariance requirements, and no Stage 2 policy promotion candidate. Deterministic partition replay, production partition analyzer soundness, operated cross-engine Stage 2 differential service, pack-grade replay governance, and C5 remain open.

Post-`calc-ubd.7` note: `w037-operated-assurance-service-pilot-001` records the W037 operated assurance and cross-engine service pilot packet. It records 16 source rows, 5 scheduled lanes, 9 differential rows, 11 history rows, 7 threshold rules, 8 quarantine rules, 10 service-readiness criteria, 4 blocked service criteria, 0 missing artifacts, 0 unexpected mismatches, and no operated continuous-assurance service, operated cross-engine service, fully independent evaluator, pack/C5, or Stage 2 promotion.
