...
| `calc-gucd.15` | integrated semantic kernel | outputs from `.2` through `.7`; `dependency.rs`, `recalc.rs`, `coordinator.rs`, `treecalc.rs`, TraceCalc machine/planner/runner | cross-phase state machine over graph, invalidation, tracker, order/read, effects, candidate, reject, publish, and trace | cross-phase preservation: no stale/rejected/cycle work publishes; publication uses accepted candidates; traces preserve semantic facts | W046 phase packets and current TraceCalc/TreeCalc roots | integrated kernel exists with checked model or exact integration blocker |
| `calc-gucd.16` | finite graph/dataflow/order strengthening | W046 graph/invalidation/evaluation Lean/TLA, Tarjan/topo Rust code, TreeCalc graph artifacts, scale graph shapes | reusable finite graph relation or Datalog-style rule table | SCC, reverse reachability, no-under-invalidation, topo order, and stable/prior read properties beyond one smoke graph | multi-shape graph fixtures or generated model cases | generalized theorem/model targets exist or exact blockers name the remaining proof gap |
| `calc-gucd.17` | proof-carrying traces and semantic replay checker | treecalc runner emission, TraceCalc contracts/assertions/replay mappings, current `docs/test-runs/core-engine` roots | checked trace schema and validator | emitted graph/invalidation/order/candidate/reject/publish/effect facts validate against semantic catalog | existing W046/W037 roots, plus additive baseline if needed | checker validates at least one baseline root or exact implementation blocker recorded |
| `calc-gucd.18` | Rust refinement bridge | `.15` kernel, `.16` strengthened facts, `.17` checker, all Rust semantic state emitters | implementation-to-semantics mapping and refinement relation over emitted artifacts | selected Rust runs refine the integrated semantic kernel or produce exact blockers | TreeCalc/CoreEngine and TraceCalc selected runs | Rust artifact refinement evidence exists; .6 dynamic/invalidation blockers resolved or sharpened |
| `calc-gucd.8` | evidence layer over semantic spine | outputs from `.2` through `.7` and `.15` through `.18`; archived classifiers only as predecessor context | proof-service/evidence coverage ledger over semantic objects | no classifier row promotes beyond direct semantic/checker-backed evidence | current model/proof/replay/checker outputs | classifier/evidence layer recast over semantic spine |
| `calc-gucd.9` | scale and phase timings as semantic regression evidence | treecalc-scale profiles, phase timing names, integrated kernel, trace checker, Rust refinement bridge | semantic regression signature table | timing never substitutes for correctness; phase timings bind to semantic phases and checker-backed facts | million-node grid/fanout/indirect/rebind artifacts plus checker output | performance evidence mapped to semantic signatures |
| `calc-gucd.10` | downstream consequence reassessment | outputs from `.2` through `.18`, `.8`, and `.9` | consequence matrix for Stage 2, pack, C5, operated service, release-readiness | promotion only from direct semantic evidence, trace checker evidence, Rust refinement evidence, or exact blockers | current semantic-spine outputs | readiness consequences classified honestly |
...
### 7.1 Post `.6` Deep-Proof Redirection

The review after `calc-gucd.6` found the direction correct but still too shallow for a deep proof of the engine. The remaining path now inserts four semantic-proof beads before the evidence/readiness layer:

1. integrated semantic kernel,
2. finite graph/dataflow/order strengthening,
3. proof-carrying trace checker,
4. Rust refinement bridge.

This means `calc-gucd.8` and later beads are deliberately downstream of implementation-trace refinement, not merely downstream of the initial phase-local models.
