...
4. `calc-gucd.5` is now the next real gap: evaluation order and working-value reads are implementation-backed, but the proof objects do not yet state the read-discipline invariant sharply.
