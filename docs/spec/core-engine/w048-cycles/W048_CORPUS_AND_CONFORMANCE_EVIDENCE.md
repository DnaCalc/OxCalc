...
| Cross-corpus conformance summary | `docs/test-runs/core-engine/w048-conformance-002/w048_conformance_summary.json` | `status=passed_with_named_excel_blockers` |
...
| root/report-cell behavior | blocker | `BLK-W048-EXCEL-ROOT` |
| blank/text/error prior values | blocker | `BLK-W048-EXCEL-NONNUMERIC` |
| cross-version and multithread variants | blocker | `BLK-W048-EXCEL-VERSION`, `BLK-W048-EXCEL-MT` |
...
```text
w048 conformance passed with named blockers: docs/test-runs/core-engine/w048-conformance-002/w048_conformance_summary.json
```
...
7. The five named Excel blockers are missing.
8. Any status axis incorrectly claims final full closure while blockers remain.
...
2. The conformance status is `passed_with_named_excel_blockers`, with all three status axes partial while blockers remain.
...
Fresh-eyes result: `calc-zci1.15` satisfies the reopened conformance audit gate for declared coverage, but W048 parent closure remains blocked by explicit Excel-match blockers and whole-workset final audit/disposition.
...
- open_lanes:
  - `BLK-W048-EXCEL-ROOT`;
  - `BLK-W048-EXCEL-INITIAL`;
  - `BLK-W048-EXCEL-NONNUMERIC`;
  - `BLK-W048-EXCEL-VERSION`;
  - `BLK-W048-EXCEL-MT`;
  - whole-workset fresh-eyes audit and user disposition for blockers before parent closure.
