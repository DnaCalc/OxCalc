...
## 79. W050 B8 Session Corpus Evidence Packet
OxCalc has added a deterministic TreeCalc session-path evidence packet for
the current local corpus.

Current OxCalc code shape is:
1. `TreeCalcRunner` emits `session_path_evidence.json` at the TreeCalc run
   root.
2. The B8 checked-in root is
   `docs/test-runs/core-engine/treecalc-local/w050-b8-treecalc-session-corpus-001`.
3. The root is generated by
   `cargo run -p oxcalc-tracecalc-cli -- treecalc w050-b8-treecalc-session-corpus-001`.
4. Each packet entry records candidate, commit, reject, returned-value
   surface, replay-facing diagnostic, result/trace artifact path, and
   no-publish/non-mutation facts for an initial or post-edit case
   execution.
5. The TreeCalc replay-appliance projection now references the packet and
   validates it in `replay-appliance/validation/bundle_validation.json`.

Current OxCalc gap routed to B9 / `HANDOFF-CALC-002`:
1. the B8 packet proves which current V1 facts OxCalc can preserve
   deterministically, but it is an OxCalc evidence schema rather than a
   canonical OxFml shared schema,
2. some correlation facts still arrive through diagnostics; OxCalc needs
   the compatibility ledger to name which should become stable structured
   public facade fields,
3. likely structured fields include candidate result id, commit attempt id,
   reject trace correlation id, returned-value surface classification, and
   replay-facing diagnostic categories.

Current non-assumptions:
1. OxCalc is not asking for private OxFml internals.
2. OxCalc is not treating the B8 packet as a final shared replay schema.
