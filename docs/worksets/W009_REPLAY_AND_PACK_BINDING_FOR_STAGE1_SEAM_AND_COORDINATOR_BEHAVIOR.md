...
| `R3` fence-compatible accept/reject split | Compatibility and artifact-token basis are preserved through `OxCalcTreeContext recalculation configuration` and result diagnostics. | Explicit incompatible-fence TreeCalc corpus breadth remains future if promotion pressure requires it. |
