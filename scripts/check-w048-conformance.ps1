...
if ($s.status -notin @("passed_with_named_excel_blockers", "passed_with_named_excel_version_blocker", "passed_single_host_scoped")) { Add-Err "unexpected conformance status observed $($s.status)" }
if (@($s.errors).Count -ne 0) { Add-Err "summary has errors: $($s.errors -join '; ')" }
if ($s.status -eq "passed_single_host_scoped") {
  if ($s.scope_completeness -ne "scope_complete_single_host") { Add-Err "single-host scope_completeness mismatch" }
  if ($s.target_completeness -ne "target_complete_single_host") { Add-Err "single-host target_completeness mismatch" }
  if ($s.integration_completeness -ne "integrated_single_host") { Add-Err "single-host integration_completeness mismatch" }
  if (-not $s.accepted_scope) { Add-Err "single-host conformance must include accepted_scope" }
  if (($s.documented_limitations -join ";") -notmatch "cross-version") { Add-Err "single-host conformance must document cross-version limitation" }
} else {
...
if ($s.status -eq "passed_single_host_scoped") {
  if (@($s.blockers).Count -ne 0) { Add-Err "single-host scoped conformance should not list open blockers" }
} else {
...
Write-Host "w048 conformance ok: status=$($s.status) path=$SummaryPath"
