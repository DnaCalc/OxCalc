...
public sealed class TraceCalcReferenceMachine
{
    ...
    {
        var state = ReferenceState.Create(scenario);
        ...
        var assertionFailures = TraceCalcAssertionEvaluator.Evaluate(
            scenario,
            state.ReadPublishedValues(),
            state.ReadPinnedViews(),
            state.TraceEvents,
            state.Counters,
            state.Rejects);
    ...
    private static void PinView(ReferenceState state, TraceCalcScenario scenario, TraceCalcStep step)
    {
    ...
    private static void UnpinView(ReferenceState state, TraceCalcStep step)
    {
    ...
    private static void MarkStale(ReferenceState state, TraceCalcStep step)
    {
    ...
    private static void AdmitWork(ReferenceState state, TraceCalcScenario scenario, TraceCalcStep step)
    {
    ...
    private static void EmitPlanArtifacts(ReferenceState state, string stepId)
    {
    ...
    private static void AdvanceScheduledNodesToEvaluating(ReferenceState state, string stepId, string compatibilityBasis)
    {
    ...
    private static void EmitCandidateResult(ReferenceState state, TraceCalcScenario scenario, TraceCalcStep step)
    {
    ...
    private static void EmitReject(ReferenceState state, TraceCalcScenario scenario, TraceCalcStep step)
    {
    ...
    private static void PublishCandidate(ReferenceState state, TraceCalcStep step)
    {
    ...
    private static void VerifyClean(ReferenceState state, TraceCalcScenario scenario, TraceCalcStep step)
    {
        var compatibilityBasis = state.CurrentCompatibilityBasis ?? scenario.InitialGraph.SnapshotId;
        if (state.CurrentTargets.Count == 0)
        {
    ...
    private static void SeedOverlay(ReferenceState state, TraceCalcScenario scenario, TraceCalcStep step)
    {
        var compatibilityBasis = state.CurrentCompatibilityBasis ?? scenario.InitialGraph.SnapshotId;
    ...
    private static void ResetFixture(ReferenceState state, TraceCalcStep step)
    {
    ...
    private sealed class ReferenceState
    {
        ...
        public static ReferenceState Create(TraceCalcScenario scenario)
        {
