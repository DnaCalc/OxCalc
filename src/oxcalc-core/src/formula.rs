...
pub const W056_NON_TABLE_REFERENCE_CATEGORIES: &[W056NonTableReferenceCategory] = &[
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "implemented and green as ChildrenV1, including direct OxCalcDocumentContext raw @CHILDREN/.* product path",
        dnatreecalc_status: "active direct OxCalcDocumentContext slice",
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "direct OxCalcDocumentContext raw formula binding resolves bare walk-up and dotted descent through the host-name bind lane; RelativePath carriers still cover lower-level fixture paths",
        dnatreecalc_status: "focused active direct OxCalcDocumentContext raw slice; full Q1/cell-like precedence cases remain pending",
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "ancestor RelativePath carriers and direct OxCalcDocumentContext raw ancestor-anchor binding exist; workspace-root/alias anchor breadth remains pending",
        dnatreecalc_status: "focused active direct OxCalcDocumentContext ancestor-anchor slice; broader workspace-root anchor corpus pending",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        dnatreecalc_status: "active direct OxCalcDocumentContext typed-pending/exclusion slice for raw cross-workspace syntax",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "structural path base resolver covers bracketed path segments and direct OxCalcDocumentContext raw escaped paths for the focused slice",
        dnatreecalc_status: "focused active direct OxCalcDocumentContext escaped-path slice; broader canonical display/profile coverage pending",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "direct OxCalcDocumentContext owns is_meta state, hides meta-effective nodes from host-name lookup and sibling navigation, and exposes current metadata accessors for the active slice",
        dnatreecalc_status: "focused active direct OxCalcDocumentContext meta invisibility/accessor slice; broader ordered selectors over meta-effective snapshots pending",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "SiblingOffset and QualifiedSiblingOffset carrier/dependency lowering exist; direct OxCalcDocumentContext raw @PREV/@NEXT and base.@PREV/base.@NEXT product paths are implemented for focused tail forms and out-of-range relative-bound diagnostics",
        dnatreecalc_status: "active direct OxCalcDocumentContext sibling-offset slice covers unqualified tail forms, qualified base tail forms, and out-of-range diagnostics",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "resolved ordered selector carriers, traversal resolver, bounds, and direct OxCalcDocumentContext raw @PRECEDING/@FOLLOWING/@ANCESTORS product path implemented, including dotted-tail forms (e.g. @PRECEDING.Margin) resolved under each member",
        dnatreecalc_status: "active direct OxCalcDocumentContext ordered-selector slice plus broad raw set-membership mirror for Q1.*, children, ancestors, preceding/following, empty preceding, and recursive all/match forms",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        runnable_suite_command: "cargo test -p dnatreecalc-host --test active_ordered_corpus --test active_set_membership_corpus -- --nocapture",
        oxcalc_status: "recursive ordered selector carrier, traversal-bound resolver, and direct OxCalcDocumentContext Base.**.tail product path implemented",
        dnatreecalc_status: "active direct OxCalcDocumentContext recursive-selector slice plus broad set-membership mirror for absolute recursive tail and relative all-descendant forms",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "ReferenceLiteralArrayV1 implemented; direct OxCalcDocumentContext resolves all-reference arrays through host-reference packets; mixed scalar/reference arrays remain typed exclusions",
        dnatreecalc_status: "focused active direct OxCalcDocumentContext reference-only array slice; broader arrays corpus pending",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        runnable_suite_command: "cargo test -p dnatreecalc-host --test active_dynamic_cross_workspace_corpus -- --nocapture",
        oxcalc_status: "Direct OxCalcDocumentContext resolves basic raw INDIRECT reference text through the OxFml/OxFunc FEC resolver and records CTRO dynamic dependency edges; broader CTRO cases remain open",
        dnatreecalc_status: "active direct OxCalcDocumentContext dynamic INDIRECT corpus migration pending beyond OxCalc basic proof",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        runnable_suite_command: "cargo test -p dnatreecalc-host --test active_dynamic_cross_workspace_corpus -- --nocapture",
        oxcalc_status: "workspace-qualified carriers, reverse edges, and prepared identity implemented; direct OxCalcDocumentContext records raw cross-workspace host names as typed pending",
        dnatreecalc_status: "active direct OxCalcDocumentContext typed-pending/exclusion slice for raw cross-workspace runtime references",
        replay_status: "retained non-table replay missing",
        current_test_result: "green direct-context typed-pending runner; product runtime and retained evidence still open",
        evidence_status: W056NonTableReferenceEvidenceStatus::DirectContextTypedPending,
        specification_is_sufficient_for_cases: true,
    ...
    W056NonTableReferenceCategory {
        ...
        oxcalc_status: "W074 handoff consumed; direct OxCalcDocumentContext host-name bind lane maps TreeCalc names to the defined-name value lane with source token/span and replay identity; no OxCalc private precedence mirror",
        dnatreecalc_status: "focused active direct OxCalcDocumentContext non-cell-like host-name slice; broader name/cell-like precedence corpus pending",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        corpus_or_suite: &[
            "../DnaTreeCalc/docs/test-corpus/references/node-functions.json",
            "oxcalc-core consumer.rs treecalc_context_can_call_lambda_value_published_by_another_node",
        ],
        runnable_suite_command: "cargo test -p dnatreecalc-host --test active_node_functions_corpus -- --nocapture",
        oxcalc_status: "current defined-name-LAMBDA mapping admitted by calc-4vs8.32; direct OxCalcDocumentContext now calls lambda-valued host nodes as callable candidates (consumer.rs treecalc_context_can_call_lambda_value_published_by_another_node), not typed pending/exclusion",
        dnatreecalc_status: "active direct OxCalcDocumentContext callable slice for single-reference lambda-valued nodes; broader node-functions corpus pending",
        replay_status: "retained non-table replay missing",
        current_test_result: "green direct-context callable runner (static dependency descriptor, published Callable rich value, correct recalculation); broader active_node_functions_corpus breadth pending",
        evidence_status: W056NonTableReferenceEvidenceStatus::DirectContextSliceGreen,
        specification_is_sufficient_for_cases: true,
        blocks_w056_non_table_closure: true,
    },
    W056NonTableReferenceCategory {
        ...
        dnatreecalc_status: "active direct OxCalcDocumentContext typed-pending runner",
        replay_status: "retained non-table replay missing",
    ...
    W056NonTableReferenceCategory {
        ...
        dnatreecalc_status: "active direct OxCalcDocumentContext structural edit runner covers delete, rename propagation/no-propagation, move-out-of-scope, and insert-shadow consequences",
        replay_status: "retained non-table replay missing",
