#![forbid(unsafe_code)]

//! W038/W039/W040/W041/W042/W043/W044/W045 independent-evaluator diversity and OxFml seam-watch packet emission.

use std::fs;
use std::path::Path;

use serde_json::{Value, json};
use thiserror::Error;

const RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w038.run_summary.v1";
const SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w038.source_evidence_index.v1";
const DIVERSITY_DISPOSITION_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w038.implementation_diversity_disposition.v1";
const OXFML_SEAM_WATCH_SCHEMA_V1: &str = "oxcalc.diversity_seam.w038.oxfml_seam_watch_packet.v1";
const BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w038.exact_diversity_seam_blocker_register.v1";
const PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w038.promotion_decision.v1";
const VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w038.validation.v1";
const W039_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w039.run_summary.v1";
const W039_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w039.source_evidence_index.v1";
const W039_INDEPENDENT_EVALUATOR_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w039.independent_evaluator_row_set.v1";
const W039_CROSS_ENGINE_DIVERSITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w039.cross_engine_diversity_register.v1";
const W039_DIFFERENTIAL_AUTHORITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w039.differential_service_authority_register.v1";
const W039_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w039.exact_diversity_blocker_register.v1";
const W039_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w039.promotion_decision.v1";
const W039_VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w039.validation.v1";
const W040_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w040.run_summary.v1";
const W040_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w040.source_evidence_index.v1";
const W040_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w040.independent_scalar_evaluator_implementation.v1";
const W040_INDEPENDENT_EVALUATOR_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w040.independent_evaluator_row_set.v1";
const W040_CROSS_ENGINE_DIVERSITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w040.cross_engine_differential_register.v1";
const W040_DIFFERENTIAL_AUTHORITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w040.differential_authority_register.v1";
const W040_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w040.exact_diversity_blocker_register.v1";
const W040_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w040.promotion_decision.v1";
const W040_VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w040.validation.v1";
const W041_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w041.run_summary.v1";
const W041_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w041.source_evidence_index.v1";
const W041_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w041.independent_formula_evaluator_implementation.v1";
const W041_INDEPENDENT_EVALUATOR_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w041.independent_evaluator_breadth_register.v1";
const W041_CROSS_ENGINE_DIVERSITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w041.cross_engine_differential_service_register.v1";
const W041_MISMATCH_AUTHORITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w041.mismatch_authority_router.v1";
const W041_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w041.exact_diversity_blocker_register.v1";
const W041_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w041.promotion_decision.v1";
const W041_VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w041.validation.v1";
const W042_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w042.run_summary.v1";
const W042_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w042.source_evidence_index.v1";
const W042_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w042.independent_reference_model_implementation.v1";
const W042_INDEPENDENT_EVALUATOR_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w042.independent_evaluator_breadth_register.v1";
const W042_CROSS_ENGINE_DIVERSITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w042.cross_engine_differential_service_register.v1";
const W042_MISMATCH_AUTHORITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w042.mismatch_quarantine_authority_router.v1";
const W042_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w042.exact_diversity_blocker_register.v1";
const W042_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w042.promotion_decision.v1";
const W042_VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w042.validation.v1";
const W043_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w043.run_summary.v1";
const W043_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w043.source_evidence_index.v1";
const W043_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w043.independent_reference_model_implementation.v1";
const W043_INDEPENDENT_EVALUATOR_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w043.independent_evaluator_breadth_register.v1";
const W043_CROSS_ENGINE_DIVERSITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w043.cross_engine_differential_service_register.v1";
const W043_MISMATCH_AUTHORITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w043.mismatch_quarantine_authority_router.v1";
const W043_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w043.exact_diversity_blocker_register.v1";
const W043_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w043.promotion_decision.v1";
const W043_VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w043.validation.v1";
const W044_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w044.run_summary.v1";
const W044_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w044.source_evidence_index.v1";
const W044_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w044.independent_reference_model_implementation.v1";
const W044_INDEPENDENT_EVALUATOR_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w044.independent_evaluator_breadth_register.v1";
const W044_CROSS_ENGINE_DIVERSITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w044.cross_engine_differential_service_register.v1";
const W044_MISMATCH_AUTHORITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w044.mismatch_quarantine_authority_router.v1";
const W044_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w044.exact_diversity_blocker_register.v1";
const W044_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w044.promotion_decision.v1";
const W044_VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w044.validation.v1";
const W045_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.diversity_seam.w045.run_summary.v1";
const W045_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.diversity_seam.w045.source_evidence_index.v1";
const W045_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w045.independent_reference_model_implementation.v1";
const W045_INDEPENDENT_EVALUATOR_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w045.independent_evaluator_breadth_register.v1";
const W045_CROSS_ENGINE_DIVERSITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w045.cross_engine_differential_service_register.v1";
const W045_MISMATCH_AUTHORITY_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w045.mismatch_quarantine_authority_router.v1";
const W045_RETAINED_WITNESS_ATTACHMENT_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w045.retained_witness_attachment_register.v1";
const W045_OPERATED_DIFFERENTIAL_HARNESS_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w045.operated_differential_service_harness_register.v1";
const W045_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.diversity_seam.w045.exact_diversity_blocker_register.v1";
const W045_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w045.promotion_decision.v1";
const W045_VALIDATION_SCHEMA_V1: &str = "oxcalc.diversity_seam.w045.validation.v1";

const W036_INDEPENDENT_SUMMARY: &str = "docs/test-runs/core-engine/independent-conformance/w036-independent-diversity-differential-001/run_summary.json";
const W036_CROSS_ENGINE_SUMMARY: &str = "docs/test-runs/core-engine/cross-engine-differential/w036-independent-diversity-differential-001/run_summary.json";
const W037_DIRECT_OXFML_SUMMARY: &str =
    "docs/test-runs/core-engine/upstream-host/w037-direct-oxfml-evaluator-001/run_summary.json";
const W038_CONFORMANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w038-optimized-core-conformance-disposition-001/run_summary.json";
const W038_CONFORMANCE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w038-optimized-core-conformance-disposition-001/w038_exact_remaining_blocker_register.json";
const W038_FORMAL_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w038-proof-model-assumption-discharge-001/run_summary.json";
const W038_STAGE2_REPLAY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w038-stage2-partition-replay-001/run_summary.json";
const W038_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w038-operated-assurance-alert-quarantine-001/run_summary.json";
const W038_DIVERSITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w038-diversity-seam-watch-001/run_summary.json";
const W038_DIVERSITY_DISPOSITION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w038-diversity-seam-watch-001/implementation_diversity_disposition.json";
const W038_DIVERSITY_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w038-diversity-seam-watch-001/exact_diversity_seam_blocker_register.json";
const W039_RESIDUAL_LEDGER_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w039-residual-successor-obligation-ledger-001/run_summary.json";
const W039_FORMATTING_INTAKE: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w039-residual-successor-obligation-ledger-001/w073_formatting_intake.json";
const W039_CONFORMANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w039-optimized-core-exact-blocker-disposition-001/run_summary.json";
const W039_PROOF_MODEL_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w039-proof-model-totality-closure-001/run_summary.json";
const W039_STAGE2_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w039-stage2-production-policy-replay-governance-001/run_summary.json";
const W039_STAGE2_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w039-stage2-production-policy-replay-governance-001/w039_stage2_exact_blocker_register.json";
const W039_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w039-operated-assurance-retained-history-001/run_summary.json";
const W039_CROSS_ENGINE_SERVICE_SUBSTRATE: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w039-operated-assurance-retained-history-001/w039_cross_engine_service_substrate.json";
const W039_OPERATED_SERVICE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w039-operated-assurance-retained-history-001/w039_exact_service_blocker_register.json";
const W039_DIVERSITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w039-independent-evaluator-cross-engine-diversity-001/run_summary.json";
const W039_INDEPENDENT_ROW_SET: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w039-independent-evaluator-cross-engine-diversity-001/w039_independent_evaluator_row_set.json";
const W039_CROSS_ENGINE_DIVERSITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w039-independent-evaluator-cross-engine-diversity-001/w039_cross_engine_diversity_register.json";
const W039_DIFFERENTIAL_AUTHORITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w039-independent-evaluator-cross-engine-diversity-001/w039_differential_service_authority_register.json";
const W039_DIVERSITY_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w039-independent-evaluator-cross-engine-diversity-001/w039_exact_diversity_blocker_register.json";
const W039_DIVERSITY_DECISION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w039-independent-evaluator-cross-engine-diversity-001/promotion_decision.json";
const W040_DIRECT_OBLIGATION_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w040-direct-verification-obligation-map-001/run_summary.json";
const W040_DIRECT_OBLIGATION_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w040-direct-verification-obligation-map-001/direct_verification_obligation_map.json";
const W040_FORMATTING_INTAKE: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w040-direct-verification-obligation-map-001/w073_formatting_intake.json";
const W040_CONFORMANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w040-optimized-core-exact-blocker-fixes-differentials-001/run_summary.json";
const W040_TREECALC_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/treecalc-local/w040-optimized-core-dynamic-release-reclassification-001/run_summary.json";
const W040_RUST_TOTALITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w040-rust-totality-refinement-proof-tranche-001/run_summary.json";
const W040_LEAN_TLA_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w040-lean-tla-full-verification-discharge-001/run_summary.json";
const W040_STAGE2_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w040-stage2-production-policy-equivalence-001/run_summary.json";
const W040_STAGE2_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w040-stage2-production-policy-equivalence-001/w040_stage2_exact_blocker_register.json";
const W040_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w040-operated-assurance-retained-history-service-001/run_summary.json";
const W040_RETAINED_HISTORY_STORE: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w040-operated-assurance-retained-history-service-001/w040_retained_history_store_query.json";
const W040_CROSS_ENGINE_SERVICE_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w040-operated-assurance-retained-history-service-001/w040_cross_engine_service_register.json";
const W040_OPERATED_SERVICE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w040-operated-assurance-retained-history-service-001/w040_exact_service_blocker_register.json";
const W040_DIVERSITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w040-independent-evaluator-operated-differential-001/run_summary.json";
const W040_INDEPENDENT_IMPLEMENTATION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w040-independent-evaluator-operated-differential-001/w040_independent_scalar_evaluator_implementation.json";
const W040_INDEPENDENT_ROW_SET: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w040-independent-evaluator-operated-differential-001/w040_independent_evaluator_row_set.json";
const W040_CROSS_ENGINE_DIVERSITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w040-independent-evaluator-operated-differential-001/w040_cross_engine_differential_register.json";
const W040_DIFFERENTIAL_AUTHORITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w040-independent-evaluator-operated-differential-001/w040_differential_authority_register.json";
const W040_DIVERSITY_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w040-independent-evaluator-operated-differential-001/w040_exact_diversity_blocker_register.json";
const W040_DIVERSITY_DECISION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w040-independent-evaluator-operated-differential-001/promotion_decision.json";
const W041_OBLIGATION_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w041-residual-release-grade-successor-obligation-map-001/run_summary.json";
const W041_OBLIGATION_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w041-residual-release-grade-successor-obligation-map-001/successor_obligation_map.json";
const W041_CONFORMANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w041-optimized-core-residual-blocker-differentials-001/run_summary.json";
const W041_TREECALC_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/treecalc-local/w041-optimized-core-automatic-dynamic-transition-001/run_summary.json";
const W041_RUST_TOTALITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w041-rust-totality-refinement-proof-tranche-001/run_summary.json";
const W041_LEAN_TLA_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w041-lean-tla-full-verification-fairness-discharge-001/run_summary.json";
const W041_STAGE2_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w041-stage2-production-analyzer-pack-equivalence-001/run_summary.json";
const W041_STAGE2_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w041-stage2-production-analyzer-pack-equivalence-001/w041_stage2_exact_blocker_register.json";
const W041_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w041-operated-assurance-retained-history-alert-dispatch-001/run_summary.json";
const W041_RETAINED_HISTORY_QUERY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w041-operated-assurance-retained-history-alert-dispatch-001/w041_retained_history_service_query.json";
const W041_CROSS_ENGINE_SERVICE_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w041-operated-assurance-retained-history-alert-dispatch-001/w041_cross_engine_service_register.json";
const W041_ALERT_DISPATCH_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w041-operated-assurance-retained-history-alert-dispatch-001/w041_alert_dispatch_service_register.json";
const W041_OPERATED_SERVICE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w041-operated-assurance-retained-history-alert-dispatch-001/w041_exact_service_blocker_register.json";
const W041_DIVERSITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w041-independent-evaluator-breadth-operated-differential-001/run_summary.json";
const W041_INDEPENDENT_IMPLEMENTATION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w041-independent-evaluator-breadth-operated-differential-001/w041_independent_formula_evaluator_implementation.json";
const W041_INDEPENDENT_ROW_SET: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w041-independent-evaluator-breadth-operated-differential-001/w041_independent_evaluator_breadth_register.json";
const W041_CROSS_ENGINE_DIVERSITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w041-independent-evaluator-breadth-operated-differential-001/w041_cross_engine_differential_service_register.json";
const W041_MISMATCH_AUTHORITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w041-independent-evaluator-breadth-operated-differential-001/w041_mismatch_authority_router.json";
const W041_DIVERSITY_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w041-independent-evaluator-breadth-operated-differential-001/w041_exact_diversity_blocker_register.json";
const W041_DIVERSITY_DECISION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w041-independent-evaluator-breadth-operated-differential-001/promotion_decision.json";
const W041_PACK_DECISION: &str = "archive/test-runs-core-engine-w038-w045/pack-capability/w041-pack-grade-replay-governance-c5-reassessment-001/decision/pack_capability_decision.json";
const W042_OBLIGATION_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w042-residual-release-grade-closure-obligation-ledger-001/run_summary.json";
const W042_OBLIGATION_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w042-residual-release-grade-closure-obligation-ledger-001/closure_obligation_map.json";
const W042_FORMATTING_INTAKE: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w042-residual-release-grade-closure-obligation-ledger-001/w073_formatting_intake.json";
const W042_STAGE2_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w042-stage2-production-analyzer-pack-grade-equivalence-closure-001/run_summary.json";
const W042_STAGE2_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w042-stage2-production-analyzer-pack-grade-equivalence-closure-001/w042_stage2_exact_blocker_register.json";
const W042_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w042-operated-assurance-retained-history-retained-witness-alert-service-001/run_summary.json";
const W042_RETAINED_HISTORY_QUERY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w042-operated-assurance-retained-history-retained-witness-alert-service-001/w042_retained_history_service_query.json";
const W042_RETAINED_WITNESS_LIFECYCLE: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w042-operated-assurance-retained-history-retained-witness-alert-service-001/w042_retained_witness_lifecycle_register.json";
const W042_CROSS_ENGINE_SERVICE_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w042-operated-assurance-retained-history-retained-witness-alert-service-001/w042_cross_engine_service_register.json";
const W042_ALERT_DISPATCH_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w042-operated-assurance-retained-history-retained-witness-alert-service-001/w042_alert_dispatch_service_register.json";
const W042_OPERATED_SERVICE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w042-operated-assurance-retained-history-retained-witness-alert-service-001/w042_exact_service_blocker_register.json";
const W042_OPERATED_PROMOTION_DECISION: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w042-operated-assurance-retained-history-retained-witness-alert-service-001/promotion_decision.json";
const W042_DIVERSITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w042-independent-evaluator-breadth-mismatch-quarantine-operated-differential-001/run_summary.json";
const W042_INDEPENDENT_IMPLEMENTATION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w042-independent-evaluator-breadth-mismatch-quarantine-operated-differential-001/w042_independent_reference_model_implementation.json";
const W042_INDEPENDENT_ROW_SET: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w042-independent-evaluator-breadth-mismatch-quarantine-operated-differential-001/w042_independent_evaluator_breadth_register.json";
const W042_CROSS_ENGINE_DIVERSITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w042-independent-evaluator-breadth-mismatch-quarantine-operated-differential-001/w042_cross_engine_differential_service_register.json";
const W042_MISMATCH_AUTHORITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w042-independent-evaluator-breadth-mismatch-quarantine-operated-differential-001/w042_mismatch_quarantine_authority_router.json";
const W042_DIVERSITY_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w042-independent-evaluator-breadth-mismatch-quarantine-operated-differential-001/w042_exact_diversity_blocker_register.json";
const W042_DIVERSITY_DECISION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w042-independent-evaluator-breadth-mismatch-quarantine-operated-differential-001/promotion_decision.json";
const W042_PACK_DECISION: &str = "archive/test-runs-core-engine-w038-w045/pack-capability/w042-pack-grade-replay-governance-c5-reassessment-001/decision/pack_capability_decision.json";
const W043_OBLIGATION_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w043-residual-release-grade-proof-service-obligation-map-001/run_summary.json";
const W043_OBLIGATION_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w043-residual-release-grade-proof-service-obligation-map-001/proof_service_obligation_map.json";
const W043_OXFML_INBOUND_INTAKE: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w043-residual-release-grade-proof-service-obligation-map-001/oxfml_inbound_observation_intake.json";
const W043_STAGE2_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w043-stage2-production-partition-analyzer-scheduler-equivalence-001/run_summary.json";
const W043_STAGE2_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w043-stage2-production-partition-analyzer-scheduler-equivalence-001/w043_stage2_exact_blocker_register.json";
const W043_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w043-operated-assurance-retained-history-witness-slo-alert-service-001/run_summary.json";
const W043_RETAINED_HISTORY_QUERY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w043-operated-assurance-retained-history-witness-slo-alert-service-001/w043_retained_history_service_query.json";
const W043_RETAINED_WITNESS_LIFECYCLE: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w043-operated-assurance-retained-history-witness-slo-alert-service-001/w043_retained_witness_lifecycle_register.json";
const W043_CROSS_ENGINE_SERVICE_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w043-operated-assurance-retained-history-witness-slo-alert-service-001/w043_cross_engine_service_register.json";
const W043_ALERT_DISPATCH_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w043-operated-assurance-retained-history-witness-slo-alert-service-001/w043_alert_dispatch_service_register.json";
const W043_OPERATED_SERVICE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w043-operated-assurance-retained-history-witness-slo-alert-service-001/w043_exact_service_blocker_register.json";
const W043_OPERATED_PROMOTION_DECISION: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w043-operated-assurance-retained-history-witness-slo-alert-service-001/promotion_decision.json";
const W043_DIVERSITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w043-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/run_summary.json";
const W043_INDEPENDENT_IMPLEMENTATION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w043-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w043_independent_reference_model_implementation.json";
const W043_INDEPENDENT_ROW_SET: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w043-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w043_independent_evaluator_breadth_register.json";
const W043_CROSS_ENGINE_DIVERSITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w043-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w043_cross_engine_differential_service_register.json";
const W043_MISMATCH_AUTHORITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w043-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w043_mismatch_quarantine_authority_router.json";
const W043_DIVERSITY_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w043-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w043_exact_diversity_blocker_register.json";
const W043_DIVERSITY_DECISION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w043-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/promotion_decision.json";
const W043_PACK_DECISION: &str = "archive/test-runs-core-engine-w038-w045/pack-capability/w043-pack-grade-replay-governance-c5-release-reassessment-001/decision/pack_capability_decision.json";
const W044_OBLIGATION_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w044-residual-release-grade-blocker-reclassification-map-001/run_summary.json";
const W044_BLOCKER_RECLASSIFICATION_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w044-residual-release-grade-blocker-reclassification-map-001/blocker_reclassification_map.json";
const W044_PROMOTION_CONTRACT_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w044-residual-release-grade-blocker-reclassification-map-001/promotion_contract_map.json";
const W044_OXFML_INBOUND_INTAKE: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w044-residual-release-grade-blocker-reclassification-map-001/oxfml_inbound_observation_intake.json";
const W044_CONFORMANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w044-optimized-core-dynamic-transition-callable-metadata-001/run_summary.json";
const W044_DYNAMIC_TRANSITION_EVIDENCE: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w044-optimized-core-dynamic-transition-callable-metadata-001/w044_dynamic_transition_evidence.json";
const W044_CALLABLE_METADATA_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w044-optimized-core-dynamic-transition-callable-metadata-001/w044_callable_metadata_projection_register.json";
const W044_CONFORMANCE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w044-optimized-core-dynamic-transition-callable-metadata-001/w044_exact_remaining_blocker_register.json";
const W044_RUST_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w044-rust-totality-refinement-panic-surface-expansion-001/run_summary.json";
const W044_RUST_REFINEMENT_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w044-rust-totality-refinement-panic-surface-expansion-001/w044_rust_refinement_register.json";
const W044_LEAN_TLA_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w044-lean-tla-unbounded-fairness-full-verification-expansion-001/run_summary.json";
const W044_TLA_MODEL_BOUND_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w044-lean-tla-unbounded-fairness-full-verification-expansion-001/w044_tla_model_bound_register.json";
const W044_STAGE2_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w044-stage2-production-partition-analyzer-scheduler-equivalence-001/run_summary.json";
const W044_STAGE2_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w044-stage2-production-partition-analyzer-scheduler-equivalence-001/w044_stage2_exact_blocker_register.json";
const W044_STAGE2_PROMOTION_DECISION: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w044-stage2-production-partition-analyzer-scheduler-equivalence-001/promotion_decision.json";
const W044_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/run_summary.json";
const W044_SERVICE_READINESS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/w044_service_readiness_register.json";
const W044_RETAINED_HISTORY_QUERY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/w044_retained_history_service_query.json";
const W044_RETAINED_WITNESS_LIFECYCLE: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/w044_retained_witness_lifecycle_register.json";
const W044_CROSS_ENGINE_SERVICE_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/w044_cross_engine_service_register.json";
const W044_ALERT_DISPATCH_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/w044_alert_dispatch_service_register.json";
const W044_OPERATED_SERVICE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/w044_exact_service_blocker_register.json";
const W044_OPERATED_PROMOTION_DECISION: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w044-operated-assurance-retained-history-witness-slo-alert-service-001/promotion_decision.json";
const W044_DIVERSITY_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w044-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/run_summary.json";
const W044_INDEPENDENT_IMPLEMENTATION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w044-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w044_independent_reference_model_implementation.json";
const W044_INDEPENDENT_ROW_SET: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w044-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w044_independent_evaluator_breadth_register.json";
const W044_CROSS_ENGINE_DIVERSITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w044-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w044_cross_engine_differential_service_register.json";
const W044_MISMATCH_AUTHORITY: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w044-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w044_mismatch_quarantine_authority_router.json";
const W044_DIVERSITY_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w044-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/w044_exact_diversity_blocker_register.json";
const W044_DIVERSITY_DECISION: &str = "archive/test-runs-core-engine-w038-w045/diversity-seam/w044-independent-evaluator-breadth-mismatch-quarantine-differential-service-001/promotion_decision.json";
const W044_PACK_DECISION: &str = "archive/test-runs-core-engine-w038-w045/pack-capability/w044-pack-grade-replay-governance-service-c5-reassessment-001/decision/pack_capability_decision.json";
const W045_RESIDUAL_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w045-residual-release-grade-successor-obligation-current-oxfml-intake-map-001/run_summary.json";
const W045_SUCCESSOR_OBLIGATION_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w045-residual-release-grade-successor-obligation-current-oxfml-intake-map-001/successor_obligation_map.json";
const W045_PROMOTION_CONTRACT_MAP: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w045-residual-release-grade-successor-obligation-current-oxfml-intake-map-001/promotion_contract_map.json";
const W045_OXFML_INBOUND_INTAKE: &str = "archive/test-runs-core-engine-w038-w045/release-grade-ledger/w045-residual-release-grade-successor-obligation-current-oxfml-intake-map-001/oxfml_inbound_observation_intake.json";
const W045_CONFORMANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w045-optimized-core-counterpart-callable-metadata-001/run_summary.json";
const W045_DYNAMIC_TRANSITION_COVERAGE: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w045-optimized-core-counterpart-callable-metadata-001/w045_dynamic_transition_coverage_register.json";
const W045_CALLABLE_METADATA_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w045-optimized-core-counterpart-callable-metadata-001/w045_callable_metadata_projection_register.json";
const W045_CONFORMANCE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w045-optimized-core-counterpart-callable-metadata-001/w045_exact_remaining_blocker_register.json";
const W045_MATCH_PROMOTION_GUARD: &str = "archive/test-runs-core-engine-w038-w045/implementation-conformance/w045-optimized-core-counterpart-callable-metadata-001/w045_match_promotion_guard.json";
const W045_RUST_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w045-rust-totality-refinement-panic-surface-hardening-001/run_summary.json";
const W045_RUST_REFINEMENT_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w045-rust-totality-refinement-panic-surface-hardening-001/w045_rust_refinement_register.json";
const W045_RUST_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w045-rust-totality-refinement-panic-surface-hardening-001/w045_rust_exact_blocker_register.json";
const W045_LEAN_TLA_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w045-lean-tla-verification-fairness-totality-discharge-001/run_summary.json";
const W045_TLA_MODEL_BOUND_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w045-lean-tla-verification-fairness-totality-discharge-001/w045_tla_model_bound_register.json";
const W045_LEAN_TLA_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/formal-assurance/w045-lean-tla-verification-fairness-totality-discharge-001/w045_lean_tla_exact_blocker_register.json";
const W045_STAGE2_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w045-stage2-production-partition-pack-grade-equivalence-service-001/run_summary.json";
const W045_STAGE2_SERVICE_GATE: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w045-stage2-production-partition-pack-grade-equivalence-service-001/w045_stage2_service_gate_register.json";
const W045_STAGE2_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w045-stage2-production-partition-pack-grade-equivalence-service-001/w045_stage2_exact_blocker_register.json";
const W045_STAGE2_PROMOTION_DECISION: &str = "archive/test-runs-core-engine-w038-w045/stage2-replay/w045-stage2-production-partition-pack-grade-equivalence-service-001/promotion_decision.json";
const W045_OPERATED_ASSURANCE_SUMMARY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/run_summary.json";
const W045_SERVICE_READINESS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/w045_service_readiness_register.json";
const W045_SERVICE_HARNESS_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/w045_operated_service_harness_register.json";
const W045_RETAINED_HISTORY_QUERY: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/w045_retained_history_service_query.json";
const W045_RETAINED_WITNESS_LIFECYCLE: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/w045_retained_witness_lifecycle_register.json";
const W045_CROSS_ENGINE_SERVICE_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/w045_cross_engine_service_register.json";
const W045_ALERT_DISPATCH_REGISTER: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/w045_alert_dispatch_service_register.json";
const W045_OPERATED_SERVICE_BLOCKERS: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/w045_exact_service_blocker_register.json";
const W045_OPERATED_PROMOTION_DECISION: &str = "archive/test-runs-core-engine-w038-w045/operated-assurance/w045-operated-assurance-retained-history-retained-witness-slo-service-001/promotion_decision.json";
const OXFML_INBOUND_NOTES: &str = "../OxFml/docs/upstream/NOTES_FOR_OXCALC.md";

#[derive(Debug, Error)]
pub enum DiversitySeamError {
    #[error("failed to create artifact directory {path}: {source}")]
    CreateDirectory {
        path: String,
        source: std::io::Error,
    },
    #[error("failed to remove existing artifact root {path}: {source}")]
    RemoveDirectory {
        path: String,
        source: std::io::Error,
    },
    #[error("failed to read artifact {path}: {source}")]
    ReadArtifact {
        path: String,
        source: std::io::Error,
    },
    #[error("failed to parse JSON artifact {path}: {source}")]
    ParseJson {
        path: String,
        source: serde_json::Error,
    },
    #[error("failed to write artifact {path}: {source}")]
    WriteFile {
        path: String,
        source: std::io::Error,
    },
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DiversitySeamRunSummary {
    pub run_id: String,
    pub schema_version: String,
    pub source_evidence_row_count: usize,
    pub diversity_disposition_row_count: usize,
    pub seam_watch_row_count: usize,
    pub aligned_seam_watch_row_count: usize,
    pub accepted_boundary_count: usize,
    pub exact_blocker_count: usize,
    pub failed_row_count: usize,
    pub fully_independent_evaluator_promoted: bool,
    pub artifact_root: String,
}

#[derive(Debug, Clone, Default)]
pub struct DiversitySeamRunner;

impl DiversitySeamRunner {
    #[must_use]
    pub fn new() -> Self {
        Self
    }

    pub fn execute(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        if run_id.starts_with("w045-") || run_id.starts_with("test-w045-") {
            return self.execute_w045(repo_root, run_id);
        }
        if run_id.starts_with("w044-") || run_id.starts_with("test-w044-") {
            return self.execute_w044(repo_root, run_id);
        }
        if run_id.starts_with("w043-") || run_id.starts_with("test-w043-") {
            return self.execute_w043(repo_root, run_id);
        }
        if run_id.starts_with("w042-") || run_id.starts_with("test-w042-") {
            return self.execute_w042(repo_root, run_id);
        }
        if run_id.starts_with("w041-") || run_id.starts_with("test-w041-") {
            return self.execute_w041(repo_root, run_id);
        }
        if run_id.starts_with("w040-") || run_id.starts_with("test-w040-") {
            return self.execute_w040(repo_root, run_id);
        }
        if run_id.starts_with("w039-") || run_id.starts_with("test-w039-") {
            return self.execute_w039(repo_root, run_id);
        }

        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w036_independent = read_json(repo_root, W036_INDEPENDENT_SUMMARY)?;
        let w036_cross_engine = read_json(repo_root, W036_CROSS_ENGINE_SUMMARY)?;
        let w037_direct_oxfml = read_json(repo_root, W037_DIRECT_OXFML_SUMMARY)?;
        let w038_conformance = read_json(repo_root, W038_CONFORMANCE_SUMMARY)?;
        let w038_conformance_blockers = read_json(repo_root, W038_CONFORMANCE_BLOCKERS)?;
        let w038_formal = read_json(repo_root, W038_FORMAL_ASSURANCE_SUMMARY)?;
        let w038_stage2 = read_json(repo_root, W038_STAGE2_REPLAY_SUMMARY)?;
        let w038_operated = read_json(repo_root, W038_OPERATED_ASSURANCE_SUMMARY)?;

        let source_rows = source_rows(
            &w036_independent,
            &w036_cross_engine,
            &w037_direct_oxfml,
            &w038_conformance,
            &w038_formal,
            &w038_stage2,
            &w038_operated,
        );
        let diversity_rows = diversity_rows(&w036_independent, &w036_cross_engine);
        let seam_watch_rows = seam_watch_rows(
            repo_root,
            &w037_direct_oxfml,
            &w038_conformance,
            &w038_conformance_blockers,
            &w038_formal,
            &w038_stage2,
        );
        let blockers = exact_blockers();
        let source_failures = source_validation_failures(&source_rows);
        let seam_failures = seam_validation_failures(&seam_watch_rows);
        let failed_row_count = source_failures.len() + seam_failures.len();
        let accepted_boundary_count = diversity_rows
            .iter()
            .chain(seam_watch_rows.iter())
            .filter(|row| {
                row.get("disposition_kind").and_then(Value::as_str) == Some("accepted_boundary")
            })
            .count();
        let aligned_seam_watch_row_count = seam_watch_rows
            .iter()
            .filter(|row| row.get("watch_state").and_then(Value::as_str) == Some("aligned"))
            .count();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let diversity_disposition_path =
            format!("{relative_artifact_root}/implementation_diversity_disposition.json");
        let seam_watch_path = format!("{relative_artifact_root}/oxfml_seam_watch_packet.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/exact_diversity_seam_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w036_independent_summary": W036_INDEPENDENT_SUMMARY,
                "w036_cross_engine_summary": W036_CROSS_ENGINE_SUMMARY,
                "w037_direct_oxfml_summary": W037_DIRECT_OXFML_SUMMARY,
                "w038_conformance_summary": W038_CONFORMANCE_SUMMARY,
                "w038_conformance_blockers": W038_CONFORMANCE_BLOCKERS,
                "w038_formal_assurance_summary": W038_FORMAL_ASSURANCE_SUMMARY,
                "w038_stage2_replay_summary": W038_STAGE2_REPLAY_SUMMARY,
                "w038_operated_assurance_summary": W038_OPERATED_ASSURANCE_SUMMARY,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let diversity_disposition = json!({
            "schema_version": DIVERSITY_DISPOSITION_SCHEMA_V1,
            "run_id": run_id,
            "row_count": diversity_rows.len(),
            "fully_independent_evaluator_promoted": false,
            "rows": diversity_rows
        });
        let seam_watch = json!({
            "schema_version": OXFML_SEAM_WATCH_SCHEMA_V1,
            "run_id": run_id,
            "row_count": seam_watch_rows.len(),
            "aligned_row_count": aligned_seam_watch_row_count,
            "handoff_triggered": false,
            "rows": seam_watch_rows
        });
        let blocker_register = json!({
            "schema_version": BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w038_diversity_and_oxfml_seam_watch_bound_without_full_independence_promotion",
            "fully_independent_evaluator_promoted": false,
            "general_oxfunc_kernel_promoted": false,
            "callable_metadata_projection_promoted": false,
            "oxfml_handoff_triggered": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "diversity_disposition_row_count": diversity_rows.len(),
            "seam_watch_row_count": seam_watch_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This runner classifies implementation diversity and OxFml seam-watch evidence only. It does not change evaluator kernels, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let mut validation_failures = source_failures;
        validation_failures.extend(seam_failures);
        let validation = json!({
            "schema_version": VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w038_diversity_seam_packet_valid"
            } else {
                "w038_diversity_seam_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "diversity_disposition_row_count": diversity_rows.len(),
            "seam_watch_row_count": seam_watch_rows.len(),
            "aligned_seam_watch_row_count": aligned_seam_watch_row_count,
            "accepted_boundary_count": accepted_boundary_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "implementation_diversity_disposition_path": diversity_disposition_path,
            "oxfml_seam_watch_packet_path": seam_watch_path,
            "exact_diversity_seam_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "diversity_disposition_row_count": diversity_rows.len(),
            "seam_watch_row_count": seam_watch_rows.len(),
            "aligned_seam_watch_row_count": aligned_seam_watch_row_count,
            "accepted_boundary_count": accepted_boundary_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "oxfml_handoff_triggered": false
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("implementation_diversity_disposition.json"),
            &diversity_disposition,
        )?;
        write_json(
            &artifact_root.join("oxfml_seam_watch_packet.json"),
            &seam_watch,
        )?;
        write_json(
            &artifact_root.join("exact_diversity_seam_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: diversity_rows.len(),
            seam_watch_row_count: seam_watch_rows.len(),
            aligned_seam_watch_row_count,
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }

    fn execute_w045(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w045_residual_summary = read_json(repo_root, W045_RESIDUAL_SUMMARY)?;
        let w045_obligation_map = read_json(repo_root, W045_SUCCESSOR_OBLIGATION_MAP)?;
        let w045_promotion_contract = read_json(repo_root, W045_PROMOTION_CONTRACT_MAP)?;
        let w045_oxfml_intake = read_json(repo_root, W045_OXFML_INBOUND_INTAKE)?;
        let w044_diversity = read_json(repo_root, W044_DIVERSITY_SUMMARY)?;
        let w044_implementation = read_json(repo_root, W044_INDEPENDENT_IMPLEMENTATION)?;
        let w044_independent = read_json(repo_root, W044_INDEPENDENT_ROW_SET)?;
        let w044_cross_engine = read_json(repo_root, W044_CROSS_ENGINE_DIVERSITY)?;
        let w044_authority = read_json(repo_root, W044_MISMATCH_AUTHORITY)?;
        let w044_blockers = read_json(repo_root, W044_DIVERSITY_BLOCKERS)?;
        let w044_decision = read_json(repo_root, W044_DIVERSITY_DECISION)?;
        let w045_conformance = read_json(repo_root, W045_CONFORMANCE_SUMMARY)?;
        let w045_dynamic = read_json(repo_root, W045_DYNAMIC_TRANSITION_COVERAGE)?;
        let w045_callable = read_json(repo_root, W045_CALLABLE_METADATA_REGISTER)?;
        let w045_conformance_blockers = read_json(repo_root, W045_CONFORMANCE_BLOCKERS)?;
        let w045_match_guard = read_json(repo_root, W045_MATCH_PROMOTION_GUARD)?;
        let w045_rust = read_json(repo_root, W045_RUST_SUMMARY)?;
        let w045_rust_refinement = read_json(repo_root, W045_RUST_REFINEMENT_REGISTER)?;
        let w045_rust_blockers = read_json(repo_root, W045_RUST_BLOCKERS)?;
        let w045_lean_tla = read_json(repo_root, W045_LEAN_TLA_SUMMARY)?;
        let w045_tla_model_bound = read_json(repo_root, W045_TLA_MODEL_BOUND_REGISTER)?;
        let w045_lean_tla_blockers = read_json(repo_root, W045_LEAN_TLA_BLOCKERS)?;
        let w045_stage2 = read_json(repo_root, W045_STAGE2_SUMMARY)?;
        let w045_stage2_service_gate = read_json(repo_root, W045_STAGE2_SERVICE_GATE)?;
        let w045_stage2_blockers = read_json(repo_root, W045_STAGE2_BLOCKERS)?;
        let w045_stage2_decision = read_json(repo_root, W045_STAGE2_PROMOTION_DECISION)?;
        let w045_operated = read_json(repo_root, W045_OPERATED_ASSURANCE_SUMMARY)?;
        let w045_readiness = read_json(repo_root, W045_SERVICE_READINESS)?;
        let w045_harness = read_json(repo_root, W045_SERVICE_HARNESS_REGISTER)?;
        let w045_retained = read_json(repo_root, W045_RETAINED_HISTORY_QUERY)?;
        let w045_witness = read_json(repo_root, W045_RETAINED_WITNESS_LIFECYCLE)?;
        let w045_cross_engine_service = read_json(repo_root, W045_CROSS_ENGINE_SERVICE_REGISTER)?;
        let w045_alert_dispatch = read_json(repo_root, W045_ALERT_DISPATCH_REGISTER)?;
        let w045_service_blockers = read_json(repo_root, W045_OPERATED_SERVICE_BLOCKERS)?;
        let w045_operated_decision = read_json(repo_root, W045_OPERATED_PROMOTION_DECISION)?;
        let w044_pack_decision = read_json(repo_root, W044_PACK_DECISION)?;

        let implementation_rows = w045_independent_reference_model_rows();
        let source_rows = w045_source_rows(
            &w045_residual_summary,
            &w045_obligation_map,
            &w045_promotion_contract,
            &w045_oxfml_intake,
            &w044_diversity,
            &w044_implementation,
            &w044_independent,
            &w044_cross_engine,
            &w044_authority,
            &w044_blockers,
            &w044_decision,
            &w045_conformance,
            &w045_dynamic,
            &w045_callable,
            &w045_conformance_blockers,
            &w045_match_guard,
            &w045_rust,
            &w045_rust_refinement,
            &w045_rust_blockers,
            &w045_lean_tla,
            &w045_tla_model_bound,
            &w045_lean_tla_blockers,
            &w045_stage2,
            &w045_stage2_service_gate,
            &w045_stage2_blockers,
            &w045_stage2_decision,
            &w045_operated,
            &w045_readiness,
            &w045_harness,
            &w045_retained,
            &w045_witness,
            &w045_cross_engine_service,
            &w045_alert_dispatch,
            &w045_service_blockers,
            &w045_operated_decision,
            &w044_pack_decision,
        );
        let independent_rows = w045_independent_evaluator_rows(
            &w044_independent,
            &w044_implementation,
            &w044_diversity,
            &w045_conformance,
            &w045_dynamic,
            &w045_callable,
            &w045_rust,
            &w045_lean_tla,
            &w045_stage2,
            &w045_operated,
            &w045_readiness,
            &w045_harness,
            &w045_retained,
            &w045_witness,
            &w045_alert_dispatch,
            &w045_cross_engine_service,
            &implementation_rows,
        );
        let cross_engine_rows = w045_cross_engine_differential_rows(
            &w044_cross_engine,
            &w045_stage2,
            &w045_stage2_service_gate,
            &w045_operated,
            &w045_readiness,
            &w045_harness,
            &w045_retained,
            &w045_witness,
            &w045_cross_engine_service,
            &w045_alert_dispatch,
            &w045_oxfml_intake,
            &w045_dynamic,
            &w045_callable,
            &w044_pack_decision,
            &implementation_rows,
        );
        let authority_rows = w045_mismatch_authority_rows(
            &w044_authority,
            &w045_operated,
            &w045_readiness,
            &w045_harness,
            &w045_cross_engine_service,
            &w045_alert_dispatch,
            &w045_witness,
            &w045_stage2,
            &w045_stage2_service_gate,
            &w045_oxfml_intake,
            &w044_pack_decision,
            &implementation_rows,
        );
        let retained_attachment_rows = w045_retained_witness_attachment_rows(
            &w045_witness,
            &w045_retained,
            &w045_harness,
            &w045_alert_dispatch,
            &w045_cross_engine_service,
            &w045_stage2,
            &w044_pack_decision,
        );
        let differential_harness_rows = w045_operated_differential_harness_rows(&w045_harness);
        let blockers = w045_exact_blockers(
            &w044_blockers,
            &w045_stage2_blockers,
            &w045_service_blockers,
            &w045_cross_engine_service,
            &w045_conformance_blockers,
            &w045_lean_tla_blockers,
            &w045_rust_blockers,
            &w045_oxfml_intake,
        );
        let accepted_boundary_count = independent_rows
            .iter()
            .chain(cross_engine_rows.iter())
            .chain(authority_rows.iter())
            .chain(retained_attachment_rows.iter())
            .chain(differential_harness_rows.iter())
            .filter(|row| {
                matches!(
                    row.get("disposition_kind").and_then(Value::as_str),
                    Some("accepted_boundary")
                        | Some("accepted_external_slice")
                        | Some("broadened_independent_implementation")
                        | Some("service_contract_boundary")
                        | Some("local_harness_boundary")
                )
            })
            .count();
        let service_blocked_count = cross_engine_rows
            .iter()
            .chain(authority_rows.iter())
            .chain(retained_attachment_rows.iter())
            .filter(|row| {
                row.get("service_state").and_then(Value::as_str) == Some("blocked")
                    || row
                        .get("authority_state")
                        .and_then(Value::as_str)
                        .is_some_and(|state| state.starts_with("blocked"))
                    || row
                        .get("attachment_state")
                        .and_then(Value::as_str)
                        .is_some_and(|state| state.starts_with("blocked"))
            })
            .count();

        let mut validation_failures = source_validation_failures(&source_rows);
        validation_failures.extend(w045_diversity_validation_failures(
            &implementation_rows,
            &independent_rows,
            &cross_engine_rows,
            &authority_rows,
            &retained_attachment_rows,
            &differential_harness_rows,
            &blockers,
        ));
        let failed_row_count = validation_failures.len();
        let implementation_case_count = implementation_rows.len();
        let implementation_match_count = implementation_rows
            .iter()
            .filter(|row| bool_at(row, "matches_expected"))
            .count();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let independent_implementation_path = format!(
            "{relative_artifact_root}/w045_independent_reference_model_implementation.json"
        );
        let independent_row_set_path =
            format!("{relative_artifact_root}/w045_independent_evaluator_breadth_register.json");
        let cross_engine_path = format!(
            "{relative_artifact_root}/w045_cross_engine_differential_service_register.json"
        );
        let mismatch_authority_path =
            format!("{relative_artifact_root}/w045_mismatch_quarantine_authority_router.json");
        let retained_attachment_path =
            format!("{relative_artifact_root}/w045_retained_witness_attachment_register.json");
        let differential_harness_path = format!(
            "{relative_artifact_root}/w045_operated_differential_service_harness_register.json"
        );
        let blocker_register_path =
            format!("{relative_artifact_root}/w045_exact_diversity_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": W045_SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w045_residual_summary": W045_RESIDUAL_SUMMARY,
                "w045_successor_obligation_map": W045_SUCCESSOR_OBLIGATION_MAP,
                "w045_promotion_contract_map": W045_PROMOTION_CONTRACT_MAP,
                "w045_oxfml_inbound_intake": W045_OXFML_INBOUND_INTAKE,
                "w044_diversity_summary": W044_DIVERSITY_SUMMARY,
                "w044_independent_implementation": W044_INDEPENDENT_IMPLEMENTATION,
                "w044_independent_row_set": W044_INDEPENDENT_ROW_SET,
                "w044_cross_engine_differential": W044_CROSS_ENGINE_DIVERSITY,
                "w044_mismatch_authority": W044_MISMATCH_AUTHORITY,
                "w044_diversity_blockers": W044_DIVERSITY_BLOCKERS,
                "w044_diversity_decision": W044_DIVERSITY_DECISION,
                "w045_conformance_summary": W045_CONFORMANCE_SUMMARY,
                "w045_dynamic_transition_coverage": W045_DYNAMIC_TRANSITION_COVERAGE,
                "w045_callable_metadata_register": W045_CALLABLE_METADATA_REGISTER,
                "w045_conformance_blockers": W045_CONFORMANCE_BLOCKERS,
                "w045_match_promotion_guard": W045_MATCH_PROMOTION_GUARD,
                "w045_rust_summary": W045_RUST_SUMMARY,
                "w045_rust_refinement_register": W045_RUST_REFINEMENT_REGISTER,
                "w045_rust_blockers": W045_RUST_BLOCKERS,
                "w045_lean_tla_summary": W045_LEAN_TLA_SUMMARY,
                "w045_tla_model_bound_register": W045_TLA_MODEL_BOUND_REGISTER,
                "w045_lean_tla_blockers": W045_LEAN_TLA_BLOCKERS,
                "w045_stage2_summary": W045_STAGE2_SUMMARY,
                "w045_stage2_service_gate": W045_STAGE2_SERVICE_GATE,
                "w045_stage2_blockers": W045_STAGE2_BLOCKERS,
                "w045_stage2_promotion_decision": W045_STAGE2_PROMOTION_DECISION,
                "w045_operated_assurance_summary": W045_OPERATED_ASSURANCE_SUMMARY,
                "w045_service_readiness": W045_SERVICE_READINESS,
                "w045_service_harness_register": W045_SERVICE_HARNESS_REGISTER,
                "w045_retained_history_query": W045_RETAINED_HISTORY_QUERY,
                "w045_retained_witness_lifecycle": W045_RETAINED_WITNESS_LIFECYCLE,
                "w045_cross_engine_service_register": W045_CROSS_ENGINE_SERVICE_REGISTER,
                "w045_alert_dispatch_register": W045_ALERT_DISPATCH_REGISTER,
                "w045_operated_service_blockers": W045_OPERATED_SERVICE_BLOCKERS,
                "w045_operated_promotion_decision": W045_OPERATED_PROMOTION_DECISION,
                "w044_pack_decision": W044_PACK_DECISION,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let independent_implementation = json!({
            "schema_version": W045_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1,
            "run_id": run_id,
            "implementation_kind": "independent_reference_model_evaluator",
            "implementation_authority": "local_w045_diversity_runner_independent_from_tracecalc_optimized_core_treecalc_oxfml_and_oxfunc",
            "case_count": implementation_case_count,
            "match_count": implementation_match_count,
            "failed_case_count": implementation_case_count - implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, deterministic before/after edit checks, fan-in checksum, mismatch-routing, soft-reference rebind, retained-witness attachment controls, local harness boundary checks, and retained-history replay-correlation controls",
            "rows": implementation_rows
        });
        let independent_row_set = json!({
            "schema_version": W045_INDEPENDENT_EVALUATOR_SCHEMA_V1,
            "run_id": run_id,
            "row_count": independent_rows.len(),
            "w045_independent_reference_model_case_count": implementation_case_count,
            "w045_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "rows": independent_rows
        });
        let cross_engine = json!({
            "schema_version": W045_CROSS_ENGINE_DIVERSITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": cross_engine_rows.len(),
            "service_blocked_count": service_blocked_count,
            "w045_independent_reference_model_case_count": implementation_case_count,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "w073_downstream_typed_rule_request_construction_verified": false,
            "rows": cross_engine_rows
        });
        let mismatch_authority = json!({
            "schema_version": W045_MISMATCH_AUTHORITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "w045_independent_reference_model_case_count": implementation_case_count,
            "operated_service_authority_promoted": false,
            "fully_independent_evaluator_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "rows": authority_rows
        });
        let retained_attachment = json!({
            "schema_version": W045_RETAINED_WITNESS_ATTACHMENT_SCHEMA_V1,
            "run_id": run_id,
            "row_count": retained_attachment_rows.len(),
            "retained_witness_attachment_service_promoted": false,
            "retained_witness_lifecycle_service_promoted": false,
            "retention_slo_enforcement_promoted": false,
            "rows": retained_attachment_rows
        });
        let differential_harness = json!({
            "schema_version": W045_OPERATED_DIFFERENTIAL_HARNESS_SCHEMA_V1,
            "run_id": run_id,
            "row_count": differential_harness_rows.len(),
            "local_harness_runnable": bool_at(&w045_harness, "local_harness_runnable"),
            "service_endpoint_present": bool_at(&w045_harness, "service_endpoint_present"),
            "operated_cross_engine_differential_service_promoted": false,
            "rows": differential_harness_rows
        });
        let blocker_register = json!({
            "schema_version": W045_BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": W045_PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w045_independent_reference_model_and_local_harness_bound_without_full_diversity_or_service_promotion",
            "independent_reference_model_evaluator_present": true,
            "w045_independent_reference_model_case_count": implementation_case_count,
            "w045_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "independent_evaluator_breadth_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "cross_engine_diversity_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "local_harness_runnable": bool_at(&w045_harness, "local_harness_runnable"),
            "local_harness_promoted_as_service": false,
            "broad_oxfml_seam_promoted": false,
            "public_consumer_migration_promoted": false,
            "callable_metadata_projection_promoted": false,
            "w073_formatting_handoff_triggered": false,
            "w073_downstream_typed_rule_request_construction_verified": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "retained_witness_attachment_row_count": retained_attachment_rows.len(),
            "operated_differential_harness_row_count": differential_harness_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This W045 diversity runner widens the bounded independent named-reference model with local harness boundary, retained-witness attachment, mismatch-routing, and retained-history replay-correlation controls, binds W045 operated-assurance, Stage 2 service-gate, proof/model, optimized-core, W073, and W044 pack evidence, and classifies diversity, differential-service, mismatch-quarantine, retained-witness, and spec-evolution authority rows only. It does not change evaluator kernels used by OxCalc, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let validation = json!({
            "schema_version": W045_VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w045_independent_reference_model_diversity_packet_valid"
            } else {
                "w045_independent_reference_model_diversity_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "w045_independent_reference_model_case_count": implementation_case_count,
            "w045_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "retained_witness_attachment_row_count": retained_attachment_rows.len(),
            "operated_differential_harness_row_count": differential_harness_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": W045_RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "w045_independent_reference_model_implementation_path": independent_implementation_path,
            "w045_independent_evaluator_breadth_register_path": independent_row_set_path,
            "w045_cross_engine_differential_service_register_path": cross_engine_path,
            "w045_mismatch_quarantine_authority_router_path": mismatch_authority_path,
            "w045_retained_witness_attachment_register_path": retained_attachment_path,
            "w045_operated_differential_service_harness_register_path": differential_harness_path,
            "w045_exact_diversity_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "w045_independent_reference_model_case_count": implementation_case_count,
            "w045_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "retained_witness_attachment_row_count": retained_attachment_rows.len(),
            "operated_differential_harness_row_count": differential_harness_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "w073_downstream_typed_rule_request_construction_verified": false
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("w045_independent_reference_model_implementation.json"),
            &independent_implementation,
        )?;
        write_json(
            &artifact_root.join("w045_independent_evaluator_breadth_register.json"),
            &independent_row_set,
        )?;
        write_json(
            &artifact_root.join("w045_cross_engine_differential_service_register.json"),
            &cross_engine,
        )?;
        write_json(
            &artifact_root.join("w045_mismatch_quarantine_authority_router.json"),
            &mismatch_authority,
        )?;
        write_json(
            &artifact_root.join("w045_retained_witness_attachment_register.json"),
            &retained_attachment,
        )?;
        write_json(
            &artifact_root.join("w045_operated_differential_service_harness_register.json"),
            &differential_harness,
        )?;
        write_json(
            &artifact_root.join("w045_exact_diversity_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: W045_RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: independent_rows.len(),
            seam_watch_row_count: cross_engine_rows.len(),
            aligned_seam_watch_row_count: authority_rows.len(),
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }

    fn execute_w044(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w044_obligation_summary = read_json(repo_root, W044_OBLIGATION_SUMMARY)?;
        let w044_blocker_map = read_json(repo_root, W044_BLOCKER_RECLASSIFICATION_MAP)?;
        let w044_promotion_contract = read_json(repo_root, W044_PROMOTION_CONTRACT_MAP)?;
        let w044_oxfml_intake = read_json(repo_root, W044_OXFML_INBOUND_INTAKE)?;
        let w043_diversity = read_json(repo_root, W043_DIVERSITY_SUMMARY)?;
        let w043_implementation = read_json(repo_root, W043_INDEPENDENT_IMPLEMENTATION)?;
        let w043_independent = read_json(repo_root, W043_INDEPENDENT_ROW_SET)?;
        let w043_cross_engine = read_json(repo_root, W043_CROSS_ENGINE_DIVERSITY)?;
        let w043_authority = read_json(repo_root, W043_MISMATCH_AUTHORITY)?;
        let w043_blockers = read_json(repo_root, W043_DIVERSITY_BLOCKERS)?;
        let w043_decision = read_json(repo_root, W043_DIVERSITY_DECISION)?;
        let w043_pack_decision = read_json(repo_root, W043_PACK_DECISION)?;
        let w044_conformance = read_json(repo_root, W044_CONFORMANCE_SUMMARY)?;
        let w044_dynamic = read_json(repo_root, W044_DYNAMIC_TRANSITION_EVIDENCE)?;
        let w044_callable = read_json(repo_root, W044_CALLABLE_METADATA_REGISTER)?;
        let w044_conformance_blockers = read_json(repo_root, W044_CONFORMANCE_BLOCKERS)?;
        let w044_rust = read_json(repo_root, W044_RUST_SUMMARY)?;
        let w044_rust_refinement = read_json(repo_root, W044_RUST_REFINEMENT_REGISTER)?;
        let w044_lean_tla = read_json(repo_root, W044_LEAN_TLA_SUMMARY)?;
        let w044_tla_model_bound = read_json(repo_root, W044_TLA_MODEL_BOUND_REGISTER)?;
        let w044_stage2 = read_json(repo_root, W044_STAGE2_SUMMARY)?;
        let w044_stage2_blockers = read_json(repo_root, W044_STAGE2_BLOCKERS)?;
        let w044_stage2_decision = read_json(repo_root, W044_STAGE2_PROMOTION_DECISION)?;
        let w044_operated = read_json(repo_root, W044_OPERATED_ASSURANCE_SUMMARY)?;
        let w044_readiness = read_json(repo_root, W044_SERVICE_READINESS)?;
        let w044_retained = read_json(repo_root, W044_RETAINED_HISTORY_QUERY)?;
        let w044_witness = read_json(repo_root, W044_RETAINED_WITNESS_LIFECYCLE)?;
        let w044_cross_engine_service = read_json(repo_root, W044_CROSS_ENGINE_SERVICE_REGISTER)?;
        let w044_alert_dispatch = read_json(repo_root, W044_ALERT_DISPATCH_REGISTER)?;
        let w044_service_blockers = read_json(repo_root, W044_OPERATED_SERVICE_BLOCKERS)?;
        let w044_operated_decision = read_json(repo_root, W044_OPERATED_PROMOTION_DECISION)?;

        let implementation_rows = w044_independent_reference_model_rows();
        let source_rows = w044_source_rows(
            &w044_obligation_summary,
            &w044_blocker_map,
            &w044_promotion_contract,
            &w044_oxfml_intake,
            &w043_diversity,
            &w043_implementation,
            &w043_independent,
            &w043_cross_engine,
            &w043_authority,
            &w043_blockers,
            &w043_decision,
            &w044_conformance,
            &w044_dynamic,
            &w044_callable,
            &w044_rust,
            &w044_rust_refinement,
            &w044_lean_tla,
            &w044_stage2,
            &w044_stage2_blockers,
            &w044_stage2_decision,
            &w044_operated,
            &w044_readiness,
            &w044_retained,
            &w044_witness,
            &w044_cross_engine_service,
            &w044_alert_dispatch,
            &w044_service_blockers,
            &w044_operated_decision,
            &w043_pack_decision,
        );
        let independent_rows = w044_independent_evaluator_rows(
            &w043_independent,
            &w043_implementation,
            &w043_diversity,
            &w044_conformance,
            &w044_dynamic,
            &w044_rust,
            &w044_lean_tla,
            &w044_stage2,
            &w044_operated,
            &implementation_rows,
        );
        let cross_engine_rows = w044_cross_engine_differential_rows(
            &w043_cross_engine,
            &w044_stage2,
            &w044_operated,
            &w044_readiness,
            &w044_retained,
            &w044_witness,
            &w044_cross_engine_service,
            &w044_alert_dispatch,
            &w044_oxfml_intake,
            &w044_dynamic,
            &w044_callable,
            &implementation_rows,
        );
        let authority_rows = w044_mismatch_authority_rows(
            &w043_authority,
            &w044_operated,
            &w044_cross_engine_service,
            &w044_alert_dispatch,
            &w044_witness,
            &w044_stage2,
            &implementation_rows,
        );
        let blockers = w044_exact_blockers(
            &w043_blockers,
            &w044_stage2_blockers,
            &w044_service_blockers,
            &w044_cross_engine_service,
            &w044_conformance_blockers,
            &w044_tla_model_bound,
        );
        let accepted_boundary_count = independent_rows
            .iter()
            .chain(cross_engine_rows.iter())
            .chain(authority_rows.iter())
            .filter(|row| {
                matches!(
                    row.get("disposition_kind").and_then(Value::as_str),
                    Some("accepted_boundary")
                        | Some("accepted_external_slice")
                        | Some("broadened_independent_implementation")
                        | Some("service_contract_boundary")
                )
            })
            .count();
        let service_blocked_count = cross_engine_rows
            .iter()
            .chain(authority_rows.iter())
            .filter(|row| {
                row.get("service_state").and_then(Value::as_str) == Some("blocked")
                    || row
                        .get("authority_state")
                        .and_then(Value::as_str)
                        .is_some_and(|state| state.starts_with("blocked"))
            })
            .count();

        let mut validation_failures = source_validation_failures(&source_rows);
        validation_failures.extend(w044_diversity_validation_failures(
            &implementation_rows,
            &independent_rows,
            &cross_engine_rows,
            &authority_rows,
            &blockers,
        ));
        let failed_row_count = validation_failures.len();
        let implementation_case_count = implementation_rows.len();
        let implementation_match_count = implementation_rows
            .iter()
            .filter(|row| bool_at(row, "matches_expected"))
            .count();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let independent_implementation_path = format!(
            "{relative_artifact_root}/w044_independent_reference_model_implementation.json"
        );
        let independent_row_set_path =
            format!("{relative_artifact_root}/w044_independent_evaluator_breadth_register.json");
        let cross_engine_path = format!(
            "{relative_artifact_root}/w044_cross_engine_differential_service_register.json"
        );
        let mismatch_authority_path =
            format!("{relative_artifact_root}/w044_mismatch_quarantine_authority_router.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/w044_exact_diversity_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": W044_SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w044_obligation_summary": W044_OBLIGATION_SUMMARY,
                "w044_blocker_reclassification_map": W044_BLOCKER_RECLASSIFICATION_MAP,
                "w044_promotion_contract_map": W044_PROMOTION_CONTRACT_MAP,
                "w044_oxfml_inbound_intake": W044_OXFML_INBOUND_INTAKE,
                "w043_diversity_summary": W043_DIVERSITY_SUMMARY,
                "w043_independent_implementation": W043_INDEPENDENT_IMPLEMENTATION,
                "w043_independent_row_set": W043_INDEPENDENT_ROW_SET,
                "w043_cross_engine_differential": W043_CROSS_ENGINE_DIVERSITY,
                "w043_mismatch_authority": W043_MISMATCH_AUTHORITY,
                "w043_diversity_blockers": W043_DIVERSITY_BLOCKERS,
                "w043_diversity_decision": W043_DIVERSITY_DECISION,
                "w043_pack_decision": W043_PACK_DECISION,
                "w044_conformance_summary": W044_CONFORMANCE_SUMMARY,
                "w044_dynamic_transition_evidence": W044_DYNAMIC_TRANSITION_EVIDENCE,
                "w044_callable_metadata_register": W044_CALLABLE_METADATA_REGISTER,
                "w044_conformance_blockers": W044_CONFORMANCE_BLOCKERS,
                "w044_rust_summary": W044_RUST_SUMMARY,
                "w044_rust_refinement_register": W044_RUST_REFINEMENT_REGISTER,
                "w044_lean_tla_summary": W044_LEAN_TLA_SUMMARY,
                "w044_tla_model_bound_register": W044_TLA_MODEL_BOUND_REGISTER,
                "w044_stage2_summary": W044_STAGE2_SUMMARY,
                "w044_stage2_blockers": W044_STAGE2_BLOCKERS,
                "w044_stage2_promotion_decision": W044_STAGE2_PROMOTION_DECISION,
                "w044_operated_assurance_summary": W044_OPERATED_ASSURANCE_SUMMARY,
                "w044_service_readiness": W044_SERVICE_READINESS,
                "w044_retained_history_query": W044_RETAINED_HISTORY_QUERY,
                "w044_retained_witness_lifecycle": W044_RETAINED_WITNESS_LIFECYCLE,
                "w044_cross_engine_service_register": W044_CROSS_ENGINE_SERVICE_REGISTER,
                "w044_alert_dispatch_register": W044_ALERT_DISPATCH_REGISTER,
                "w044_operated_service_blockers": W044_OPERATED_SERVICE_BLOCKERS,
                "w044_operated_promotion_decision": W044_OPERATED_PROMOTION_DECISION,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let independent_implementation = json!({
            "schema_version": W044_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1,
            "run_id": run_id,
            "implementation_kind": "independent_reference_model_evaluator",
            "implementation_authority": "local_w044_diversity_runner_independent_from_tracecalc_optimized_core_treecalc_oxfml_and_oxfunc",
            "case_count": implementation_case_count,
            "match_count": implementation_match_count,
            "failed_case_count": implementation_case_count - implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, deterministic before/after edit checks, fan-in checksum, mismatch-routing, soft-reference rebind, and retained-witness attachment controls",
            "rows": implementation_rows
        });
        let independent_row_set = json!({
            "schema_version": W044_INDEPENDENT_EVALUATOR_SCHEMA_V1,
            "run_id": run_id,
            "row_count": independent_rows.len(),
            "w044_independent_reference_model_case_count": implementation_case_count,
            "w044_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "rows": independent_rows
        });
        let cross_engine = json!({
            "schema_version": W044_CROSS_ENGINE_DIVERSITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": cross_engine_rows.len(),
            "service_blocked_count": service_blocked_count,
            "w044_independent_reference_model_case_count": implementation_case_count,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "w073_downstream_typed_rule_request_construction_verified": false,
            "rows": cross_engine_rows
        });
        let mismatch_authority = json!({
            "schema_version": W044_MISMATCH_AUTHORITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "w044_independent_reference_model_case_count": implementation_case_count,
            "operated_service_authority_promoted": false,
            "fully_independent_evaluator_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "rows": authority_rows
        });
        let blocker_register = json!({
            "schema_version": W044_BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": W044_PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w044_independent_reference_model_extended_without_full_diversity_or_service_promotion",
            "independent_reference_model_evaluator_present": true,
            "w044_independent_reference_model_case_count": implementation_case_count,
            "w044_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "independent_evaluator_breadth_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "cross_engine_diversity_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "broad_oxfml_seam_promoted": false,
            "public_consumer_migration_promoted": false,
            "callable_metadata_projection_promoted": false,
            "w073_formatting_handoff_triggered": false,
            "w073_downstream_typed_rule_request_construction_verified": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This W044 diversity runner extends the bounded independent named-reference model with soft-reference rebind and retained-witness attachment controls, binds W044 operated-assurance, Stage 2, proof/model, and optimized-core evidence, and classifies diversity, differential-service, mismatch-quarantine, retained-witness, and spec-evolution authority rows only. It does not change evaluator kernels used by OxCalc, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let validation = json!({
            "schema_version": W044_VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w044_independent_reference_model_diversity_packet_valid"
            } else {
                "w044_independent_reference_model_diversity_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "w044_independent_reference_model_case_count": implementation_case_count,
            "w044_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": W044_RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "w044_independent_reference_model_implementation_path": independent_implementation_path,
            "w044_independent_evaluator_breadth_register_path": independent_row_set_path,
            "w044_cross_engine_differential_service_register_path": cross_engine_path,
            "w044_mismatch_quarantine_authority_router_path": mismatch_authority_path,
            "w044_exact_diversity_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "w044_independent_reference_model_case_count": implementation_case_count,
            "w044_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "retained_witness_attachment_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "w073_downstream_typed_rule_request_construction_verified": false
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("w044_independent_reference_model_implementation.json"),
            &independent_implementation,
        )?;
        write_json(
            &artifact_root.join("w044_independent_evaluator_breadth_register.json"),
            &independent_row_set,
        )?;
        write_json(
            &artifact_root.join("w044_cross_engine_differential_service_register.json"),
            &cross_engine,
        )?;
        write_json(
            &artifact_root.join("w044_mismatch_quarantine_authority_router.json"),
            &mismatch_authority,
        )?;
        write_json(
            &artifact_root.join("w044_exact_diversity_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: W044_RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: independent_rows.len(),
            seam_watch_row_count: cross_engine_rows.len(),
            aligned_seam_watch_row_count: authority_rows.len(),
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }

    fn execute_w043(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w043_obligation_summary = read_json(repo_root, W043_OBLIGATION_SUMMARY)?;
        let w043_obligation_map = read_json(repo_root, W043_OBLIGATION_MAP)?;
        let w043_oxfml_intake = read_json(repo_root, W043_OXFML_INBOUND_INTAKE)?;
        let w042_diversity = read_json(repo_root, W042_DIVERSITY_SUMMARY)?;
        let w042_implementation = read_json(repo_root, W042_INDEPENDENT_IMPLEMENTATION)?;
        let w042_independent = read_json(repo_root, W042_INDEPENDENT_ROW_SET)?;
        let w042_cross_engine = read_json(repo_root, W042_CROSS_ENGINE_DIVERSITY)?;
        let w042_authority = read_json(repo_root, W042_MISMATCH_AUTHORITY)?;
        let w042_blockers = read_json(repo_root, W042_DIVERSITY_BLOCKERS)?;
        let w042_decision = read_json(repo_root, W042_DIVERSITY_DECISION)?;
        let w043_stage2 = read_json(repo_root, W043_STAGE2_SUMMARY)?;
        let w043_stage2_blockers = read_json(repo_root, W043_STAGE2_BLOCKERS)?;
        let w043_operated = read_json(repo_root, W043_OPERATED_ASSURANCE_SUMMARY)?;
        let w043_retained = read_json(repo_root, W043_RETAINED_HISTORY_QUERY)?;
        let w043_witness = read_json(repo_root, W043_RETAINED_WITNESS_LIFECYCLE)?;
        let w043_cross_engine_service = read_json(repo_root, W043_CROSS_ENGINE_SERVICE_REGISTER)?;
        let w043_alert_dispatch = read_json(repo_root, W043_ALERT_DISPATCH_REGISTER)?;
        let w043_service_blockers = read_json(repo_root, W043_OPERATED_SERVICE_BLOCKERS)?;
        let w043_operated_decision = read_json(repo_root, W043_OPERATED_PROMOTION_DECISION)?;
        let w042_pack_decision = read_json(repo_root, W042_PACK_DECISION)?;

        let implementation_rows = w043_independent_reference_model_rows();
        let source_rows = w043_source_rows(
            &w043_obligation_summary,
            &w043_obligation_map,
            &w043_oxfml_intake,
            &w042_diversity,
            &w042_implementation,
            &w042_independent,
            &w042_cross_engine,
            &w042_authority,
            &w042_blockers,
            &w042_decision,
            &w043_stage2,
            &w043_operated,
            &w043_retained,
            &w043_witness,
            &w043_cross_engine_service,
            &w043_alert_dispatch,
            &w043_service_blockers,
            &w043_operated_decision,
            &w042_pack_decision,
        );
        let independent_rows = w043_independent_evaluator_rows(
            &w042_independent,
            &w042_implementation,
            &w042_diversity,
            &w043_stage2,
            &w043_operated,
            &implementation_rows,
        );
        let cross_engine_rows = w043_cross_engine_differential_rows(
            &w042_cross_engine,
            &w043_stage2,
            &w043_operated,
            &w043_retained,
            &w043_witness,
            &w043_cross_engine_service,
            &w043_alert_dispatch,
            &w043_oxfml_intake,
            &implementation_rows,
        );
        let authority_rows = w043_mismatch_authority_rows(
            &w042_authority,
            &w043_operated,
            &w043_cross_engine_service,
            &w043_alert_dispatch,
            &implementation_rows,
        );
        let blockers = w043_exact_blockers(
            &w042_blockers,
            &w043_stage2_blockers,
            &w043_service_blockers,
            &w043_cross_engine_service,
        );
        let accepted_boundary_count = independent_rows
            .iter()
            .chain(cross_engine_rows.iter())
            .chain(authority_rows.iter())
            .filter(|row| {
                matches!(
                    row.get("disposition_kind").and_then(Value::as_str),
                    Some("accepted_boundary")
                        | Some("accepted_external_slice")
                        | Some("broadened_independent_implementation")
                        | Some("service_contract_boundary")
                )
            })
            .count();
        let service_blocked_count = cross_engine_rows
            .iter()
            .chain(authority_rows.iter())
            .filter(|row| {
                row.get("service_state").and_then(Value::as_str) == Some("blocked")
                    || row
                        .get("authority_state")
                        .and_then(Value::as_str)
                        .is_some_and(|state| state.starts_with("blocked"))
            })
            .count();

        let mut validation_failures = source_validation_failures(&source_rows);
        validation_failures.extend(w043_diversity_validation_failures(
            &implementation_rows,
            &independent_rows,
            &cross_engine_rows,
            &authority_rows,
            &blockers,
        ));
        let failed_row_count = validation_failures.len();
        let implementation_case_count = implementation_rows.len();
        let implementation_match_count = implementation_rows
            .iter()
            .filter(|row| bool_at(row, "matches_expected"))
            .count();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let independent_implementation_path = format!(
            "{relative_artifact_root}/w043_independent_reference_model_implementation.json"
        );
        let independent_row_set_path =
            format!("{relative_artifact_root}/w043_independent_evaluator_breadth_register.json");
        let cross_engine_path = format!(
            "{relative_artifact_root}/w043_cross_engine_differential_service_register.json"
        );
        let mismatch_authority_path =
            format!("{relative_artifact_root}/w043_mismatch_quarantine_authority_router.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/w043_exact_diversity_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": W043_SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w043_obligation_summary": W043_OBLIGATION_SUMMARY,
                "w043_obligation_map": W043_OBLIGATION_MAP,
                "w043_oxfml_inbound_intake": W043_OXFML_INBOUND_INTAKE,
                "w042_diversity_summary": W042_DIVERSITY_SUMMARY,
                "w042_independent_implementation": W042_INDEPENDENT_IMPLEMENTATION,
                "w042_independent_row_set": W042_INDEPENDENT_ROW_SET,
                "w042_cross_engine_differential": W042_CROSS_ENGINE_DIVERSITY,
                "w042_mismatch_authority": W042_MISMATCH_AUTHORITY,
                "w042_diversity_blockers": W042_DIVERSITY_BLOCKERS,
                "w042_diversity_decision": W042_DIVERSITY_DECISION,
                "w043_stage2_summary": W043_STAGE2_SUMMARY,
                "w043_stage2_blockers": W043_STAGE2_BLOCKERS,
                "w043_operated_assurance_summary": W043_OPERATED_ASSURANCE_SUMMARY,
                "w043_retained_history_query": W043_RETAINED_HISTORY_QUERY,
                "w043_retained_witness_lifecycle": W043_RETAINED_WITNESS_LIFECYCLE,
                "w043_cross_engine_service_register": W043_CROSS_ENGINE_SERVICE_REGISTER,
                "w043_alert_dispatch_register": W043_ALERT_DISPATCH_REGISTER,
                "w043_operated_service_blockers": W043_OPERATED_SERVICE_BLOCKERS,
                "w043_operated_promotion_decision": W043_OPERATED_PROMOTION_DECISION,
                "w042_pack_decision": W042_PACK_DECISION,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let independent_implementation = json!({
            "schema_version": W043_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1,
            "run_id": run_id,
            "implementation_kind": "independent_reference_model_evaluator",
            "implementation_authority": "local_w043_diversity_runner_independent_from_tracecalc_optimized_core_treecalc_oxfml_and_oxfunc",
            "case_count": implementation_case_count,
            "match_count": implementation_match_count,
            "failed_case_count": implementation_case_count - implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, deterministic before/after edit checks, fan-in checksum rows, and mismatch-routing control rows",
            "rows": implementation_rows
        });
        let independent_row_set = json!({
            "schema_version": W043_INDEPENDENT_EVALUATOR_SCHEMA_V1,
            "run_id": run_id,
            "row_count": independent_rows.len(),
            "w043_independent_reference_model_case_count": implementation_case_count,
            "w043_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "rows": independent_rows
        });
        let cross_engine = json!({
            "schema_version": W043_CROSS_ENGINE_DIVERSITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": cross_engine_rows.len(),
            "service_blocked_count": service_blocked_count,
            "w043_independent_reference_model_case_count": implementation_case_count,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "rows": cross_engine_rows
        });
        let mismatch_authority = json!({
            "schema_version": W043_MISMATCH_AUTHORITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "w043_independent_reference_model_case_count": implementation_case_count,
            "operated_service_authority_promoted": false,
            "fully_independent_evaluator_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "rows": authority_rows
        });
        let blocker_register = json!({
            "schema_version": W043_BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": W043_PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w043_independent_reference_model_extended_without_full_diversity_or_service_promotion",
            "independent_reference_model_evaluator_present": true,
            "w043_independent_reference_model_case_count": implementation_case_count,
            "w043_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "independent_evaluator_breadth_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "cross_engine_diversity_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "broad_oxfml_seam_promoted": false,
            "callable_metadata_projection_promoted": false,
            "w073_formatting_handoff_triggered": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This W043 diversity runner extends the bounded independent named-reference model, binds W043 operated-assurance and Stage 2 evidence, and classifies diversity, differential-service, mismatch-quarantine, and spec-evolution authority rows only. It does not change evaluator kernels used by OxCalc, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let validation = json!({
            "schema_version": W043_VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w043_independent_reference_model_diversity_packet_valid"
            } else {
                "w043_independent_reference_model_diversity_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "w043_independent_reference_model_case_count": implementation_case_count,
            "w043_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": W043_RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "w043_independent_reference_model_implementation_path": independent_implementation_path,
            "w043_independent_evaluator_breadth_register_path": independent_row_set_path,
            "w043_cross_engine_differential_service_register_path": cross_engine_path,
            "w043_mismatch_quarantine_authority_router_path": mismatch_authority_path,
            "w043_exact_diversity_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "w043_independent_reference_model_case_count": implementation_case_count,
            "w043_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("w043_independent_reference_model_implementation.json"),
            &independent_implementation,
        )?;
        write_json(
            &artifact_root.join("w043_independent_evaluator_breadth_register.json"),
            &independent_row_set,
        )?;
        write_json(
            &artifact_root.join("w043_cross_engine_differential_service_register.json"),
            &cross_engine,
        )?;
        write_json(
            &artifact_root.join("w043_mismatch_quarantine_authority_router.json"),
            &mismatch_authority,
        )?;
        write_json(
            &artifact_root.join("w043_exact_diversity_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: W043_RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: independent_rows.len(),
            seam_watch_row_count: cross_engine_rows.len(),
            aligned_seam_watch_row_count: authority_rows.len(),
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }

    fn execute_w042(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w042_obligation_summary = read_json(repo_root, W042_OBLIGATION_SUMMARY)?;
        let w042_obligation_map = read_json(repo_root, W042_OBLIGATION_MAP)?;
        let w042_formatting_intake = read_json(repo_root, W042_FORMATTING_INTAKE)?;
        let w041_diversity = read_json(repo_root, W041_DIVERSITY_SUMMARY)?;
        let w041_implementation = read_json(repo_root, W041_INDEPENDENT_IMPLEMENTATION)?;
        let w041_independent = read_json(repo_root, W041_INDEPENDENT_ROW_SET)?;
        let w041_cross_engine = read_json(repo_root, W041_CROSS_ENGINE_DIVERSITY)?;
        let w041_authority = read_json(repo_root, W041_MISMATCH_AUTHORITY)?;
        let w041_blockers = read_json(repo_root, W041_DIVERSITY_BLOCKERS)?;
        let w041_decision = read_json(repo_root, W041_DIVERSITY_DECISION)?;
        let w042_stage2 = read_json(repo_root, W042_STAGE2_SUMMARY)?;
        let w042_stage2_blockers = read_json(repo_root, W042_STAGE2_BLOCKERS)?;
        let w042_operated = read_json(repo_root, W042_OPERATED_ASSURANCE_SUMMARY)?;
        let w042_retained = read_json(repo_root, W042_RETAINED_HISTORY_QUERY)?;
        let w042_witness = read_json(repo_root, W042_RETAINED_WITNESS_LIFECYCLE)?;
        let w042_cross_engine_service = read_json(repo_root, W042_CROSS_ENGINE_SERVICE_REGISTER)?;
        let w042_alert_dispatch = read_json(repo_root, W042_ALERT_DISPATCH_REGISTER)?;
        let w042_service_blockers = read_json(repo_root, W042_OPERATED_SERVICE_BLOCKERS)?;
        let w042_operated_decision = read_json(repo_root, W042_OPERATED_PROMOTION_DECISION)?;
        let w041_pack_decision = read_json(repo_root, W041_PACK_DECISION)?;

        let implementation_rows = w042_independent_reference_model_rows();
        let source_rows = w042_source_rows(
            &w042_obligation_summary,
            &w042_obligation_map,
            &w042_formatting_intake,
            &w041_diversity,
            &w041_implementation,
            &w041_independent,
            &w041_cross_engine,
            &w041_authority,
            &w041_blockers,
            &w041_decision,
            &w042_stage2,
            &w042_operated,
            &w042_retained,
            &w042_witness,
            &w042_cross_engine_service,
            &w042_alert_dispatch,
            &w042_service_blockers,
            &w042_operated_decision,
            &w041_pack_decision,
        );
        let independent_rows = w042_independent_evaluator_rows(
            &w041_independent,
            &w041_implementation,
            &w041_diversity,
            &w042_stage2,
            &w042_operated,
            &implementation_rows,
        );
        let cross_engine_rows = w042_cross_engine_differential_rows(
            &w041_cross_engine,
            &w042_stage2,
            &w042_operated,
            &w042_retained,
            &w042_witness,
            &w042_cross_engine_service,
            &w042_alert_dispatch,
            &w042_formatting_intake,
            &implementation_rows,
        );
        let authority_rows = w042_mismatch_authority_rows(
            &w041_authority,
            &w042_operated,
            &w042_cross_engine_service,
            &w042_alert_dispatch,
            &implementation_rows,
        );
        let blockers = w042_exact_blockers(
            &w041_blockers,
            &w042_stage2_blockers,
            &w042_service_blockers,
            &w042_cross_engine_service,
        );
        let accepted_boundary_count = independent_rows
            .iter()
            .chain(cross_engine_rows.iter())
            .chain(authority_rows.iter())
            .filter(|row| {
                matches!(
                    row.get("disposition_kind").and_then(Value::as_str),
                    Some("accepted_boundary")
                        | Some("accepted_external_slice")
                        | Some("broadened_independent_implementation")
                        | Some("service_contract_boundary")
                )
            })
            .count();
        let service_blocked_count = cross_engine_rows
            .iter()
            .chain(authority_rows.iter())
            .filter(|row| {
                row.get("service_state").and_then(Value::as_str) == Some("blocked")
                    || row
                        .get("authority_state")
                        .and_then(Value::as_str)
                        .is_some_and(|state| state.starts_with("blocked"))
            })
            .count();

        let mut validation_failures = source_validation_failures(&source_rows);
        validation_failures.extend(w042_diversity_validation_failures(
            &implementation_rows,
            &independent_rows,
            &cross_engine_rows,
            &authority_rows,
            &blockers,
        ));
        let failed_row_count = validation_failures.len();
        let implementation_case_count = implementation_rows.len();
        let implementation_match_count = implementation_rows
            .iter()
            .filter(|row| bool_at(row, "matches_expected"))
            .count();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let independent_implementation_path = format!(
            "{relative_artifact_root}/w042_independent_reference_model_implementation.json"
        );
        let independent_row_set_path =
            format!("{relative_artifact_root}/w042_independent_evaluator_breadth_register.json");
        let cross_engine_path = format!(
            "{relative_artifact_root}/w042_cross_engine_differential_service_register.json"
        );
        let mismatch_authority_path =
            format!("{relative_artifact_root}/w042_mismatch_quarantine_authority_router.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/w042_exact_diversity_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": W042_SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w042_obligation_summary": W042_OBLIGATION_SUMMARY,
                "w042_obligation_map": W042_OBLIGATION_MAP,
                "w042_formatting_intake": W042_FORMATTING_INTAKE,
                "w041_diversity_summary": W041_DIVERSITY_SUMMARY,
                "w041_independent_implementation": W041_INDEPENDENT_IMPLEMENTATION,
                "w041_independent_row_set": W041_INDEPENDENT_ROW_SET,
                "w041_cross_engine_differential": W041_CROSS_ENGINE_DIVERSITY,
                "w041_mismatch_authority": W041_MISMATCH_AUTHORITY,
                "w041_diversity_blockers": W041_DIVERSITY_BLOCKERS,
                "w041_diversity_decision": W041_DIVERSITY_DECISION,
                "w042_stage2_summary": W042_STAGE2_SUMMARY,
                "w042_stage2_blockers": W042_STAGE2_BLOCKERS,
                "w042_operated_assurance_summary": W042_OPERATED_ASSURANCE_SUMMARY,
                "w042_retained_history_query": W042_RETAINED_HISTORY_QUERY,
                "w042_retained_witness_lifecycle": W042_RETAINED_WITNESS_LIFECYCLE,
                "w042_cross_engine_service_register": W042_CROSS_ENGINE_SERVICE_REGISTER,
                "w042_alert_dispatch_register": W042_ALERT_DISPATCH_REGISTER,
                "w042_operated_service_blockers": W042_OPERATED_SERVICE_BLOCKERS,
                "w042_operated_promotion_decision": W042_OPERATED_PROMOTION_DECISION,
                "w041_pack_decision": W041_PACK_DECISION,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let independent_implementation = json!({
            "schema_version": W042_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1,
            "run_id": run_id,
            "implementation_kind": "independent_reference_model_evaluator",
            "implementation_authority": "local_w042_diversity_runner_independent_from_tracecalc_optimized_core_treecalc_oxfml_and_oxfunc",
            "case_count": implementation_case_count,
            "match_count": implementation_match_count,
            "failed_case_count": implementation_case_count - implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, and deterministic before/after edit checks",
            "rows": implementation_rows
        });
        let independent_row_set = json!({
            "schema_version": W042_INDEPENDENT_EVALUATOR_SCHEMA_V1,
            "run_id": run_id,
            "row_count": independent_rows.len(),
            "w042_independent_reference_model_case_count": implementation_case_count,
            "w042_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "rows": independent_rows
        });
        let cross_engine = json!({
            "schema_version": W042_CROSS_ENGINE_DIVERSITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": cross_engine_rows.len(),
            "service_blocked_count": service_blocked_count,
            "w042_independent_reference_model_case_count": implementation_case_count,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "rows": cross_engine_rows
        });
        let mismatch_authority = json!({
            "schema_version": W042_MISMATCH_AUTHORITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "w042_independent_reference_model_case_count": implementation_case_count,
            "operated_service_authority_promoted": false,
            "fully_independent_evaluator_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "rows": authority_rows
        });
        let blocker_register = json!({
            "schema_version": W042_BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": W042_PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w042_independent_reference_model_bound_without_full_diversity_or_service_promotion",
            "independent_reference_model_evaluator_present": true,
            "w042_independent_reference_model_case_count": implementation_case_count,
            "w042_independent_reference_model_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "independent_evaluator_breadth_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "cross_engine_diversity_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "broad_oxfml_seam_promoted": false,
            "callable_metadata_projection_promoted": false,
            "w073_formatting_handoff_triggered": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This W042 diversity runner adds an independent named-reference model evaluator and classifies diversity, differential service, and mismatch-quarantine authority evidence only. It does not change evaluator kernels used by OxCalc, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let validation = json!({
            "schema_version": W042_VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w042_independent_reference_model_diversity_packet_valid"
            } else {
                "w042_independent_reference_model_diversity_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "w042_independent_reference_model_case_count": implementation_case_count,
            "w042_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": W042_RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "w042_independent_reference_model_implementation_path": independent_implementation_path,
            "w042_independent_evaluator_breadth_register_path": independent_row_set_path,
            "w042_cross_engine_differential_service_register_path": cross_engine_path,
            "w042_mismatch_quarantine_authority_router_path": mismatch_authority_path,
            "w042_exact_diversity_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "w042_independent_reference_model_case_count": implementation_case_count,
            "w042_independent_reference_model_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("w042_independent_reference_model_implementation.json"),
            &independent_implementation,
        )?;
        write_json(
            &artifact_root.join("w042_independent_evaluator_breadth_register.json"),
            &independent_row_set,
        )?;
        write_json(
            &artifact_root.join("w042_cross_engine_differential_service_register.json"),
            &cross_engine,
        )?;
        write_json(
            &artifact_root.join("w042_mismatch_quarantine_authority_router.json"),
            &mismatch_authority,
        )?;
        write_json(
            &artifact_root.join("w042_exact_diversity_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: W042_RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: independent_rows.len(),
            seam_watch_row_count: cross_engine_rows.len(),
            aligned_seam_watch_row_count: authority_rows.len(),
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }

    fn execute_w041(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w041_obligation_summary = read_json(repo_root, W041_OBLIGATION_SUMMARY)?;
        let w041_obligation_map = read_json(repo_root, W041_OBLIGATION_MAP)?;
        let w040_formatting_intake = read_json(repo_root, W040_FORMATTING_INTAKE)?;
        let w040_diversity = read_json(repo_root, W040_DIVERSITY_SUMMARY)?;
        let w040_implementation = read_json(repo_root, W040_INDEPENDENT_IMPLEMENTATION)?;
        let w040_independent = read_json(repo_root, W040_INDEPENDENT_ROW_SET)?;
        let w040_cross_engine = read_json(repo_root, W040_CROSS_ENGINE_DIVERSITY)?;
        let w040_authority = read_json(repo_root, W040_DIFFERENTIAL_AUTHORITY)?;
        let w040_blockers = read_json(repo_root, W040_DIVERSITY_BLOCKERS)?;
        let w040_decision = read_json(repo_root, W040_DIVERSITY_DECISION)?;
        let w041_conformance = read_json(repo_root, W041_CONFORMANCE_SUMMARY)?;
        let w041_treecalc = read_json(repo_root, W041_TREECALC_SUMMARY)?;
        let w041_rust = read_json(repo_root, W041_RUST_TOTALITY_SUMMARY)?;
        let w041_lean_tla = read_json(repo_root, W041_LEAN_TLA_SUMMARY)?;
        let w041_stage2 = read_json(repo_root, W041_STAGE2_SUMMARY)?;
        let w041_stage2_blockers = read_json(repo_root, W041_STAGE2_BLOCKERS)?;
        let w041_operated = read_json(repo_root, W041_OPERATED_ASSURANCE_SUMMARY)?;
        let w041_retained = read_json(repo_root, W041_RETAINED_HISTORY_QUERY)?;
        let w041_cross_engine_service = read_json(repo_root, W041_CROSS_ENGINE_SERVICE_REGISTER)?;
        let w041_alert_dispatch = read_json(repo_root, W041_ALERT_DISPATCH_REGISTER)?;
        let w041_service_blockers = read_json(repo_root, W041_OPERATED_SERVICE_BLOCKERS)?;

        let implementation_rows = w041_independent_formula_evaluator_rows();
        let source_rows = w041_source_rows(
            &w041_obligation_summary,
            &w041_obligation_map,
            &w040_formatting_intake,
            &w040_diversity,
            &w040_implementation,
            &w040_decision,
            &w041_conformance,
            &w041_treecalc,
            &w041_rust,
            &w041_lean_tla,
            &w041_stage2,
            &w041_operated,
            &w041_cross_engine_service,
            &w041_alert_dispatch,
        );
        let independent_rows = w041_independent_evaluator_rows(
            &w040_independent,
            &w040_diversity,
            &w041_conformance,
            &w041_treecalc,
            &w041_rust,
            &w041_lean_tla,
            &w041_stage2,
            &w041_operated,
            &implementation_rows,
        );
        let cross_engine_rows = w041_cross_engine_differential_rows(
            &w040_cross_engine,
            &w041_stage2,
            &w041_operated,
            &w041_retained,
            &w041_cross_engine_service,
            &w041_alert_dispatch,
            &w040_formatting_intake,
            &implementation_rows,
        );
        let authority_rows = w041_mismatch_authority_rows(
            &w040_authority,
            &w041_conformance,
            &w041_operated,
            &w041_cross_engine_service,
            &w041_alert_dispatch,
            &implementation_rows,
        );
        let blockers = w041_exact_blockers(
            &w040_blockers,
            &w041_stage2_blockers,
            &w041_service_blockers,
            &w041_cross_engine_service,
        );
        let accepted_boundary_count = independent_rows
            .iter()
            .chain(cross_engine_rows.iter())
            .chain(authority_rows.iter())
            .filter(|row| {
                matches!(
                    row.get("disposition_kind").and_then(Value::as_str),
                    Some("accepted_boundary")
                        | Some("accepted_external_slice")
                        | Some("broadened_independent_implementation")
                        | Some("service_contract_boundary")
                )
            })
            .count();
        let service_blocked_count = cross_engine_rows
            .iter()
            .chain(authority_rows.iter())
            .filter(|row| {
                row.get("service_state").and_then(Value::as_str) == Some("blocked")
                    || row
                        .get("authority_state")
                        .and_then(Value::as_str)
                        .is_some_and(|state| state.starts_with("blocked"))
            })
            .count();

        let mut validation_failures = source_validation_failures(&source_rows);
        validation_failures.extend(w041_diversity_validation_failures(
            &implementation_rows,
            &independent_rows,
            &cross_engine_rows,
            &authority_rows,
            &blockers,
        ));
        let failed_row_count = validation_failures.len();
        let implementation_case_count = implementation_rows.len();
        let implementation_match_count = implementation_rows
            .iter()
            .filter(|row| bool_at(row, "matches_expected"))
            .count();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let independent_implementation_path = format!(
            "{relative_artifact_root}/w041_independent_formula_evaluator_implementation.json"
        );
        let independent_row_set_path =
            format!("{relative_artifact_root}/w041_independent_evaluator_breadth_register.json");
        let cross_engine_path = format!(
            "{relative_artifact_root}/w041_cross_engine_differential_service_register.json"
        );
        let mismatch_authority_path =
            format!("{relative_artifact_root}/w041_mismatch_authority_router.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/w041_exact_diversity_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": W041_SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w041_obligation_summary": W041_OBLIGATION_SUMMARY,
                "w041_obligation_map": W041_OBLIGATION_MAP,
                "w040_formatting_intake": W040_FORMATTING_INTAKE,
                "w040_diversity_summary": W040_DIVERSITY_SUMMARY,
                "w040_independent_implementation": W040_INDEPENDENT_IMPLEMENTATION,
                "w040_independent_row_set": W040_INDEPENDENT_ROW_SET,
                "w040_cross_engine_differential": W040_CROSS_ENGINE_DIVERSITY,
                "w040_differential_authority": W040_DIFFERENTIAL_AUTHORITY,
                "w040_diversity_blockers": W040_DIVERSITY_BLOCKERS,
                "w040_diversity_decision": W040_DIVERSITY_DECISION,
                "w041_conformance_summary": W041_CONFORMANCE_SUMMARY,
                "w041_treecalc_summary": W041_TREECALC_SUMMARY,
                "w041_rust_totality_summary": W041_RUST_TOTALITY_SUMMARY,
                "w041_lean_tla_summary": W041_LEAN_TLA_SUMMARY,
                "w041_stage2_summary": W041_STAGE2_SUMMARY,
                "w041_stage2_blockers": W041_STAGE2_BLOCKERS,
                "w041_operated_assurance_summary": W041_OPERATED_ASSURANCE_SUMMARY,
                "w041_retained_history_query": W041_RETAINED_HISTORY_QUERY,
                "w041_cross_engine_service_register": W041_CROSS_ENGINE_SERVICE_REGISTER,
                "w041_alert_dispatch_register": W041_ALERT_DISPATCH_REGISTER,
                "w041_operated_service_blockers": W041_OPERATED_SERVICE_BLOCKERS,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let independent_implementation = json!({
            "schema_version": W041_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1,
            "run_id": run_id,
            "implementation_kind": "broadened_independent_formula_fragment_evaluator",
            "implementation_authority": "local_w041_diversity_runner_independent_from_tracecalc_optimized_core_oxfml_and_oxfunc",
            "case_count": implementation_case_count,
            "match_count": implementation_match_count,
            "failed_case_count": implementation_case_count - implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "scope_limit": "integer names and literals with +, -, *, /, parentheses, comparisons, and IF(condition, true_expr, false_expr)",
            "rows": implementation_rows
        });
        let independent_row_set = json!({
            "schema_version": W041_INDEPENDENT_EVALUATOR_SCHEMA_V1,
            "run_id": run_id,
            "row_count": independent_rows.len(),
            "broadened_independent_implementation_case_count": implementation_case_count,
            "broadened_independent_implementation_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "rows": independent_rows
        });
        let cross_engine = json!({
            "schema_version": W041_CROSS_ENGINE_DIVERSITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": cross_engine_rows.len(),
            "service_blocked_count": service_blocked_count,
            "broadened_independent_implementation_case_count": implementation_case_count,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "rows": cross_engine_rows
        });
        let mismatch_authority = json!({
            "schema_version": W041_MISMATCH_AUTHORITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "broadened_independent_implementation_case_count": implementation_case_count,
            "operated_service_authority_promoted": false,
            "fully_independent_evaluator_promoted": false,
            "rows": authority_rows
        });
        let blocker_register = json!({
            "schema_version": W041_BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": W041_PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w041_broadened_independent_formula_fragment_bound_without_diversity_or_service_promotion",
            "broadened_independent_formula_evaluator_present": true,
            "broadened_independent_implementation_case_count": implementation_case_count,
            "broadened_independent_implementation_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "independent_evaluator_breadth_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "cross_engine_diversity_service_promoted": false,
            "mismatch_quarantine_service_promoted": false,
            "broad_oxfml_seam_promoted": false,
            "callable_metadata_projection_promoted": false,
            "w073_formatting_handoff_triggered": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This W041 diversity runner adds a broadened independent formula-fragment evaluator and classifies diversity, differential service, and mismatch-authority evidence only. It does not change evaluator kernels used by OxCalc, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let validation = json!({
            "schema_version": W041_VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w041_broadened_independent_evaluator_diversity_packet_valid"
            } else {
                "w041_broadened_independent_evaluator_diversity_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "broadened_independent_implementation_case_count": implementation_case_count,
            "broadened_independent_implementation_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": W041_RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "w041_independent_formula_evaluator_implementation_path": independent_implementation_path,
            "w041_independent_evaluator_breadth_register_path": independent_row_set_path,
            "w041_cross_engine_differential_service_register_path": cross_engine_path,
            "w041_mismatch_authority_router_path": mismatch_authority_path,
            "w041_exact_diversity_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "broadened_independent_implementation_case_count": implementation_case_count,
            "broadened_independent_implementation_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "mismatch_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("w041_independent_formula_evaluator_implementation.json"),
            &independent_implementation,
        )?;
        write_json(
            &artifact_root.join("w041_independent_evaluator_breadth_register.json"),
            &independent_row_set,
        )?;
        write_json(
            &artifact_root.join("w041_cross_engine_differential_service_register.json"),
            &cross_engine,
        )?;
        write_json(
            &artifact_root.join("w041_mismatch_authority_router.json"),
            &mismatch_authority,
        )?;
        write_json(
            &artifact_root.join("w041_exact_diversity_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: W041_RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: independent_rows.len(),
            seam_watch_row_count: cross_engine_rows.len(),
            aligned_seam_watch_row_count: accepted_boundary_count,
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }

    fn execute_w040(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w040_direct_summary = read_json(repo_root, W040_DIRECT_OBLIGATION_SUMMARY)?;
        let w040_direct_map = read_json(repo_root, W040_DIRECT_OBLIGATION_MAP)?;
        let w040_formatting_intake = read_json(repo_root, W040_FORMATTING_INTAKE)?;
        let w039_diversity = read_json(repo_root, W039_DIVERSITY_SUMMARY)?;
        let w039_independent = read_json(repo_root, W039_INDEPENDENT_ROW_SET)?;
        let w039_cross_engine = read_json(repo_root, W039_CROSS_ENGINE_DIVERSITY)?;
        let w039_authority = read_json(repo_root, W039_DIFFERENTIAL_AUTHORITY)?;
        let w039_blockers = read_json(repo_root, W039_DIVERSITY_BLOCKERS)?;
        let w039_decision = read_json(repo_root, W039_DIVERSITY_DECISION)?;
        let w040_conformance = read_json(repo_root, W040_CONFORMANCE_SUMMARY)?;
        let w040_treecalc = read_json(repo_root, W040_TREECALC_SUMMARY)?;
        let w040_rust = read_json(repo_root, W040_RUST_TOTALITY_SUMMARY)?;
        let w040_lean_tla = read_json(repo_root, W040_LEAN_TLA_SUMMARY)?;
        let w040_stage2 = read_json(repo_root, W040_STAGE2_SUMMARY)?;
        let w040_stage2_blockers = read_json(repo_root, W040_STAGE2_BLOCKERS)?;
        let w040_operated = read_json(repo_root, W040_OPERATED_ASSURANCE_SUMMARY)?;
        let w040_retained = read_json(repo_root, W040_RETAINED_HISTORY_STORE)?;
        let w040_cross_engine_service = read_json(repo_root, W040_CROSS_ENGINE_SERVICE_REGISTER)?;
        let w040_service_blockers = read_json(repo_root, W040_OPERATED_SERVICE_BLOCKERS)?;

        let implementation_rows = w040_independent_scalar_evaluator_rows();
        let source_rows = w040_source_rows(
            &w040_direct_summary,
            &w040_direct_map,
            &w040_formatting_intake,
            &w039_diversity,
            &w039_decision,
            &w040_conformance,
            &w040_treecalc,
            &w040_rust,
            &w040_lean_tla,
            &w040_stage2,
            &w040_operated,
        );
        let independent_rows = w040_independent_evaluator_rows(
            &w039_independent,
            &w039_diversity,
            &w040_conformance,
            &w040_treecalc,
            &w040_rust,
            &w040_lean_tla,
            &implementation_rows,
        );
        let cross_engine_rows = w040_cross_engine_differential_rows(
            &w039_cross_engine,
            &w040_stage2,
            &w040_operated,
            &w040_retained,
            &w040_cross_engine_service,
            &w040_formatting_intake,
            &implementation_rows,
        );
        let authority_rows = w040_differential_authority_rows(
            &w039_authority,
            &w040_conformance,
            &w040_operated,
            &w040_cross_engine_service,
            &implementation_rows,
        );
        let blockers = w040_exact_blockers(
            &w039_blockers,
            &w040_stage2_blockers,
            &w040_service_blockers,
        );
        let accepted_boundary_count = independent_rows
            .iter()
            .chain(cross_engine_rows.iter())
            .chain(authority_rows.iter())
            .filter(|row| {
                matches!(
                    row.get("disposition_kind").and_then(Value::as_str),
                    Some("accepted_boundary")
                        | Some("accepted_external_slice")
                        | Some("bounded_independent_implementation")
                )
            })
            .count();
        let service_blocked_count = cross_engine_rows
            .iter()
            .chain(authority_rows.iter())
            .filter(|row| {
                row.get("service_state").and_then(Value::as_str) == Some("blocked")
                    || row.get("authority_state").and_then(Value::as_str)
                        == Some("blocked_no_operated_service")
            })
            .count();

        let mut validation_failures = source_validation_failures(&source_rows);
        validation_failures.extend(w040_diversity_validation_failures(
            &implementation_rows,
            &independent_rows,
            &cross_engine_rows,
            &authority_rows,
            &blockers,
        ));
        let failed_row_count = validation_failures.len();
        let implementation_case_count = implementation_rows.len();
        let implementation_match_count = implementation_rows
            .iter()
            .filter(|row| bool_at(row, "matches_expected"))
            .count();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let independent_implementation_path = format!(
            "{relative_artifact_root}/w040_independent_scalar_evaluator_implementation.json"
        );
        let independent_row_set_path =
            format!("{relative_artifact_root}/w040_independent_evaluator_row_set.json");
        let cross_engine_path =
            format!("{relative_artifact_root}/w040_cross_engine_differential_register.json");
        let differential_authority_path =
            format!("{relative_artifact_root}/w040_differential_authority_register.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/w040_exact_diversity_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": W040_SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w040_direct_obligation_summary": W040_DIRECT_OBLIGATION_SUMMARY,
                "w040_direct_obligation_map": W040_DIRECT_OBLIGATION_MAP,
                "w040_formatting_intake": W040_FORMATTING_INTAKE,
                "w039_diversity_summary": W039_DIVERSITY_SUMMARY,
                "w039_independent_row_set": W039_INDEPENDENT_ROW_SET,
                "w039_cross_engine_diversity": W039_CROSS_ENGINE_DIVERSITY,
                "w039_differential_authority": W039_DIFFERENTIAL_AUTHORITY,
                "w039_diversity_blockers": W039_DIVERSITY_BLOCKERS,
                "w039_diversity_decision": W039_DIVERSITY_DECISION,
                "w040_conformance_summary": W040_CONFORMANCE_SUMMARY,
                "w040_treecalc_summary": W040_TREECALC_SUMMARY,
                "w040_rust_totality_summary": W040_RUST_TOTALITY_SUMMARY,
                "w040_lean_tla_summary": W040_LEAN_TLA_SUMMARY,
                "w040_stage2_summary": W040_STAGE2_SUMMARY,
                "w040_stage2_blockers": W040_STAGE2_BLOCKERS,
                "w040_operated_assurance_summary": W040_OPERATED_ASSURANCE_SUMMARY,
                "w040_retained_history_store": W040_RETAINED_HISTORY_STORE,
                "w040_cross_engine_service_register": W040_CROSS_ENGINE_SERVICE_REGISTER,
                "w040_operated_service_blockers": W040_OPERATED_SERVICE_BLOCKERS,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let independent_implementation = json!({
            "schema_version": W040_INDEPENDENT_IMPLEMENTATION_SCHEMA_V1,
            "run_id": run_id,
            "implementation_kind": "bounded_independent_scalar_arithmetic_evaluator",
            "implementation_authority": "local_w040_diversity_runner_independent_from_tracecalc_and_optimized_core",
            "case_count": implementation_case_count,
            "match_count": implementation_match_count,
            "failed_case_count": implementation_case_count - implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "scope_limit": "integer arithmetic literals, names, +, *, and parentheses only",
            "rows": implementation_rows
        });
        let independent_row_set = json!({
            "schema_version": W040_INDEPENDENT_EVALUATOR_SCHEMA_V1,
            "run_id": run_id,
            "row_count": independent_rows.len(),
            "bounded_independent_implementation_case_count": implementation_case_count,
            "bounded_independent_implementation_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "rows": independent_rows
        });
        let cross_engine = json!({
            "schema_version": W040_CROSS_ENGINE_DIVERSITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": cross_engine_rows.len(),
            "service_blocked_count": service_blocked_count,
            "bounded_independent_implementation_case_count": implementation_case_count,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "rows": cross_engine_rows
        });
        let differential_authority = json!({
            "schema_version": W040_DIFFERENTIAL_AUTHORITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "bounded_independent_implementation_case_count": implementation_case_count,
            "operated_service_authority_promoted": false,
            "fully_independent_evaluator_promoted": false,
            "rows": authority_rows
        });
        let blocker_register = json!({
            "schema_version": W040_BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": W040_PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w040_bounded_independent_scalar_evaluator_bound_without_diversity_promotion",
            "bounded_independent_scalar_evaluator_present": true,
            "bounded_independent_implementation_case_count": implementation_case_count,
            "bounded_independent_implementation_match_count": implementation_match_count,
            "fully_independent_evaluator_promoted": false,
            "independent_evaluator_row_set_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "cross_engine_diversity_service_promoted": false,
            "broad_oxfml_seam_promoted": false,
            "callable_metadata_projection_promoted": false,
            "w073_formatting_handoff_triggered": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "differential_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This W040 diversity runner adds a bounded independent scalar arithmetic evaluator and classifies diversity/service authority evidence only. It does not change evaluator kernels used by OxCalc, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let validation = json!({
            "schema_version": W040_VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w040_bounded_independent_evaluator_diversity_packet_valid"
            } else {
                "w040_bounded_independent_evaluator_diversity_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "bounded_independent_implementation_case_count": implementation_case_count,
            "bounded_independent_implementation_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "differential_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": W040_RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "w040_independent_scalar_evaluator_implementation_path": independent_implementation_path,
            "w040_independent_evaluator_row_set_path": independent_row_set_path,
            "w040_cross_engine_differential_register_path": cross_engine_path,
            "w040_differential_authority_register_path": differential_authority_path,
            "w040_exact_diversity_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "bounded_independent_implementation_case_count": implementation_case_count,
            "bounded_independent_implementation_match_count": implementation_match_count,
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_differential_row_count": cross_engine_rows.len(),
            "differential_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("w040_independent_scalar_evaluator_implementation.json"),
            &independent_implementation,
        )?;
        write_json(
            &artifact_root.join("w040_independent_evaluator_row_set.json"),
            &independent_row_set,
        )?;
        write_json(
            &artifact_root.join("w040_cross_engine_differential_register.json"),
            &cross_engine,
        )?;
        write_json(
            &artifact_root.join("w040_differential_authority_register.json"),
            &differential_authority,
        )?;
        write_json(
            &artifact_root.join("w040_exact_diversity_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: W040_RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: independent_rows.len(),
            seam_watch_row_count: cross_engine_rows.len(),
            aligned_seam_watch_row_count: accepted_boundary_count,
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }

    fn execute_w039(
        &self,
        repo_root: &Path,
        run_id: &str,
    ) -> Result<DiversitySeamRunSummary, DiversitySeamError> {
        let relative_artifact_root =
            relative_artifact_path(&["docs", "test-runs", "core-engine", "diversity-seam", run_id]);
        let artifact_root = repo_root.join(&relative_artifact_root);
        if artifact_root.exists() {
            fs::remove_dir_all(&artifact_root).map_err(|source| {
                DiversitySeamError::RemoveDirectory {
                    path: artifact_root.display().to_string(),
                    source,
                }
            })?;
        }
        fs::create_dir_all(&artifact_root).map_err(|source| {
            DiversitySeamError::CreateDirectory {
                path: artifact_root.display().to_string(),
                source,
            }
        })?;

        let w036_independent = read_json(repo_root, W036_INDEPENDENT_SUMMARY)?;
        let w036_cross_engine = read_json(repo_root, W036_CROSS_ENGINE_SUMMARY)?;
        let w037_direct_oxfml = read_json(repo_root, W037_DIRECT_OXFML_SUMMARY)?;
        let w038_diversity = read_json(repo_root, W038_DIVERSITY_SUMMARY)?;
        let w038_diversity_disposition = read_json(repo_root, W038_DIVERSITY_DISPOSITION)?;
        let w038_diversity_blockers = read_json(repo_root, W038_DIVERSITY_BLOCKERS)?;
        let w039_residual_ledger = read_json(repo_root, W039_RESIDUAL_LEDGER_SUMMARY)?;
        let w039_formatting_intake = read_json(repo_root, W039_FORMATTING_INTAKE)?;
        let w039_conformance = read_json(repo_root, W039_CONFORMANCE_SUMMARY)?;
        let w039_proof_model = read_json(repo_root, W039_PROOF_MODEL_SUMMARY)?;
        let w039_stage2 = read_json(repo_root, W039_STAGE2_SUMMARY)?;
        let w039_stage2_blockers = read_json(repo_root, W039_STAGE2_BLOCKERS)?;
        let w039_operated = read_json(repo_root, W039_OPERATED_ASSURANCE_SUMMARY)?;
        let w039_cross_engine_substrate =
            read_json(repo_root, W039_CROSS_ENGINE_SERVICE_SUBSTRATE)?;
        let w039_operated_service_blockers = read_json(repo_root, W039_OPERATED_SERVICE_BLOCKERS)?;

        let source_rows = w039_source_rows(
            repo_root,
            &w036_independent,
            &w036_cross_engine,
            &w037_direct_oxfml,
            &w038_diversity,
            &w039_residual_ledger,
            &w039_formatting_intake,
            &w039_conformance,
            &w039_proof_model,
            &w039_stage2,
            &w039_operated,
        );
        let independent_rows = w039_independent_evaluator_rows(
            &w036_independent,
            &w036_cross_engine,
            &w037_direct_oxfml,
            &w038_diversity,
            &w038_diversity_disposition,
            &w039_conformance,
            &w039_proof_model,
        );
        let cross_engine_rows = w039_cross_engine_diversity_rows(
            &w036_cross_engine,
            &w038_diversity,
            &w039_stage2,
            &w039_operated,
            &w039_cross_engine_substrate,
            &w039_formatting_intake,
        );
        let authority_rows = w039_differential_authority_rows(
            &w036_independent,
            &w036_cross_engine,
            &w037_direct_oxfml,
            &w039_conformance,
            &w039_operated,
            &w039_cross_engine_substrate,
        );
        let blockers = w039_exact_blockers(
            &w038_diversity_blockers,
            &w039_stage2_blockers,
            &w039_operated_service_blockers,
        );
        let accepted_boundary_count = independent_rows
            .iter()
            .chain(cross_engine_rows.iter())
            .chain(authority_rows.iter())
            .filter(|row| {
                row.get("disposition_kind").and_then(Value::as_str) == Some("accepted_boundary")
                    || row.get("disposition_kind").and_then(Value::as_str)
                        == Some("accepted_external_slice")
            })
            .count();
        let service_blocked_count = cross_engine_rows
            .iter()
            .chain(authority_rows.iter())
            .filter(|row| {
                row.get("service_state").and_then(Value::as_str) == Some("blocked")
                    || row.get("authority_state").and_then(Value::as_str)
                        == Some("blocked_no_operated_service")
            })
            .count();

        let mut validation_failures = source_validation_failures(&source_rows);
        validation_failures.extend(w039_diversity_validation_failures(
            &independent_rows,
            &cross_engine_rows,
            &authority_rows,
            &blockers,
        ));
        let failed_row_count = validation_failures.len();

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let independent_row_set_path =
            format!("{relative_artifact_root}/w039_independent_evaluator_row_set.json");
        let cross_engine_diversity_path =
            format!("{relative_artifact_root}/w039_cross_engine_diversity_register.json");
        let differential_authority_path =
            format!("{relative_artifact_root}/w039_differential_service_authority_register.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/w039_exact_diversity_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");

        let source_evidence_index = json!({
            "schema_version": W039_SOURCE_INDEX_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_row_count": source_rows.len(),
            "rows": source_rows,
            "source_artifacts": {
                "w036_independent_summary": W036_INDEPENDENT_SUMMARY,
                "w036_cross_engine_summary": W036_CROSS_ENGINE_SUMMARY,
                "w037_direct_oxfml_summary": W037_DIRECT_OXFML_SUMMARY,
                "w038_diversity_summary": W038_DIVERSITY_SUMMARY,
                "w038_diversity_disposition": W038_DIVERSITY_DISPOSITION,
                "w038_diversity_blockers": W038_DIVERSITY_BLOCKERS,
                "w039_residual_ledger_summary": W039_RESIDUAL_LEDGER_SUMMARY,
                "w039_formatting_intake": W039_FORMATTING_INTAKE,
                "w039_conformance_summary": W039_CONFORMANCE_SUMMARY,
                "w039_proof_model_summary": W039_PROOF_MODEL_SUMMARY,
                "w039_stage2_summary": W039_STAGE2_SUMMARY,
                "w039_operated_assurance_summary": W039_OPERATED_ASSURANCE_SUMMARY,
                "w039_cross_engine_service_substrate": W039_CROSS_ENGINE_SERVICE_SUBSTRATE,
                "oxfml_inbound_notes": OXFML_INBOUND_NOTES
            }
        });
        let independent_row_set = json!({
            "schema_version": W039_INDEPENDENT_EVALUATOR_SCHEMA_V1,
            "run_id": run_id,
            "row_count": independent_rows.len(),
            "fully_independent_evaluator_promoted": false,
            "independent_implementation_row_count": 0,
            "rows": independent_rows
        });
        let cross_engine_diversity = json!({
            "schema_version": W039_CROSS_ENGINE_DIVERSITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": cross_engine_rows.len(),
            "service_blocked_count": service_blocked_count,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true,
            "rows": cross_engine_rows
        });
        let differential_authority = json!({
            "schema_version": W039_DIFFERENTIAL_AUTHORITY_SCHEMA_V1,
            "run_id": run_id,
            "row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "operated_service_authority_promoted": false,
            "independent_evaluator_authority_promoted": false,
            "rows": authority_rows
        });
        let blocker_register = json!({
            "schema_version": W039_BLOCKER_REGISTER_SCHEMA_V1,
            "run_id": run_id,
            "exact_blocker_count": blockers.len(),
            "rows": blockers
        });
        let promotion_decision = json!({
            "schema_version": W039_PROMOTION_DECISION_SCHEMA_V1,
            "run_id": run_id,
            "decision_state": "w039_independent_evaluator_and_cross_engine_diversity_bound_without_promotion",
            "fully_independent_evaluator_promoted": false,
            "independent_evaluator_row_set_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "cross_engine_diversity_service_promoted": false,
            "broad_oxfml_seam_promoted": false,
            "callable_metadata_projection_promoted": false,
            "w073_formatting_handoff_triggered": false,
            "pack_grade_replay_promoted": false,
            "c5_promoted": false,
            "stage2_policy_promoted": false,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_diversity_row_count": cross_engine_rows.len(),
            "differential_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "exact_blocker_count": blockers.len(),
            "blockers": blockers
                .iter()
                .map(|row| row["blocker_id"].clone())
                .collect::<Vec<_>>(),
            "semantic_equivalence_statement": "This W039 diversity runner binds source evidence, independent-evaluator authority rows, cross-engine diversity rows, and service-authority blockers only. It does not change evaluator kernels, coordinator scheduling, recalc, publication, replay, pack, service, TraceCalc, TreeCalc, OxFml, OxFunc, Lean, or TLA semantics."
        });
        let validation = json!({
            "schema_version": W039_VALIDATION_SCHEMA_V1,
            "run_id": run_id,
            "status": if validation_failures.is_empty() {
                "w039_independent_evaluator_cross_engine_diversity_packet_valid"
            } else {
                "w039_independent_evaluator_cross_engine_diversity_packet_invalid"
            },
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_diversity_row_count": cross_engine_rows.len(),
            "differential_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "validation_failures": validation_failures
        });
        let run_summary = json!({
            "schema_version": W039_RUN_SUMMARY_SCHEMA_V1,
            "run_id": run_id,
            "artifact_root": relative_artifact_root,
            "source_evidence_index_path": source_evidence_index_path,
            "w039_independent_evaluator_row_set_path": independent_row_set_path,
            "w039_cross_engine_diversity_register_path": cross_engine_diversity_path,
            "w039_differential_service_authority_register_path": differential_authority_path,
            "w039_exact_diversity_blocker_register_path": blocker_register_path,
            "promotion_decision_path": promotion_decision_path,
            "validation_path": validation_path,
            "source_evidence_row_count": source_rows.len(),
            "independent_evaluator_row_count": independent_rows.len(),
            "cross_engine_diversity_row_count": cross_engine_rows.len(),
            "differential_authority_row_count": authority_rows.len(),
            "accepted_boundary_count": accepted_boundary_count,
            "service_blocked_count": service_blocked_count,
            "exact_blocker_count": blockers.len(),
            "failed_row_count": failed_row_count,
            "fully_independent_evaluator_promoted": false,
            "operated_cross_engine_differential_service_promoted": false,
            "w073_typed_only_formatting_guard_retained": true
        });

        write_json(
            &artifact_root.join("source_evidence_index.json"),
            &source_evidence_index,
        )?;
        write_json(
            &artifact_root.join("w039_independent_evaluator_row_set.json"),
            &independent_row_set,
        )?;
        write_json(
            &artifact_root.join("w039_cross_engine_diversity_register.json"),
            &cross_engine_diversity,
        )?;
        write_json(
            &artifact_root.join("w039_differential_service_authority_register.json"),
            &differential_authority,
        )?;
        write_json(
            &artifact_root.join("w039_exact_diversity_blocker_register.json"),
            &blocker_register,
        )?;
        write_json(
            &artifact_root.join("promotion_decision.json"),
            &promotion_decision,
        )?;
        write_json(&artifact_root.join("validation.json"), &validation)?;
        write_json(&artifact_root.join("run_summary.json"), &run_summary)?;

        Ok(DiversitySeamRunSummary {
            run_id: run_id.to_string(),
            schema_version: W039_RUN_SUMMARY_SCHEMA_V1.to_string(),
            source_evidence_row_count: source_rows.len(),
            diversity_disposition_row_count: independent_rows.len(),
            seam_watch_row_count: cross_engine_rows.len(),
            aligned_seam_watch_row_count: accepted_boundary_count,
            accepted_boundary_count,
            exact_blocker_count: blockers.len(),
            failed_row_count,
            fully_independent_evaluator_promoted: false,
            artifact_root: relative_artifact_root,
        })
    }
}

fn source_rows(
    w036_independent: &Value,
    w036_cross_engine: &Value,
    w037_direct_oxfml: &Value,
    w038_conformance: &Value,
    w038_formal: &Value,
    w038_stage2: &Value,
    w038_operated: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "source.w036_independent_conformance",
            "artifact": W036_INDEPENDENT_SUMMARY,
            "missing_artifact_count": number_at(w036_independent, "missing_artifact_count"),
            "unexpected_mismatch_count": number_at(w036_independent, "unexpected_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w036_independent, "w036_full_independent_evaluator_promoted"),
            "semantic_state": "w036_independent_projection_differential_bound"
        }),
        json!({
            "row_id": "source.w036_cross_engine_differential",
            "artifact": W036_CROSS_ENGINE_SUMMARY,
            "missing_artifact_count": number_at(w036_cross_engine, "missing_artifact_count"),
            "unexpected_mismatch_count": number_at(w036_cross_engine, "unexpected_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w036_cross_engine, "full_independent_evaluator_promoted")
                && !bool_at(w036_cross_engine, "continuous_cross_engine_service_promoted"),
            "semantic_state": "w036_file_backed_cross_engine_differential_bound"
        }),
        json!({
            "row_id": "source.w037_direct_oxfml",
            "artifact": W037_DIRECT_OXFML_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": number_at(w037_direct_oxfml, "expectation_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": !bool_at(&w037_direct_oxfml["promotion_limits"], "general_oxfunc_kernel_claimed")
                && !bool_at(&w037_direct_oxfml["promotion_limits"], "pack_grade_replay_promoted")
                && !bool_at(&w037_direct_oxfml["promotion_limits"], "c5_promoted"),
            "semantic_state": "direct_oxfml_runtime_slice_bound"
        }),
        json!({
            "row_id": "source.w038_conformance",
            "artifact": W038_CONFORMANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w038_conformance, "failed_row_count"),
            "promotion_guard": number_at(w038_conformance, "w038_match_promoted_count") == 0,
            "semantic_state": "w038_conformance_blockers_bound"
        }),
        json!({
            "row_id": "source.w038_formal_assurance",
            "artifact": W038_FORMAL_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w038_formal, "failed_row_count"),
            "promotion_guard": !bool_at(&w038_formal["promotion_claims"], "general_oxfunc_kernel_promoted"),
            "semantic_state": "w038_formal_external_kernel_boundary_bound"
        }),
        json!({
            "row_id": "source.w038_stage2_replay",
            "artifact": W038_STAGE2_REPLAY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w038_stage2, "failed_row_count"),
            "promotion_guard": !bool_at(w038_stage2, "stage2_policy_promoted"),
            "semantic_state": "w038_stage2_formatting_watch_bound"
        }),
        json!({
            "row_id": "source.w038_operated_assurance",
            "artifact": W038_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w038_operated, "failed_row_count"),
            "promotion_guard": !bool_at(w038_operated, "operated_continuous_assurance_service_promoted")
                && !bool_at(w038_operated, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w038_operated_service_blockers_bound"
        }),
    ]
}

fn diversity_rows(w036_independent: &Value, w036_cross_engine: &Value) -> Vec<Value> {
    vec![
        json!({
            "row_id": "diversity.tracecalc_reference_machine",
            "disposition_kind": "accepted_boundary",
            "diversity_state": "reference_oracle_not_independent_production_evaluator",
            "evidence": W036_INDEPENDENT_SUMMARY,
            "comparison_row_count": number_at(w036_independent, "comparison_row_count"),
            "promotion_consequence": "TraceCalc remains correctness oracle for covered reference behavior, not a fully independent optimized implementation."
        }),
        json!({
            "row_id": "diversity.treecalc_core_projection",
            "disposition_kind": "projection_evidence_not_independent",
            "diversity_state": "shared_implementation_projection",
            "evidence": W036_INDEPENDENT_SUMMARY,
            "exact_value_match_count": number_at(w036_independent, "exact_value_match_count"),
            "promotion_consequence": "TreeCalc/CoreEngine projection rows are useful differential evidence but cannot promote full independent evaluator diversity."
        }),
        json!({
            "row_id": "diversity.file_backed_cross_engine_differential",
            "disposition_kind": "accepted_boundary",
            "diversity_state": "file_backed_differential_not_operated_service",
            "evidence": W036_CROSS_ENGINE_SUMMARY,
            "differential_row_count": number_at(w036_cross_engine, "differential_row_count"),
            "promotion_consequence": "File-backed differential rows support assurance but do not promote an operated cross-engine service or independent evaluator."
        }),
        json!({
            "row_id": "diversity.direct_oxfml_external_evaluator_slice",
            "disposition_kind": "accepted_external_slice",
            "diversity_state": "external_formula_evaluator_slice_not_full_core_engine",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "promotion_consequence": "Direct OxFml evidence strengthens seam confidence for formulas, LET/LAMBDA carrier, and W073 formatting, but does not independently implement OxCalc coordination."
        }),
        json!({
            "row_id": "diversity.fully_independent_evaluator_absent",
            "disposition_kind": "exact_remaining_blocker",
            "diversity_state": "blocked",
            "evidence": W036_INDEPENDENT_SUMMARY,
            "promotion_consequence": "Fully independent evaluator claims remain blocked until an independently implemented row set exists."
        }),
    ]
}

fn seam_watch_rows(
    repo_root: &Path,
    w037_direct_oxfml: &Value,
    w038_conformance: &Value,
    w038_conformance_blockers: &Value,
    w038_formal: &Value,
    w038_stage2: &Value,
) -> Vec<Value> {
    let oxfml_notes_present = repo_root.join(OXFML_INBOUND_NOTES).exists();
    let callable_metadata_blocker_present = row_with_field_exists(
        w038_conformance_blockers,
        "row_id",
        "w038_disposition_callable_metadata_projection_exact_blocker",
    );

    vec![
        json!({
            "row_id": "seam.oxfml_w073_typed_formatting",
            "watch_state": "aligned",
            "disposition_kind": "watch_input_bound",
            "source": W037_DIRECT_OXFML_SUMMARY,
            "evidence": {
                "w073_typed_rule_case_count": number_at(w037_direct_oxfml, "w073_typed_rule_case_count"),
                "w038_stage2_formatting_watch_row_count": number_at(w038_stage2, "formatting_watch_row_count")
            },
            "current_read": "W073 aggregate and visualization conditional-formatting metadata is typed_rule-only for the watched families; legacy thresholds are not interpreted there."
        }),
        json!({
            "row_id": "seam.format_delta_display_delta_distinct",
            "watch_state": "aligned",
            "disposition_kind": "watch_input_bound",
            "source": OXFML_INBOUND_NOTES,
            "evidence": {
                "oxfml_notes_present": oxfml_notes_present,
                "w038_stage2_formatting_watch_row_count": number_at(w038_stage2, "formatting_watch_row_count")
            },
            "current_read": "format_delta and display_delta remain distinct; broad display-facing closure is not inferred."
        }),
        json!({
            "row_id": "seam.direct_oxfml_runtime_facade",
            "watch_state": "aligned",
            "disposition_kind": "direct_external_slice_bound",
            "source": W037_DIRECT_OXFML_SUMMARY,
            "evidence": {
                "direct_oxfml_case_count": number_at(w037_direct_oxfml, "direct_oxfml_case_count"),
                "expectation_mismatch_count": number_at(w037_direct_oxfml, "expectation_mismatch_count")
            },
            "current_read": "OxCalc consumes direct OxFml runtime evidence through the current facade slice without reopening OxFml ownership."
        }),
        json!({
            "row_id": "seam.let_lambda_narrow_carrier",
            "watch_state": "aligned",
            "disposition_kind": "accepted_boundary",
            "source": W037_DIRECT_OXFML_SUMMARY,
            "evidence": {
                "let_lambda_case_count": number_at(w037_direct_oxfml, "let_lambda_case_count"),
                "general_oxfunc_kernel_promoted": bool_at(&w038_formal["promotion_claims"], "general_oxfunc_kernel_promoted")
            },
            "current_read": "LET/LAMBDA is included as a narrow carrier seam; general OxFunc kernels remain external."
        }),
        json!({
            "row_id": "seam.callable_metadata_projection",
            "watch_state": if callable_metadata_blocker_present { "blocked_exact" } else { "missing_blocker" },
            "disposition_kind": "exact_remaining_blocker",
            "source": W038_CONFORMANCE_BLOCKERS,
            "evidence": {
                "callable_metadata_blocker_present": callable_metadata_blocker_present,
                "w038_exact_remaining_blocker_count": number_at(w038_conformance, "w038_exact_remaining_blocker_count")
            },
            "current_read": "value-only callable carrier evidence is bound; callable metadata projection remains unpromoted."
        }),
        json!({
            "row_id": "seam.host_runtime_and_public_consumer_surface",
            "watch_state": "aligned",
            "disposition_kind": "note_level_watch",
            "source": OXFML_INBOUND_NOTES,
            "evidence": { "oxfml_notes_present": oxfml_notes_present },
            "current_read": "current OxFml consumer surface points ordinary downstream use at consumer::runtime, consumer::editor, and consumer::replay; OxCalc does not build a new long-term wrapper around the older flat root."
        }),
        json!({
            "row_id": "seam.structured_reference_table_packet",
            "watch_state": "aligned",
            "disposition_kind": "note_level_watch",
            "source": OXFML_INBOUND_NOTES,
            "evidence": { "oxfml_notes_present": oxfml_notes_present },
            "current_read": "table_catalog, enclosing_table_ref, and caller_table_region remain the aligned first structured-reference packet direction."
        }),
        json!({
            "row_id": "seam.registered_external_packet",
            "watch_state": "aligned",
            "disposition_kind": "note_level_watch",
            "source": OXFML_INBOUND_NOTES,
            "evidence": { "oxfml_notes_present": oxfml_notes_present },
            "current_read": "registered-external direct packet naming and seven-field descriptor read are note-level converged, without coordinator API freeze."
        }),
    ]
}

fn exact_blockers() -> Vec<Value> {
    vec![
        json!({
            "blocker_id": "diversity.fully_independent_evaluator_absent",
            "owner": "calc-zsr.7; calc-zsr.9",
            "status_after_run": "exact_remaining_blocker",
            "reason": "TraceCalc, TreeCalc/CoreEngine projection, direct OxFml, and file-backed differential rows do not constitute an independently implemented OxCalc evaluator row set.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "diversity.operated_cross_engine_service_absent",
            "owner": "calc-zsr.6; calc-zsr.7",
            "status_after_run": "exact_remaining_blocker",
            "reason": "cross-engine evidence remains file-backed and does not run as an operated differential service.",
            "promotion_consequence": "continuous cross-engine diversity claims remain unpromoted"
        }),
        json!({
            "blocker_id": "seam.callable_metadata_projection_absent",
            "owner": "calc-zsr.7; external:OxFunc",
            "status_after_run": "exact_remaining_blocker",
            "reason": "narrow LET/LAMBDA value carrier evidence is present, but callable metadata projection is not exercised.",
            "promotion_consequence": "callable metadata projection remains unpromoted"
        }),
        json!({
            "blocker_id": "seam.broad_oxfml_display_and_publication_breadth_unfrozen",
            "owner": "calc-zsr.7; OxFml watch lane",
            "status_after_run": "exact_remaining_watch_blocker",
            "reason": "format_delta/display_delta distinction and current publication consequences are aligned, but broad display-facing and publication/topology breadth remains narrower than full future scope.",
            "promotion_consequence": "broad OxFml seam closure remains unpromoted until exercised evidence requires and supports it"
        }),
    ]
}

fn w039_source_rows(
    repo_root: &Path,
    w036_independent: &Value,
    w036_cross_engine: &Value,
    w037_direct_oxfml: &Value,
    w038_diversity: &Value,
    w039_residual_ledger: &Value,
    w039_formatting_intake: &Value,
    w039_conformance: &Value,
    w039_proof_model: &Value,
    w039_stage2: &Value,
    w039_operated: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "source.w036_independent_conformance",
            "artifact": W036_INDEPENDENT_SUMMARY,
            "missing_artifact_count": number_at(w036_independent, "missing_artifact_count"),
            "unexpected_mismatch_count": number_at(w036_independent, "unexpected_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w036_independent, "w036_full_independent_evaluator_promoted"),
            "semantic_state": "w036_tracecalc_treecalc_projection_differential_bound"
        }),
        json!({
            "row_id": "source.w036_cross_engine_differential",
            "artifact": W036_CROSS_ENGINE_SUMMARY,
            "missing_artifact_count": number_at(w036_cross_engine, "missing_artifact_count"),
            "unexpected_mismatch_count": number_at(w036_cross_engine, "unexpected_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w036_cross_engine, "full_independent_evaluator_promoted")
                && !bool_at(w036_cross_engine, "continuous_cross_engine_service_promoted"),
            "semantic_state": "w036_file_backed_cross_engine_differential_bound"
        }),
        json!({
            "row_id": "source.w037_direct_oxfml",
            "artifact": W037_DIRECT_OXFML_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": number_at(w037_direct_oxfml, "expectation_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": !bool_at(&w037_direct_oxfml["promotion_limits"], "general_oxfunc_kernel_claimed")
                && !bool_at(&w037_direct_oxfml["promotion_limits"], "pack_grade_replay_promoted")
                && !bool_at(&w037_direct_oxfml["promotion_limits"], "c5_promoted"),
            "semantic_state": "direct_oxfml_runtime_slice_and_w073_guard_bound"
        }),
        json!({
            "row_id": "source.w038_diversity_seam_watch",
            "artifact": W038_DIVERSITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w038_diversity, "failed_row_count"),
            "promotion_guard": !bool_at(w038_diversity, "fully_independent_evaluator_promoted"),
            "semantic_state": "w038_diversity_and_seam_watch_bound_without_promotion"
        }),
        json!({
            "row_id": "source.w039_residual_successor_ledger",
            "artifact": W039_RESIDUAL_LEDGER_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w039_residual_ledger, "w073_formatting_intake") == "typed_only_guard_recorded",
            "semantic_state": "w039_obligation_and_promotion_map_bound"
        }),
        json!({
            "row_id": "source.w039_w073_formatting_intake",
            "artifact": W039_FORMATTING_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w039_formatting_intake, "thresholds_rule").contains("intentionally ignored")
                && array_len_at(w039_formatting_intake, "typed_only_families") == 7,
            "semantic_state": "current_oxfml_w073_typed_only_formatting_intake_retained",
            "oxfml_inbound_notes_present": repo_root.join(OXFML_INBOUND_NOTES).exists()
        }),
        json!({
            "row_id": "source.w039_optimized_core_conformance",
            "artifact": W039_CONFORMANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w039_conformance, "failed_row_count"),
            "promotion_guard": number_at(w039_conformance, "w039_match_promoted_count") == 0,
            "semantic_state": "optimized_core_exact_blockers_retained_without_full_core_promotion"
        }),
        json!({
            "row_id": "source.w039_proof_model",
            "artifact": W039_PROOF_MODEL_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w039_proof_model, "failed_row_count"),
            "promotion_guard": !bool_at(&w039_proof_model["promotion_claims"], "full_lean_verification_promoted")
                && !bool_at(&w039_proof_model["promotion_claims"], "full_tla_verification_promoted")
                && !bool_at(&w039_proof_model["promotion_claims"], "general_oxfunc_kernel_promoted"),
            "semantic_state": "proof_model_totality_bound_without_full_verification_promotion"
        }),
        json!({
            "row_id": "source.w039_stage2_policy_governance",
            "artifact": W039_STAGE2_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w039_stage2, "failed_row_count"),
            "promotion_guard": !bool_at(w039_stage2, "stage2_policy_promoted"),
            "semantic_state": "stage2_policy_governance_bound_without_production_policy_promotion"
        }),
        json!({
            "row_id": "source.w039_operated_assurance_substrate",
            "artifact": W039_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w039_operated, "failed_row_count"),
            "promotion_guard": !bool_at(w039_operated, "operated_continuous_assurance_service_promoted")
                && !bool_at(w039_operated, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w039_operated, "retained_history_service_promoted"),
            "semantic_state": "operated_assurance_substrate_bound_without_operated_service_promotion"
        }),
    ]
}

fn w039_independent_evaluator_rows(
    w036_independent: &Value,
    w036_cross_engine: &Value,
    w037_direct_oxfml: &Value,
    w038_diversity: &Value,
    w038_diversity_disposition: &Value,
    w039_conformance: &Value,
    w039_proof_model: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "independent.tracecalc_reference_oracle",
            "disposition_kind": "accepted_boundary",
            "independence_state": "reference_oracle_not_independent_production_evaluator",
            "evidence": W036_INDEPENDENT_SUMMARY,
            "comparison_row_count": number_at(w036_independent, "comparison_row_count"),
            "promotion_consequence": "TraceCalc remains the correctness oracle for covered behavior, not a second production evaluator."
        }),
        json!({
            "row_id": "independent.treecalc_core_projection",
            "disposition_kind": "projection_evidence_not_independent",
            "independence_state": "shared_projection_over_oxcalc_core_behavior",
            "evidence": W036_INDEPENDENT_SUMMARY,
            "exact_value_match_count": number_at(w036_independent, "exact_value_match_count"),
            "promotion_consequence": "Shared TreeCalc/CoreEngine projection evidence cannot count as independent implementation authority."
        }),
        json!({
            "row_id": "independent.optimized_core_exact_blocker_disposition",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "optimized_core_disposition_same_authority_family",
            "evidence": W039_CONFORMANCE_SUMMARY,
            "w039_direct_evidence_bound_count": number_at(w039_conformance, "w039_direct_evidence_bound_count"),
            "w039_exact_remaining_blocker_count": number_at(w039_conformance, "w039_exact_remaining_blocker_count"),
            "promotion_consequence": "Optimized/core evidence strengthens conformance, but does not establish a separate evaluator implementation."
        }),
        json!({
            "row_id": "independent.direct_oxfml_external_formula_slice",
            "disposition_kind": "accepted_external_slice",
            "independence_state": "external_formula_evaluator_slice_not_oxcalc_coordinator",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "direct_oxfml_case_count": number_at(w037_direct_oxfml, "direct_oxfml_case_count"),
            "let_lambda_case_count": number_at(w037_direct_oxfml, "let_lambda_case_count"),
            "promotion_consequence": "Direct OxFml evidence counts for the consumed formula seam, including narrow LET/LAMBDA and W073 guards, but not for OxCalc coordinator implementation diversity."
        }),
        json!({
            "row_id": "independent.file_backed_cross_engine_differential",
            "disposition_kind": "accepted_boundary",
            "independence_state": "file_backed_differential_not_operated_implementation",
            "evidence": W036_CROSS_ENGINE_SUMMARY,
            "differential_row_count": number_at(w036_cross_engine, "differential_row_count"),
            "promotion_consequence": "File-backed differential rows remain useful assurance evidence and do not promote an independent evaluator."
        }),
        json!({
            "row_id": "independent.formal_model_not_runtime_evaluator",
            "disposition_kind": "accepted_boundary",
            "independence_state": "proof_model_not_executable_runtime_evaluator",
            "evidence": W039_PROOF_MODEL_SUMMARY,
            "local_proof_row_count": number_at(w039_proof_model, "local_proof_row_count"),
            "totality_boundary_count": number_at(w039_proof_model, "totality_boundary_count"),
            "promotion_consequence": "Lean/TLA evidence strengthens specification and proof obligations but is not a separately implemented evaluator row set."
        }),
        json!({
            "row_id": "independent.fully_independent_evaluator_row_set_absent",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "independent_implementation_absent",
            "evidence": W038_DIVERSITY_DISPOSITION,
            "w038_diversity_disposition_row_count": number_at(w038_diversity, "diversity_disposition_row_count"),
            "w038_rows_present": row_count_at(w038_diversity_disposition),
            "promotion_consequence": "Full independent-evaluator diversity remains blocked until an independently implemented row set exists and has replay/differential evidence."
        }),
    ]
}

fn w039_cross_engine_diversity_rows(
    w036_cross_engine: &Value,
    w038_diversity: &Value,
    w039_stage2: &Value,
    w039_operated: &Value,
    w039_cross_engine_substrate: &Value,
    w039_formatting_intake: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "cross_engine.file_backed_differential_harness",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_not_operated",
            "evidence": W036_CROSS_ENGINE_SUMMARY,
            "differential_row_count": number_at(w036_cross_engine, "differential_row_count"),
            "promotion_consequence": "The harness is retained as deterministic evidence, not as an operated service."
        }),
        json!({
            "row_id": "cross_engine.w038_diversity_seam_baseline",
            "disposition_kind": "accepted_boundary",
            "service_state": "baseline_bound",
            "evidence": W038_DIVERSITY_SUMMARY,
            "diversity_disposition_row_count": number_at(w038_diversity, "diversity_disposition_row_count"),
            "seam_watch_row_count": number_at(w038_diversity, "seam_watch_row_count"),
            "promotion_consequence": "W038 baseline is carried forward as evidence lineage only."
        }),
        json!({
            "row_id": "cross_engine.stage2_service_dependency",
            "disposition_kind": "service_dependency_blocked",
            "service_state": "blocked",
            "evidence": W039_STAGE2_SUMMARY,
            "stage2_policy_promoted": bool_at(w039_stage2, "stage2_policy_promoted"),
            "exact_remaining_blocker_count": number_at(w039_stage2, "exact_remaining_blocker_count"),
            "promotion_consequence": "Stage 2 service-dependent diversity cannot promote while production policy and operated Stage 2 differential evidence are blocked."
        }),
        json!({
            "row_id": "cross_engine.operated_service_substrate",
            "disposition_kind": "service_substrate_bound",
            "service_state": if bool_at(w039_cross_engine_substrate, "operated_cross_engine_differential_service_present") { "present" } else { "blocked" },
            "evidence": W039_CROSS_ENGINE_SERVICE_SUBSTRATE,
            "file_backed_pilot_present": bool_at(w039_cross_engine_substrate, "file_backed_pilot_present"),
            "operated_cross_engine_differential_service_present": bool_at(w039_cross_engine_substrate, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "The substrate records service shape and blocked claims, but does not promote an operated cross-engine service."
        }),
        json!({
            "row_id": "cross_engine.retained_history_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_history_only",
            "evidence": W039_OPERATED_ASSURANCE_SUMMARY,
            "multi_run_history_row_count": number_at(w039_operated, "multi_run_history_row_count"),
            "retained_history_service_promoted": bool_at(w039_operated, "retained_history_service_promoted"),
            "promotion_consequence": "Retained history supports future service operation and replay audit, but the retained-history service remains unpromoted."
        }),
        json!({
            "row_id": "cross_engine.w073_typed_only_formatting_guard",
            "disposition_kind": "accepted_boundary",
            "service_state": "observable_guard_retained",
            "evidence": W039_FORMATTING_INTAKE,
            "typed_only_family_count": array_len_at(w039_formatting_intake, "typed_only_families"),
            "thresholds_rule": text_at(w039_formatting_intake, "thresholds_rule"),
            "promotion_consequence": "Diversity evidence must not compare formatting rows that still rely on W072 bounded threshold strings for W073 aggregate or visualization families."
        }),
        json!({
            "row_id": "cross_engine.mismatch_triage_and_quarantine_service",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W039_OPERATED_ASSURANCE_SUMMARY,
            "alert_decision_count": number_at(w039_operated, "alert_decision_count"),
            "quarantine_decision_count": number_at(w039_operated, "quarantine_decision_count"),
            "promotion_consequence": "Cross-engine diversity needs an operated mismatch triage/quarantine path before service-level promotion."
        }),
    ]
}

fn w039_differential_authority_rows(
    w036_independent: &Value,
    w036_cross_engine: &Value,
    w037_direct_oxfml: &Value,
    w039_conformance: &Value,
    w039_operated: &Value,
    w039_cross_engine_substrate: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "authority.tracecalc_correctness_oracle",
            "disposition_kind": "accepted_boundary",
            "authority_state": "reference_oracle",
            "evidence": W036_INDEPENDENT_SUMMARY,
            "comparison_row_count": number_at(w036_independent, "comparison_row_count"),
            "authority_limit": "oracle authority for covered observable behavior only"
        }),
        json!({
            "row_id": "authority.optimized_core_same_implementation_family",
            "disposition_kind": "same_authority_family",
            "authority_state": "not_independent",
            "evidence": W039_CONFORMANCE_SUMMARY,
            "w039_exact_remaining_blocker_count": number_at(w039_conformance, "w039_exact_remaining_blocker_count"),
            "authority_limit": "optimized/core conformance does not supply a separate implementation authority"
        }),
        json!({
            "row_id": "authority.direct_oxfml_external_formula_evaluator",
            "disposition_kind": "accepted_external_slice",
            "authority_state": "external_formula_evaluator_only",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "direct_oxfml_case_count": number_at(w037_direct_oxfml, "direct_oxfml_case_count"),
            "authority_limit": "external evaluator authority is limited to the consumed OxFml seam and does not implement OxCalc scheduling/publication"
        }),
        json!({
            "row_id": "authority.file_backed_cross_engine_differential",
            "disposition_kind": "accepted_boundary",
            "authority_state": "file_backed_artifact_authority",
            "evidence": W036_CROSS_ENGINE_SUMMARY,
            "differential_row_count": number_at(w036_cross_engine, "differential_row_count"),
            "authority_limit": "file-backed row agreement cannot substitute for an operated differential service"
        }),
        json!({
            "row_id": "authority.operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_operated_service",
            "evidence": W039_CROSS_ENGINE_SERVICE_SUBSTRATE,
            "operated_cross_engine_differential_service_present": bool_at(w039_cross_engine_substrate, "operated_cross_engine_differential_service_present"),
            "authority_limit": "no recurring service endpoint, retained service history, or service-level mismatch action path exists"
        }),
        json!({
            "row_id": "authority.independent_evaluator_implementation",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_independent_implementation",
            "evidence": W039_OPERATED_ASSURANCE_SUMMARY,
            "operated_cross_engine_differential_service_promoted": bool_at(w039_operated, "operated_cross_engine_differential_service_promoted"),
            "authority_limit": "no independently implemented evaluator row set exists"
        }),
        json!({
            "row_id": "authority.release_grade_diversity_promotion",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_release_grade_authority",
            "evidence": W039_OPERATED_ASSURANCE_SUMMARY,
            "exact_service_blocker_count": number_at(w039_operated, "exact_service_blocker_count"),
            "authority_limit": "release-grade diversity authority remains unavailable while independent implementation and operated service blockers remain"
        }),
    ]
}

fn w039_exact_blockers(
    w038_diversity_blockers: &Value,
    w039_stage2_blockers: &Value,
    w039_operated_service_blockers: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "blocker_id": "w039_diversity.fully_independent_evaluator_implementation_absent",
            "owner": "calc-f7o.6",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W038_DIVERSITY_BLOCKERS,
            "predecessor_blocker_present": row_with_field_exists(
                w038_diversity_blockers,
                "blocker_id",
                "diversity.fully_independent_evaluator_absent"
            ),
            "reason": "TraceCalc, TreeCalc/CoreEngine projection, direct OxFml, formal model, and file-backed differential rows do not constitute an independently implemented OxCalc evaluator row set.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "w039_diversity.independent_evaluator_authority_absent",
            "owner": "calc-f7o.6",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W039_CONFORMANCE_SUMMARY,
            "reason": "No row set is backed by a separate implementation authority with its own execution kernel and replay evidence.",
            "promotion_consequence": "independent evaluator row-set promotion remains unavailable"
        }),
        json!({
            "blocker_id": "w039_diversity.operated_cross_engine_service_absent",
            "owner": "calc-f7o.6; calc-f7o.5",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W039_CROSS_ENGINE_SERVICE_SUBSTRATE,
            "predecessor_blocker_present": row_with_field_exists(
                w038_diversity_blockers,
                "blocker_id",
                "diversity.operated_cross_engine_service_absent"
            ),
            "reason": "W039 has a file-backed cross-engine substrate, not an operated recurring differential service.",
            "promotion_consequence": "operated cross-engine diversity service remains unpromoted"
        }),
        json!({
            "blocker_id": "w039_diversity.stage2_differential_service_dependency_absent",
            "owner": "calc-f7o.4; calc-f7o.6",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W039_STAGE2_BLOCKERS,
            "stage2_blocker_rows": row_count_at(w039_stage2_blockers),
            "reason": "Stage 2 diversity needs production policy, partition soundness, and operated differential service evidence before strategy-level diversity promotion.",
            "promotion_consequence": "Stage 2 diversity remains a blocked dependency for release-grade promotion"
        }),
        json!({
            "blocker_id": "w039_diversity.mismatch_triage_and_quarantine_service_absent",
            "owner": "calc-f7o.5; calc-f7o.6",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W039_OPERATED_SERVICE_BLOCKERS,
            "service_blocker_rows": row_count_at(w039_operated_service_blockers),
            "reason": "The retained history and alert rules are bound, but no external alert dispatcher or mismatch quarantine service is operated.",
            "promotion_consequence": "service-level diversity assurance remains unpromoted"
        }),
        json!({
            "blocker_id": "w039_diversity.release_grade_promotion_authority_absent",
            "owner": "calc-f7o.9",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W039_OPERATED_ASSURANCE_SUMMARY,
            "reason": "Release-grade diversity cannot be claimed from proxy evidence, file-backed rows, shared projections, or retained blockers.",
            "promotion_consequence": "release-grade verification, pack/C5, and diversity promotion remain unavailable to this bead"
        }),
    ]
}

fn w039_diversity_validation_failures(
    independent_rows: &[Value],
    cross_engine_rows: &[Value],
    authority_rows: &[Value],
    blockers: &[Value],
) -> Vec<String> {
    let mut failures = Vec::new();
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("independent_implementation_absent")
    }) {
        failures.push("w039_diversity.independent_implementation_absent_row_missing".to_string());
    }
    if !cross_engine_rows
        .iter()
        .any(|row| row.get("service_state").and_then(Value::as_str) == Some("blocked"))
    {
        failures.push("w039_diversity.blocked_cross_engine_service_row_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_operated_service")
    }) {
        failures.push("w039_diversity.operated_service_authority_blocker_missing".to_string());
    }
    if blockers.len() < 6 {
        failures.push("w039_diversity.exact_blocker_count_below_gate".to_string());
    }
    if independent_rows
        .iter()
        .chain(cross_engine_rows.iter())
        .chain(authority_rows.iter())
        .any(|row| row.get("disposition_kind").and_then(Value::as_str) == Some("promoted"))
    {
        failures.push("w039_diversity.unexpected_promotion_row".to_string());
    }
    failures
}

#[derive(Debug, Clone)]
struct IndependentScalarCase {
    case_id: &'static str,
    expression: &'static str,
    environment: &'static [(&'static str, i64)],
    expected: i64,
}

fn independent_scalar_cases() -> Vec<IndependentScalarCase> {
    const EMPTY_ENV: &[(&str, i64)] = &[];
    const X3_ENV: &[(&str, i64)] = &[("X", 3)];
    const XY_ENV: &[(&str, i64)] = &[("X", 3), ("Y", 60)];

    vec![
        IndependentScalarCase {
            case_id: "scalar.literal",
            expression: "2",
            environment: EMPTY_ENV,
            expected: 2,
        },
        IndependentScalarCase {
            case_id: "scalar.precedence",
            expression: "2+3*4",
            environment: EMPTY_ENV,
            expected: 14,
        },
        IndependentScalarCase {
            case_id: "scalar.parentheses",
            expression: "(2+3)*4",
            environment: EMPTY_ENV,
            expected: 20,
        },
        IndependentScalarCase {
            case_id: "scalar.named_reference_y",
            expression: "X*20",
            environment: X3_ENV,
            expected: 60,
        },
        IndependentScalarCase {
            case_id: "scalar.incremental_chain_z",
            expression: "X+Y",
            environment: XY_ENV,
            expected: 63,
        },
    ]
}

fn eval_scalar_expression(
    expression: &str,
    environment: &[(&'static str, i64)],
) -> Result<i64, String> {
    let mut parser = ScalarParser {
        input: expression.as_bytes(),
        position: 0,
        environment,
    };
    let value = parser.parse_expr()?;
    parser.skip_ws();
    if parser.position == parser.input.len() {
        Ok(value)
    } else {
        Err(format!(
            "unexpected trailing input at byte {}",
            parser.position
        ))
    }
}

struct ScalarParser<'a> {
    input: &'a [u8],
    position: usize,
    environment: &'a [(&'static str, i64)],
}

impl ScalarParser<'_> {
    fn parse_expr(&mut self) -> Result<i64, String> {
        let mut value = self.parse_term()?;
        loop {
            self.skip_ws();
            if !self.consume(b'+') {
                return Ok(value);
            }
            value += self.parse_term()?;
        }
    }

    fn parse_term(&mut self) -> Result<i64, String> {
        let mut value = self.parse_factor()?;
        loop {
            self.skip_ws();
            if !self.consume(b'*') {
                return Ok(value);
            }
            value *= self.parse_factor()?;
        }
    }

    fn parse_factor(&mut self) -> Result<i64, String> {
        self.skip_ws();
        if self.consume(b'(') {
            let value = self.parse_expr()?;
            self.skip_ws();
            if self.consume(b')') {
                return Ok(value);
            }
            return Err(format!("missing ')' at byte {}", self.position));
        }
        if self.peek().is_some_and(|byte| byte.is_ascii_digit()) {
            return self.parse_number();
        }
        if self
            .peek()
            .is_some_and(|byte| byte.is_ascii_alphabetic() || byte == b'_')
        {
            return self.parse_identifier();
        }
        Err(format!("expected factor at byte {}", self.position))
    }

    fn parse_number(&mut self) -> Result<i64, String> {
        let start = self.position;
        while self.peek().is_some_and(|byte| byte.is_ascii_digit()) {
            self.position += 1;
        }
        std::str::from_utf8(&self.input[start..self.position])
            .map_err(|err| err.to_string())?
            .parse::<i64>()
            .map_err(|err| err.to_string())
    }

    fn parse_identifier(&mut self) -> Result<i64, String> {
        let start = self.position;
        while self
            .peek()
            .is_some_and(|byte| byte.is_ascii_alphanumeric() || byte == b'_')
        {
            self.position += 1;
        }
        let name = std::str::from_utf8(&self.input[start..self.position])
            .map_err(|err| err.to_string())?;
        self.environment
            .iter()
            .find_map(|(candidate, value)| (*candidate == name).then_some(*value))
            .ok_or_else(|| format!("unknown name {name}"))
    }

    fn skip_ws(&mut self) {
        while self.peek().is_some_and(|byte| byte.is_ascii_whitespace()) {
            self.position += 1;
        }
    }

    fn consume(&mut self, expected: u8) -> bool {
        if self.peek() == Some(expected) {
            self.position += 1;
            true
        } else {
            false
        }
    }

    fn peek(&self) -> Option<u8> {
        self.input.get(self.position).copied()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum FormulaValue {
    Int(i64),
    Bool(bool),
}

impl FormulaValue {
    fn as_int(self) -> Result<i64, String> {
        match self {
            Self::Int(value) => Ok(value),
            Self::Bool(value) => Err(format!("expected integer value, found boolean {value}")),
        }
    }

    fn as_bool(self) -> Result<bool, String> {
        match self {
            Self::Bool(value) => Ok(value),
            Self::Int(value) => Err(format!("expected boolean value, found integer {value}")),
        }
    }
}

#[derive(Debug, Clone)]
struct IndependentFormulaCase {
    case_id: &'static str,
    expression: &'static str,
    environment: &'static [(&'static str, FormulaValue)],
    expected: FormulaValue,
}

fn w041_independent_formula_cases() -> Vec<IndependentFormulaCase> {
    const EMPTY_ENV: &[(&str, FormulaValue)] = &[];
    const X3_ENV: &[(&str, FormulaValue)] = &[("X", FormulaValue::Int(3))];
    const XY_ENV: &[(&str, FormulaValue)] =
        &[("X", FormulaValue::Int(3)), ("Y", FormulaValue::Int(60))];

    vec![
        IndependentFormulaCase {
            case_id: "formula.literal",
            expression: "2",
            environment: EMPTY_ENV,
            expected: FormulaValue::Int(2),
        },
        IndependentFormulaCase {
            case_id: "formula.precedence",
            expression: "2+3*4",
            environment: EMPTY_ENV,
            expected: FormulaValue::Int(14),
        },
        IndependentFormulaCase {
            case_id: "formula.subtraction_unary",
            expression: "-(X-5)",
            environment: X3_ENV,
            expected: FormulaValue::Int(2),
        },
        IndependentFormulaCase {
            case_id: "formula.division",
            expression: "Y/X+1",
            environment: XY_ENV,
            expected: FormulaValue::Int(21),
        },
        IndependentFormulaCase {
            case_id: "formula.comparison_equal",
            expression: "X*20=Y",
            environment: XY_ENV,
            expected: FormulaValue::Bool(true),
        },
        IndependentFormulaCase {
            case_id: "formula.if_branch",
            expression: "IF(X>2,Y,X)",
            environment: XY_ENV,
            expected: FormulaValue::Int(60),
        },
        IndependentFormulaCase {
            case_id: "formula.incremental_chain_z",
            expression: "X+Y",
            environment: XY_ENV,
            expected: FormulaValue::Int(63),
        },
        IndependentFormulaCase {
            case_id: "formula.not_equal_guard",
            expression: "IF(X<>2,63,0)",
            environment: X3_ENV,
            expected: FormulaValue::Int(63),
        },
    ]
}

fn eval_formula_fragment(
    expression: &str,
    environment: &[(&'static str, FormulaValue)],
) -> Result<FormulaValue, String> {
    let mut parser = FormulaParser {
        input: expression.as_bytes(),
        position: 0,
        environment,
    };
    let value = parser.parse_expression()?;
    parser.skip_ws();
    if parser.position == parser.input.len() {
        Ok(value)
    } else {
        Err(format!(
            "unexpected trailing input at byte {}",
            parser.position
        ))
    }
}

struct FormulaParser<'a> {
    input: &'a [u8],
    position: usize,
    environment: &'a [(&'static str, FormulaValue)],
}

impl FormulaParser<'_> {
    fn parse_expression(&mut self) -> Result<FormulaValue, String> {
        self.parse_comparison()
    }

    fn parse_comparison(&mut self) -> Result<FormulaValue, String> {
        let left = self.parse_additive()?;
        self.skip_ws();
        let operator = if self.consume_two(b'>', b'=') {
            Some(">=")
        } else if self.consume_two(b'<', b'=') {
            Some("<=")
        } else if self.consume_two(b'<', b'>') {
            Some("<>")
        } else if self.consume(b'>') {
            Some(">")
        } else if self.consume(b'<') {
            Some("<")
        } else if self.consume(b'=') {
            Some("=")
        } else {
            None
        };

        let Some(operator) = operator else {
            return Ok(left);
        };
        let right = self.parse_additive()?;
        let left = left.as_int()?;
        let right = right.as_int()?;
        Ok(FormulaValue::Bool(match operator {
            ">" => left > right,
            ">=" => left >= right,
            "<" => left < right,
            "<=" => left <= right,
            "=" => left == right,
            "<>" => left != right,
            _ => unreachable!("operator is constrained above"),
        }))
    }

    fn parse_additive(&mut self) -> Result<FormulaValue, String> {
        let mut value = self.parse_term()?.as_int()?;
        loop {
            self.skip_ws();
            if self.consume(b'+') {
                value += self.parse_term()?.as_int()?;
            } else if self.consume(b'-') {
                value -= self.parse_term()?.as_int()?;
            } else {
                return Ok(FormulaValue::Int(value));
            }
        }
    }

    fn parse_term(&mut self) -> Result<FormulaValue, String> {
        let mut value = self.parse_factor()?.as_int()?;
        loop {
            self.skip_ws();
            if self.consume(b'*') {
                value *= self.parse_factor()?.as_int()?;
            } else if self.consume(b'/') {
                let divisor = self.parse_factor()?.as_int()?;
                if divisor == 0 {
                    return Err(format!("division by zero at byte {}", self.position));
                }
                value /= divisor;
            } else {
                return Ok(FormulaValue::Int(value));
            }
        }
    }

    fn parse_factor(&mut self) -> Result<FormulaValue, String> {
        self.skip_ws();
        if self.consume(b'-') {
            return Ok(FormulaValue::Int(-self.parse_factor()?.as_int()?));
        }
        if self.consume(b'(') {
            let value = self.parse_expression()?;
            self.skip_ws();
            if self.consume(b')') {
                return Ok(value);
            }
            return Err(format!("missing ')' at byte {}", self.position));
        }
        if self.peek().is_some_and(|byte| byte.is_ascii_digit()) {
            return self.parse_number().map(FormulaValue::Int);
        }
        if self
            .peek()
            .is_some_and(|byte| byte.is_ascii_alphabetic() || byte == b'_')
        {
            let name = self.parse_name()?;
            if name.eq_ignore_ascii_case("IF") {
                return self.parse_if_call();
            }
            return self
                .environment
                .iter()
                .find_map(|(candidate, value)| (*candidate == name).then_some(*value))
                .ok_or_else(|| format!("unknown name {name}"));
        }
        Err(format!("expected factor at byte {}", self.position))
    }

    fn parse_if_call(&mut self) -> Result<FormulaValue, String> {
        self.skip_ws();
        if !self.consume(b'(') {
            return Err(format!("expected '(' after IF at byte {}", self.position));
        }
        let condition = self.parse_expression()?.as_bool()?;
        self.skip_ws();
        if !self.consume(b',') {
            return Err(format!("expected first IF comma at byte {}", self.position));
        }
        let true_value = self.parse_expression()?;
        self.skip_ws();
        if !self.consume(b',') {
            return Err(format!(
                "expected second IF comma at byte {}",
                self.position
            ));
        }
        let false_value = self.parse_expression()?;
        self.skip_ws();
        if !self.consume(b')') {
            return Err(format!("missing ')' after IF at byte {}", self.position));
        }
        Ok(if condition { true_value } else { false_value })
    }

    fn parse_number(&mut self) -> Result<i64, String> {
        let start = self.position;
        while self.peek().is_some_and(|byte| byte.is_ascii_digit()) {
            self.position += 1;
        }
        std::str::from_utf8(&self.input[start..self.position])
            .map_err(|err| err.to_string())?
            .parse::<i64>()
            .map_err(|err| err.to_string())
    }

    fn parse_name(&mut self) -> Result<String, String> {
        let start = self.position;
        while self
            .peek()
            .is_some_and(|byte| byte.is_ascii_alphanumeric() || byte == b'_')
        {
            self.position += 1;
        }
        std::str::from_utf8(&self.input[start..self.position])
            .map(str::to_string)
            .map_err(|err| err.to_string())
    }

    fn skip_ws(&mut self) {
        while self.peek().is_some_and(|byte| byte.is_ascii_whitespace()) {
            self.position += 1;
        }
    }

    fn consume(&mut self, expected: u8) -> bool {
        if self.peek() == Some(expected) {
            self.position += 1;
            true
        } else {
            false
        }
    }

    fn consume_two(&mut self, first: u8, second: u8) -> bool {
        if self.input.get(self.position..self.position + 2) == Some([first, second].as_slice()) {
            self.position += 2;
            true
        } else {
            false
        }
    }

    fn peek(&self) -> Option<u8> {
        self.input.get(self.position).copied()
    }
}

fn formula_value_json(value: Option<&FormulaValue>) -> Value {
    match value {
        Some(FormulaValue::Int(value)) => json!(value),
        Some(FormulaValue::Bool(value)) => json!(value),
        None => Value::Null,
    }
}

fn w041_independent_formula_evaluator_rows() -> Vec<Value> {
    w041_independent_formula_cases()
        .into_iter()
        .map(|case| {
            let result = eval_formula_fragment(case.expression, case.environment);
            let (actual_value, error) = match result {
                Ok(value) => (Some(value), None),
                Err(err) => (None, Some(err)),
            };
            let environment = case
                .environment
                .iter()
                .map(|(name, value)| {
                    json!({
                        "name": name,
                        "value": formula_value_json(Some(value))
                    })
                })
                .collect::<Vec<_>>();
            json!({
                "case_id": case.case_id,
                "expression": case.expression,
                "environment": environment,
                "expected_value": formula_value_json(Some(&case.expected)),
                "actual_value": formula_value_json(actual_value.as_ref()),
                "error": error,
                "matches_expected": actual_value == Some(case.expected),
                "implementation_path": "src/oxcalc-tracecalc/src/diversity_seam.rs::eval_formula_fragment",
                "implementation_kind": "broadened_independent_formula_fragment_evaluator",
                "scope_limit": "integer names and literals with +, -, *, /, parentheses, comparisons, and IF(condition, true_expr, false_expr)",
                "independent_from": ["TraceCalc", "optimized_core", "TreeCalc", "OxFml", "OxFunc"]
            })
        })
        .collect()
}

#[derive(Debug, Clone)]
struct ReferenceModelCase {
    case_id: &'static str,
    assignments: &'static [(&'static str, &'static str)],
    expected: &'static [(&'static str, FormulaValue)],
}

fn w042_reference_model_cases() -> Vec<ReferenceModelCase> {
    const BASIC_X2_ASSIGNMENTS: &[(&str, &str)] = &[("X", "2"), ("Y", "X*20"), ("Z", "X+Y")];
    const BASIC_X2_EXPECTED: &[(&str, FormulaValue)] = &[
        ("X", FormulaValue::Int(2)),
        ("Y", FormulaValue::Int(40)),
        ("Z", FormulaValue::Int(42)),
    ];
    const BASIC_X3_ASSIGNMENTS: &[(&str, &str)] = &[("X", "3"), ("Y", "X*20"), ("Z", "X+Y")];
    const BASIC_X3_EXPECTED: &[(&str, FormulaValue)] = &[
        ("X", FormulaValue::Int(3)),
        ("Y", FormulaValue::Int(60)),
        ("Z", FormulaValue::Int(63)),
    ];
    const ROW_COLUMN_ASSIGNMENTS: &[(&str, &str)] = &[
        ("Top", "5"),
        ("Left", "7"),
        ("Sum", "Top+Left"),
        ("Gate", "IF(Sum>10,Sum*2,0)"),
    ];
    const ROW_COLUMN_EXPECTED: &[(&str, FormulaValue)] = &[
        ("Top", FormulaValue::Int(5)),
        ("Left", FormulaValue::Int(7)),
        ("Sum", FormulaValue::Int(12)),
        ("Gate", FormulaValue::Int(24)),
    ];
    const EDIT_DELTA_ASSIGNMENTS: &[(&str, &str)] = &[
        ("BeforeX", "2"),
        ("BeforeY", "BeforeX*20"),
        ("BeforeZ", "BeforeX+BeforeY"),
        ("AfterX", "3"),
        ("AfterY", "AfterX*20"),
        ("AfterZ", "AfterX+AfterY"),
        ("DeltaZ", "AfterZ-BeforeZ"),
    ];
    const EDIT_DELTA_EXPECTED: &[(&str, FormulaValue)] = &[
        ("BeforeX", FormulaValue::Int(2)),
        ("BeforeY", FormulaValue::Int(40)),
        ("BeforeZ", FormulaValue::Int(42)),
        ("AfterX", FormulaValue::Int(3)),
        ("AfterY", FormulaValue::Int(60)),
        ("AfterZ", FormulaValue::Int(63)),
        ("DeltaZ", FormulaValue::Int(21)),
    ];

    vec![
        ReferenceModelCase {
            case_id: "reference_model.xyz_x2",
            assignments: BASIC_X2_ASSIGNMENTS,
            expected: BASIC_X2_EXPECTED,
        },
        ReferenceModelCase {
            case_id: "reference_model.xyz_x3",
            assignments: BASIC_X3_ASSIGNMENTS,
            expected: BASIC_X3_EXPECTED,
        },
        ReferenceModelCase {
            case_id: "reference_model.row_column_gate",
            assignments: ROW_COLUMN_ASSIGNMENTS,
            expected: ROW_COLUMN_EXPECTED,
        },
        ReferenceModelCase {
            case_id: "reference_model.edit_delta",
            assignments: EDIT_DELTA_ASSIGNMENTS,
            expected: EDIT_DELTA_EXPECTED,
        },
    ]
}

fn eval_reference_model(
    assignments: &[(&'static str, &'static str)],
) -> Result<Vec<(&'static str, FormulaValue)>, String> {
    let mut environment = Vec::new();
    for (name, expression) in assignments {
        let value = eval_formula_fragment(expression, &environment)?;
        environment.push((*name, value));
    }
    Ok(environment)
}

fn reference_bindings_json(bindings: &[(&'static str, FormulaValue)]) -> Value {
    Value::Array(
        bindings
            .iter()
            .map(|(name, value)| {
                json!({
                    "name": name,
                    "value": formula_value_json(Some(value))
                })
            })
            .collect(),
    )
}

fn w042_independent_reference_model_rows() -> Vec<Value> {
    w042_reference_model_cases()
        .into_iter()
        .map(|case| {
            let result = eval_reference_model(case.assignments);
            let (actual_bindings, error) = match result {
                Ok(bindings) => (bindings, None),
                Err(err) => (Vec::new(), Some(err)),
            };
            let matches_expected = error.is_none() && actual_bindings == case.expected;
            let assignment_rows = case
                .assignments
                .iter()
                .map(|(name, expression)| {
                    json!({
                        "name": name,
                        "formula": expression
                    })
                })
                .collect::<Vec<_>>();
            json!({
                "case_id": case.case_id,
                "assignments": assignment_rows,
                "expected_bindings": reference_bindings_json(case.expected),
                "actual_bindings": reference_bindings_json(&actual_bindings),
                "error": error,
                "matches_expected": matches_expected,
                "implementation_path": "src/oxcalc-tracecalc/src/diversity_seam.rs::eval_reference_model",
                "implementation_kind": "independent_reference_model_evaluator",
                "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, and deterministic before/after edit checks",
                "independent_from": ["TraceCalc", "optimized_core", "TreeCalc", "OxFml", "OxFunc"]
            })
        })
        .collect()
}

#[allow(clippy::too_many_arguments)]
fn w042_source_rows(
    w042_obligation_summary: &Value,
    w042_obligation_map: &Value,
    w042_formatting_intake: &Value,
    w041_diversity: &Value,
    w041_implementation: &Value,
    w041_independent: &Value,
    w041_cross_engine: &Value,
    w041_authority: &Value,
    w041_blockers: &Value,
    w041_decision: &Value,
    w042_stage2: &Value,
    w042_operated: &Value,
    w042_retained: &Value,
    w042_witness: &Value,
    w042_cross_engine_service: &Value,
    w042_alert_dispatch: &Value,
    w042_service_blockers: &Value,
    w042_operated_decision: &Value,
    w041_pack_decision: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "source.w042_closure_obligation_map",
            "artifact": W042_OBLIGATION_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w042_obligation_summary, "status") == "residual_release_grade_closure_obligation_ledger_validated"
                && number_at(w042_obligation_map, "obligation_count") == 33
                && bool_at(w042_obligation_summary, "oxfml_formatting_update_incorporated"),
            "semantic_state": "w042_diversity_obligations_bound_with_formatting_guard"
        }),
        json!({
            "row_id": "source.w042_w073_typed_formatting_guard",
            "artifact": W042_FORMATTING_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w042_formatting_intake, "status") == "typed_only_direct_replacement_guard_retained"
                && array_len_at(w042_formatting_intake, "typed_rule_only_families") == 7
                && !bool_at(w042_formatting_intake, "threshold_fallback_allowed_for_typed_families"),
            "semantic_state": "w073_typed_rule_only_formatting_guard_retained_for_w042_diversity"
        }),
        json!({
            "row_id": "source.w041_diversity_predecessor",
            "artifact": W041_DIVERSITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w041_diversity, "failed_row_count"),
            "promotion_guard": !bool_at(w041_diversity, "fully_independent_evaluator_promoted")
                && !bool_at(w041_diversity, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w041_decision, "fully_independent_evaluator_promoted")
                && number_at(w041_diversity, "exact_blocker_count") == 6,
            "semantic_state": "w041_diversity_predecessor_bound_without_promotion"
        }),
        json!({
            "row_id": "source.w041_formula_fragment_predecessor",
            "artifact": W041_INDEPENDENT_IMPLEMENTATION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w041_implementation, "failed_case_count"),
            "promotion_guard": number_at(w041_implementation, "case_count") == 8
                && number_at(w041_implementation, "match_count") == 8
                && !bool_at(w041_implementation, "fully_independent_evaluator_promoted"),
            "semantic_state": "w041_formula_fragment_narrowed_by_w042_reference_model"
        }),
        json!({
            "row_id": "source.w041_independent_breadth_register",
            "artifact": W041_INDEPENDENT_ROW_SET,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w041_independent, "row_count") == 10
                && !bool_at(w041_independent, "fully_independent_evaluator_promoted"),
            "semantic_state": "w041_independent_breadth_register_bound"
        }),
        json!({
            "row_id": "source.w041_cross_engine_differential_predecessor",
            "artifact": W041_CROSS_ENGINE_DIVERSITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w041_cross_engine, "row_count") == 9
                && !bool_at(w041_cross_engine, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w041_file_backed_cross_engine_rows_bound_without_operated_service"
        }),
        json!({
            "row_id": "source.w041_mismatch_authority_predecessor",
            "artifact": W041_MISMATCH_AUTHORITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w041_authority, "row_count") == 9
                && !bool_at(w041_authority, "fully_independent_evaluator_promoted")
                && !bool_at(w041_authority, "operated_service_authority_promoted"),
            "semantic_state": "w041_mismatch_authority_bound_without_service_promotion"
        }),
        json!({
            "row_id": "source.w041_diversity_blockers",
            "artifact": W041_DIVERSITY_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w041_blockers, "exact_blocker_count") == 6,
            "semantic_state": "w041_exact_diversity_blockers_bound_for_w042"
        }),
        json!({
            "row_id": "source.w042_stage2_service_dependency",
            "artifact": W042_STAGE2_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w042_stage2, "failed_row_count"),
            "promotion_guard": !bool_at(w042_stage2, "stage2_policy_promoted")
                && !bool_at(w042_stage2, "stage2_promotion_candidate")
                && number_at(w042_stage2, "exact_remaining_blocker_count") == 6,
            "semantic_state": "w042_stage2_service_dependency_bound_without_policy_promotion"
        }),
        json!({
            "row_id": "source.w042_operated_assurance_service_packet",
            "artifact": W042_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w042_operated, "failed_row_count"),
            "promotion_guard": bool_at(w042_operated, "file_backed_service_envelope_present")
                && bool_at(w042_operated, "retained_history_query_api_contract_present")
                && bool_at(w042_operated, "retained_witness_lifecycle_register_present")
                && !bool_at(w042_operated, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w042_file_backed_service_packet_bound_without_operated_service_promotion"
        }),
        json!({
            "row_id": "source.w042_retained_history_witness_support",
            "artifact": W042_RETAINED_HISTORY_QUERY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w042_retained, "query_register_row_count") == 10
                && number_at(w042_retained, "replay_correlation_row_count") == 8
                && bool_at(w042_witness, "retained_witness_lifecycle_register_present")
                && number_at(w042_witness, "pack_eligible_witness_count") == 0
                && !bool_at(w042_witness, "retained_witness_lifecycle_service_promoted"),
            "semantic_state": "w042_retained_history_and_witness_support_bound"
        }),
        json!({
            "row_id": "source.w042_cross_engine_service_blocker",
            "artifact": W042_CROSS_ENGINE_SERVICE_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w042_cross_engine_service, "file_backed_cross_engine_substrate_present")
                && !bool_at(w042_cross_engine_service, "operated_cross_engine_differential_service_present")
                && array_contains_string(
                    w042_cross_engine_service,
                    "blocked_service_claims",
                    "recurring_cross_engine_diff_scheduler"
                ),
            "semantic_state": "w042_cross_engine_service_blocker_retained"
        }),
        json!({
            "row_id": "source.w042_alert_quarantine_contract",
            "artifact": W042_ALERT_DISPATCH_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w042_alert_dispatch, "evaluated_rule_count") == 23
                && number_at(w042_alert_dispatch, "quarantine_decision_count") == 0
                && !bool_at(w042_alert_dispatch, "external_alert_dispatcher_promoted")
                && !bool_at(w042_alert_dispatch, "quarantine_service_promoted"),
            "semantic_state": "w042_local_alert_quarantine_contract_clean"
        }),
        json!({
            "row_id": "source.w042_service_and_pack_blockers",
            "artifact": W042_OPERATED_SERVICE_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w042_service_blockers, "exact_service_blocker_count") == 6
                && text_at(w041_pack_decision, "decision_status") == "capability_not_promoted"
                && !bool_at(w041_pack_decision, "capability_promoted"),
            "semantic_state": "w042_service_and_pack_no_promotion_blockers_bound"
        }),
        json!({
            "row_id": "source.w042_diversity_no_proxy_promotion_guard",
            "artifact": W042_OPERATED_PROMOTION_DECISION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w041_decision, "fully_independent_evaluator_promoted")
                && !bool_at(w041_decision, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w042_operated_decision, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w042_operated_decision, "quarantine_service_promoted")
                && !bool_at(w042_stage2, "stage2_policy_promoted")
                && !bool_at(w041_pack_decision, "capability_promoted"),
            "semantic_state": "w042_diversity_stage2_pack_service_promotion_guard_clean"
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w042_independent_evaluator_rows(
    w041_independent: &Value,
    w041_implementation: &Value,
    w041_diversity: &Value,
    w042_stage2: &Value,
    w042_operated: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "independent.w042_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "independence_state": "independent_reference_model_slice_present",
            "evidence": "w042_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "W042 broadens the independent slice to named-reference model evaluation, but still does not cover full TreeCalc/OxCalc evaluator breadth."
        }),
        json!({
            "row_id": "independent.w041_formula_fragment_predecessor_narrowed",
            "disposition_kind": "accepted_boundary",
            "independence_state": "predecessor_formula_fragment_subsumed_by_reference_model_slice",
            "evidence": W041_INDEPENDENT_IMPLEMENTATION,
            "w041_case_count": number_at(w041_implementation, "case_count"),
            "w041_row_count": number_at(w041_independent, "row_count"),
            "w041_fully_independent_evaluator_promoted": bool_at(w041_diversity, "fully_independent_evaluator_promoted"),
            "promotion_consequence": "W041 formula-fragment evidence remains evidence lineage only."
        }),
        json!({
            "row_id": "independent.tracecalc_reference_oracle_boundary",
            "disposition_kind": "accepted_boundary",
            "independence_state": "tracecalc_is_correctness_oracle_not_independent_implementation",
            "evidence": "TraceCalc",
            "promotion_consequence": "TraceCalc remains the correctness oracle for covered behavior, not a diversity implementation."
        }),
        json!({
            "row_id": "independent.optimized_core_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "optimized_core_same_implementation_family",
            "evidence": "W042 optimized/core counterpart packet",
            "promotion_consequence": "Optimized/core evidence strengthens conformance but is not independent implementation authority."
        }),
        json!({
            "row_id": "independent.treecalc_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "treecalc_same_oxcalc_family",
            "evidence": "W042 TreeCalc counterpart evidence",
            "promotion_consequence": "TreeCalc evidence is core conformance evidence, not independent evaluator diversity."
        }),
        json!({
            "row_id": "independent.direct_oxfml_external_formula_slice",
            "disposition_kind": "accepted_external_slice",
            "independence_state": "external_formula_evaluator_not_oxcalc_coordinator",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "promotion_consequence": "OxFml formula evidence remains an external formula-evaluator slice, including narrow LET/LAMBDA seam evidence, not OxCalc coordinator breadth."
        }),
        json!({
            "row_id": "independent.proof_model_not_runtime_evaluator",
            "disposition_kind": "accepted_boundary",
            "independence_state": "proof_model_not_executable_runtime_evaluator",
            "evidence": "W042 Lean/TLA proof/model packet",
            "promotion_consequence": "Proof/model evidence supports specification and refinement, but does not supply runtime diversity."
        }),
        json!({
            "row_id": "independent.stage2_declared_profile_not_independent_runtime",
            "disposition_kind": "accepted_boundary",
            "independence_state": "stage2_policy_profiles_not_second_evaluator",
            "evidence": W042_STAGE2_SUMMARY,
            "policy_row_count": number_at(w042_stage2, "policy_row_count"),
            "stage2_policy_promoted": bool_at(w042_stage2, "stage2_policy_promoted"),
            "promotion_consequence": "Stage 2 profile evidence remains policy/replay evidence, not separate evaluator implementation authority."
        }),
        json!({
            "row_id": "independent.w042_service_packet_not_independent_runtime",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "service_packet_file_backed_not_independent_evaluator",
            "evidence": W042_OPERATED_ASSURANCE_SUMMARY,
            "service_readiness_blocked_count": number_at(w042_operated, "service_readiness_blocked_count"),
            "promotion_consequence": "Service evidence improves retention and authority routing but is not independent evaluator breadth."
        }),
        json!({
            "row_id": "independent.full_independent_evaluator_breadth_blocked",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "full_independent_implementation_breadth_absent",
            "evidence": W042_OBLIGATION_MAP,
            "promotion_consequence": "Full independent-evaluator diversity remains blocked beyond the named-reference model slice."
        }),
        json!({
            "row_id": "independent.oxfml_callable_breadth_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "callable_and_publication_breadth_dependency_retained",
            "evidence": W042_OBLIGATION_MAP,
            "promotion_consequence": "LET/LAMBDA and callable-carrier interaction remains in scope for later breadth, with OxFunc peeking only where the seam threads through OxCalc/OxFml."
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w042_cross_engine_differential_rows(
    w041_cross_engine: &Value,
    w042_stage2: &Value,
    w042_operated: &Value,
    w042_retained: &Value,
    w042_witness: &Value,
    w042_cross_engine_service: &Value,
    w042_alert_dispatch: &Value,
    w042_formatting_intake: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "cross_engine.w042_reference_model_differential",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_reference_model_rows",
            "evidence": "w042_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "Reference-model rows can feed later cross-engine service work but are not an operated service."
        }),
        json!({
            "row_id": "cross_engine.w041_file_backed_differential_predecessor",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_not_operated",
            "evidence": W041_CROSS_ENGINE_DIVERSITY,
            "w041_row_count": number_at(w041_cross_engine, "row_count"),
            "promotion_consequence": "W041 file-backed diversity remains evidence lineage, not service promotion."
        }),
        json!({
            "row_id": "cross_engine.w042_service_envelope_contract",
            "disposition_kind": "service_contract_boundary",
            "service_state": "file_backed_service_envelope_only",
            "evidence": W042_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w042_operated, "service_envelope_row_count"),
            "file_backed_service_envelope_present": bool_at(w042_operated, "file_backed_service_envelope_present"),
            "promotion_consequence": "The service envelope records rows needed by a future operated service without claiming operation."
        }),
        json!({
            "row_id": "cross_engine.w042_retained_history_query_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_history_query_and_correlation_only",
            "evidence": W042_RETAINED_HISTORY_QUERY,
            "query_register_row_count": number_at(w042_retained, "query_register_row_count"),
            "replay_correlation_row_count": number_at(w042_retained, "replay_correlation_row_count"),
            "promotion_consequence": "Retained query/correlation artifacts support later differential service operation but are not themselves a service."
        }),
        json!({
            "row_id": "cross_engine.w042_retained_witness_lifecycle_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_witness_lifecycle_only",
            "evidence": W042_RETAINED_WITNESS_LIFECYCLE,
            "retained_witness_lifecycle_row_count": number_at(w042_witness, "witness_lifecycle_row_count"),
            "pack_eligible_witness_count": number_at(w042_witness, "pack_eligible_witness_count"),
            "promotion_consequence": "Retained-witness lifecycle rows support later differential and pack governance work but are not an operated lifecycle service."
        }),
        json!({
            "row_id": "cross_engine.w042_stage2_service_dependency",
            "disposition_kind": "service_dependency_blocked",
            "service_state": "blocked",
            "evidence": W042_STAGE2_SUMMARY,
            "stage2_policy_promoted": bool_at(w042_stage2, "stage2_policy_promoted"),
            "exact_remaining_blocker_count": number_at(w042_stage2, "exact_remaining_blocker_count"),
            "promotion_consequence": "Stage 2 service-dependent diversity remains blocked by operated service and pack-governance dependencies."
        }),
        json!({
            "row_id": "cross_engine.w042_cross_engine_service_register",
            "disposition_kind": "service_substrate_bound",
            "service_state": if bool_at(w042_cross_engine_service, "operated_cross_engine_differential_service_present") { "present" } else { "blocked" },
            "evidence": W042_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w042_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w042_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "The W042 service register records the operated-service blocker exactly."
        }),
        json!({
            "row_id": "cross_engine.w042_mismatch_triage_and_quarantine_contract",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W042_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w042_alert_dispatch, "evaluated_rule_count"),
            "external_alert_dispatcher_promoted": bool_at(w042_alert_dispatch, "external_alert_dispatcher_promoted"),
            "quarantine_service_promoted": bool_at(w042_alert_dispatch, "quarantine_service_promoted"),
            "promotion_consequence": "Local alert/quarantine rules are evaluated, but no external mismatch triage/quarantine service is operated."
        }),
        json!({
            "row_id": "cross_engine.w073_typed_only_formatting_guard",
            "disposition_kind": "accepted_boundary",
            "service_state": "observable_guard_retained",
            "evidence": W042_FORMATTING_INTAKE,
            "typed_rule_only_family_count": array_len_at(w042_formatting_intake, "typed_rule_only_families"),
            "threshold_fallback_allowed_for_typed_families": bool_at(w042_formatting_intake, "threshold_fallback_allowed_for_typed_families"),
            "promotion_consequence": "Diversity comparisons must preserve the W073 typed_rule-only aggregate/visualization formatting guard."
        }),
        json!({
            "row_id": "cross_engine.pack_governance_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W041_PACK_DECISION,
            "promotion_consequence": "Pack/C5 and release-grade diversity remain unavailable while operated differential service, mismatch quarantine, and independent breadth are blocked."
        }),
    ]
}

fn w042_mismatch_authority_rows(
    w041_authority: &Value,
    w042_operated: &Value,
    w042_cross_engine_service: &Value,
    w042_alert_dispatch: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "authority.w042_independent_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "authority_state": "independent_reference_model_runtime",
            "evidence": "w042_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "authority_limit": "named-reference model slice only; no scheduling, publication, effects, callable-carrier, or full OxFml/OxFunc kernel authority"
        }),
        json!({
            "row_id": "authority.w041_formula_fragment_predecessor",
            "disposition_kind": "accepted_boundary",
            "authority_state": "predecessor_formula_fragment_runtime",
            "evidence": W041_MISMATCH_AUTHORITY,
            "w041_authority_row_count": number_at(w041_authority, "row_count"),
            "authority_limit": "predecessor authority remains a narrower formula-fragment slice"
        }),
        json!({
            "row_id": "authority.tracecalc_correctness_oracle",
            "disposition_kind": "accepted_boundary",
            "authority_state": "reference_oracle",
            "evidence": "TraceCalc",
            "authority_limit": "oracle authority for covered observable behavior only"
        }),
        json!({
            "row_id": "authority.optimized_core_same_implementation_family",
            "disposition_kind": "same_authority_family",
            "authority_state": "not_independent",
            "evidence": "W042 optimized/core counterpart packet",
            "authority_limit": "optimized/core conformance does not supply separate implementation authority"
        }),
        json!({
            "row_id": "authority.direct_oxfml_external_formula_evaluator",
            "disposition_kind": "accepted_external_slice",
            "authority_state": "external_formula_evaluator_only",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "authority_limit": "external evaluator authority is limited to the consumed OxFml seam and does not implement OxCalc scheduling/publication"
        }),
        json!({
            "row_id": "authority.service_envelope_file_backed_authority",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "file_backed_artifact_authority",
            "evidence": W042_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w042_operated, "service_envelope_row_count"),
            "authority_limit": "file-backed row agreement and retained artifacts cannot substitute for an operated differential service"
        }),
        json!({
            "row_id": "authority.mismatch_router_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_router_contract_not_service",
            "evidence": W042_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w042_alert_dispatch, "evaluated_rule_count"),
            "authority_limit": "local alert dispatch classification is a contract row set, not an external quarantine service"
        }),
        json!({
            "row_id": "authority.operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_operated_service",
            "evidence": W042_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w042_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "authority_limit": "no recurring service endpoint, retained service history, or service-level mismatch action path exists"
        }),
        json!({
            "row_id": "authority.full_independent_evaluator_breadth",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_beyond_reference_model_slice",
            "evidence": "w042_independent_reference_model_implementation.json",
            "authority_limit": "reference model slice does not cover TreeCalc scheduling, references outside declared order, publication, effects, full OxFml seam breadth, or callable metadata"
        }),
        json!({
            "row_id": "authority.release_grade_diversity_promotion",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_release_grade_authority",
            "evidence": W042_OBLIGATION_MAP,
            "exact_service_blocker_count": number_at(w042_operated, "exact_service_blocker_count"),
            "authority_limit": "release-grade diversity authority remains unavailable while full independent implementation and operated service blockers remain"
        }),
    ]
}

fn w042_exact_blockers(
    w041_blockers: &Value,
    w042_stage2_blockers: &Value,
    w042_service_blockers: &Value,
    w042_cross_engine_service: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "blocker_id": "w042_diversity.full_independent_evaluator_breadth_absent",
            "owner": "calc-czd.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": "w042_independent_reference_model_implementation.json",
            "predecessor_blocker_present": row_with_field_exists(
                w041_blockers,
                "blocker_id",
                "w041_diversity.full_independent_evaluator_breadth_absent"
            ),
            "reason": "W042 introduces an independent named-reference model evaluator, but full TreeCalc/OxCalc evaluator breadth remains absent.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "w042_diversity.operated_cross_engine_differential_service_absent",
            "owner": "calc-czd.6; calc-czd.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W042_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w042_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w042_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "reason": "W042 service artifacts are file-backed and do not operate a recurring cross-engine differential service.",
            "promotion_consequence": "operated cross-engine diversity service remains unpromoted"
        }),
        json!({
            "blocker_id": "w042_diversity.mismatch_quarantine_service_absent",
            "owner": "calc-czd.6; calc-czd.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W042_OPERATED_SERVICE_BLOCKERS,
            "service_blocker_rows": row_count_at(w042_service_blockers),
            "reason": "Local alert/quarantine rows are evaluated, but no external mismatch triage/quarantine service is operated.",
            "promotion_consequence": "service-level diversity assurance and mismatch quarantine remain unpromoted"
        }),
        json!({
            "blocker_id": "w042_diversity.stage2_operated_differential_dependency_absent",
            "owner": "calc-czd.5; calc-czd.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W042_STAGE2_BLOCKERS,
            "stage2_blocker_rows": row_count_at(w042_stage2_blockers),
            "reason": "Stage 2 diversity still needs production policy, partition soundness, pack governance, and operated differential service evidence before strategy-level diversity promotion.",
            "promotion_consequence": "Stage 2 diversity remains a blocked dependency for release-grade promotion"
        }),
        json!({
            "blocker_id": "w042_diversity.oxfml_callable_breadth_dependency_absent",
            "owner": "calc-czd.7; calc-czd.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W042_OBLIGATION_MAP,
            "reason": "The independent reference-model slice does not settle broad OxFml display publication, callable-carrier sufficiency, registered-external callable projection, or LET/LAMBDA threading through OxFunc-adjacent behavior.",
            "promotion_consequence": "broad OxFml callable/display publication remains a successor dependency"
        }),
        json!({
            "blocker_id": "w042_diversity.pack_grade_replay_governance_dependency_absent",
            "owner": "calc-czd.7; calc-czd.9",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W041_PACK_DECISION,
            "reason": "Pack-grade replay governance cannot be inferred from local retained-history, retained-witness, or file-backed differential rows.",
            "promotion_consequence": "pack-grade replay, C5, and release-grade verification remain unpromoted"
        }),
        json!({
            "blocker_id": "w042_diversity.release_grade_promotion_authority_absent",
            "owner": "calc-czd.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W042_OBLIGATION_MAP,
            "reason": "Release-grade diversity cannot be claimed from bounded reference-model rows, proxy evidence, file-backed rows, shared projections, or retained blockers.",
            "promotion_consequence": "release-grade verification, pack/C5, and diversity promotion remain unavailable to this bead"
        }),
    ]
}

fn w042_diversity_validation_failures(
    implementation_rows: &[Value],
    independent_rows: &[Value],
    cross_engine_rows: &[Value],
    authority_rows: &[Value],
    blockers: &[Value],
) -> Vec<String> {
    let mut failures = Vec::new();
    if implementation_rows.len() < 4
        || implementation_rows
            .iter()
            .any(|row| !bool_at(row, "matches_expected"))
    {
        failures.push("w042_diversity.independent_reference_model_case_mismatch".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("independent_reference_model_slice_present")
    }) {
        failures.push("w042_diversity.reference_model_row_missing".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("full_independent_implementation_breadth_absent")
    }) {
        failures.push("w042_diversity.full_breadth_blocker_row_missing".to_string());
    }
    if !cross_engine_rows
        .iter()
        .any(|row| row.get("service_state").and_then(Value::as_str) == Some("blocked"))
    {
        failures.push("w042_diversity.blocked_cross_engine_service_row_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_operated_service")
    }) {
        failures.push("w042_diversity.operated_service_authority_blocker_missing".to_string());
    }
    if blockers.len() < 7 {
        failures.push("w042_diversity.exact_blocker_count_too_low".to_string());
    }
    if independent_rows
        .iter()
        .chain(cross_engine_rows.iter())
        .chain(authority_rows.iter())
        .any(|row| {
            bool_at(row, "fully_independent_evaluator_promoted")
                || bool_at(row, "operated_cross_engine_differential_service_promoted")
                || bool_at(row, "mismatch_quarantine_service_promoted")
        })
    {
        failures.push("w042_diversity.unexpected_promotion_row".to_string());
    }
    failures
}

fn w044_reference_model_cases() -> Vec<ReferenceModelCase> {
    const SOFT_REBIND_ASSIGNMENTS: &[(&str, &str)] = &[
        ("Base", "5"),
        ("Target", "Base*4"),
        ("Selector", "1"),
        ("Resolved", "IF(Selector=1,Target,0)"),
        ("Final", "Resolved+3"),
    ];
    const SOFT_REBIND_EXPECTED: &[(&str, FormulaValue)] = &[
        ("Base", FormulaValue::Int(5)),
        ("Target", FormulaValue::Int(20)),
        ("Selector", FormulaValue::Int(1)),
        ("Resolved", FormulaValue::Int(20)),
        ("Final", FormulaValue::Int(23)),
    ];
    const RETAINED_WITNESS_ASSIGNMENTS: &[(&str, &str)] = &[
        ("Actual", "21"),
        ("Oracle", "20"),
        ("Mismatch", "IF(Actual=Oracle,0,1)"),
        ("WitnessAttached", "IF(Mismatch=1,1,0)"),
        ("QuarantineRoute", "IF(WitnessAttached=1,1,0)"),
    ];
    const RETAINED_WITNESS_EXPECTED: &[(&str, FormulaValue)] = &[
        ("Actual", FormulaValue::Int(21)),
        ("Oracle", FormulaValue::Int(20)),
        ("Mismatch", FormulaValue::Int(1)),
        ("WitnessAttached", FormulaValue::Int(1)),
        ("QuarantineRoute", FormulaValue::Int(1)),
    ];

    let mut cases = w043_reference_model_cases();
    cases.push(ReferenceModelCase {
        case_id: "reference_model.w044_soft_reference_rebind_control",
        assignments: SOFT_REBIND_ASSIGNMENTS,
        expected: SOFT_REBIND_EXPECTED,
    });
    cases.push(ReferenceModelCase {
        case_id: "reference_model.w044_retained_witness_attachment_control",
        assignments: RETAINED_WITNESS_ASSIGNMENTS,
        expected: RETAINED_WITNESS_EXPECTED,
    });
    cases
}

fn w044_independent_reference_model_rows() -> Vec<Value> {
    w044_reference_model_cases()
        .into_iter()
        .map(|case| {
            let result = eval_reference_model(case.assignments);
            let (actual_bindings, error) = match result {
                Ok(bindings) => (bindings, None),
                Err(err) => (Vec::new(), Some(err)),
            };
            let matches_expected = error.is_none() && actual_bindings == case.expected;
            let assignment_rows = case
                .assignments
                .iter()
                .map(|(name, expression)| {
                    json!({
                        "name": name,
                        "formula": expression
                    })
                })
                .collect::<Vec<_>>();
            json!({
                "case_id": case.case_id,
                "assignments": assignment_rows,
                "expected_bindings": reference_bindings_json(case.expected),
                "actual_bindings": reference_bindings_json(&actual_bindings),
                "error": error,
                "matches_expected": matches_expected,
                "implementation_path": "src/oxcalc-tracecalc/src/diversity_seam.rs::eval_reference_model",
                "implementation_kind": "independent_reference_model_evaluator",
                "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, deterministic before/after edit checks, fan-in checksum, mismatch-routing, soft-reference rebind, and retained-witness attachment controls",
                "independent_from": ["TraceCalc", "optimized_core", "TreeCalc", "OxFml", "OxFunc"]
            })
        })
        .collect()
}

#[allow(clippy::too_many_arguments)]
fn w044_source_rows(
    w044_obligation_summary: &Value,
    w044_blocker_map: &Value,
    w044_promotion_contract: &Value,
    w044_oxfml_intake: &Value,
    w043_diversity: &Value,
    w043_implementation: &Value,
    w043_independent: &Value,
    w043_cross_engine: &Value,
    w043_authority: &Value,
    w043_blockers: &Value,
    w043_decision: &Value,
    w044_conformance: &Value,
    w044_dynamic: &Value,
    w044_callable: &Value,
    w044_rust: &Value,
    w044_rust_refinement: &Value,
    w044_lean_tla: &Value,
    w044_stage2: &Value,
    w044_stage2_blockers: &Value,
    w044_stage2_decision: &Value,
    w044_operated: &Value,
    w044_readiness: &Value,
    w044_retained: &Value,
    w044_witness: &Value,
    w044_cross_engine_service: &Value,
    w044_alert_dispatch: &Value,
    w044_service_blockers: &Value,
    w044_operated_decision: &Value,
    w043_pack_decision: &Value,
) -> Vec<Value> {
    let w073 = w044_oxfml_intake
        .get("w073_formatting")
        .unwrap_or(&Value::Null);
    vec![
        json!({
            "row_id": "source.w044_release_grade_blocker_map",
            "artifact": W044_BLOCKER_RECLASSIFICATION_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w044_obligation_summary, "status") == "residual_release_grade_blocker_reclassification_map_validated"
                && number_at(w044_blocker_map, "obligation_count") == 45
                && number_at(w044_blocker_map, "source_residual_lane_count") == 20,
            "semantic_state": "w044_diversity_obligations_bound_with_no_proxy_guard"
        }),
        json!({
            "row_id": "source.w044_promotion_contract_map",
            "artifact": W044_PROMOTION_CONTRACT_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_promotion_contract, "promotion_contract_count") == 17,
            "semantic_state": "w044_promotion_contracts_retained_without_promotion"
        }),
        json!({
            "row_id": "source.w044_w073_typed_formatting_guard",
            "artifact": W044_OXFML_INBOUND_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": array_len_at(w073, "typed_rule_only_families") == 7
                && !bool_at(w073, "threshold_fallback_allowed_for_typed_families")
                && bool_at(w073, "downstream_typed_rule_request_construction_required")
                && !bool_at(w073, "downstream_uptake_verified_by_oxcalc"),
            "semantic_state": "w073_typed_rule_only_formatting_guard_retained_for_w044_diversity"
        }),
        json!({
            "row_id": "source.w043_diversity_predecessor_summary",
            "artifact": W043_DIVERSITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w043_diversity, "failed_row_count"),
            "promotion_guard": number_at(w043_diversity, "w043_independent_reference_model_case_count") == 6
                && number_at(w043_diversity, "exact_blocker_count") == 8
                && !bool_at(w043_diversity, "fully_independent_evaluator_promoted")
                && !bool_at(w043_decision, "mismatch_quarantine_service_promoted"),
            "semantic_state": "w043_diversity_predecessor_bound_without_promotion"
        }),
        json!({
            "row_id": "source.w043_reference_model_predecessor",
            "artifact": W043_INDEPENDENT_IMPLEMENTATION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w043_implementation, "failed_case_count"),
            "promotion_guard": number_at(w043_implementation, "case_count") == 6
                && number_at(w043_implementation, "match_count") == 6
                && number_at(w043_independent, "row_count") == 13,
            "semantic_state": "w043_reference_model_extended_by_w044_cases"
        }),
        json!({
            "row_id": "source.w043_cross_engine_predecessor",
            "artifact": W043_CROSS_ENGINE_DIVERSITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w043_cross_engine, "row_count") == 12
                && !bool_at(w043_cross_engine, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w043_cross_engine_rows_bound_without_operated_service"
        }),
        json!({
            "row_id": "source.w043_mismatch_authority_predecessor",
            "artifact": W043_MISMATCH_AUTHORITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w043_authority, "row_count") == 12
                && !bool_at(w043_authority, "mismatch_quarantine_service_promoted"),
            "semantic_state": "w043_mismatch_authority_bound_without_service_promotion"
        }),
        json!({
            "row_id": "source.w043_diversity_blockers_and_decision",
            "artifact": W043_DIVERSITY_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w043_blockers, "exact_blocker_count") == 8
                && !bool_at(w043_decision, "pack_grade_replay_promoted")
                && !bool_at(w043_pack_decision, "capability_promoted"),
            "semantic_state": "w043_diversity_pack_blockers_retained"
        }),
        json!({
            "row_id": "source.w044_optimized_core_dynamic_transition",
            "artifact": W044_CONFORMANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_conformance, "failed_row_count"),
            "promotion_guard": number_at(w044_conformance, "w044_direct_evidence_bound_count") == 2
                && number_at(w044_conformance, "w044_exact_remaining_blocker_count") == 4
                && bool_at(w044_dynamic, "direct_evidence_bound"),
            "semantic_state": "w044_mixed_dynamic_transition_evidence_bound_without_full_core_promotion"
        }),
        json!({
            "row_id": "source.w044_callable_metadata_projection_blocker",
            "artifact": W044_CALLABLE_METADATA_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_callable, "row_count") == 3
                && !bool_at(w044_callable, "callable_metadata_projection_promoted"),
            "semantic_state": "w044_callable_metadata_projection_remains_exact_blocker"
        }),
        json!({
            "row_id": "source.w044_rust_refinement_bridge",
            "artifact": W044_RUST_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_rust, "failed_row_count"),
            "promotion_guard": number_at(w044_rust, "refinement_row_count") == 9
                && number_at(w044_rust, "exact_remaining_blocker_count") == 6
                && row_count_at(w044_rust_refinement) > 0,
            "semantic_state": "w044_rust_refinement_rows_bound_without_totality_promotion"
        }),
        json!({
            "row_id": "source.w044_lean_tla_model_bound",
            "artifact": W044_LEAN_TLA_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_lean_tla, "failed_row_count"),
            "promotion_guard": number_at(w044_lean_tla, "bounded_model_row_count") == 4
                && number_at(w044_lean_tla, "exact_remaining_blocker_count") == 5,
            "semantic_state": "w044_lean_tla_model_bound_rows_bound_without_full_verification_promotion"
        }),
        json!({
            "row_id": "source.w044_stage2_scheduler_equivalence",
            "artifact": W044_STAGE2_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_stage2, "failed_row_count"),
            "promotion_guard": number_at(w044_stage2, "policy_row_count") == 25
                && number_at(w044_stage2, "exact_remaining_blocker_count") == 8
                && bool_at(w044_stage2, "declared_scheduler_equivalence_evidenced")
                && !bool_at(w044_stage2, "stage2_policy_promoted"),
            "semantic_state": "w044_stage2_scheduler_equivalence_bound_without_policy_promotion"
        }),
        json!({
            "row_id": "source.w044_stage2_blockers_and_decision",
            "artifact": W044_STAGE2_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_stage2_blockers, "exact_remaining_blocker_count") == 8
                && !bool_at(w044_stage2_decision, "pack_grade_replay_promoted")
                && !bool_at(w044_stage2_decision, "stage2_policy_promoted"),
            "semantic_state": "w044_stage2_blockers_bound_for_diversity"
        }),
        json!({
            "row_id": "source.w044_operated_assurance_service_packet",
            "artifact": W044_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_operated, "failed_row_count"),
            "promotion_guard": bool_at(w044_operated, "file_backed_service_envelope_present")
                && bool_at(w044_operated, "retained_history_query_api_contract_present")
                && bool_at(w044_operated, "retained_witness_lifecycle_register_present")
                && !bool_at(w044_operated, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w044_file_backed_service_packet_bound_without_operated_service_promotion"
        }),
        json!({
            "row_id": "source.w044_service_readiness_register",
            "artifact": W044_SERVICE_READINESS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_readiness, "criteria_count") == 25
                && number_at(w044_readiness, "blocked_criteria_count") == 6
                && bool_at(w044_readiness, "w073_typed_rule_only_formatting_guard_carried"),
            "semantic_state": "w044_service_readiness_bound_with_exact_blockers"
        }),
        json!({
            "row_id": "source.w044_retained_history_query",
            "artifact": W044_RETAINED_HISTORY_QUERY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_retained, "query_register_row_count") == 17
                && number_at(w044_retained, "replay_correlation_row_count") == 15
                && !bool_at(w044_retained, "retained_history_service_promoted"),
            "semantic_state": "w044_retained_history_query_support_bound"
        }),
        json!({
            "row_id": "source.w044_retained_witness_lifecycle",
            "artifact": W044_RETAINED_WITNESS_LIFECYCLE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w044_witness, "retained_witness_lifecycle_register_present")
                && number_at(w044_witness, "witness_lifecycle_row_count") == 11
                && number_at(w044_witness, "pack_eligible_witness_count") == 0
                && !bool_at(w044_witness, "retained_witness_lifecycle_service_promoted"),
            "semantic_state": "w044_retained_witness_lifecycle_support_bound"
        }),
        json!({
            "row_id": "source.w044_cross_engine_service_blocker",
            "artifact": W044_CROSS_ENGINE_SERVICE_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w044_cross_engine_service, "file_backed_cross_engine_substrate_present")
                && !bool_at(w044_cross_engine_service, "operated_cross_engine_differential_service_present")
                && array_contains_string(
                    w044_cross_engine_service,
                    "blocked_service_claims",
                    "operated_mismatch_quarantine_dispatcher"
                ),
            "semantic_state": "w044_cross_engine_service_blocker_retained"
        }),
        json!({
            "row_id": "source.w044_alert_quarantine_contract",
            "artifact": W044_ALERT_DISPATCH_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_alert_dispatch, "evaluated_rule_count") == 35
                && number_at(w044_alert_dispatch, "quarantine_decision_count") == 0
                && number_at(w044_alert_dispatch, "alert_decision_count") == 0
                && !bool_at(w044_alert_dispatch, "external_alert_dispatcher_promoted")
                && !bool_at(w044_alert_dispatch, "quarantine_service_promoted"),
            "semantic_state": "w044_local_alert_quarantine_contract_clean"
        }),
        json!({
            "row_id": "source.w044_service_pack_no_proxy_guard",
            "artifact": W044_OPERATED_SERVICE_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_service_blockers, "exact_service_blocker_count") == 6
                && !bool_at(w044_operated_decision, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w044_operated_decision, "quarantine_service_promoted")
                && !bool_at(w043_pack_decision, "capability_promoted"),
            "semantic_state": "w044_service_pack_no_proxy_promotion_guard_clean"
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w044_independent_evaluator_rows(
    w043_independent: &Value,
    w043_implementation: &Value,
    w043_diversity: &Value,
    w044_conformance: &Value,
    w044_dynamic: &Value,
    w044_rust: &Value,
    w044_lean_tla: &Value,
    w044_stage2: &Value,
    w044_operated: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "independent.w044_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "independence_state": "independent_reference_model_slice_present",
            "evidence": "w044_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "W044 broadens the independent slice with soft-reference rebind and retained-witness attachment controls, but still does not cover full TreeCalc/OxCalc evaluator breadth."
        }),
        json!({
            "row_id": "independent.w043_reference_model_predecessor",
            "disposition_kind": "accepted_boundary",
            "independence_state": "predecessor_reference_model_slice_subsumed_by_w044_slice",
            "evidence": W043_INDEPENDENT_IMPLEMENTATION,
            "w043_case_count": number_at(w043_implementation, "case_count"),
            "w043_row_count": number_at(w043_independent, "row_count"),
            "w043_fully_independent_evaluator_promoted": bool_at(w043_diversity, "fully_independent_evaluator_promoted"),
            "promotion_consequence": "W043 reference-model evidence remains evidence lineage only."
        }),
        json!({
            "row_id": "independent.w044_soft_reference_rebind_control",
            "disposition_kind": "accepted_boundary",
            "independence_state": "soft_reference_rebind_control_present",
            "evidence": "w044_independent_reference_model_implementation.json",
            "promotion_consequence": "The W044 soft-reference case is a deterministic control row for later differential-service comparison, not broad dynamic-reference authority."
        }),
        json!({
            "row_id": "independent.w044_retained_witness_attachment_control",
            "disposition_kind": "accepted_boundary",
            "independence_state": "retained_witness_attachment_control_present",
            "evidence": "w044_independent_reference_model_implementation.json",
            "promotion_consequence": "The W044 retained-witness case models mismatch attachment intent without operating a witness lifecycle service."
        }),
        json!({
            "row_id": "independent.tracecalc_reference_oracle_boundary",
            "disposition_kind": "accepted_boundary",
            "independence_state": "tracecalc_is_correctness_oracle_not_independent_implementation",
            "evidence": "TraceCalc",
            "promotion_consequence": "TraceCalc remains the correctness oracle for covered behavior, not a diversity implementation."
        }),
        json!({
            "row_id": "independent.optimized_core_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "optimized_core_same_implementation_family",
            "evidence": W044_CONFORMANCE_SUMMARY,
            "exact_remaining_blocker_count": number_at(w044_conformance, "w044_exact_remaining_blocker_count"),
            "promotion_consequence": "Optimized/core evidence strengthens conformance but is not independent implementation authority."
        }),
        json!({
            "row_id": "independent.treecalc_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "treecalc_same_oxcalc_family",
            "evidence": W044_DYNAMIC_TRANSITION_EVIDENCE,
            "direct_evidence_bound": bool_at(w044_dynamic, "direct_evidence_bound"),
            "promotion_consequence": "TreeCalc evidence is core conformance evidence, not independent evaluator diversity."
        }),
        json!({
            "row_id": "independent.direct_oxfml_external_formula_slice",
            "disposition_kind": "accepted_external_slice",
            "independence_state": "external_formula_evaluator_not_oxcalc_coordinator",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "promotion_consequence": "OxFml formula evidence remains an external formula-evaluator slice, including narrow LET/LAMBDA seam evidence, not OxCalc coordinator breadth."
        }),
        json!({
            "row_id": "independent.proof_model_not_runtime_evaluator",
            "disposition_kind": "accepted_boundary",
            "independence_state": "proof_model_not_executable_runtime_evaluator",
            "evidence": W044_LEAN_TLA_SUMMARY,
            "bounded_model_row_count": number_at(w044_lean_tla, "bounded_model_row_count"),
            "promotion_consequence": "Proof/model evidence supports specification and refinement, but does not supply runtime diversity."
        }),
        json!({
            "row_id": "independent.rust_refinement_not_runtime_diversity",
            "disposition_kind": "accepted_boundary",
            "independence_state": "rust_refinement_bridge_not_second_evaluator",
            "evidence": W044_RUST_SUMMARY,
            "refinement_row_count": number_at(w044_rust, "refinement_row_count"),
            "promotion_consequence": "Rust refinement bridge evidence supports the main implementation family and does not establish independent evaluator breadth."
        }),
        json!({
            "row_id": "independent.stage2_policy_not_independent_runtime",
            "disposition_kind": "accepted_boundary",
            "independence_state": "stage2_policy_profiles_not_second_evaluator",
            "evidence": W044_STAGE2_SUMMARY,
            "policy_row_count": number_at(w044_stage2, "policy_row_count"),
            "stage2_policy_promoted": bool_at(w044_stage2, "stage2_policy_promoted"),
            "promotion_consequence": "Stage 2 scheduler-equivalence evidence remains policy/replay evidence, not separate evaluator implementation authority."
        }),
        json!({
            "row_id": "independent.w044_service_packet_not_independent_runtime",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "service_packet_file_backed_not_independent_evaluator",
            "evidence": W044_OPERATED_ASSURANCE_SUMMARY,
            "service_readiness_blocked_count": number_at(w044_operated, "service_readiness_blocked_count"),
            "promotion_consequence": "Service evidence improves retention and authority routing but is not independent evaluator breadth."
        }),
        json!({
            "row_id": "independent.mismatch_quarantine_contract_not_independent_runtime",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "local_quarantine_contract_not_independent_evaluator",
            "evidence": W044_ALERT_DISPATCH_REGISTER,
            "promotion_consequence": "Local mismatch/quarantine rules are not a second evaluator implementation."
        }),
        json!({
            "row_id": "independent.callable_metadata_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "callable_metadata_projection_blocked",
            "evidence": W044_CALLABLE_METADATA_REGISTER,
            "promotion_consequence": "Callable metadata and LET/LAMBDA carrier sufficiency remain successor dependencies."
        }),
        json!({
            "row_id": "independent.full_independent_implementation_breadth_absent",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "full_independent_implementation_breadth_absent",
            "evidence": "w044_independent_reference_model_implementation.json",
            "promotion_consequence": "Full evaluator diversity remains blocked until a broader independent implementation covers scheduler, references, publication, effects, broad OxFml, and callable metadata."
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w044_cross_engine_differential_rows(
    w043_cross_engine: &Value,
    w044_stage2: &Value,
    w044_operated: &Value,
    w044_readiness: &Value,
    w044_retained: &Value,
    w044_witness: &Value,
    w044_cross_engine_service: &Value,
    w044_alert_dispatch: &Value,
    w044_oxfml_intake: &Value,
    w044_dynamic: &Value,
    w044_callable: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let w073 = w044_oxfml_intake
        .get("w073_formatting")
        .unwrap_or(&Value::Null);
    vec![
        json!({
            "row_id": "cross_engine.w044_reference_model_outputs_available",
            "disposition_kind": "accepted_boundary",
            "service_state": "reference_model_outputs_available",
            "evidence": "w044_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "promotion_consequence": "Reference-model rows are comparable evidence inputs, not an operated cross-engine service."
        }),
        json!({
            "row_id": "cross_engine.w043_cross_engine_predecessor",
            "disposition_kind": "accepted_boundary",
            "service_state": "predecessor_cross_engine_rows_bound",
            "evidence": W043_CROSS_ENGINE_DIVERSITY,
            "row_count": number_at(w043_cross_engine, "row_count"),
            "promotion_consequence": "W043 file-backed differential rows remain predecessor evidence only."
        }),
        json!({
            "row_id": "cross_engine.w044_service_envelope",
            "disposition_kind": "service_contract_boundary",
            "service_state": "file_backed_service_envelope_present",
            "evidence": W044_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w044_operated, "service_envelope_row_count"),
            "file_backed_service_envelope_present": bool_at(w044_operated, "file_backed_service_envelope_present"),
            "promotion_consequence": "The W044 service envelope records rows needed by a future operated service without claiming operation."
        }),
        json!({
            "row_id": "cross_engine.w044_retained_history_query_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_history_query_and_correlation_only",
            "evidence": W044_RETAINED_HISTORY_QUERY,
            "query_register_row_count": number_at(w044_retained, "query_register_row_count"),
            "replay_correlation_row_count": number_at(w044_retained, "replay_correlation_row_count"),
            "promotion_consequence": "Retained query/correlation artifacts support later differential service operation but are not themselves a service."
        }),
        json!({
            "row_id": "cross_engine.w044_retained_witness_lifecycle_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_witness_lifecycle_only",
            "evidence": W044_RETAINED_WITNESS_LIFECYCLE,
            "retained_witness_lifecycle_row_count": number_at(w044_witness, "witness_lifecycle_row_count"),
            "pack_eligible_witness_count": number_at(w044_witness, "pack_eligible_witness_count"),
            "promotion_consequence": "Retained-witness lifecycle rows support later differential and pack governance work but are not an operated lifecycle service."
        }),
        json!({
            "row_id": "cross_engine.w044_mixed_dynamic_transition_input",
            "disposition_kind": "accepted_boundary",
            "service_state": "dynamic_transition_input_bound",
            "evidence": W044_DYNAMIC_TRANSITION_EVIDENCE,
            "direct_evidence_bound": bool_at(w044_dynamic, "direct_evidence_bound"),
            "promotion_consequence": "Mixed dynamic transition evidence is a comparison input only."
        }),
        json!({
            "row_id": "cross_engine.w044_callable_metadata_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W044_CALLABLE_METADATA_REGISTER,
            "callable_metadata_projection_promoted": bool_at(w044_callable, "callable_metadata_projection_promoted"),
            "promotion_consequence": "Callable metadata projection remains blocked and cannot be inferred from diversity rows."
        }),
        json!({
            "row_id": "cross_engine.w044_stage2_service_dependency",
            "disposition_kind": "service_dependency_blocked",
            "service_state": "blocked",
            "evidence": W044_STAGE2_SUMMARY,
            "stage2_policy_promoted": bool_at(w044_stage2, "stage2_policy_promoted"),
            "exact_remaining_blocker_count": number_at(w044_stage2, "exact_remaining_blocker_count"),
            "promotion_consequence": "Stage 2 diversity remains blocked by operated service, retained-witness, and pack-governance dependencies."
        }),
        json!({
            "row_id": "cross_engine.w044_cross_engine_service_register",
            "disposition_kind": "service_substrate_bound",
            "service_state": if bool_at(w044_cross_engine_service, "operated_cross_engine_differential_service_present") { "present" } else { "blocked" },
            "evidence": W044_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w044_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w044_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "The W044 service register records the operated-service blocker exactly."
        }),
        json!({
            "row_id": "cross_engine.w044_mismatch_triage_and_quarantine_contract",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W044_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w044_alert_dispatch, "evaluated_rule_count"),
            "external_alert_dispatcher_promoted": bool_at(w044_alert_dispatch, "external_alert_dispatcher_promoted"),
            "quarantine_service_promoted": bool_at(w044_alert_dispatch, "quarantine_service_promoted"),
            "promotion_consequence": "Local alert/quarantine rules are evaluated, but no external mismatch triage/quarantine service is operated."
        }),
        json!({
            "row_id": "cross_engine.w044_retained_witness_attachment_service",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W044_RETAINED_WITNESS_LIFECYCLE,
            "retained_witness_lifecycle_service_promoted": bool_at(w044_witness, "retained_witness_lifecycle_service_promoted"),
            "promotion_consequence": "Retained-witness attachment is represented by control rows and lifecycle evidence, not by an operated attachment service."
        }),
        json!({
            "row_id": "cross_engine.w073_typed_only_formatting_guard",
            "disposition_kind": "accepted_boundary",
            "service_state": "observable_guard_retained",
            "evidence": W044_OXFML_INBOUND_INTAKE,
            "typed_rule_only_family_count": array_len_at(w073, "typed_rule_only_families"),
            "threshold_fallback_allowed_for_typed_families": bool_at(w073, "threshold_fallback_allowed_for_typed_families"),
            "downstream_typed_rule_request_construction_required": bool_at(w073, "downstream_typed_rule_request_construction_required"),
            "promotion_consequence": "Diversity comparisons must preserve the W073 typed_rule-only aggregate/visualization formatting guard."
        }),
        json!({
            "row_id": "cross_engine.w073_downstream_request_construction",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W044_OXFML_INBOUND_INTAKE,
            "downstream_uptake_verified_by_oxcalc": bool_at(w073, "downstream_uptake_verified_by_oxcalc"),
            "promotion_consequence": "Downstream W073 typed-rule request construction remains routed to calc-b1t.8."
        }),
        json!({
            "row_id": "cross_engine.w044_spec_evolution_mismatch_route",
            "disposition_kind": "service_contract_boundary",
            "service_state": "spec_evolution_route_declared_not_operated",
            "evidence": W044_BLOCKER_RECLASSIFICATION_MAP,
            "promotion_consequence": "Mismatch handling can route to spec correction or implementation fault only after operated differential and quarantine evidence exists."
        }),
        json!({
            "row_id": "cross_engine.pack_governance_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W043_PACK_DECISION,
            "promotion_consequence": "Pack/C5 and release-grade diversity remain unavailable while operated differential service, mismatch quarantine, retained-witness service, and independent breadth are blocked."
        }),
        json!({
            "row_id": "cross_engine.public_migration_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W044_OXFML_INBOUND_INTAKE,
            "w073_downstream_request_verified": bool_at(w044_readiness, "w073_downstream_typed_rule_request_construction_verified"),
            "promotion_consequence": "Public migration and broad OxFml display/publication coverage remain successor dependencies and cannot be inferred from diversity rows."
        }),
    ]
}

fn w044_mismatch_authority_rows(
    w043_authority: &Value,
    w044_operated: &Value,
    w044_cross_engine_service: &Value,
    w044_alert_dispatch: &Value,
    w044_witness: &Value,
    w044_stage2: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "authority.w044_independent_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "authority_state": "independent_reference_model_runtime",
            "evidence": "w044_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "authority_limit": "named-reference model slice only; no scheduling, publication, effects, callable-carrier, broad OxFml seam, or full OxFunc kernel authority"
        }),
        json!({
            "row_id": "authority.w043_reference_model_predecessor",
            "disposition_kind": "accepted_boundary",
            "authority_state": "predecessor_reference_model_runtime",
            "evidence": W043_MISMATCH_AUTHORITY,
            "w043_authority_row_count": number_at(w043_authority, "row_count"),
            "authority_limit": "predecessor authority remains a narrower reference-model slice"
        }),
        json!({
            "row_id": "authority.tracecalc_correctness_oracle",
            "disposition_kind": "accepted_boundary",
            "authority_state": "reference_oracle",
            "evidence": "TraceCalc",
            "authority_limit": "oracle authority for covered observable behavior only"
        }),
        json!({
            "row_id": "authority.optimized_core_same_implementation_family",
            "disposition_kind": "same_authority_family",
            "authority_state": "not_independent",
            "evidence": W044_CONFORMANCE_SUMMARY,
            "authority_limit": "optimized/core conformance does not supply separate implementation authority"
        }),
        json!({
            "row_id": "authority.direct_oxfml_external_formula_evaluator",
            "disposition_kind": "accepted_external_slice",
            "authority_state": "external_formula_evaluator_only",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "authority_limit": "external evaluator authority is limited to the consumed OxFml seam and does not implement OxCalc scheduling/publication"
        }),
        json!({
            "row_id": "authority.service_envelope_file_backed_authority",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "file_backed_artifact_authority",
            "evidence": W044_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w044_operated, "service_envelope_row_count"),
            "authority_limit": "file-backed row agreement and retained artifacts cannot substitute for an operated differential service"
        }),
        json!({
            "row_id": "authority.mismatch_router_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_router_contract_not_service",
            "evidence": W044_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w044_alert_dispatch, "evaluated_rule_count"),
            "authority_limit": "local alert dispatch classification is a contract row set, not an external quarantine service"
        }),
        json!({
            "row_id": "authority.retained_witness_attachment_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_attachment_contract_not_service",
            "evidence": W044_RETAINED_WITNESS_LIFECYCLE,
            "witness_lifecycle_row_count": number_at(w044_witness, "witness_lifecycle_row_count"),
            "authority_limit": "retained-witness rows describe lifecycle and control intent, not an operated attachment service"
        }),
        json!({
            "row_id": "authority.spec_evolution_route",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "spec_evolution_route_not_service",
            "evidence": W044_BLOCKER_RECLASSIFICATION_MAP,
            "authority_limit": "agreement, mismatch, implementation-fault, and spec-correction routes remain declared classification until operated service evidence exists"
        }),
        json!({
            "row_id": "authority.operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_operated_service",
            "evidence": W044_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w044_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "authority_limit": "no recurring service endpoint, retained service history, or service-level mismatch action path exists"
        }),
        json!({
            "row_id": "authority.mismatch_quarantine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_quarantine_service",
            "evidence": W044_ALERT_DISPATCH_REGISTER,
            "quarantine_service_promoted": bool_at(w044_alert_dispatch, "quarantine_service_promoted"),
            "authority_limit": "local clean dispatch rows cannot stand in for an enforcing mismatch quarantine service"
        }),
        json!({
            "row_id": "authority.retained_witness_attachment_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_retained_witness_attachment_service",
            "evidence": W044_RETAINED_WITNESS_LIFECYCLE,
            "retained_witness_lifecycle_service_promoted": bool_at(w044_witness, "retained_witness_lifecycle_service_promoted"),
            "authority_limit": "no operated service attaches mismatch witnesses to quarantine, retained history, or pack governance"
        }),
        json!({
            "row_id": "authority.full_independent_evaluator_breadth",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_beyond_reference_model_slice",
            "evidence": "w044_independent_reference_model_implementation.json",
            "authority_limit": "reference model slice does not cover TreeCalc scheduling, references outside declared order, publication, effects, broad OxFml seam breadth, or callable metadata"
        }),
        json!({
            "row_id": "authority.callable_metadata_and_public_surface",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_callable_and_public_surface",
            "evidence": W044_OXFML_INBOUND_INTAKE,
            "authority_limit": "W073 downstream request construction, public migration, callable metadata, and broad display/publication remain successor dependencies"
        }),
        json!({
            "row_id": "authority.release_grade_diversity_promotion",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_release_grade_authority",
            "evidence": W044_STAGE2_SUMMARY,
            "exact_stage2_blocker_count": number_at(w044_stage2, "exact_remaining_blocker_count"),
            "authority_limit": "release-grade diversity authority remains unavailable while full independent implementation and operated service blockers remain"
        }),
    ]
}

fn w044_exact_blockers(
    w043_blockers: &Value,
    w044_stage2_blockers: &Value,
    w044_service_blockers: &Value,
    w044_cross_engine_service: &Value,
    w044_conformance_blockers: &Value,
    w044_tla_model_bound: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "blocker_id": "w044_diversity.full_independent_evaluator_breadth_absent",
            "owner": "calc-b1t.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": "w044_independent_reference_model_implementation.json",
            "predecessor_blocker_present": row_with_field_exists(
                w043_blockers,
                "blocker_id",
                "w043_diversity.full_independent_evaluator_breadth_absent"
            ),
            "reason": "W044 extends the independent named-reference model evaluator with soft-reference and retained-witness controls, but full TreeCalc/OxCalc evaluator breadth remains absent.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "w044_diversity.operated_cross_engine_differential_service_absent",
            "owner": "calc-b1t.6; calc-b1t.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w044_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w044_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "reason": "W044 service artifacts are file-backed and do not operate a recurring cross-engine differential service.",
            "promotion_consequence": "operated cross-engine diversity service remains unpromoted"
        }),
        json!({
            "blocker_id": "w044_diversity.mismatch_quarantine_service_absent",
            "owner": "calc-b1t.6; calc-b1t.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_OPERATED_SERVICE_BLOCKERS,
            "service_blocker_rows": row_count_at(w044_service_blockers),
            "reason": "Local alert/quarantine rows are evaluated, but no external mismatch triage/quarantine service is operated.",
            "promotion_consequence": "service-level diversity assurance and mismatch quarantine remain unpromoted"
        }),
        json!({
            "blocker_id": "w044_diversity.stage2_operated_differential_dependency_absent",
            "owner": "calc-b1t.5; calc-b1t.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_STAGE2_BLOCKERS,
            "stage2_blocker_rows": row_count_at(w044_stage2_blockers),
            "reason": "Stage 2 diversity still needs operated differential service, retained-witness lifecycle, retention SLO, pack governance, and release-grade decision evidence before strategy-level diversity promotion.",
            "promotion_consequence": "Stage 2 diversity remains a blocked dependency for release-grade promotion"
        }),
        json!({
            "blocker_id": "w044_diversity.retained_witness_attachment_service_absent",
            "owner": "calc-b1t.6; calc-b1t.7; calc-b1t.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_OPERATED_SERVICE_BLOCKERS,
            "reason": "Retained-witness lifecycle rows and W044 control rows exist, but no service attaches mismatch witnesses to quarantine, pack replay, or retained-history lifecycle operation.",
            "promotion_consequence": "mismatch quarantine, pack-grade replay, C5, and release-grade diversity remain blocked"
        }),
        json!({
            "blocker_id": "w044_diversity.oxfml_callable_public_migration_dependency_absent",
            "owner": "calc-b1t.7; calc-b1t.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_BLOCKER_RECLASSIFICATION_MAP,
            "reason": "The independent reference-model slice does not settle broad OxFml display publication, public migration, callable-carrier sufficiency, registered-external callable projection, or LET/LAMBDA threading through OxFunc-adjacent behavior.",
            "promotion_consequence": "broad OxFml callable/display publication and public migration remain successor dependencies"
        }),
        json!({
            "blocker_id": "w044_diversity.optimized_core_callable_metadata_dependency_absent",
            "owner": "calc-b1t.2; calc-b1t.7; calc-b1t.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_CONFORMANCE_BLOCKERS,
            "conformance_blocker_rows": row_count_at(w044_conformance_blockers),
            "reason": "Callable metadata projection and registered external/provider publication semantics remain outside the W044 independent reference-model slice.",
            "promotion_consequence": "callable metadata projection remains unpromoted"
        }),
        json!({
            "blocker_id": "w044_diversity.formal_model_bound_dependency_absent",
            "owner": "calc-b1t.4; calc-b1t.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_TLA_MODEL_BOUND_REGISTER,
            "model_bound_rows": row_count_at(w044_tla_model_bound),
            "reason": "Lean/TLA model bounds and unbounded fairness blockers remain proof/model dependencies for release-grade diversity.",
            "promotion_consequence": "full Lean/TLA and scheduler fairness remain unpromoted"
        }),
        json!({
            "blocker_id": "w044_diversity.pack_grade_replay_governance_dependency_absent",
            "owner": "calc-b1t.7; calc-b1t.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W043_PACK_DECISION,
            "reason": "Pack-grade replay governance cannot be inferred from local retained-history, retained-witness, file-backed differential, or bounded mismatch-control rows.",
            "promotion_consequence": "pack-grade replay, C5, and release-grade verification remain unpromoted"
        }),
    ]
}

fn w044_diversity_validation_failures(
    implementation_rows: &[Value],
    independent_rows: &[Value],
    cross_engine_rows: &[Value],
    authority_rows: &[Value],
    blockers: &[Value],
) -> Vec<String> {
    let mut failures = Vec::new();
    if implementation_rows.len() < 8
        || implementation_rows
            .iter()
            .any(|row| !bool_at(row, "matches_expected"))
    {
        failures.push("w044_diversity.independent_reference_model_case_mismatch".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("independent_reference_model_slice_present")
    }) {
        failures.push("w044_diversity.reference_model_row_missing".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("full_independent_implementation_breadth_absent")
    }) {
        failures.push("w044_diversity.full_breadth_blocker_row_missing".to_string());
    }
    if !cross_engine_rows
        .iter()
        .any(|row| row.get("service_state").and_then(Value::as_str) == Some("blocked"))
    {
        failures.push("w044_diversity.blocked_cross_engine_service_row_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_operated_service")
    }) {
        failures.push("w044_diversity.operated_service_authority_blocker_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_quarantine_service")
    }) {
        failures.push("w044_diversity.mismatch_quarantine_authority_blocker_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str)
            == Some("blocked_no_retained_witness_attachment_service")
    }) {
        failures.push("w044_diversity.retained_witness_attachment_blocker_missing".to_string());
    }
    if blockers.len() < 9 {
        failures.push("w044_diversity.exact_blocker_count_too_low".to_string());
    }
    if independent_rows
        .iter()
        .chain(cross_engine_rows.iter())
        .chain(authority_rows.iter())
        .any(|row| {
            bool_at(row, "fully_independent_evaluator_promoted")
                || bool_at(row, "operated_cross_engine_differential_service_promoted")
                || bool_at(row, "mismatch_quarantine_service_promoted")
                || bool_at(row, "retained_witness_attachment_service_promoted")
        })
    {
        failures.push("w044_diversity.unexpected_promotion_row".to_string());
    }
    failures
}

fn w045_reference_model_cases() -> Vec<ReferenceModelCase> {
    const LOCAL_HARNESS_ASSIGNMENTS: &[(&str, &str)] = &[
        ("Baseline", "10"),
        ("Candidate", "Baseline+2"),
        ("Reference", "12"),
        ("Match", "IF(Candidate=Reference,1,0)"),
        ("Quarantine", "IF(Match=1,0,1)"),
        ("HarnessBoundary", "IF(Quarantine=0,1,0)"),
    ];
    const LOCAL_HARNESS_EXPECTED: &[(&str, FormulaValue)] = &[
        ("Baseline", FormulaValue::Int(10)),
        ("Candidate", FormulaValue::Int(12)),
        ("Reference", FormulaValue::Int(12)),
        ("Match", FormulaValue::Int(1)),
        ("Quarantine", FormulaValue::Int(0)),
        ("HarnessBoundary", FormulaValue::Int(1)),
    ];
    const RETAINED_HISTORY_ASSIGNMENTS: &[(&str, &str)] = &[
        ("RunBefore", "42"),
        ("RunAfter", "63"),
        ("Delta", "RunAfter-RunBefore"),
        ("ReplayCorrelation", "IF(Delta=21,1,0)"),
        ("WitnessAttachment", "ReplayCorrelation*7"),
        ("PackEligible", "IF(WitnessAttachment=7,0,1)"),
    ];
    const RETAINED_HISTORY_EXPECTED: &[(&str, FormulaValue)] = &[
        ("RunBefore", FormulaValue::Int(42)),
        ("RunAfter", FormulaValue::Int(63)),
        ("Delta", FormulaValue::Int(21)),
        ("ReplayCorrelation", FormulaValue::Int(1)),
        ("WitnessAttachment", FormulaValue::Int(7)),
        ("PackEligible", FormulaValue::Int(0)),
    ];

    let mut cases = w044_reference_model_cases();
    cases.push(ReferenceModelCase {
        case_id: "reference_model.w045_local_harness_differential_boundary",
        assignments: LOCAL_HARNESS_ASSIGNMENTS,
        expected: LOCAL_HARNESS_EXPECTED,
    });
    cases.push(ReferenceModelCase {
        case_id: "reference_model.w045_retained_history_replay_correlation_control",
        assignments: RETAINED_HISTORY_ASSIGNMENTS,
        expected: RETAINED_HISTORY_EXPECTED,
    });
    cases
}

fn w045_independent_reference_model_rows() -> Vec<Value> {
    w045_reference_model_cases()
        .into_iter()
        .map(|case| {
            let result = eval_reference_model(case.assignments);
            let (actual_bindings, error) = match result {
                Ok(bindings) => (bindings, None),
                Err(err) => (Vec::new(), Some(err)),
            };
            let matches_expected = error.is_none() && actual_bindings == case.expected;
            let assignment_rows = case
                .assignments
                .iter()
                .map(|(name, expression)| {
                    json!({
                        "name": name,
                        "formula": expression
                    })
                })
                .collect::<Vec<_>>();
            json!({
                "case_id": case.case_id,
                "assignments": assignment_rows,
                "expected_bindings": reference_bindings_json(case.expected),
                "actual_bindings": reference_bindings_json(&actual_bindings),
                "error": error,
                "matches_expected": matches_expected,
                "implementation_path": "src/oxcalc-tracecalc/src/diversity_seam.rs::eval_reference_model",
                "implementation_kind": "independent_reference_model_evaluator",
                "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, deterministic before/after edit checks, fan-in checksum, mismatch-routing, soft-reference rebind, retained-witness attachment controls, local harness boundary checks, and retained-history replay-correlation controls",
                "independent_from": ["TraceCalc", "optimized_core", "TreeCalc", "OxFml", "OxFunc"]
            })
        })
        .collect()
}

#[allow(clippy::too_many_arguments)]
fn w045_source_rows(
    w045_residual_summary: &Value,
    w045_obligation_map: &Value,
    w045_promotion_contract: &Value,
    w045_oxfml_intake: &Value,
    w044_diversity: &Value,
    w044_implementation: &Value,
    w044_independent: &Value,
    w044_cross_engine: &Value,
    w044_authority: &Value,
    w044_blockers: &Value,
    w044_decision: &Value,
    w045_conformance: &Value,
    w045_dynamic: &Value,
    w045_callable: &Value,
    w045_conformance_blockers: &Value,
    w045_match_guard: &Value,
    w045_rust: &Value,
    w045_rust_refinement: &Value,
    w045_rust_blockers: &Value,
    w045_lean_tla: &Value,
    w045_tla_model_bound: &Value,
    w045_lean_tla_blockers: &Value,
    w045_stage2: &Value,
    w045_stage2_service_gate: &Value,
    w045_stage2_blockers: &Value,
    w045_stage2_decision: &Value,
    w045_operated: &Value,
    w045_readiness: &Value,
    w045_harness: &Value,
    w045_retained: &Value,
    w045_witness: &Value,
    w045_cross_engine_service: &Value,
    w045_alert_dispatch: &Value,
    w045_service_blockers: &Value,
    w045_operated_decision: &Value,
    w044_pack_decision: &Value,
) -> Vec<Value> {
    let w073 = w045_oxfml_intake
        .get("w073_formatting")
        .unwrap_or(&Value::Null);
    vec![
        json!({
            "row_id": "source.w045_residual_successor_map",
            "artifact": W045_RESIDUAL_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w045_residual_summary, "status") == "residual_successor_obligation_current_oxfml_intake_map_validated"
                && number_at(w045_residual_summary, "successor_obligation_count") == 36
                && number_at(w045_residual_summary, "source_residual_lane_count") == 22,
            "semantic_state": "w045_diversity_obligations_bound_with_no_proxy_guard"
        }),
        json!({
            "row_id": "source.w045_successor_obligation_rows",
            "artifact": W045_SUCCESSOR_OBLIGATION_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_obligation_map, "successor_obligation_count") == 36
                && number_at(w045_obligation_map, "source_residual_lane_count") == 22
                && row_count_at(w045_obligation_map) >= 18,
            "semantic_state": "w045_successor_obligation_rows_bound_for_diversity"
        }),
        json!({
            "row_id": "source.w045_promotion_contract_map",
            "artifact": W045_PROMOTION_CONTRACT_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_promotion_contract, "promotion_contract_count") == 18
                && array_len_at(w045_promotion_contract, "contracts") == 18,
            "semantic_state": "w045_no_proxy_promotion_contracts_retained"
        }),
        json!({
            "row_id": "source.w045_w073_typed_formatting_guard",
            "artifact": W045_OXFML_INBOUND_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": array_len_at(w073, "typed_rule_only_families") == 7
                && !bool_at(w073, "threshold_fallback_allowed_for_typed_families")
                && bool_at(w073, "old_bounded_string_non_interpretation_evidence_reviewed")
                && bool_at(w073, "downstream_typed_rule_request_construction_required")
                && !bool_at(w073, "downstream_uptake_verified_by_oxcalc"),
            "semantic_state": "w073_typed_rule_only_formatting_guard_retained_for_w045_diversity"
        }),
        json!({
            "row_id": "source.w044_diversity_predecessor_summary",
            "artifact": W044_DIVERSITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_diversity, "failed_row_count"),
            "promotion_guard": number_at(w044_diversity, "w044_independent_reference_model_case_count") == 8
                && number_at(w044_diversity, "exact_blocker_count") == 9
                && !bool_at(w044_diversity, "fully_independent_evaluator_promoted")
                && !bool_at(w044_decision, "mismatch_quarantine_service_promoted"),
            "semantic_state": "w044_diversity_predecessor_bound_without_promotion"
        }),
        json!({
            "row_id": "source.w044_reference_model_predecessor",
            "artifact": W044_INDEPENDENT_IMPLEMENTATION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_implementation, "failed_case_count"),
            "promotion_guard": number_at(w044_implementation, "case_count") == 8
                && number_at(w044_implementation, "match_count") == 8
                && number_at(w044_independent, "row_count") == 15,
            "semantic_state": "w044_reference_model_extended_by_w045_cases"
        }),
        json!({
            "row_id": "source.w044_cross_engine_predecessor",
            "artifact": W044_CROSS_ENGINE_DIVERSITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_cross_engine, "row_count") == 16
                && !bool_at(w044_cross_engine, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w044_cross_engine_rows_bound_without_operated_service"
        }),
        json!({
            "row_id": "source.w044_mismatch_authority_predecessor",
            "artifact": W044_MISMATCH_AUTHORITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_authority, "row_count") == 15
                && !bool_at(w044_authority, "mismatch_quarantine_service_promoted"),
            "semantic_state": "w044_mismatch_authority_bound_without_service_promotion"
        }),
        json!({
            "row_id": "source.w044_diversity_blockers_and_decision",
            "artifact": W044_DIVERSITY_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w044_blockers, "exact_blocker_count") == 9
                && !bool_at(w044_decision, "pack_grade_replay_promoted"),
            "semantic_state": "w044_diversity_blockers_retained"
        }),
        json!({
            "row_id": "source.w045_optimized_core_counterpart",
            "artifact": W045_CONFORMANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w045_conformance, "failed_row_count"),
            "promotion_guard": number_at(w045_conformance, "w045_direct_evidence_bound_count") == 2
                && number_at(w045_conformance, "w045_exact_remaining_blocker_count") == 5
                && number_at(w045_conformance, "w045_match_promoted_count") == 0,
            "semantic_state": "w045_optimized_core_evidence_bound_without_full_core_promotion"
        }),
        json!({
            "row_id": "source.w045_dynamic_transition_and_indirect_blocker",
            "artifact": W045_DYNAMIC_TRANSITION_COVERAGE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_dynamic, "dynamic_transition_row_count") == 3
                && number_at(w045_dynamic, "direct_evidence_bound_count") == 2
                && number_at(w045_dynamic, "exact_remaining_blocker_count") == 2,
            "semantic_state": "w045_dynamic_and_soft_reference_indirect_blockers_bound"
        }),
        json!({
            "row_id": "source.w045_callable_metadata_projection_blocker",
            "artifact": W045_CALLABLE_METADATA_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_callable, "row_count") == 4
                && !bool_at(w045_callable, "callable_metadata_projection_promoted"),
            "semantic_state": "w045_callable_metadata_projection_remains_exact_blocker"
        }),
        json!({
            "row_id": "source.w045_conformance_blockers_and_match_guard",
            "artifact": W045_CONFORMANCE_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_conformance_blockers, "exact_remaining_blocker_count") == 5
                && row_count_at(w045_conformance_blockers) == 5
                && !bool_at(w045_match_guard, "match_promotion_allowed"),
            "semantic_state": "w045_conformance_no_proxy_match_guard_bound"
        }),
        json!({
            "row_id": "source.w045_rust_refinement_bridge",
            "artifact": W045_RUST_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w045_rust, "failed_row_count"),
            "promotion_guard": number_at(w045_rust, "refinement_row_count") == 9
                && number_at(w045_rust, "exact_remaining_blocker_count") == 7
                && row_count_at(w045_rust_refinement) > 0,
            "semantic_state": "w045_rust_refinement_rows_bound_without_totality_promotion"
        }),
        json!({
            "row_id": "source.w045_rust_blockers",
            "artifact": W045_RUST_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_rust_blockers, "exact_remaining_blocker_count") == 7
                && row_count_at(w045_rust_blockers) == 7,
            "semantic_state": "w045_rust_exact_blockers_retained"
        }),
        json!({
            "row_id": "source.w045_lean_tla_model_bound",
            "artifact": W045_LEAN_TLA_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w045_lean_tla, "failed_row_count"),
            "promotion_guard": number_at(w045_lean_tla, "bounded_model_row_count") == 4
                && number_at(w045_lean_tla, "exact_remaining_blocker_count") == 6,
            "semantic_state": "w045_lean_tla_model_bound_rows_bound_without_full_verification_promotion"
        }),
        json!({
            "row_id": "source.w045_lean_tla_blockers",
            "artifact": W045_LEAN_TLA_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_lean_tla_blockers, "exact_remaining_blocker_count") == 6
                && row_count_at(w045_tla_model_bound) == 4,
            "semantic_state": "w045_lean_tla_exact_blockers_retained"
        }),
        json!({
            "row_id": "source.w045_stage2_service_evidence",
            "artifact": W045_STAGE2_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w045_stage2, "failed_row_count"),
            "promotion_guard": number_at(w045_stage2, "policy_row_count") == 29
                && number_at(w045_stage2, "exact_remaining_blocker_count") == 10
                && bool_at(w045_stage2, "service_gate_classification_evidenced")
                && !bool_at(w045_stage2, "stage2_policy_promoted"),
            "semantic_state": "w045_stage2_service_gate_bound_without_policy_promotion"
        }),
        json!({
            "row_id": "source.w045_stage2_service_gate",
            "artifact": W045_STAGE2_SERVICE_GATE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_stage2_service_gate, "row_count") == 10
                && number_at(w045_stage2_service_gate, "exact_remaining_blocker_count") == 4,
            "semantic_state": "w045_stage2_service_gate_rows_bound"
        }),
        json!({
            "row_id": "source.w045_stage2_blockers_and_decision",
            "artifact": W045_STAGE2_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_stage2_blockers, "exact_remaining_blocker_count") == 10
                && !bool_at(w045_stage2_decision, "pack_grade_replay_promoted")
                && !bool_at(w045_stage2_decision, "stage2_policy_promoted")
                && !bool_at(w045_stage2_decision, "service_gate_promoted"),
            "semantic_state": "w045_stage2_service_blockers_bound_for_diversity"
        }),
        json!({
            "row_id": "source.w045_operated_assurance_service_packet",
            "artifact": W045_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w045_operated, "failed_row_count"),
            "promotion_guard": bool_at(w045_operated, "local_operated_service_harness_runnable")
                && number_at(w045_operated, "service_harness_operation_count") == 8
                && number_at(w045_operated, "exact_service_blocker_count") == 6
                && !bool_at(w045_operated, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w045_local_harness_service_packet_bound_without_operated_service_promotion"
        }),
        json!({
            "row_id": "source.w045_service_readiness_register",
            "artifact": W045_SERVICE_READINESS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_readiness, "criteria_count") == 31
                && number_at(w045_readiness, "blocked_criteria_count") == 6
                && bool_at(w045_readiness, "w073_typed_rule_only_formatting_guard_carried")
                && !bool_at(w045_readiness, "w073_downstream_typed_rule_request_construction_verified"),
            "semantic_state": "w045_service_readiness_bound_with_exact_blockers"
        }),
        json!({
            "row_id": "source.w045_service_harness_register",
            "artifact": W045_SERVICE_HARNESS_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w045_harness, "local_harness_runnable")
                && number_at(w045_harness, "operation_count") == 8
                && !bool_at(w045_harness, "service_endpoint_present")
                && !bool_at(w045_harness, "external_dispatcher_present"),
            "semantic_state": "w045_local_harness_boundary_bound_not_operated_endpoint"
        }),
        json!({
            "row_id": "source.w045_retained_history_query",
            "artifact": W045_RETAINED_HISTORY_QUERY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w045_retained, "file_backed_retained_history_store_present")
                && bool_at(w045_retained, "local_harness_runnable")
                && array_len_at(w045_retained, "query_register") == 22,
            "semantic_state": "w045_retained_history_query_support_bound"
        }),
        json!({
            "row_id": "source.w045_retained_witness_lifecycle",
            "artifact": W045_RETAINED_WITNESS_LIFECYCLE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w045_witness, "retained_witness_lifecycle_register_present")
                && number_at(w045_witness, "witness_lifecycle_row_count") == 15
                && number_at(w045_witness, "pack_eligible_witness_count") == 0
                && !bool_at(w045_witness, "retained_witness_lifecycle_service_promoted"),
            "semantic_state": "w045_retained_witness_lifecycle_support_bound"
        }),
        json!({
            "row_id": "source.w045_cross_engine_service_blocker",
            "artifact": W045_CROSS_ENGINE_SERVICE_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w045_cross_engine_service, "file_backed_cross_engine_substrate_present")
                && !bool_at(w045_cross_engine_service, "operated_cross_engine_differential_service_present")
                && array_contains_string(
                    w045_cross_engine_service,
                    "blocked_service_claims",
                    "operated_mismatch_quarantine_dispatcher"
                ),
            "semantic_state": "w045_cross_engine_service_blocker_retained"
        }),
        json!({
            "row_id": "source.w045_alert_quarantine_contract",
            "artifact": W045_ALERT_DISPATCH_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_alert_dispatch, "evaluated_rule_count") == 42
                && number_at(w045_alert_dispatch, "quarantine_decision_count") == 0
                && number_at(w045_alert_dispatch, "alert_decision_count") == 0
                && !bool_at(w045_alert_dispatch, "external_alert_dispatcher_promoted")
                && !bool_at(w045_alert_dispatch, "quarantine_service_promoted"),
            "semantic_state": "w045_local_alert_quarantine_contract_clean"
        }),
        json!({
            "row_id": "source.w045_service_pack_no_proxy_guard",
            "artifact": W045_OPERATED_SERVICE_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w045_service_blockers, "exact_service_blocker_count") == 6
                && !bool_at(w045_operated_decision, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w045_operated_decision, "quarantine_service_promoted")
                && !bool_at(w045_operated_decision, "local_harness_promoted_as_service"),
            "semantic_state": "w045_service_pack_no_proxy_promotion_guard_clean"
        }),
        json!({
            "row_id": "source.w044_pack_decision",
            "artifact": W044_PACK_DECISION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w044_pack_decision, "missing_artifact_count"),
            "promotion_guard": text_at(w044_pack_decision, "decision_status") == "capability_not_promoted"
                && !bool_at(w044_pack_decision, "capability_promoted"),
            "semantic_state": "w044_pack_c5_blockers_bound_for_w045_diversity"
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w045_independent_evaluator_rows(
    w044_independent: &Value,
    w044_implementation: &Value,
    w044_diversity: &Value,
    w045_conformance: &Value,
    w045_dynamic: &Value,
    w045_callable: &Value,
    w045_rust: &Value,
    w045_lean_tla: &Value,
    w045_stage2: &Value,
    _w045_operated: &Value,
    w045_readiness: &Value,
    w045_harness: &Value,
    w045_retained: &Value,
    w045_witness: &Value,
    w045_alert_dispatch: &Value,
    w045_cross_engine_service: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "independent.w045_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "independence_state": "independent_reference_model_slice_present",
            "evidence": "w045_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "W045 broadens the independent slice with local harness and retained-history replay-correlation controls, but still does not cover full TreeCalc/OxCalc evaluator breadth."
        }),
        json!({
            "row_id": "independent.w044_reference_model_predecessor",
            "disposition_kind": "accepted_boundary",
            "independence_state": "predecessor_reference_model_slice_subsumed_by_w045_slice",
            "evidence": W044_INDEPENDENT_IMPLEMENTATION,
            "w044_case_count": number_at(w044_implementation, "case_count"),
            "w044_row_count": number_at(w044_independent, "row_count"),
            "w044_fully_independent_evaluator_promoted": bool_at(w044_diversity, "fully_independent_evaluator_promoted"),
            "promotion_consequence": "W044 reference-model controls remain predecessor evidence only."
        }),
        json!({
            "row_id": "independent.w045_operated_differential_case_pair",
            "disposition_kind": "broadened_independent_implementation",
            "independence_state": "local_harness_differential_boundary_case_present",
            "evidence": "reference_model.w045_local_harness_differential_boundary",
            "promotion_consequence": "The local harness differential boundary is executable reference-model evidence, not operated service evidence."
        }),
        json!({
            "row_id": "independent.w045_retained_history_replay_correlation_case",
            "disposition_kind": "broadened_independent_implementation",
            "independence_state": "retained_history_replay_correlation_case_present",
            "evidence": "reference_model.w045_retained_history_replay_correlation_control",
            "promotion_consequence": "Retained-history replay-correlation is represented in the reference model without pack eligibility promotion."
        }),
        json!({
            "row_id": "independent.optimized_core_same_family",
            "disposition_kind": "same_authority_family",
            "independence_state": "not_independent",
            "evidence": W045_CONFORMANCE_SUMMARY,
            "w045_disposition_row_count": number_at(w045_conformance, "w045_disposition_row_count"),
            "promotion_consequence": "Optimized/core conformance rows are same-family evidence and do not supply independent implementation authority."
        }),
        json!({
            "row_id": "independent.w045_dynamic_transition_bound",
            "disposition_kind": "accepted_boundary",
            "independence_state": "dynamic_transition_input_bound",
            "evidence": W045_DYNAMIC_TRANSITION_COVERAGE,
            "direct_evidence_bound_count": number_at(w045_dynamic, "direct_evidence_bound_count"),
            "promotion_consequence": "Dynamic-transition rows are bound as source evidence only."
        }),
        json!({
            "row_id": "independent.w045_soft_reference_indirect_breadth_blocker",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "soft_reference_indirect_breadth_absent",
            "evidence": W045_DYNAMIC_TRANSITION_COVERAGE,
            "promotion_consequence": "Soft-reference and INDIRECT late-resolution breadth remains outside the independent reference-model slice."
        }),
        json!({
            "row_id": "independent.w045_rust_refinement_bridge",
            "disposition_kind": "accepted_boundary",
            "independence_state": "rust_refinement_bridge_bound",
            "evidence": W045_RUST_SUMMARY,
            "refinement_row_count": number_at(w045_rust, "refinement_row_count"),
            "promotion_consequence": "Rust refinement rows are proof inputs, not independent evaluator breadth."
        }),
        json!({
            "row_id": "independent.w045_lean_tla_model_bound",
            "disposition_kind": "accepted_boundary",
            "independence_state": "lean_tla_model_bound_bound",
            "evidence": W045_LEAN_TLA_SUMMARY,
            "bounded_model_row_count": number_at(w045_lean_tla, "bounded_model_row_count"),
            "promotion_consequence": "Lean/TLA rows remain bounded proof/model evidence."
        }),
        json!({
            "row_id": "independent.w045_stage2_service_gate",
            "disposition_kind": "accepted_boundary",
            "independence_state": "stage2_service_gate_classification_bound",
            "evidence": W045_STAGE2_SUMMARY,
            "service_gate_row_count": number_at(w045_stage2, "service_gate_row_count"),
            "promotion_consequence": "Stage 2 service-gate rows classify dependencies without Stage 2 policy promotion."
        }),
        json!({
            "row_id": "independent.w045_local_service_harness_boundary",
            "disposition_kind": "local_harness_boundary",
            "independence_state": "local_harness_not_service",
            "evidence": W045_SERVICE_HARNESS_REGISTER,
            "local_harness_runnable": bool_at(w045_harness, "local_harness_runnable"),
            "service_endpoint_present": bool_at(w045_harness, "service_endpoint_present"),
            "promotion_consequence": "The W045 local harness is runnable artifact orchestration, not an operated service."
        }),
        json!({
            "row_id": "independent.w045_retained_history_query_boundary",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "retained_history_query_contract_bound",
            "evidence": W045_RETAINED_HISTORY_QUERY,
            "query_register_row_count": array_len_at(w045_retained, "query_register"),
            "promotion_consequence": "Retained-history rows remain local query-contract evidence."
        }),
        json!({
            "row_id": "independent.w045_retained_witness_lifecycle_boundary",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "retained_witness_lifecycle_contract_bound",
            "evidence": W045_RETAINED_WITNESS_LIFECYCLE,
            "witness_lifecycle_row_count": number_at(w045_witness, "witness_lifecycle_row_count"),
            "promotion_consequence": "Retained-witness lifecycle rows do not promote a lifecycle service."
        }),
        json!({
            "row_id": "independent.w045_alert_dispatch_boundary",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "alert_dispatch_contract_bound",
            "evidence": W045_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w045_alert_dispatch, "evaluated_rule_count"),
            "promotion_consequence": "Local alert/quarantine rules are evaluated but no external dispatcher is operated."
        }),
        json!({
            "row_id": "independent.w045_cross_engine_service_blocker",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "operated_cross_engine_service_absent",
            "evidence": W045_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w045_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "Cross-engine service authority remains file-backed/local-harness only."
        }),
        json!({
            "row_id": "independent.w045_callable_metadata_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "callable_metadata_projection_absent",
            "evidence": W045_CALLABLE_METADATA_REGISTER,
            "callable_metadata_projection_promoted": bool_at(w045_callable, "callable_metadata_projection_promoted"),
            "promotion_consequence": "Callable metadata projection remains a successor dependency."
        }),
        json!({
            "row_id": "independent.w045_w073_downstream_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "w073_downstream_typed_rule_request_construction_unverified",
            "evidence": W045_SERVICE_READINESS,
            "w073_downstream_typed_rule_request_construction_verified": bool_at(w045_readiness, "w073_downstream_typed_rule_request_construction_verified"),
            "promotion_consequence": "W073 downstream typed-rule request construction remains routed to calc-zkio.8."
        }),
        json!({
            "row_id": "independent.full_breadth_absent",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "full_independent_implementation_breadth_absent",
            "evidence": "w045_independent_reference_model_implementation.json",
            "promotion_consequence": "Full independent evaluator breadth remains unavailable until scheduling, publication, effects, references, broad OxFml, and callable metadata are independently covered."
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w045_cross_engine_differential_rows(
    w044_cross_engine: &Value,
    w045_stage2: &Value,
    w045_stage2_service_gate: &Value,
    w045_operated: &Value,
    w045_readiness: &Value,
    w045_harness: &Value,
    w045_retained: &Value,
    w045_witness: &Value,
    w045_cross_engine_service: &Value,
    w045_alert_dispatch: &Value,
    w045_oxfml_intake: &Value,
    w045_dynamic: &Value,
    w045_callable: &Value,
    w044_pack_decision: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let w073 = w045_oxfml_intake
        .get("w073_formatting")
        .unwrap_or(&Value::Null);
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "cross_engine.w045_reference_model_matches",
            "disposition_kind": "broadened_independent_implementation",
            "service_state": "reference_model_clean",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "Reference-model cases match expected values but do not create an operated cross-engine service."
        }),
        json!({
            "row_id": "cross_engine.w044_predecessor_register",
            "disposition_kind": "accepted_boundary",
            "service_state": "predecessor_rows_bound",
            "evidence": W044_CROSS_ENGINE_DIVERSITY,
            "w044_row_count": number_at(w044_cross_engine, "row_count"),
            "promotion_consequence": "W044 cross-engine rows are retained predecessor evidence."
        }),
        json!({
            "row_id": "cross_engine.w045_operated_assurance_packet",
            "disposition_kind": "service_contract_boundary",
            "service_state": "local_harness_packet_bound",
            "evidence": W045_OPERATED_ASSURANCE_SUMMARY,
            "service_harness_operation_count": number_at(w045_operated, "service_harness_operation_count"),
            "promotion_consequence": "W045 operated-assurance artifacts add a runnable local harness but not an operated service endpoint."
        }),
        json!({
            "row_id": "cross_engine.w045_local_harness_boundary",
            "disposition_kind": "local_harness_boundary",
            "service_state": "local_harness_not_service",
            "evidence": W045_SERVICE_HARNESS_REGISTER,
            "operation_count": number_at(w045_harness, "operation_count"),
            "service_endpoint_present": bool_at(w045_harness, "service_endpoint_present"),
            "promotion_consequence": "Local harness operations are artifact orchestration, not recurring differential service operation."
        }),
        json!({
            "row_id": "cross_engine.w045_service_readiness",
            "disposition_kind": "service_contract_boundary",
            "service_state": "readiness_classified",
            "evidence": W045_SERVICE_READINESS,
            "criteria_count": number_at(w045_readiness, "criteria_count"),
            "blocked_criteria_count": number_at(w045_readiness, "blocked_criteria_count"),
            "promotion_consequence": "Service readiness is classified with exact blockers retained."
        }),
        json!({
            "row_id": "cross_engine.w045_retained_history_query_contract",
            "disposition_kind": "service_contract_boundary",
            "service_state": "retained_history_contract_bound",
            "evidence": W045_RETAINED_HISTORY_QUERY,
            "query_register_row_count": array_len_at(w045_retained, "query_register"),
            "promotion_consequence": "Retained-history query rows support replay correlation but remain file-backed."
        }),
        json!({
            "row_id": "cross_engine.w045_retained_witness_lifecycle_contract",
            "disposition_kind": "service_contract_boundary",
            "service_state": "retained_witness_contract_bound",
            "evidence": W045_RETAINED_WITNESS_LIFECYCLE,
            "witness_lifecycle_row_count": number_at(w045_witness, "witness_lifecycle_row_count"),
            "promotion_consequence": "Retained-witness lifecycle rows remain non-pack-eligible local evidence."
        }),
        json!({
            "row_id": "cross_engine.w045_stage2_service_gate",
            "disposition_kind": "service_contract_boundary",
            "service_state": "stage2_service_gate_classified",
            "evidence": W045_STAGE2_SERVICE_GATE,
            "row_count": number_at(w045_stage2_service_gate, "row_count"),
            "promotion_consequence": "Stage 2 service gates classify dependencies without service promotion."
        }),
        json!({
            "row_id": "cross_engine.w045_dynamic_soft_reference_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_DYNAMIC_TRANSITION_COVERAGE,
            "exact_remaining_blocker_count": number_at(w045_dynamic, "exact_remaining_blocker_count"),
            "promotion_consequence": "Soft-reference and INDIRECT breadth remains a service-relevant blocker."
        }),
        json!({
            "row_id": "cross_engine.w045_cross_engine_service_blocker",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w045_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "Cross-engine evidence remains file-backed and not operated."
        }),
        json!({
            "row_id": "cross_engine.w045_alert_quarantine_blocker",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_ALERT_DISPATCH_REGISTER,
            "quarantine_service_promoted": bool_at(w045_alert_dispatch, "quarantine_service_promoted"),
            "promotion_consequence": "Local clean alert rows cannot stand in for an enforcing quarantine service."
        }),
        json!({
            "row_id": "cross_engine.w045_retained_witness_attachment_service",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_RETAINED_WITNESS_LIFECYCLE,
            "retained_witness_lifecycle_service_promoted": bool_at(w045_witness, "retained_witness_lifecycle_service_promoted"),
            "promotion_consequence": "Retained-witness attachment is represented by local rows, not by an operated attachment service."
        }),
        json!({
            "row_id": "cross_engine.w045_operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_OPERATED_ASSURANCE_SUMMARY,
            "operated_cross_engine_differential_service_promoted": bool_at(w045_operated, "operated_cross_engine_differential_service_promoted"),
            "promotion_consequence": "No recurring cross-engine service endpoint exists."
        }),
        json!({
            "row_id": "cross_engine.stage2_operated_service_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_STAGE2_SUMMARY,
            "service_gate_promoted": bool_at(w045_stage2, "service_gate_promoted"),
            "promotion_consequence": "Stage 2 operated differential dependency remains blocked."
        }),
        json!({
            "row_id": "cross_engine.pack_governance_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W044_PACK_DECISION,
            "capability_promoted": bool_at(w044_pack_decision, "capability_promoted"),
            "promotion_consequence": "Pack/C5 diversity remains unavailable while operated differential and retained-witness services are blocked."
        }),
        json!({
            "row_id": "cross_engine.callable_public_migration_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_CALLABLE_METADATA_REGISTER,
            "callable_metadata_projection_promoted": bool_at(w045_callable, "callable_metadata_projection_promoted"),
            "promotion_consequence": "Callable metadata, public migration, registered-external, and provider publication remain successor dependencies."
        }),
        json!({
            "row_id": "cross_engine.w073_typed_only_formatting_guard",
            "disposition_kind": "accepted_boundary",
            "service_state": "observable_guard_retained",
            "evidence": W045_OXFML_INBOUND_INTAKE,
            "typed_rule_only_family_count": array_len_at(w073, "typed_rule_only_families"),
            "threshold_fallback_allowed_for_typed_families": bool_at(w073, "threshold_fallback_allowed_for_typed_families"),
            "downstream_typed_rule_request_construction_required": bool_at(w073, "downstream_typed_rule_request_construction_required"),
            "promotion_consequence": "Diversity comparisons must preserve the W073 typed_rule-only aggregate/visualization formatting guard."
        }),
        json!({
            "row_id": "cross_engine.w073_downstream_request_construction",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W045_OXFML_INBOUND_INTAKE,
            "downstream_uptake_verified_by_oxcalc": bool_at(w073, "downstream_uptake_verified_by_oxcalc"),
            "promotion_consequence": "Downstream W073 typed-rule request construction remains routed to calc-zkio.8."
        }),
        json!({
            "row_id": "cross_engine.w045_spec_evolution_mismatch_route",
            "disposition_kind": "service_contract_boundary",
            "service_state": "spec_evolution_route_declared_not_operated",
            "evidence": W045_SUCCESSOR_OBLIGATION_MAP,
            "promotion_consequence": "Mismatch handling can route to spec correction or implementation fault only after operated differential and quarantine evidence exists."
        }),
        json!({
            "row_id": "cross_engine.w045_no_proxy_local_harness_guard",
            "disposition_kind": "local_harness_boundary",
            "service_state": "local_harness_not_promotion",
            "evidence": W045_SERVICE_HARNESS_REGISTER,
            "local_harness_runnable": bool_at(w045_harness, "local_harness_runnable"),
            "promotion_consequence": "Local harness evidence cannot promote service, Stage 2, pack, C5, or release-grade claims."
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w045_mismatch_authority_rows(
    w044_authority: &Value,
    w045_operated: &Value,
    w045_readiness: &Value,
    w045_harness: &Value,
    w045_cross_engine_service: &Value,
    w045_alert_dispatch: &Value,
    w045_witness: &Value,
    w045_stage2: &Value,
    w045_stage2_service_gate: &Value,
    w045_oxfml_intake: &Value,
    w044_pack_decision: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "authority.w045_independent_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "authority_state": "independent_reference_model_runtime",
            "evidence": "w045_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "authority_limit": "named-reference model slice only; no scheduling, publication, effects, callable-carrier, broad OxFml seam, or full OxFunc kernel authority"
        }),
        json!({
            "row_id": "authority.w044_reference_model_predecessor",
            "disposition_kind": "accepted_boundary",
            "authority_state": "predecessor_reference_model_runtime",
            "evidence": W044_MISMATCH_AUTHORITY,
            "w044_authority_row_count": number_at(w044_authority, "row_count"),
            "authority_limit": "predecessor authority remains a narrower reference-model slice"
        }),
        json!({
            "row_id": "authority.tracecalc_correctness_oracle",
            "disposition_kind": "accepted_boundary",
            "authority_state": "reference_oracle",
            "evidence": "TraceCalc",
            "authority_limit": "oracle authority for covered observable behavior only"
        }),
        json!({
            "row_id": "authority.optimized_core_same_implementation_family",
            "disposition_kind": "same_authority_family",
            "authority_state": "not_independent",
            "evidence": W045_CONFORMANCE_SUMMARY,
            "authority_limit": "optimized/core conformance does not supply separate implementation authority"
        }),
        json!({
            "row_id": "authority.direct_oxfml_external_formula_evaluator",
            "disposition_kind": "accepted_external_slice",
            "authority_state": "external_formula_evaluator_only",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "authority_limit": "external evaluator authority is limited to the consumed OxFml seam and does not implement OxCalc scheduling/publication"
        }),
        json!({
            "row_id": "authority.local_harness_file_backed_authority",
            "disposition_kind": "local_harness_boundary",
            "authority_state": "local_harness_artifact_authority",
            "evidence": W045_SERVICE_HARNESS_REGISTER,
            "operation_count": number_at(w045_harness, "operation_count"),
            "authority_limit": "local runnable harness cannot substitute for operated differential-service authority"
        }),
        json!({
            "row_id": "authority.service_envelope_file_backed_authority",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "file_backed_artifact_authority",
            "evidence": W045_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w045_operated, "service_envelope_row_count"),
            "authority_limit": "file-backed row agreement and retained artifacts cannot substitute for an operated differential service"
        }),
        json!({
            "row_id": "authority.retained_history_query_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_retained_history_query_contract",
            "evidence": W045_SERVICE_READINESS,
            "query_register_row_count": number_at(w045_readiness, "query_register_row_count"),
            "authority_limit": "query contract is deterministic evidence, not an operated retained-history endpoint"
        }),
        json!({
            "row_id": "authority.retained_witness_attachment_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_attachment_contract_not_service",
            "evidence": W045_RETAINED_WITNESS_LIFECYCLE,
            "witness_lifecycle_row_count": number_at(w045_witness, "witness_lifecycle_row_count"),
            "authority_limit": "retained-witness rows describe lifecycle and control intent, not an operated attachment service"
        }),
        json!({
            "row_id": "authority.mismatch_router_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_router_contract_not_service",
            "evidence": W045_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w045_alert_dispatch, "evaluated_rule_count"),
            "authority_limit": "local alert dispatch classification is a contract row set, not an external quarantine service"
        }),
        json!({
            "row_id": "authority.spec_evolution_route",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "spec_evolution_route_not_service",
            "evidence": W045_SUCCESSOR_OBLIGATION_MAP,
            "authority_limit": "agreement, mismatch, implementation-fault, and spec-correction routes remain declared classification until operated service evidence exists"
        }),
        json!({
            "row_id": "authority.operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_operated_service",
            "evidence": W045_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w045_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "authority_limit": "no recurring service endpoint, retained service history, or service-level mismatch action path exists"
        }),
        json!({
            "row_id": "authority.mismatch_quarantine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_quarantine_service",
            "evidence": W045_ALERT_DISPATCH_REGISTER,
            "quarantine_service_promoted": bool_at(w045_alert_dispatch, "quarantine_service_promoted"),
            "authority_limit": "local clean dispatch rows cannot stand in for an enforcing mismatch quarantine service"
        }),
        json!({
            "row_id": "authority.retained_witness_attachment_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_retained_witness_attachment_service",
            "evidence": W045_RETAINED_WITNESS_LIFECYCLE,
            "retained_witness_lifecycle_service_promoted": bool_at(w045_witness, "retained_witness_lifecycle_service_promoted"),
            "authority_limit": "no operated service attaches mismatch witnesses to quarantine, retained history, or pack governance"
        }),
        json!({
            "row_id": "authority.full_independent_evaluator_breadth",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_beyond_reference_model_slice",
            "evidence": "w045_independent_reference_model_implementation.json",
            "authority_limit": "reference model slice does not cover TreeCalc scheduling, references outside declared order, publication, effects, broad OxFml seam breadth, or callable metadata"
        }),
        json!({
            "row_id": "authority.callable_metadata_and_public_surface",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_callable_and_public_surface",
            "evidence": W045_OXFML_INBOUND_INTAKE,
            "w073_downstream_uptake_verified": bool_at(
                w045_oxfml_intake.get("w073_formatting").unwrap_or(&Value::Null),
                "downstream_uptake_verified_by_oxcalc"
            ),
            "authority_limit": "W073 downstream request construction, public migration, callable metadata, and broad display/publication remain successor dependencies"
        }),
        json!({
            "row_id": "authority.stage2_service_gate_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_stage2_service_gate",
            "evidence": W045_STAGE2_SERVICE_GATE,
            "exact_service_gate_blocker_count": number_at(w045_stage2_service_gate, "exact_remaining_blocker_count"),
            "authority_limit": "Stage 2 service gates remain classified but unpromoted"
        }),
        json!({
            "row_id": "authority.pack_governance_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_pack_governance",
            "evidence": W044_PACK_DECISION,
            "capability_promoted": bool_at(w044_pack_decision, "capability_promoted"),
            "authority_limit": "pack/C5 cannot be inferred from W045 diversity or local harness rows"
        }),
        json!({
            "row_id": "authority.release_grade_diversity_promotion",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_release_grade_authority",
            "evidence": W045_STAGE2_SUMMARY,
            "exact_stage2_blocker_count": number_at(w045_stage2, "exact_remaining_blocker_count"),
            "authority_limit": "release-grade diversity authority remains unavailable while full independent implementation and operated service blockers remain"
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w045_retained_witness_attachment_rows(
    w045_witness: &Value,
    w045_retained: &Value,
    w045_harness: &Value,
    w045_alert_dispatch: &Value,
    w045_cross_engine_service: &Value,
    w045_stage2: &Value,
    w044_pack_decision: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "attachment.local_reference_model_control",
            "disposition_kind": "broadened_independent_implementation",
            "attachment_state": "local_reference_model_control",
            "evidence": "reference_model.w045_retained_history_replay_correlation_control",
            "promotion_consequence": "reference-model attachment control is local evidence only"
        }),
        json!({
            "row_id": "attachment.retained_witness_lifecycle_register",
            "disposition_kind": "service_contract_boundary",
            "attachment_state": "local_attachment_contract",
            "evidence": W045_RETAINED_WITNESS_LIFECYCLE,
            "witness_lifecycle_row_count": number_at(w045_witness, "witness_lifecycle_row_count"),
            "pack_eligible_witness_count": number_at(w045_witness, "pack_eligible_witness_count"),
            "promotion_consequence": "retained-witness lifecycle is registered without pack eligibility"
        }),
        json!({
            "row_id": "attachment.retained_history_query_correlation",
            "disposition_kind": "service_contract_boundary",
            "attachment_state": "local_history_correlation_contract",
            "evidence": W045_RETAINED_HISTORY_QUERY,
            "query_register_row_count": array_len_at(w045_retained, "query_register"),
            "promotion_consequence": "retained-history correlation is query-contract evidence"
        }),
        json!({
            "row_id": "attachment.local_harness_emit_operation",
            "disposition_kind": "local_harness_boundary",
            "attachment_state": "local_harness_attachment_projection",
            "evidence": W045_SERVICE_HARNESS_REGISTER,
            "local_harness_runnable": bool_at(w045_harness, "local_harness_runnable"),
            "promotion_consequence": "local harness can emit witness lifecycle rows but is not an attachment service"
        }),
        json!({
            "row_id": "attachment.alert_dispatch_contract",
            "disposition_kind": "service_contract_boundary",
            "attachment_state": "local_alert_contract",
            "evidence": W045_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w045_alert_dispatch, "evaluated_rule_count"),
            "promotion_consequence": "local alert dispatch is a clean contract without external quarantine"
        }),
        json!({
            "row_id": "attachment.operated_attachment_service",
            "disposition_kind": "exact_remaining_blocker",
            "attachment_state": "blocked_no_retained_witness_attachment_service",
            "evidence": W045_CROSS_ENGINE_SERVICE_REGISTER,
            "service_state": "blocked",
            "operated_cross_engine_differential_service_present": bool_at(w045_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "no operated service attaches retained witnesses to mismatch quarantine"
        }),
        json!({
            "row_id": "attachment.stage2_retained_witness_pack_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "attachment_state": "blocked_stage2_pack_dependency",
            "evidence": W045_STAGE2_SUMMARY,
            "service_state": "blocked",
            "retained_witness_lifecycle_promoted": bool_at(w045_stage2, "retained_witness_lifecycle_promoted"),
            "promotion_consequence": "Stage 2 and pack remain blocked by retained-witness lifecycle dependency"
        }),
        json!({
            "row_id": "attachment.pack_governance_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "attachment_state": "blocked_pack_governance_dependency",
            "evidence": W044_PACK_DECISION,
            "service_state": "blocked",
            "capability_promoted": bool_at(w044_pack_decision, "capability_promoted"),
            "promotion_consequence": "pack-grade replay governance cannot be inferred from local witness rows"
        }),
    ]
}

fn w045_operated_differential_harness_rows(w045_harness: &Value) -> Vec<Value> {
    w045_harness
        .get("operations")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .map(|operation| {
            json!({
                "row_id": format!(
                    "harness.{}",
                    operation
                        .get("operation_id")
                        .and_then(Value::as_str)
                        .unwrap_or("unknown")
                ),
                "disposition_kind": "local_harness_boundary",
                "operation_id": operation.get("operation_id").cloned().unwrap_or(Value::Null),
                "operation_kind": operation.get("operation_kind").cloned().unwrap_or(Value::Null),
                "result_state": operation.get("result_state").cloned().unwrap_or(Value::Null),
                "local_runnable": operation
                    .get("local_runnable")
                    .and_then(Value::as_bool)
                    .unwrap_or(false),
                "service_endpoint_present": bool_at(w045_harness, "service_endpoint_present"),
                "operated_service_promoted": false,
                "promotion_consequence": "local harness operation is deterministic artifact orchestration only"
            })
        })
        .collect()
}

#[allow(clippy::too_many_arguments)]
fn w045_exact_blockers(
    w044_blockers: &Value,
    w045_stage2_blockers: &Value,
    w045_service_blockers: &Value,
    w045_cross_engine_service: &Value,
    w045_conformance_blockers: &Value,
    w045_lean_tla_blockers: &Value,
    w045_rust_blockers: &Value,
    w045_oxfml_intake: &Value,
) -> Vec<Value> {
    let w073 = w045_oxfml_intake
        .get("w073_formatting")
        .unwrap_or(&Value::Null);
    vec![
        json!({
            "blocker_id": "w045_diversity.full_independent_evaluator_breadth_absent",
            "owner": "calc-zkio.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": "w045_independent_reference_model_implementation.json",
            "predecessor_blocker_present": row_with_field_exists(
                w044_blockers,
                "blocker_id",
                "w044_diversity.full_independent_evaluator_breadth_absent"
            ),
            "reason": "W045 extends the independent named-reference model evaluator with local harness and retained-history replay-correlation controls, but full TreeCalc/OxCalc evaluator breadth remains absent.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "w045_diversity.operated_cross_engine_differential_service_absent",
            "owner": "calc-zkio.6; calc-zkio.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w045_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w045_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "reason": "W045 service artifacts are local-harness/file-backed and do not operate a recurring cross-engine differential service.",
            "promotion_consequence": "operated cross-engine diversity service remains unpromoted"
        }),
        json!({
            "blocker_id": "w045_diversity.mismatch_quarantine_service_absent",
            "owner": "calc-zkio.6; calc-zkio.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_OPERATED_SERVICE_BLOCKERS,
            "service_blocker_rows": row_count_at(w045_service_blockers),
            "reason": "Local alert/quarantine rows are evaluated, but no external mismatch triage/quarantine service is operated.",
            "promotion_consequence": "service-level diversity assurance and mismatch quarantine remain unpromoted"
        }),
        json!({
            "blocker_id": "w045_diversity.stage2_operated_differential_dependency_absent",
            "owner": "calc-zkio.5; calc-zkio.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_STAGE2_BLOCKERS,
            "stage2_blocker_rows": row_count_at(w045_stage2_blockers),
            "reason": "Stage 2 diversity still needs operated differential service, retained-witness lifecycle, retention SLO, pack governance, and release-grade decision evidence before strategy-level diversity promotion.",
            "promotion_consequence": "Stage 2 diversity remains a blocked dependency for release-grade promotion"
        }),
        json!({
            "blocker_id": "w045_diversity.retained_witness_attachment_service_absent",
            "owner": "calc-zkio.6; calc-zkio.7; calc-zkio.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_OPERATED_SERVICE_BLOCKERS,
            "reason": "Retained-witness lifecycle rows and W045 local attachment controls exist, but no service attaches mismatch witnesses to quarantine, pack replay, or retained-history lifecycle operation.",
            "promotion_consequence": "mismatch quarantine, pack-grade replay, C5, and release-grade diversity remain blocked"
        }),
        json!({
            "blocker_id": "w045_diversity.oxfml_callable_public_migration_dependency_absent",
            "owner": "calc-zkio.7; calc-zkio.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_OXFML_INBOUND_INTAKE,
            "reason": "The independent reference-model slice does not settle broad OxFml display publication, public migration, callable-carrier sufficiency, registered-external callable projection, or LET/LAMBDA threading through OxFunc-adjacent behavior.",
            "promotion_consequence": "broad OxFml callable/display publication and public migration remain successor dependencies"
        }),
        json!({
            "blocker_id": "w045_diversity.optimized_core_callable_metadata_dependency_absent",
            "owner": "calc-zkio.2; calc-zkio.7; calc-zkio.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_CONFORMANCE_BLOCKERS,
            "conformance_blocker_rows": row_count_at(w045_conformance_blockers),
            "reason": "Callable metadata projection and registered external/provider publication semantics remain outside the W045 independent reference-model slice.",
            "promotion_consequence": "callable metadata projection remains unpromoted"
        }),
        json!({
            "blocker_id": "w045_diversity.formal_model_bound_dependency_absent",
            "owner": "calc-zkio.4; calc-zkio.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_LEAN_TLA_BLOCKERS,
            "lean_tla_blocker_rows": row_count_at(w045_lean_tla_blockers),
            "rust_blocker_rows": row_count_at(w045_rust_blockers),
            "reason": "Lean/TLA model bounds, Rust totality/refinement, and unbounded fairness blockers remain proof/model dependencies for release-grade diversity.",
            "promotion_consequence": "full Rust, Lean/TLA, and scheduler fairness remain unpromoted"
        }),
        json!({
            "blocker_id": "w045_diversity.pack_grade_replay_governance_dependency_absent",
            "owner": "calc-zkio.7; calc-zkio.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W044_PACK_DECISION,
            "reason": "Pack-grade replay governance cannot be inferred from local retained-history, retained-witness, harness, file-backed differential, or bounded mismatch-control rows.",
            "promotion_consequence": "pack-grade replay, C5, and release-grade verification remain unpromoted"
        }),
        json!({
            "blocker_id": "w045_diversity.w073_downstream_typed_rule_request_construction_absent",
            "owner": "calc-zkio.7; calc-zkio.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W045_OXFML_INBOUND_INTAKE,
            "typed_rule_only_family_count": array_len_at(w073, "typed_rule_only_families"),
            "downstream_uptake_verified_by_oxcalc": bool_at(w073, "downstream_uptake_verified_by_oxcalc"),
            "reason": "W073 typed-rule-only direct-replacement semantics are carried, but downstream request construction uptake is not verified in W045.7.",
            "promotion_consequence": "W073 downstream typed request construction remains routed to calc-zkio.8"
        }),
    ]
}

fn w045_diversity_validation_failures(
    implementation_rows: &[Value],
    independent_rows: &[Value],
    cross_engine_rows: &[Value],
    authority_rows: &[Value],
    retained_attachment_rows: &[Value],
    differential_harness_rows: &[Value],
    blockers: &[Value],
) -> Vec<String> {
    let mut failures = Vec::new();
    if implementation_rows.len() < 10
        || implementation_rows
            .iter()
            .any(|row| !bool_at(row, "matches_expected"))
    {
        failures.push("w045_diversity.independent_reference_model_case_mismatch".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("independent_reference_model_slice_present")
    }) {
        failures.push("w045_diversity.reference_model_row_missing".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("full_independent_implementation_breadth_absent")
    }) {
        failures.push("w045_diversity.full_breadth_blocker_row_missing".to_string());
    }
    if !cross_engine_rows
        .iter()
        .any(|row| row.get("service_state").and_then(Value::as_str) == Some("blocked"))
    {
        failures.push("w045_diversity.blocked_cross_engine_service_row_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_operated_service")
    }) {
        failures.push("w045_diversity.operated_service_authority_blocker_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_quarantine_service")
    }) {
        failures.push("w045_diversity.mismatch_quarantine_authority_blocker_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str)
            == Some("blocked_no_retained_witness_attachment_service")
    }) {
        failures.push("w045_diversity.retained_witness_attachment_blocker_missing".to_string());
    }
    if retained_attachment_rows.len() < 8
        || !retained_attachment_rows.iter().any(|row| {
            row.get("attachment_state").and_then(Value::as_str) == Some("local_attachment_contract")
        })
    {
        failures.push("w045_diversity.retained_witness_attachment_rows_missing".to_string());
    }
    if differential_harness_rows.len() < 8
        || differential_harness_rows
            .iter()
            .any(|row| !bool_at(row, "local_runnable"))
    {
        failures.push("w045_diversity.local_harness_rows_missing".to_string());
    }
    if blockers.len() < 10 {
        failures.push("w045_diversity.exact_blocker_count_too_low".to_string());
    }
    if independent_rows
        .iter()
        .chain(cross_engine_rows.iter())
        .chain(authority_rows.iter())
        .chain(retained_attachment_rows.iter())
        .chain(differential_harness_rows.iter())
        .any(|row| {
            bool_at(row, "fully_independent_evaluator_promoted")
                || bool_at(row, "operated_cross_engine_differential_service_promoted")
                || bool_at(row, "mismatch_quarantine_service_promoted")
                || bool_at(row, "retained_witness_attachment_service_promoted")
                || bool_at(row, "operated_service_promoted")
        })
    {
        failures.push("w045_diversity.unexpected_promotion_row".to_string());
    }
    failures
}

fn w043_reference_model_cases() -> Vec<ReferenceModelCase> {
    const FAN_IN_CHECKSUM_ASSIGNMENTS: &[(&str, &str)] = &[
        ("Top", "4"),
        ("Left", "6"),
        ("Bias", "3"),
        ("Row", "Top+Left"),
        ("Column", "Top*Left"),
        ("Checksum", "Row+Column+Bias"),
        ("Gate", "IF(Checksum>=37,Checksum,0)"),
    ];
    const FAN_IN_CHECKSUM_EXPECTED: &[(&str, FormulaValue)] = &[
        ("Top", FormulaValue::Int(4)),
        ("Left", FormulaValue::Int(6)),
        ("Bias", FormulaValue::Int(3)),
        ("Row", FormulaValue::Int(10)),
        ("Column", FormulaValue::Int(24)),
        ("Checksum", FormulaValue::Int(37)),
        ("Gate", FormulaValue::Int(37)),
    ];
    const MISMATCH_ROUTING_ASSIGNMENTS: &[(&str, &str)] = &[
        ("Left", "8"),
        ("Top", "5"),
        ("Actual", "Left+Top"),
        ("Oracle", "14"),
        ("Match", "IF(Actual=Oracle,1,0)"),
        ("Quarantine", "IF(Match=1,0,1)"),
    ];
    const MISMATCH_ROUTING_EXPECTED: &[(&str, FormulaValue)] = &[
        ("Left", FormulaValue::Int(8)),
        ("Top", FormulaValue::Int(5)),
        ("Actual", FormulaValue::Int(13)),
        ("Oracle", FormulaValue::Int(14)),
        ("Match", FormulaValue::Int(0)),
        ("Quarantine", FormulaValue::Int(1)),
    ];

    let mut cases = w042_reference_model_cases();
    cases.push(ReferenceModelCase {
        case_id: "reference_model.fan_in_checksum",
        assignments: FAN_IN_CHECKSUM_ASSIGNMENTS,
        expected: FAN_IN_CHECKSUM_EXPECTED,
    });
    cases.push(ReferenceModelCase {
        case_id: "reference_model.mismatch_routing_control",
        assignments: MISMATCH_ROUTING_ASSIGNMENTS,
        expected: MISMATCH_ROUTING_EXPECTED,
    });
    cases
}

fn w043_independent_reference_model_rows() -> Vec<Value> {
    w043_reference_model_cases()
        .into_iter()
        .map(|case| {
            let result = eval_reference_model(case.assignments);
            let (actual_bindings, error) = match result {
                Ok(bindings) => (bindings, None),
                Err(err) => (Vec::new(), Some(err)),
            };
            let matches_expected = error.is_none() && actual_bindings == case.expected;
            let assignment_rows = case
                .assignments
                .iter()
                .map(|(name, expression)| {
                    json!({
                        "name": name,
                        "formula": expression
                    })
                })
                .collect::<Vec<_>>();
            json!({
                "case_id": case.case_id,
                "assignments": assignment_rows,
                "expected_bindings": reference_bindings_json(case.expected),
                "actual_bindings": reference_bindings_json(&actual_bindings),
                "error": error,
                "matches_expected": matches_expected,
                "implementation_path": "src/oxcalc-tracecalc/src/diversity_seam.rs::eval_reference_model",
                "implementation_kind": "independent_reference_model_evaluator",
                "scope_limit": "acyclic named-reference models evaluated in declaration order with integer formulas, IF, comparisons, deterministic before/after edit checks, fan-in checksum rows, and mismatch-routing control rows",
                "independent_from": ["TraceCalc", "optimized_core", "TreeCalc", "OxFml", "OxFunc"]
            })
        })
        .collect()
}

#[allow(clippy::too_many_arguments)]
fn w043_source_rows(
    w043_obligation_summary: &Value,
    w043_obligation_map: &Value,
    w043_oxfml_intake: &Value,
    w042_diversity: &Value,
    w042_implementation: &Value,
    w042_independent: &Value,
    w042_cross_engine: &Value,
    w042_authority: &Value,
    w042_blockers: &Value,
    w042_decision: &Value,
    w043_stage2: &Value,
    w043_operated: &Value,
    w043_retained: &Value,
    w043_witness: &Value,
    w043_cross_engine_service: &Value,
    w043_alert_dispatch: &Value,
    w043_service_blockers: &Value,
    w043_operated_decision: &Value,
    w042_pack_decision: &Value,
) -> Vec<Value> {
    let w073 = w043_oxfml_intake
        .get("w073_formatting")
        .unwrap_or(&Value::Null);
    vec![
        json!({
            "row_id": "source.w043_proof_service_obligation_map",
            "artifact": W043_OBLIGATION_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w043_obligation_summary, "status") == "residual_release_grade_proof_service_obligation_map_validated"
                && number_at(w043_obligation_map, "obligation_count") == 36
                && bool_at(w043_obligation_summary, "oxfml_formatting_update_incorporated"),
            "semantic_state": "w043_diversity_obligations_bound_with_no_proxy_guard"
        }),
        json!({
            "row_id": "source.w043_w073_typed_formatting_guard",
            "artifact": W043_OXFML_INBOUND_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": w073.get("status").and_then(Value::as_str) == Some("typed_only_direct_replacement_guard_retained")
                && array_len_at(w073, "typed_rule_only_families") == 7
                && !bool_at(w073, "threshold_fallback_allowed_for_typed_families")
                && !bool_at(w073, "old_aggregate_visualization_option_strings_interpreted_as_typed_metadata"),
            "semantic_state": "w073_typed_rule_only_formatting_guard_retained_for_w043_diversity"
        }),
        json!({
            "row_id": "source.w043_oxfml_watch_lanes",
            "artifact": W043_OXFML_INBOUND_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w043_oxfml_intake, "status") == "reviewed_as_watch_and_handoff_trigger_inputs"
                && array_len_at(w043_oxfml_intake, "note_level_watch_lanes") == 7
                && !bool_at(w043_oxfml_intake, "handoff_required"),
            "semantic_state": "w043_oxfml_notes_are_watch_lanes_not_seam_freeze_text"
        }),
        json!({
            "row_id": "source.w042_diversity_predecessor",
            "artifact": W042_DIVERSITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w042_diversity, "failed_row_count"),
            "promotion_guard": !bool_at(w042_diversity, "fully_independent_evaluator_promoted")
                && !bool_at(w042_diversity, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w042_diversity, "mismatch_quarantine_service_promoted")
                && !bool_at(w042_decision, "fully_independent_evaluator_promoted")
                && number_at(w042_diversity, "exact_blocker_count") == 7,
            "semantic_state": "w042_diversity_predecessor_bound_without_promotion"
        }),
        json!({
            "row_id": "source.w042_reference_model_predecessor",
            "artifact": W042_INDEPENDENT_IMPLEMENTATION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w042_implementation, "failed_case_count"),
            "promotion_guard": number_at(w042_implementation, "case_count") == 4
                && number_at(w042_implementation, "match_count") == 4
                && !bool_at(w042_implementation, "fully_independent_evaluator_promoted"),
            "semantic_state": "w042_reference_model_extended_by_w043_cases"
        }),
        json!({
            "row_id": "source.w042_independent_breadth_register",
            "artifact": W042_INDEPENDENT_ROW_SET,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w042_independent, "row_count") == 11
                && !bool_at(w042_independent, "fully_independent_evaluator_promoted"),
            "semantic_state": "w042_independent_breadth_register_bound"
        }),
        json!({
            "row_id": "source.w042_cross_engine_differential_predecessor",
            "artifact": W042_CROSS_ENGINE_DIVERSITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w042_cross_engine, "row_count") == 10
                && !bool_at(w042_cross_engine, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w042_file_backed_cross_engine_rows_bound_without_operated_service"
        }),
        json!({
            "row_id": "source.w042_mismatch_authority_predecessor",
            "artifact": W042_MISMATCH_AUTHORITY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w042_authority, "row_count") == 10
                && !bool_at(w042_authority, "fully_independent_evaluator_promoted")
                && !bool_at(w042_authority, "operated_service_authority_promoted")
                && !bool_at(w042_authority, "mismatch_quarantine_service_promoted"),
            "semantic_state": "w042_mismatch_authority_bound_without_service_promotion"
        }),
        json!({
            "row_id": "source.w042_diversity_blockers",
            "artifact": W042_DIVERSITY_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w042_blockers, "exact_blocker_count") == 7,
            "semantic_state": "w042_exact_diversity_blockers_bound_for_w043"
        }),
        json!({
            "row_id": "source.w043_stage2_service_dependency",
            "artifact": W043_STAGE2_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w043_stage2, "failed_row_count"),
            "promotion_guard": !bool_at(w043_stage2, "stage2_policy_promoted")
                && !bool_at(w043_stage2, "stage2_promotion_candidate")
                && number_at(w043_stage2, "exact_remaining_blocker_count") == 6
                && number_at(w043_stage2, "policy_row_count") == 20,
            "semantic_state": "w043_stage2_service_dependency_bound_without_policy_promotion"
        }),
        json!({
            "row_id": "source.w043_operated_assurance_service_packet",
            "artifact": W043_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w043_operated, "failed_row_count"),
            "promotion_guard": bool_at(w043_operated, "file_backed_service_envelope_present")
                && bool_at(w043_operated, "retained_history_query_api_contract_present")
                && bool_at(w043_operated, "retained_witness_lifecycle_register_present")
                && !bool_at(w043_operated, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w043_file_backed_service_packet_bound_without_operated_service_promotion"
        }),
        json!({
            "row_id": "source.w043_retained_history_witness_support",
            "artifact": W043_RETAINED_HISTORY_QUERY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w043_retained, "query_register_row_count") == 13
                && number_at(w043_retained, "replay_correlation_row_count") == 11
                && bool_at(w043_witness, "retained_witness_lifecycle_register_present")
                && number_at(w043_witness, "pack_eligible_witness_count") == 0
                && !bool_at(w043_witness, "retained_witness_lifecycle_service_promoted"),
            "semantic_state": "w043_retained_history_and_witness_support_bound"
        }),
        json!({
            "row_id": "source.w043_cross_engine_service_blocker",
            "artifact": W043_CROSS_ENGINE_SERVICE_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w043_cross_engine_service, "file_backed_cross_engine_substrate_present")
                && !bool_at(w043_cross_engine_service, "operated_cross_engine_differential_service_present")
                && array_contains_string(
                    w043_cross_engine_service,
                    "blocked_service_claims",
                    "operated_mismatch_quarantine_dispatcher"
                ),
            "semantic_state": "w043_cross_engine_service_blocker_retained"
        }),
        json!({
            "row_id": "source.w043_alert_quarantine_contract",
            "artifact": W043_ALERT_DISPATCH_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w043_alert_dispatch, "evaluated_rule_count") == 29
                && number_at(w043_alert_dispatch, "quarantine_decision_count") == 0
                && number_at(w043_alert_dispatch, "alert_decision_count") == 0
                && !bool_at(w043_alert_dispatch, "external_alert_dispatcher_promoted")
                && !bool_at(w043_alert_dispatch, "quarantine_service_promoted"),
            "semantic_state": "w043_local_alert_quarantine_contract_clean"
        }),
        json!({
            "row_id": "source.w043_service_and_pack_blockers",
            "artifact": W043_OPERATED_SERVICE_BLOCKERS,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w043_service_blockers, "exact_service_blocker_count") == 6
                && text_at(w042_pack_decision, "decision_status") == "capability_not_promoted"
                && !bool_at(w042_pack_decision, "capability_promoted"),
            "semantic_state": "w043_service_and_pack_no_promotion_blockers_bound"
        }),
        json!({
            "row_id": "source.w043_diversity_no_proxy_promotion_guard",
            "artifact": W043_OPERATED_PROMOTION_DECISION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w042_decision, "fully_independent_evaluator_promoted")
                && !bool_at(w042_decision, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w043_operated_decision, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w043_operated_decision, "quarantine_service_promoted")
                && !bool_at(w043_stage2, "stage2_policy_promoted")
                && !bool_at(w042_pack_decision, "capability_promoted"),
            "semantic_state": "w043_diversity_stage2_pack_service_promotion_guard_clean"
        }),
        json!({
            "row_id": "source.w043_diversity_obligation_coverage",
            "artifact": W043_OBLIGATION_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": number_at(w043_obligation_map, "obligation_count") == 36
                && number_at(w043_obligation_map, "source_residual_lane_count") == 18,
            "semantic_state": "w043_obligations_023_026_diversity_service_spec_evolution_lanes_bound"
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w043_independent_evaluator_rows(
    w042_independent: &Value,
    w042_implementation: &Value,
    w042_diversity: &Value,
    w043_stage2: &Value,
    w043_operated: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "independent.w043_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "independence_state": "independent_reference_model_slice_present",
            "evidence": "w043_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "W043 broadens the independent slice with fan-in checksum and mismatch-routing control rows, but still does not cover full TreeCalc/OxCalc evaluator breadth."
        }),
        json!({
            "row_id": "independent.w042_reference_model_predecessor",
            "disposition_kind": "accepted_boundary",
            "independence_state": "predecessor_reference_model_slice_subsumed_by_w043_slice",
            "evidence": W042_INDEPENDENT_IMPLEMENTATION,
            "w042_case_count": number_at(w042_implementation, "case_count"),
            "w042_row_count": number_at(w042_independent, "row_count"),
            "w042_fully_independent_evaluator_promoted": bool_at(w042_diversity, "fully_independent_evaluator_promoted"),
            "promotion_consequence": "W042 reference-model evidence remains evidence lineage only."
        }),
        json!({
            "row_id": "independent.w043_fan_in_and_mismatch_control_rows",
            "disposition_kind": "accepted_boundary",
            "independence_state": "bounded_fan_in_and_mismatch_routing_controls_present",
            "evidence": "w043_independent_reference_model_implementation.json",
            "promotion_consequence": "The added W043 cases are deterministic control rows for later service comparisons, not broad evaluator authority."
        }),
        json!({
            "row_id": "independent.tracecalc_reference_oracle_boundary",
            "disposition_kind": "accepted_boundary",
            "independence_state": "tracecalc_is_correctness_oracle_not_independent_implementation",
            "evidence": "TraceCalc",
            "promotion_consequence": "TraceCalc remains the correctness oracle for covered behavior, not a diversity implementation."
        }),
        json!({
            "row_id": "independent.optimized_core_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "optimized_core_same_implementation_family",
            "evidence": "W043 optimized/core counterpart packet",
            "promotion_consequence": "Optimized/core evidence strengthens conformance but is not independent implementation authority."
        }),
        json!({
            "row_id": "independent.treecalc_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "treecalc_same_oxcalc_family",
            "evidence": "W043 TreeCalc counterpart evidence",
            "promotion_consequence": "TreeCalc evidence is core conformance evidence, not independent evaluator diversity."
        }),
        json!({
            "row_id": "independent.direct_oxfml_external_formula_slice",
            "disposition_kind": "accepted_external_slice",
            "independence_state": "external_formula_evaluator_not_oxcalc_coordinator",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "promotion_consequence": "OxFml formula evidence remains an external formula-evaluator slice, including narrow LET/LAMBDA seam evidence, not OxCalc coordinator breadth."
        }),
        json!({
            "row_id": "independent.proof_model_not_runtime_evaluator",
            "disposition_kind": "accepted_boundary",
            "independence_state": "proof_model_not_executable_runtime_evaluator",
            "evidence": "W043 Lean/TLA proof/model packet",
            "promotion_consequence": "Proof/model evidence supports specification and refinement, but does not supply runtime diversity."
        }),
        json!({
            "row_id": "independent.stage2_declared_profile_not_independent_runtime",
            "disposition_kind": "accepted_boundary",
            "independence_state": "stage2_policy_profiles_not_second_evaluator",
            "evidence": W043_STAGE2_SUMMARY,
            "policy_row_count": number_at(w043_stage2, "policy_row_count"),
            "stage2_policy_promoted": bool_at(w043_stage2, "stage2_policy_promoted"),
            "promotion_consequence": "Stage 2 profile evidence remains policy/replay evidence, not separate evaluator implementation authority."
        }),
        json!({
            "row_id": "independent.w043_service_packet_not_independent_runtime",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "service_packet_file_backed_not_independent_evaluator",
            "evidence": W043_OPERATED_ASSURANCE_SUMMARY,
            "service_readiness_blocked_count": number_at(w043_operated, "service_readiness_blocked_count"),
            "promotion_consequence": "Service evidence improves retention and authority routing but is not independent evaluator breadth."
        }),
        json!({
            "row_id": "independent.mismatch_quarantine_contract_not_independent_runtime",
            "disposition_kind": "service_contract_boundary",
            "independence_state": "mismatch_contract_not_second_evaluator",
            "evidence": W043_ALERT_DISPATCH_REGISTER,
            "promotion_consequence": "Mismatch/quarantine contract rows classify later service behavior but do not add evaluator implementation breadth."
        }),
        json!({
            "row_id": "independent.full_independent_evaluator_breadth_blocked",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "full_independent_implementation_breadth_absent",
            "evidence": W043_OBLIGATION_MAP,
            "promotion_consequence": "Full independent-evaluator diversity remains blocked beyond the bounded named-reference model slice."
        }),
        json!({
            "row_id": "independent.oxfml_callable_breadth_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "callable_and_publication_breadth_dependency_retained",
            "evidence": W043_OBLIGATION_MAP,
            "promotion_consequence": "LET/LAMBDA and callable-carrier interaction remains in scope for later breadth, with OxFunc peeking only where the seam threads through OxCalc/OxFml."
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w043_cross_engine_differential_rows(
    w042_cross_engine: &Value,
    w043_stage2: &Value,
    w043_operated: &Value,
    w043_retained: &Value,
    w043_witness: &Value,
    w043_cross_engine_service: &Value,
    w043_alert_dispatch: &Value,
    w043_oxfml_intake: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let w073 = w043_oxfml_intake
        .get("w073_formatting")
        .unwrap_or(&Value::Null);
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "cross_engine.w043_reference_model_differential",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_reference_model_rows",
            "evidence": "w043_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "Reference-model rows can feed later cross-engine service work but are not an operated service."
        }),
        json!({
            "row_id": "cross_engine.w042_file_backed_differential_predecessor",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_not_operated",
            "evidence": W042_CROSS_ENGINE_DIVERSITY,
            "w042_row_count": number_at(w042_cross_engine, "row_count"),
            "promotion_consequence": "W042 file-backed diversity remains evidence lineage, not service promotion."
        }),
        json!({
            "row_id": "cross_engine.w043_service_envelope_contract",
            "disposition_kind": "service_contract_boundary",
            "service_state": "file_backed_service_envelope_only",
            "evidence": W043_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w043_operated, "service_envelope_row_count"),
            "file_backed_service_envelope_present": bool_at(w043_operated, "file_backed_service_envelope_present"),
            "promotion_consequence": "The W043 service envelope records rows needed by a future operated service without claiming operation."
        }),
        json!({
            "row_id": "cross_engine.w043_retained_history_query_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_history_query_and_correlation_only",
            "evidence": W043_RETAINED_HISTORY_QUERY,
            "query_register_row_count": number_at(w043_retained, "query_register_row_count"),
            "replay_correlation_row_count": number_at(w043_retained, "replay_correlation_row_count"),
            "promotion_consequence": "Retained query/correlation artifacts support later differential service operation but are not themselves a service."
        }),
        json!({
            "row_id": "cross_engine.w043_retained_witness_lifecycle_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_witness_lifecycle_only",
            "evidence": W043_RETAINED_WITNESS_LIFECYCLE,
            "retained_witness_lifecycle_row_count": number_at(w043_witness, "witness_lifecycle_row_count"),
            "pack_eligible_witness_count": number_at(w043_witness, "pack_eligible_witness_count"),
            "promotion_consequence": "Retained-witness lifecycle rows support later differential and pack governance work but are not an operated lifecycle service."
        }),
        json!({
            "row_id": "cross_engine.w043_stage2_service_dependency",
            "disposition_kind": "service_dependency_blocked",
            "service_state": "blocked",
            "evidence": W043_STAGE2_SUMMARY,
            "stage2_policy_promoted": bool_at(w043_stage2, "stage2_policy_promoted"),
            "exact_remaining_blocker_count": number_at(w043_stage2, "exact_remaining_blocker_count"),
            "promotion_consequence": "Stage 2 service-dependent diversity remains blocked by operated service and pack-governance dependencies."
        }),
        json!({
            "row_id": "cross_engine.w043_cross_engine_service_register",
            "disposition_kind": "service_substrate_bound",
            "service_state": if bool_at(w043_cross_engine_service, "operated_cross_engine_differential_service_present") { "present" } else { "blocked" },
            "evidence": W043_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w043_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w043_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "The W043 service register records the operated-service blocker exactly."
        }),
        json!({
            "row_id": "cross_engine.w043_mismatch_triage_and_quarantine_contract",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W043_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w043_alert_dispatch, "evaluated_rule_count"),
            "external_alert_dispatcher_promoted": bool_at(w043_alert_dispatch, "external_alert_dispatcher_promoted"),
            "quarantine_service_promoted": bool_at(w043_alert_dispatch, "quarantine_service_promoted"),
            "promotion_consequence": "Local alert/quarantine rules are evaluated, but no external mismatch triage/quarantine service is operated."
        }),
        json!({
            "row_id": "cross_engine.w073_typed_only_formatting_guard",
            "disposition_kind": "accepted_boundary",
            "service_state": "observable_guard_retained",
            "evidence": W043_OXFML_INBOUND_INTAKE,
            "typed_rule_only_family_count": array_len_at(w073, "typed_rule_only_families"),
            "threshold_fallback_allowed_for_typed_families": bool_at(w073, "threshold_fallback_allowed_for_typed_families"),
            "old_aggregate_visualization_option_strings_interpreted_as_typed_metadata": bool_at(w073, "old_aggregate_visualization_option_strings_interpreted_as_typed_metadata"),
            "promotion_consequence": "Diversity comparisons must preserve the W073 typed_rule-only aggregate/visualization formatting guard."
        }),
        json!({
            "row_id": "cross_engine.w043_spec_evolution_mismatch_route",
            "disposition_kind": "service_contract_boundary",
            "service_state": "spec_evolution_route_declared_not_operated",
            "evidence": W043_OBLIGATION_MAP,
            "promotion_consequence": "Mismatch handling can route to spec correction or implementation fault only after operated differential and quarantine evidence exists."
        }),
        json!({
            "row_id": "cross_engine.pack_governance_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W042_PACK_DECISION,
            "promotion_consequence": "Pack/C5 and release-grade diversity remain unavailable while operated differential service, mismatch quarantine, and independent breadth are blocked."
        }),
        json!({
            "row_id": "cross_engine.public_migration_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W043_OXFML_INBOUND_INTAKE,
            "promotion_consequence": "Public migration and broad OxFml display/publication coverage remain successor dependencies and cannot be inferred from diversity rows."
        }),
    ]
}

fn w043_mismatch_authority_rows(
    w042_authority: &Value,
    w043_operated: &Value,
    w043_cross_engine_service: &Value,
    w043_alert_dispatch: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "authority.w043_independent_reference_model_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "authority_state": "independent_reference_model_runtime",
            "evidence": "w043_independent_reference_model_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "authority_limit": "named-reference model slice only; no scheduling, publication, effects, callable-carrier, broad OxFml seam, or full OxFunc kernel authority"
        }),
        json!({
            "row_id": "authority.w042_reference_model_predecessor",
            "disposition_kind": "accepted_boundary",
            "authority_state": "predecessor_reference_model_runtime",
            "evidence": W042_MISMATCH_AUTHORITY,
            "w042_authority_row_count": number_at(w042_authority, "row_count"),
            "authority_limit": "predecessor authority remains a narrower reference-model slice"
        }),
        json!({
            "row_id": "authority.tracecalc_correctness_oracle",
            "disposition_kind": "accepted_boundary",
            "authority_state": "reference_oracle",
            "evidence": "TraceCalc",
            "authority_limit": "oracle authority for covered observable behavior only"
        }),
        json!({
            "row_id": "authority.optimized_core_same_implementation_family",
            "disposition_kind": "same_authority_family",
            "authority_state": "not_independent",
            "evidence": "W043 optimized/core counterpart packet",
            "authority_limit": "optimized/core conformance does not supply separate implementation authority"
        }),
        json!({
            "row_id": "authority.direct_oxfml_external_formula_evaluator",
            "disposition_kind": "accepted_external_slice",
            "authority_state": "external_formula_evaluator_only",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "authority_limit": "external evaluator authority is limited to the consumed OxFml seam and does not implement OxCalc scheduling/publication"
        }),
        json!({
            "row_id": "authority.service_envelope_file_backed_authority",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "file_backed_artifact_authority",
            "evidence": W043_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w043_operated, "service_envelope_row_count"),
            "authority_limit": "file-backed row agreement and retained artifacts cannot substitute for an operated differential service"
        }),
        json!({
            "row_id": "authority.mismatch_router_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_router_contract_not_service",
            "evidence": W043_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w043_alert_dispatch, "evaluated_rule_count"),
            "authority_limit": "local alert dispatch classification is a contract row set, not an external quarantine service"
        }),
        json!({
            "row_id": "authority.spec_evolution_route",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "spec_evolution_route_not_service",
            "evidence": W043_OBLIGATION_MAP,
            "authority_limit": "agreement, mismatch, implementation-fault, and spec-correction routes remain declared classification until operated service evidence exists"
        }),
        json!({
            "row_id": "authority.operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_operated_service",
            "evidence": W043_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w043_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "authority_limit": "no recurring service endpoint, retained service history, or service-level mismatch action path exists"
        }),
        json!({
            "row_id": "authority.mismatch_quarantine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_quarantine_service",
            "evidence": W043_ALERT_DISPATCH_REGISTER,
            "quarantine_service_promoted": bool_at(w043_alert_dispatch, "quarantine_service_promoted"),
            "authority_limit": "local clean dispatch rows cannot stand in for an enforcing mismatch quarantine service"
        }),
        json!({
            "row_id": "authority.full_independent_evaluator_breadth",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_beyond_reference_model_slice",
            "evidence": "w043_independent_reference_model_implementation.json",
            "authority_limit": "reference model slice does not cover TreeCalc scheduling, references outside declared order, publication, effects, broad OxFml seam breadth, or callable metadata"
        }),
        json!({
            "row_id": "authority.release_grade_diversity_promotion",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_release_grade_authority",
            "evidence": W043_OBLIGATION_MAP,
            "exact_service_blocker_count": number_at(w043_operated, "exact_service_blocker_count"),
            "authority_limit": "release-grade diversity authority remains unavailable while full independent implementation and operated service blockers remain"
        }),
    ]
}

fn w043_exact_blockers(
    w042_blockers: &Value,
    w043_stage2_blockers: &Value,
    w043_service_blockers: &Value,
    w043_cross_engine_service: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "blocker_id": "w043_diversity.full_independent_evaluator_breadth_absent",
            "owner": "calc-2p3.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": "w043_independent_reference_model_implementation.json",
            "predecessor_blocker_present": row_with_field_exists(
                w042_blockers,
                "blocker_id",
                "w042_diversity.full_independent_evaluator_breadth_absent"
            ),
            "reason": "W043 extends the independent named-reference model evaluator with fan-in and mismatch-routing controls, but full TreeCalc/OxCalc evaluator breadth remains absent.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "w043_diversity.operated_cross_engine_differential_service_absent",
            "owner": "calc-2p3.6; calc-2p3.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W043_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w043_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w043_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "reason": "W043 service artifacts are file-backed and do not operate a recurring cross-engine differential service.",
            "promotion_consequence": "operated cross-engine diversity service remains unpromoted"
        }),
        json!({
            "blocker_id": "w043_diversity.mismatch_quarantine_service_absent",
            "owner": "calc-2p3.6; calc-2p3.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W043_OPERATED_SERVICE_BLOCKERS,
            "service_blocker_rows": row_count_at(w043_service_blockers),
            "reason": "Local alert/quarantine rows are evaluated, but no external mismatch triage/quarantine service is operated.",
            "promotion_consequence": "service-level diversity assurance and mismatch quarantine remain unpromoted"
        }),
        json!({
            "blocker_id": "w043_diversity.stage2_operated_differential_dependency_absent",
            "owner": "calc-2p3.5; calc-2p3.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W043_STAGE2_BLOCKERS,
            "stage2_blocker_rows": row_count_at(w043_stage2_blockers),
            "reason": "Stage 2 diversity still needs production partition-analyzer soundness, scheduler equivalence, pack governance, and operated differential service evidence before strategy-level diversity promotion.",
            "promotion_consequence": "Stage 2 diversity remains a blocked dependency for release-grade promotion"
        }),
        json!({
            "blocker_id": "w043_diversity.retained_witness_attachment_service_absent",
            "owner": "calc-2p3.6; calc-2p3.7; calc-2p3.9",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W043_OPERATED_SERVICE_BLOCKERS,
            "reason": "Retained-witness lifecycle rows exist, but no service attaches mismatch witnesses to quarantine, pack replay, or retained-history lifecycle operation.",
            "promotion_consequence": "mismatch quarantine, pack-grade replay, C5, and release-grade diversity remain blocked"
        }),
        json!({
            "blocker_id": "w043_diversity.oxfml_callable_breadth_dependency_absent",
            "owner": "calc-2p3.7; calc-2p3.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W043_OBLIGATION_MAP,
            "reason": "The independent reference-model slice does not settle broad OxFml display publication, callable-carrier sufficiency, registered-external callable projection, or LET/LAMBDA threading through OxFunc-adjacent behavior.",
            "promotion_consequence": "broad OxFml callable/display publication remains a successor dependency"
        }),
        json!({
            "blocker_id": "w043_diversity.pack_grade_replay_governance_dependency_absent",
            "owner": "calc-2p3.7; calc-2p3.9",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W042_PACK_DECISION,
            "reason": "Pack-grade replay governance cannot be inferred from local retained-history, retained-witness, file-backed differential, or bounded mismatch-control rows.",
            "promotion_consequence": "pack-grade replay, C5, and release-grade verification remain unpromoted"
        }),
        json!({
            "blocker_id": "w043_diversity.release_grade_promotion_authority_absent",
            "owner": "calc-2p3.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W043_OBLIGATION_MAP,
            "reason": "Release-grade diversity cannot be claimed from bounded reference-model rows, proxy evidence, file-backed rows, shared projections, or retained blockers.",
            "promotion_consequence": "release-grade verification, pack/C5, and diversity promotion remain unavailable to this bead"
        }),
    ]
}

fn w043_diversity_validation_failures(
    implementation_rows: &[Value],
    independent_rows: &[Value],
    cross_engine_rows: &[Value],
    authority_rows: &[Value],
    blockers: &[Value],
) -> Vec<String> {
    let mut failures = Vec::new();
    if implementation_rows.len() < 6
        || implementation_rows
            .iter()
            .any(|row| !bool_at(row, "matches_expected"))
    {
        failures.push("w043_diversity.independent_reference_model_case_mismatch".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("independent_reference_model_slice_present")
    }) {
        failures.push("w043_diversity.reference_model_row_missing".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("full_independent_implementation_breadth_absent")
    }) {
        failures.push("w043_diversity.full_breadth_blocker_row_missing".to_string());
    }
    if !cross_engine_rows
        .iter()
        .any(|row| row.get("service_state").and_then(Value::as_str) == Some("blocked"))
    {
        failures.push("w043_diversity.blocked_cross_engine_service_row_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_operated_service")
    }) {
        failures.push("w043_diversity.operated_service_authority_blocker_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_quarantine_service")
    }) {
        failures.push("w043_diversity.mismatch_quarantine_authority_blocker_missing".to_string());
    }
    if blockers.len() < 8 {
        failures.push("w043_diversity.exact_blocker_count_too_low".to_string());
    }
    if independent_rows
        .iter()
        .chain(cross_engine_rows.iter())
        .chain(authority_rows.iter())
        .any(|row| {
            bool_at(row, "fully_independent_evaluator_promoted")
                || bool_at(row, "operated_cross_engine_differential_service_promoted")
                || bool_at(row, "mismatch_quarantine_service_promoted")
        })
    {
        failures.push("w043_diversity.unexpected_promotion_row".to_string());
    }
    failures
}

#[allow(clippy::too_many_arguments)]
fn w041_source_rows(
    w041_obligation_summary: &Value,
    w041_obligation_map: &Value,
    w040_formatting_intake: &Value,
    w040_diversity: &Value,
    w040_implementation: &Value,
    w040_decision: &Value,
    w041_conformance: &Value,
    w041_treecalc: &Value,
    w041_rust: &Value,
    w041_lean_tla: &Value,
    w041_stage2: &Value,
    w041_operated: &Value,
    w041_cross_engine_service: &Value,
    w041_alert_dispatch: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "source.w041_successor_obligation_map",
            "artifact": W041_OBLIGATION_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w041_obligation_summary, "status") == "residual_successor_obligation_map_validated"
                && number_at(w041_obligation_map, "obligation_count") == 28
                && bool_at(w041_obligation_summary, "oxfml_formatting_update_incorporated"),
            "semantic_state": "w041_successor_obligations_bound_with_formatting_guard"
        }),
        json!({
            "row_id": "source.w040_diversity_predecessor",
            "artifact": W040_DIVERSITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w040_diversity, "failed_row_count"),
            "promotion_guard": !bool_at(w040_diversity, "fully_independent_evaluator_promoted")
                && !bool_at(w040_diversity, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w040_decision, "fully_independent_evaluator_promoted"),
            "semantic_state": "w040_diversity_predecessor_bound_without_promotion"
        }),
        json!({
            "row_id": "source.w040_independent_scalar_predecessor",
            "artifact": W040_INDEPENDENT_IMPLEMENTATION,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w040_implementation, "failed_case_count"),
            "promotion_guard": number_at(w040_implementation, "case_count") == 5
                && number_at(w040_implementation, "match_count") == 5
                && !bool_at(w040_implementation, "fully_independent_evaluator_promoted"),
            "semantic_state": "w040_bounded_scalar_slice_narrowed_by_w041_formula_fragment"
        }),
        json!({
            "row_id": "source.w041_optimized_core_conformance",
            "artifact": W041_CONFORMANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": number_at(w041_conformance, "treecalc_expectation_mismatch_count"),
            "failed_row_count": number_at(w041_conformance, "failed_row_count"),
            "promotion_guard": number_at(w041_conformance, "match_promoted_count") == 0
                && number_at(w041_conformance, "exact_remaining_blocker_count") == 3,
            "semantic_state": "w041_optimized_core_conformance_bound_without_full_verification"
        }),
        json!({
            "row_id": "source.w041_treecalc_dynamic_transition",
            "artifact": W041_TREECALC_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": number_at(w041_treecalc, "expectation_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": number_at(w041_treecalc, "case_count") == 26
                && number_at(w041_treecalc, "expectation_mismatch_count") == 0,
            "semantic_state": "w041_treecalc_dynamic_transition_bound"
        }),
        json!({
            "row_id": "source.w041_rust_refinement",
            "artifact": W041_RUST_TOTALITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w041_rust, "failed_row_count"),
            "promotion_guard": !bool_at_path(w041_rust, &["promotion_claims", "rust_engine_totality_promoted"])
                && !bool_at_path(w041_rust, &["promotion_claims", "rust_refinement_promoted"]),
            "semantic_state": "w041_rust_totality_refinement_bound_without_full_proof_promotion"
        }),
        json!({
            "row_id": "source.w041_lean_tla_boundaries",
            "artifact": W041_LEAN_TLA_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w041_lean_tla, "failed_row_count"),
            "promotion_guard": !bool_at_path(w041_lean_tla, &["promotion_claims", "full_lean_verification_promoted"])
                && !bool_at_path(w041_lean_tla, &["promotion_claims", "full_tla_verification_promoted"]),
            "semantic_state": "w041_lean_tla_boundaries_bound_without_full_model_promotion"
        }),
        json!({
            "row_id": "source.w041_stage2_service_dependency",
            "artifact": W041_STAGE2_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w041_stage2, "failed_row_count"),
            "promotion_guard": !bool_at(w041_stage2, "stage2_policy_promoted")
                && !bool_at(w041_stage2, "stage2_promotion_candidate")
                && number_at(w041_stage2, "exact_remaining_blocker_count") == 4,
            "semantic_state": "w041_stage2_service_dependency_bound_without_policy_promotion"
        }),
        json!({
            "row_id": "source.w041_operated_assurance_service_envelope",
            "artifact": W041_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w041_operated, "failed_row_count"),
            "promotion_guard": bool_at(w041_operated, "file_backed_service_envelope_present")
                && bool_at(w041_operated, "retained_history_query_api_contract_present")
                && !bool_at(w041_operated, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w041_file_backed_service_envelope_bound_without_operated_service_promotion"
        }),
        json!({
            "row_id": "source.w041_cross_engine_service_blocker",
            "artifact": W041_CROSS_ENGINE_SERVICE_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": bool_at(w041_cross_engine_service, "file_backed_cross_engine_substrate_present")
                && !bool_at(w041_cross_engine_service, "operated_cross_engine_differential_service_present")
                && array_contains_string(
                    w041_cross_engine_service,
                    "blocked_service_claims",
                    "recurring_cross_engine_diff_scheduler"
                ),
            "semantic_state": "w041_cross_engine_service_blocker_retained"
        }),
        json!({
            "row_id": "source.w041_w073_typed_formatting_guard",
            "artifact": W040_FORMATTING_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w040_formatting_intake, "contract_mode") == "direct_replacement_for_aggregate_and_visualization_metadata"
                && array_len_at(w040_formatting_intake, "typed_rule_only_families") == 7
                && !bool_at(w040_formatting_intake, "w072_threshold_fallback_allowed_for_aggregate_visualization"),
            "semantic_state": "w073_typed_rule_only_formatting_guard_retained_for_diversity"
        }),
        json!({
            "row_id": "source.w041_diversity_promotion_guard",
            "artifact": W041_ALERT_DISPATCH_REGISTER,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w040_decision, "fully_independent_evaluator_promoted")
                && !bool_at(w040_decision, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w041_stage2, "stage2_policy_promoted")
                && !bool_at(w041_operated, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w041_alert_dispatch, "external_alert_dispatcher_promoted")
                && !bool_at(w041_alert_dispatch, "quarantine_service_promoted"),
            "semantic_state": "diversity_stage2_pack_service_promotion_guard_clean"
        }),
    ]
}

#[allow(clippy::too_many_arguments)]
fn w041_independent_evaluator_rows(
    w040_independent: &Value,
    w040_diversity: &Value,
    w041_conformance: &Value,
    w041_treecalc: &Value,
    w041_rust: &Value,
    w041_lean_tla: &Value,
    w041_stage2: &Value,
    w041_operated: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "independent.w041_broadened_formula_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "independence_state": "broadened_independent_formula_fragment_present",
            "evidence": "w041_independent_formula_evaluator_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "W041 broadens the independent slice beyond W040 scalar arithmetic, but still does not cover full TreeCalc/OxCalc evaluator breadth."
        }),
        json!({
            "row_id": "independent.w040_bounded_scalar_predecessor_narrowed",
            "disposition_kind": "accepted_boundary",
            "independence_state": "predecessor_bounded_scalar_slice_subsumed_by_formula_fragment",
            "evidence": W040_INDEPENDENT_ROW_SET,
            "w040_row_count": number_at(w040_independent, "row_count"),
            "w040_fully_independent_evaluator_promoted": bool_at(w040_diversity, "fully_independent_evaluator_promoted"),
            "promotion_consequence": "W040 bounded scalar evidence remains evidence lineage only."
        }),
        json!({
            "row_id": "independent.tracecalc_reference_oracle_boundary",
            "disposition_kind": "accepted_boundary",
            "independence_state": "tracecalc_reference_oracle_not_independent_runtime",
            "evidence": W041_TREECALC_SUMMARY,
            "treecalc_case_count": number_at(w041_treecalc, "case_count"),
            "promotion_consequence": "TraceCalc-backed expectations remain oracle/reference evidence, not a second full evaluator."
        }),
        json!({
            "row_id": "independent.optimized_core_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "optimized_core_conformance_same_authority_family",
            "evidence": W041_CONFORMANCE_SUMMARY,
            "direct_evidence_bound_count": number_at(w041_conformance, "direct_evidence_bound_count"),
            "exact_remaining_blocker_count": number_at(w041_conformance, "exact_remaining_blocker_count"),
            "promotion_consequence": "Optimized/core evidence strengthens conformance but is not independent implementation authority."
        }),
        json!({
            "row_id": "independent.treecalc_dynamic_transition_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "treecalc_dynamic_transition_same_oxcalc_family",
            "evidence": W041_TREECALC_SUMMARY,
            "expectation_mismatch_count": number_at(w041_treecalc, "expectation_mismatch_count"),
            "promotion_consequence": "TreeCalc dynamic-transition evidence is core conformance evidence, not independent evaluator diversity."
        }),
        json!({
            "row_id": "independent.direct_oxfml_external_formula_slice",
            "disposition_kind": "accepted_external_slice",
            "independence_state": "external_formula_evaluator_not_oxcalc_coordinator",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "promotion_consequence": "OxFml formula evidence remains an external formula-evaluator slice, including narrow LET/LAMBDA seam evidence, not OxCalc coordinator breadth."
        }),
        json!({
            "row_id": "independent.proof_model_not_runtime_evaluator",
            "disposition_kind": "accepted_boundary",
            "independence_state": "proof_model_not_executable_runtime_evaluator",
            "evidence": W041_LEAN_TLA_SUMMARY,
            "lean_local_proof_row_count": number_at(w041_lean_tla, "local_proof_row_count"),
            "rust_refinement_row_count": number_at(w041_rust, "refinement_row_count"),
            "promotion_consequence": "Proof/model evidence supports specification and refinement, but does not supply runtime diversity."
        }),
        json!({
            "row_id": "independent.stage2_declared_profile_not_independent_runtime",
            "disposition_kind": "accepted_boundary",
            "independence_state": "stage2_policy_profiles_not_second_evaluator",
            "evidence": W041_STAGE2_SUMMARY,
            "policy_row_count": number_at(w041_stage2, "policy_row_count"),
            "stage2_policy_promoted": bool_at(w041_stage2, "stage2_policy_promoted"),
            "promotion_consequence": "Stage 2 profile evidence remains policy/replay evidence, not separate evaluator implementation authority."
        }),
        json!({
            "row_id": "independent.full_independent_evaluator_breadth_blocked",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "full_independent_implementation_breadth_absent",
            "evidence": W041_OBLIGATION_MAP,
            "service_readiness_blocked_count": number_at(w041_operated, "service_readiness_blocked_count"),
            "promotion_consequence": "Full independent-evaluator diversity remains blocked beyond the broadened formula-fragment slice."
        }),
        json!({
            "row_id": "independent.oxfml_callable_breadth_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "callable_and_publication_breadth_dependency_retained",
            "evidence": W041_OBLIGATION_MAP,
            "promotion_consequence": "LET/LAMBDA and callable-carrier interaction remains in scope for later breadth, with OxFunc peeking only where the seam threads through OxCalc/OxFml."
        }),
    ]
}

fn w041_cross_engine_differential_rows(
    w040_cross_engine: &Value,
    w041_stage2: &Value,
    w041_operated: &Value,
    w041_retained: &Value,
    w041_cross_engine_service: &Value,
    w041_alert_dispatch: &Value,
    w040_formatting_intake: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "cross_engine.w041_broadened_formula_differential",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_broadened_formula_rows",
            "evidence": "w041_independent_formula_evaluator_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "promotion_consequence": "Broadened formula rows can feed later cross-engine service work but are not an operated service."
        }),
        json!({
            "row_id": "cross_engine.w040_file_backed_differential_predecessor",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_not_operated",
            "evidence": W040_CROSS_ENGINE_DIVERSITY,
            "w040_row_count": number_at(w040_cross_engine, "row_count"),
            "promotion_consequence": "W040 file-backed diversity remains evidence lineage, not service promotion."
        }),
        json!({
            "row_id": "cross_engine.w041_service_envelope_contract",
            "disposition_kind": "service_contract_boundary",
            "service_state": "file_backed_service_envelope_only",
            "evidence": W041_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w041_operated, "service_envelope_row_count"),
            "file_backed_service_envelope_present": bool_at(w041_operated, "file_backed_service_envelope_present"),
            "promotion_consequence": "The service envelope records rows needed by a future operated service without claiming operation."
        }),
        json!({
            "row_id": "cross_engine.w041_retained_history_query_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_history_query_and_correlation_only",
            "evidence": W041_RETAINED_HISTORY_QUERY,
            "query_register_row_count": number_at(w041_retained, "query_register_row_count"),
            "replay_correlation_row_count": number_at(w041_retained, "replay_correlation_row_count"),
            "promotion_consequence": "Retained query/correlation artifacts support later differential service operation but are not themselves a service."
        }),
        json!({
            "row_id": "cross_engine.w041_stage2_service_dependency",
            "disposition_kind": "service_dependency_blocked",
            "service_state": "blocked",
            "evidence": W041_STAGE2_SUMMARY,
            "stage2_policy_promoted": bool_at(w041_stage2, "stage2_policy_promoted"),
            "exact_remaining_blocker_count": number_at(w041_stage2, "exact_remaining_blocker_count"),
            "promotion_consequence": "Stage 2 service-dependent diversity remains blocked by operated service and pack-governance dependencies."
        }),
        json!({
            "row_id": "cross_engine.w041_cross_engine_service_register",
            "disposition_kind": "service_substrate_bound",
            "service_state": if bool_at(w041_cross_engine_service, "operated_cross_engine_differential_service_present") { "present" } else { "blocked" },
            "evidence": W041_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w041_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w041_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "The W041 service register records the operated-service blocker exactly."
        }),
        json!({
            "row_id": "cross_engine.w041_mismatch_triage_and_quarantine_contract",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W041_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w041_alert_dispatch, "evaluated_rule_count"),
            "external_alert_dispatcher_promoted": bool_at(w041_alert_dispatch, "external_alert_dispatcher_promoted"),
            "quarantine_service_promoted": bool_at(w041_alert_dispatch, "quarantine_service_promoted"),
            "promotion_consequence": "Local alert/quarantine rules are evaluated, but no external mismatch triage/quarantine service is operated."
        }),
        json!({
            "row_id": "cross_engine.w073_typed_only_formatting_guard",
            "disposition_kind": "accepted_boundary",
            "service_state": "observable_guard_retained",
            "evidence": W040_FORMATTING_INTAKE,
            "typed_rule_only_family_count": array_len_at(w040_formatting_intake, "typed_rule_only_families"),
            "w072_threshold_fallback_allowed": bool_at(w040_formatting_intake, "w072_threshold_fallback_allowed_for_aggregate_visualization"),
            "promotion_consequence": "Diversity comparisons must preserve the W073 typed_rule-only aggregate/visualization formatting guard."
        }),
        json!({
            "row_id": "cross_engine.pack_governance_dependency",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W041_OBLIGATION_MAP,
            "promotion_consequence": "Pack/C5 and release-grade diversity remain unavailable while operated differential service and independent breadth are blocked."
        }),
    ]
}

fn w041_mismatch_authority_rows(
    w040_authority: &Value,
    w041_conformance: &Value,
    w041_operated: &Value,
    w041_cross_engine_service: &Value,
    w041_alert_dispatch: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    let match_count = implementation_rows
        .iter()
        .filter(|row| bool_at(row, "matches_expected"))
        .count();
    vec![
        json!({
            "row_id": "authority.w041_broadened_independent_formula_evaluator",
            "disposition_kind": "broadened_independent_implementation",
            "authority_state": "broadened_independent_formula_fragment_runtime",
            "evidence": "w041_independent_formula_evaluator_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": match_count,
            "authority_limit": "formula fragment only; no scheduling, publication, effect, callable-carrier, or full OxFml/OxFunc kernel authority"
        }),
        json!({
            "row_id": "authority.tracecalc_correctness_oracle",
            "disposition_kind": "accepted_boundary",
            "authority_state": "reference_oracle",
            "evidence": W041_TREECALC_SUMMARY,
            "authority_limit": "oracle authority for covered observable behavior only"
        }),
        json!({
            "row_id": "authority.optimized_core_same_implementation_family",
            "disposition_kind": "same_authority_family",
            "authority_state": "not_independent",
            "evidence": W041_CONFORMANCE_SUMMARY,
            "exact_remaining_blocker_count": number_at(w041_conformance, "exact_remaining_blocker_count"),
            "authority_limit": "optimized/core conformance does not supply separate implementation authority"
        }),
        json!({
            "row_id": "authority.direct_oxfml_external_formula_evaluator",
            "disposition_kind": "accepted_external_slice",
            "authority_state": "external_formula_evaluator_only",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "authority_limit": "external evaluator authority is limited to the consumed OxFml seam and does not implement OxCalc scheduling/publication"
        }),
        json!({
            "row_id": "authority.service_envelope_file_backed_authority",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "file_backed_artifact_authority",
            "evidence": W041_OPERATED_ASSURANCE_SUMMARY,
            "service_envelope_row_count": number_at(w041_operated, "service_envelope_row_count"),
            "authority_limit": "file-backed row agreement and retained artifacts cannot substitute for an operated differential service"
        }),
        json!({
            "row_id": "authority.mismatch_router_contract",
            "disposition_kind": "service_contract_boundary",
            "authority_state": "local_router_contract_not_service",
            "evidence": W041_ALERT_DISPATCH_REGISTER,
            "evaluated_rule_count": number_at(w041_alert_dispatch, "evaluated_rule_count"),
            "authority_limit": "local alert dispatch classification is a contract row set, not an external quarantine service"
        }),
        json!({
            "row_id": "authority.operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_operated_service",
            "evidence": W041_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w041_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "authority_limit": "no recurring service endpoint, retained service history, or service-level mismatch action path exists"
        }),
        json!({
            "row_id": "authority.full_independent_evaluator_breadth",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_beyond_formula_fragment",
            "evidence": "w041_independent_formula_evaluator_implementation.json",
            "authority_limit": "broadened formula fragment does not cover TreeCalc scheduling, references, publication, effects, full OxFml seam breadth, or callable metadata"
        }),
        json!({
            "row_id": "authority.release_grade_diversity_promotion",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_release_grade_authority",
            "evidence": W040_DIFFERENTIAL_AUTHORITY,
            "w040_authority_row_count": number_at(w040_authority, "row_count"),
            "exact_service_blocker_count": number_at(w041_operated, "exact_service_blocker_count"),
            "authority_limit": "release-grade diversity authority remains unavailable while full independent implementation and operated service blockers remain"
        }),
    ]
}

fn w041_exact_blockers(
    w040_blockers: &Value,
    w041_stage2_blockers: &Value,
    w041_service_blockers: &Value,
    w041_cross_engine_service: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "blocker_id": "w041_diversity.full_independent_evaluator_breadth_absent",
            "owner": "calc-sui.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": "w041_independent_formula_evaluator_implementation.json",
            "predecessor_blocker_present": row_with_field_exists(
                w040_blockers,
                "blocker_id",
                "w040_diversity.full_independent_evaluator_breadth_absent"
            ),
            "reason": "W041 introduces a broadened independent formula-fragment evaluator, but full TreeCalc/OxCalc evaluator breadth remains absent.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "w041_diversity.operated_cross_engine_differential_service_absent",
            "owner": "calc-sui.6; calc-sui.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W041_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w041_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w041_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "reason": "W041 service artifacts are file-backed and do not operate a recurring cross-engine differential service.",
            "promotion_consequence": "operated cross-engine diversity service remains unpromoted"
        }),
        json!({
            "blocker_id": "w041_diversity.mismatch_triage_and_quarantine_service_absent",
            "owner": "calc-sui.6; calc-sui.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W041_OPERATED_SERVICE_BLOCKERS,
            "service_blocker_rows": row_count_at(w041_service_blockers),
            "reason": "Local alert/quarantine rows are evaluated, but no external mismatch triage/quarantine service is operated.",
            "promotion_consequence": "service-level diversity assurance remains unpromoted"
        }),
        json!({
            "blocker_id": "w041_diversity.stage2_operated_differential_dependency_absent",
            "owner": "calc-sui.5; calc-sui.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W041_STAGE2_BLOCKERS,
            "stage2_blocker_rows": row_count_at(w041_stage2_blockers),
            "reason": "Stage 2 diversity still needs production policy, partition soundness, pack governance, and operated differential service evidence before strategy-level diversity promotion.",
            "promotion_consequence": "Stage 2 diversity remains a blocked dependency for release-grade promotion"
        }),
        json!({
            "blocker_id": "w041_diversity.oxfml_callable_breadth_dependency_absent",
            "owner": "calc-sui.7; calc-sui.8",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W041_OBLIGATION_MAP,
            "reason": "The independent fragment covers a formula slice, but does not settle broad OxFml display publication, callable-carrier sufficiency, or LET/LAMBDA threading through OxFunc-adjacent behavior.",
            "promotion_consequence": "broad OxFml callable/display publication remains a successor dependency"
        }),
        json!({
            "blocker_id": "w041_diversity.release_grade_promotion_authority_absent",
            "owner": "calc-sui.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W041_OBLIGATION_MAP,
            "reason": "Release-grade diversity cannot be claimed from bounded formula rows, proxy evidence, file-backed rows, shared projections, or retained blockers.",
            "promotion_consequence": "release-grade verification, pack/C5, and diversity promotion remain unavailable to this bead"
        }),
    ]
}

fn w041_diversity_validation_failures(
    implementation_rows: &[Value],
    independent_rows: &[Value],
    cross_engine_rows: &[Value],
    authority_rows: &[Value],
    blockers: &[Value],
) -> Vec<String> {
    let mut failures = Vec::new();
    if implementation_rows.len() < 8
        || implementation_rows
            .iter()
            .any(|row| !bool_at(row, "matches_expected"))
    {
        failures.push("w041_diversity.independent_formula_case_mismatch".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("broadened_independent_formula_fragment_present")
    }) {
        failures.push("w041_diversity.broadened_formula_row_missing".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("full_independent_implementation_breadth_absent")
    }) {
        failures.push("w041_diversity.full_breadth_blocker_row_missing".to_string());
    }
    if !cross_engine_rows
        .iter()
        .any(|row| row.get("service_state").and_then(Value::as_str) == Some("blocked"))
    {
        failures.push("w041_diversity.blocked_cross_engine_service_row_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_operated_service")
    }) {
        failures.push("w041_diversity.operated_service_authority_blocker_missing".to_string());
    }
    if blockers.len() < 6 {
        failures.push("w041_diversity.exact_blocker_count_below_gate".to_string());
    }
    if independent_rows
        .iter()
        .chain(cross_engine_rows.iter())
        .chain(authority_rows.iter())
        .any(|row| row.get("disposition_kind").and_then(Value::as_str) == Some("promoted"))
    {
        failures.push("w041_diversity.unexpected_promotion_row".to_string());
    }
    failures
}

#[allow(clippy::too_many_arguments)]
fn w040_source_rows(
    w040_direct_summary: &Value,
    w040_direct_map: &Value,
    w040_formatting_intake: &Value,
    w039_diversity: &Value,
    w039_decision: &Value,
    w040_conformance: &Value,
    w040_treecalc: &Value,
    w040_rust: &Value,
    w040_lean_tla: &Value,
    w040_stage2: &Value,
    w040_operated: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "source.w040_direct_obligation_map",
            "artifact": W040_DIRECT_OBLIGATION_MAP,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w040_direct_summary, "status") == "direct_verification_obligation_map_validated"
                && number_at(w040_direct_map, "obligation_count") == 23,
            "semantic_state": "w040_diversity_obligations_bound"
        }),
        json!({
            "row_id": "source.w040_w073_typed_formatting_guard",
            "artifact": W040_FORMATTING_INTAKE,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": text_at(w040_formatting_intake, "contract_mode") == "direct_replacement_for_aggregate_and_visualization_metadata"
                && !bool_at(w040_formatting_intake, "w072_threshold_fallback_allowed_for_aggregate_visualization"),
            "semantic_state": "w073_typed_only_formatting_guard_bound"
        }),
        json!({
            "row_id": "source.w039_diversity_predecessor",
            "artifact": W039_DIVERSITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w039_diversity, "failed_row_count"),
            "promotion_guard": !bool_at(w039_decision, "fully_independent_evaluator_promoted")
                && !bool_at(w039_decision, "operated_cross_engine_differential_service_promoted"),
            "semantic_state": "w039_diversity_predecessor_bound_without_promotion"
        }),
        json!({
            "row_id": "source.w040_optimized_core_conformance",
            "artifact": W040_CONFORMANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": number_at(w040_conformance, "treecalc_expectation_mismatch_count"),
            "failed_row_count": number_at(w040_conformance, "failed_row_count"),
            "promotion_guard": number_at(w040_conformance, "match_promoted_count") == 0,
            "semantic_state": "w040_optimized_core_conformance_bound_without_full_verification"
        }),
        json!({
            "row_id": "source.w040_treecalc_direct_evidence",
            "artifact": W040_TREECALC_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": number_at(w040_treecalc, "expectation_mismatch_count"),
            "failed_row_count": 0,
            "promotion_guard": number_at(w040_treecalc, "case_count") == 25
                && number_at(w040_treecalc, "expectation_mismatch_count") == 0,
            "semantic_state": "w040_treecalc_direct_evidence_bound"
        }),
        json!({
            "row_id": "source.w040_rust_totality_refinement",
            "artifact": W040_RUST_TOTALITY_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w040_rust, "failed_row_count"),
            "promotion_guard": !bool_at(w040_rust, "full_rust_totality_promoted")
                && !bool_at(w040_rust, "full_refinement_promoted"),
            "semantic_state": "w040_rust_totality_refinement_bound_with_blockers"
        }),
        json!({
            "row_id": "source.w040_lean_tla_boundaries",
            "artifact": W040_LEAN_TLA_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w040_lean_tla, "failed_row_count"),
            "promotion_guard": !bool_at(w040_lean_tla, "full_lean_verification_promoted")
                && !bool_at(w040_lean_tla, "full_tla_verification_promoted"),
            "semantic_state": "w040_lean_tla_boundaries_bound_without_full_verification"
        }),
        json!({
            "row_id": "source.w040_stage2_service_dependency",
            "artifact": W040_STAGE2_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w040_stage2, "failed_row_count"),
            "promotion_guard": !bool_at(w040_stage2, "stage2_policy_promoted"),
            "semantic_state": "w040_stage2_service_dependency_bound"
        }),
        json!({
            "row_id": "source.w040_operated_assurance_service_artifacts",
            "artifact": W040_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": number_at(w040_operated, "failed_row_count"),
            "promotion_guard": bool_at(w040_operated, "retained_history_artifact_store_present")
                && bool_at(w040_operated, "retained_history_query_register_present")
                && !bool_at(w040_operated, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w040_operated, "retained_history_service_promoted"),
            "semantic_state": "w040_service_artifacts_bound_without_operated_service_promotion"
        }),
        json!({
            "row_id": "source.w040_promotion_guard",
            "artifact": W040_OPERATED_ASSURANCE_SUMMARY,
            "missing_artifact_count": 0,
            "unexpected_mismatch_count": 0,
            "failed_row_count": 0,
            "promotion_guard": !bool_at(w040_operated, "operated_cross_engine_differential_service_promoted")
                && !bool_at(w040_operated, "operated_continuous_assurance_service_promoted")
                && !bool_at(w040_operated, "retained_history_service_promoted"),
            "semantic_state": "diversity_service_promotion_guard_clean"
        }),
    ]
}

fn w040_independent_scalar_evaluator_rows() -> Vec<Value> {
    independent_scalar_cases()
        .into_iter()
        .map(|case| {
            let result = eval_scalar_expression(case.expression, case.environment);
            let actual_value = result.ok();
            let matches_expected = actual_value == Some(case.expected);
            let environment = case
                .environment
                .iter()
                .map(|(name, value)| json!({ "name": name, "value": value }))
                .collect::<Vec<_>>();
            json!({
                "case_id": case.case_id,
                "expression": case.expression,
                "environment": environment,
                "expected_value": case.expected,
                "actual_value": actual_value,
                "matches_expected": matches_expected,
                "implementation_path": "src/oxcalc-tracecalc/src/diversity_seam.rs::eval_scalar_expression",
                "implementation_kind": "bounded_independent_scalar_arithmetic_evaluator",
                "scope_limit": "integer literals, names, +, *, and parentheses only",
                "independent_from": ["TraceCalc", "optimized_core", "OxFml", "OxFunc"]
            })
        })
        .collect()
}

fn w040_independent_evaluator_rows(
    w039_independent: &Value,
    w039_diversity: &Value,
    w040_conformance: &Value,
    w040_treecalc: &Value,
    w040_rust: &Value,
    w040_lean_tla: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "independent.w040_bounded_scalar_evaluator",
            "disposition_kind": "bounded_independent_implementation",
            "independence_state": "bounded_independent_scalar_implementation_present",
            "evidence": "w040_independent_scalar_evaluator_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": implementation_rows.iter().filter(|row| bool_at(row, "matches_expected")).count(),
            "promotion_consequence": "This is real independent implementation evidence for a bounded arithmetic slice, not full independent evaluator promotion."
        }),
        json!({
            "row_id": "independent.w039_predecessor_absence_narrowed",
            "disposition_kind": "accepted_boundary",
            "independence_state": "predecessor_full_independent_absence_narrowed_by_bounded_slice",
            "evidence": W039_INDEPENDENT_ROW_SET,
            "w039_independent_implementation_row_count": number_at(w039_independent, "independent_implementation_row_count"),
            "w039_fully_independent_evaluator_promoted": bool_at(w039_diversity, "fully_independent_evaluator_promoted"),
            "promotion_consequence": "W039 full absence is narrowed only for the W040 bounded scalar evaluator slice."
        }),
        json!({
            "row_id": "independent.optimized_core_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "optimized_core_conformance_same_authority_family",
            "evidence": W040_CONFORMANCE_SUMMARY,
            "direct_evidence_bound_count": number_at(w040_conformance, "direct_evidence_bound_count"),
            "exact_remaining_blocker_count": number_at(w040_conformance, "exact_remaining_blocker_count"),
            "promotion_consequence": "Optimized/core evidence strengthens conformance but is not independent implementation authority."
        }),
        json!({
            "row_id": "independent.treecalc_direct_evidence_same_family",
            "disposition_kind": "same_engine_family_not_independent",
            "independence_state": "treecalc_evidence_same_oxcalc_family",
            "evidence": W040_TREECALC_SUMMARY,
            "case_count": number_at(w040_treecalc, "case_count"),
            "expectation_mismatch_count": number_at(w040_treecalc, "expectation_mismatch_count"),
            "promotion_consequence": "TreeCalc direct evidence is core conformance evidence, not independent evaluator diversity."
        }),
        json!({
            "row_id": "independent.rust_totality_refinement_not_diversity",
            "disposition_kind": "accepted_boundary",
            "independence_state": "proof_and_refinement_support_not_second_runtime",
            "evidence": W040_RUST_TOTALITY_SUMMARY,
            "refinement_row_count": number_at(w040_rust, "refinement_row_count"),
            "totality_boundary_count": number_at(w040_rust, "totality_boundary_count"),
            "promotion_consequence": "Rust refinement evidence supports correctness, but is not a separately implemented evaluator."
        }),
        json!({
            "row_id": "independent.lean_tla_not_runtime_evaluator",
            "disposition_kind": "accepted_boundary",
            "independence_state": "proof_model_not_executable_runtime_evaluator",
            "evidence": W040_LEAN_TLA_SUMMARY,
            "local_proof_row_count": number_at(w040_lean_tla, "local_proof_row_count"),
            "bounded_model_row_count": number_at(w040_lean_tla, "bounded_model_row_count"),
            "promotion_consequence": "Lean/TLA evidence strengthens specification and model coverage but is not runtime evaluator implementation authority."
        }),
        json!({
            "row_id": "independent.general_oxfml_formula_slice_external",
            "disposition_kind": "accepted_external_slice",
            "independence_state": "external_formula_evaluator_not_oxcalc_coordinator",
            "evidence": W037_DIRECT_OXFML_SUMMARY,
            "promotion_consequence": "OxFml remains valid external formula-evaluator evidence, but does not independently implement OxCalc scheduling/publication."
        }),
        json!({
            "row_id": "independent.full_independent_evaluator_breadth_blocked",
            "disposition_kind": "exact_remaining_blocker",
            "independence_state": "full_independent_implementation_breadth_absent",
            "evidence": W040_DIRECT_OBLIGATION_MAP,
            "promotion_consequence": "Full independent-evaluator diversity remains blocked beyond the bounded scalar implementation slice."
        }),
    ]
}

fn w040_cross_engine_differential_rows(
    w039_cross_engine: &Value,
    w040_stage2: &Value,
    w040_operated: &Value,
    w040_retained: &Value,
    w040_cross_engine_service: &Value,
    w040_formatting_intake: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "cross_engine.w040_bounded_scalar_independent_differential",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_bounded_independent_rows",
            "evidence": "w040_independent_scalar_evaluator_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": implementation_rows.iter().filter(|row| bool_at(row, "matches_expected")).count(),
            "promotion_consequence": "Bounded scalar rows can feed later differential service work but are not an operated cross-engine service."
        }),
        json!({
            "row_id": "cross_engine.w039_file_backed_differential_predecessor",
            "disposition_kind": "accepted_boundary",
            "service_state": "file_backed_not_operated",
            "evidence": W039_CROSS_ENGINE_DIVERSITY,
            "w039_row_count": number_at(w039_cross_engine, "row_count"),
            "promotion_consequence": "W039 file-backed diversity remains evidence lineage, not service promotion."
        }),
        json!({
            "row_id": "cross_engine.w040_stage2_service_dependency",
            "disposition_kind": "service_dependency_blocked",
            "service_state": "blocked",
            "evidence": W040_STAGE2_SUMMARY,
            "stage2_policy_promoted": bool_at(w040_stage2, "stage2_policy_promoted"),
            "exact_remaining_blocker_count": number_at(w040_stage2, "exact_remaining_blocker_count"),
            "promotion_consequence": "Stage 2 service-dependent diversity remains blocked by operated service and pack-governance dependencies."
        }),
        json!({
            "row_id": "cross_engine.w040_operated_service_artifacts",
            "disposition_kind": "service_artifacts_bound",
            "service_state": "file_backed_artifacts_without_operated_service",
            "evidence": W040_OPERATED_ASSURANCE_SUMMARY,
            "file_backed_operated_runner_present": bool_at(w040_operated, "file_backed_operated_runner_present"),
            "operated_cross_engine_differential_service_promoted": bool_at(w040_operated, "operated_cross_engine_differential_service_promoted"),
            "promotion_consequence": "W040 service artifacts improve evidence retention and queryability but do not operate a differential service."
        }),
        json!({
            "row_id": "cross_engine.w040_cross_engine_service_register",
            "disposition_kind": "service_substrate_bound",
            "service_state": if bool_at(w040_cross_engine_service, "operated_cross_engine_differential_service_present") { "present" } else { "blocked" },
            "evidence": W040_CROSS_ENGINE_SERVICE_REGISTER,
            "file_backed_cross_engine_substrate_present": bool_at(w040_cross_engine_service, "file_backed_cross_engine_substrate_present"),
            "operated_cross_engine_differential_service_present": bool_at(w040_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "promotion_consequence": "The W040 service register records the operated-service blocker exactly."
        }),
        json!({
            "row_id": "cross_engine.w040_retained_history_query_support",
            "disposition_kind": "supporting_evidence_not_service",
            "service_state": "supporting_history_and_query_only",
            "evidence": W040_RETAINED_HISTORY_STORE,
            "store_record_count": number_at(w040_retained, "store_record_count"),
            "query_register_row_count": number_at(w040_retained, "query_register_row_count"),
            "replay_correlation_row_count": number_at(w040_retained, "replay_correlation_row_count"),
            "promotion_consequence": "Retained query/correlation artifacts support later differential service operation but are not themselves a service."
        }),
        json!({
            "row_id": "cross_engine.w073_typed_only_formatting_guard",
            "disposition_kind": "accepted_boundary",
            "service_state": "observable_guard_retained",
            "evidence": W040_FORMATTING_INTAKE,
            "w072_threshold_fallback_allowed": bool_at(w040_formatting_intake, "w072_threshold_fallback_allowed_for_aggregate_visualization"),
            "promotion_consequence": "Diversity comparisons must preserve the W073 typed-only aggregate/visualization formatting guard."
        }),
        json!({
            "row_id": "cross_engine.mismatch_triage_and_quarantine_service",
            "disposition_kind": "exact_remaining_blocker",
            "service_state": "blocked",
            "evidence": W040_OPERATED_ASSURANCE_SUMMARY,
            "alert_decision_count": number_at(w040_operated, "alert_decision_count"),
            "quarantine_decision_count": number_at(w040_operated, "quarantine_decision_count"),
            "promotion_consequence": "Cross-engine diversity needs an operated mismatch triage/quarantine service before service-level promotion."
        }),
    ]
}

fn w040_differential_authority_rows(
    w039_authority: &Value,
    w040_conformance: &Value,
    w040_operated: &Value,
    w040_cross_engine_service: &Value,
    implementation_rows: &[Value],
) -> Vec<Value> {
    vec![
        json!({
            "row_id": "authority.w040_bounded_independent_scalar_evaluator",
            "disposition_kind": "bounded_independent_implementation",
            "authority_state": "bounded_independent_runtime",
            "evidence": "w040_independent_scalar_evaluator_implementation.json",
            "case_count": implementation_rows.len(),
            "match_count": implementation_rows.iter().filter(|row| bool_at(row, "matches_expected")).count(),
            "authority_limit": "bounded integer scalar arithmetic only"
        }),
        json!({
            "row_id": "authority.w039_predecessor_authority_boundaries",
            "disposition_kind": "accepted_boundary",
            "authority_state": "predecessor_authority_boundaries_retained",
            "evidence": W039_DIFFERENTIAL_AUTHORITY,
            "row_count": number_at(w039_authority, "row_count"),
            "authority_limit": "W039 boundaries remain in force beyond the bounded scalar slice"
        }),
        json!({
            "row_id": "authority.optimized_core_same_implementation_family",
            "disposition_kind": "same_authority_family",
            "authority_state": "not_independent",
            "evidence": W040_CONFORMANCE_SUMMARY,
            "exact_remaining_blocker_count": number_at(w040_conformance, "exact_remaining_blocker_count"),
            "authority_limit": "optimized/core conformance does not supply separate implementation authority"
        }),
        json!({
            "row_id": "authority.file_backed_service_artifacts",
            "disposition_kind": "accepted_boundary",
            "authority_state": "file_backed_artifact_authority",
            "evidence": W040_OPERATED_ASSURANCE_SUMMARY,
            "retained_history_query_register_present": bool_at(w040_operated, "retained_history_query_register_present"),
            "authority_limit": "file-backed row agreement and retained artifacts cannot substitute for an operated differential service"
        }),
        json!({
            "row_id": "authority.operated_cross_engine_service",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_operated_service",
            "evidence": W040_CROSS_ENGINE_SERVICE_REGISTER,
            "operated_cross_engine_differential_service_present": bool_at(w040_cross_engine_service, "operated_cross_engine_differential_service_present"),
            "authority_limit": "no recurring service endpoint, retained service history, or service-level mismatch action path exists"
        }),
        json!({
            "row_id": "authority.full_independent_evaluator_implementation",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_beyond_bounded_scalar_slice",
            "evidence": "w040_independent_scalar_evaluator_implementation.json",
            "authority_limit": "bounded scalar evaluator does not cover TreeCalc scheduling, references, publication, effects, OxFml seam breadth, or callable metadata"
        }),
        json!({
            "row_id": "authority.release_grade_diversity_promotion",
            "disposition_kind": "exact_remaining_blocker",
            "authority_state": "blocked_no_release_grade_authority",
            "evidence": W040_OPERATED_ASSURANCE_SUMMARY,
            "exact_service_blocker_count": number_at(w040_operated, "exact_service_blocker_count"),
            "authority_limit": "release-grade diversity authority remains unavailable while full independent implementation and operated service blockers remain"
        }),
    ]
}

fn w040_exact_blockers(
    w039_blockers: &Value,
    w040_stage2_blockers: &Value,
    w040_service_blockers: &Value,
) -> Vec<Value> {
    vec![
        json!({
            "blocker_id": "w040_diversity.full_independent_evaluator_breadth_absent",
            "owner": "calc-tv5.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": "w040_independent_scalar_evaluator_implementation.json",
            "predecessor_blocker_count": number_at(w039_blockers, "exact_blocker_count"),
            "reason": "W040 introduces a bounded independent scalar arithmetic evaluator, but full TreeCalc/OxCalc evaluator breadth remains absent.",
            "promotion_consequence": "fully independent evaluator diversity remains unpromoted"
        }),
        json!({
            "blocker_id": "w040_diversity.operated_cross_engine_service_absent",
            "owner": "calc-tv5.7; calc-tv5.6",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W040_CROSS_ENGINE_SERVICE_REGISTER,
            "service_blocker_rows": row_count_at(w040_service_blockers),
            "reason": "W040 service artifacts are file-backed and do not operate a recurring cross-engine differential service.",
            "promotion_consequence": "operated cross-engine diversity service remains unpromoted"
        }),
        json!({
            "blocker_id": "w040_diversity.stage2_differential_service_dependency_absent",
            "owner": "calc-tv5.5; calc-tv5.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W040_STAGE2_BLOCKERS,
            "stage2_blocker_rows": row_count_at(w040_stage2_blockers),
            "reason": "Stage 2 diversity still needs production policy, partition soundness, pack governance, and operated differential service evidence before strategy-level diversity promotion.",
            "promotion_consequence": "Stage 2 diversity remains a blocked dependency for release-grade promotion"
        }),
        json!({
            "blocker_id": "w040_diversity.mismatch_triage_and_quarantine_service_absent",
            "owner": "calc-tv5.6; calc-tv5.7",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W040_OPERATED_SERVICE_BLOCKERS,
            "reason": "Local alert/quarantine rows are evaluated, but no external mismatch triage/quarantine service is operated.",
            "promotion_consequence": "service-level diversity assurance remains unpromoted"
        }),
        json!({
            "blocker_id": "w040_diversity.release_grade_promotion_authority_absent",
            "owner": "calc-tv5.10",
            "status_after_run": "exact_remaining_blocker",
            "evidence": W040_DIRECT_OBLIGATION_MAP,
            "reason": "Release-grade diversity cannot be claimed from bounded scalar rows, proxy evidence, file-backed rows, shared projections, or retained blockers.",
            "promotion_consequence": "release-grade verification, pack/C5, and diversity promotion remain unavailable to this bead"
        }),
    ]
}

fn w040_diversity_validation_failures(
    implementation_rows: &[Value],
    independent_rows: &[Value],
    cross_engine_rows: &[Value],
    authority_rows: &[Value],
    blockers: &[Value],
) -> Vec<String> {
    let mut failures = Vec::new();
    if implementation_rows.is_empty()
        || implementation_rows
            .iter()
            .any(|row| !bool_at(row, "matches_expected"))
    {
        failures.push("w040_diversity.independent_scalar_case_mismatch".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("bounded_independent_scalar_implementation_present")
    }) {
        failures.push("w040_diversity.bounded_independent_row_missing".to_string());
    }
    if !independent_rows.iter().any(|row| {
        row.get("independence_state").and_then(Value::as_str)
            == Some("full_independent_implementation_breadth_absent")
    }) {
        failures.push("w040_diversity.full_breadth_blocker_row_missing".to_string());
    }
    if !cross_engine_rows
        .iter()
        .any(|row| row.get("service_state").and_then(Value::as_str) == Some("blocked"))
    {
        failures.push("w040_diversity.blocked_cross_engine_service_row_missing".to_string());
    }
    if !authority_rows.iter().any(|row| {
        row.get("authority_state").and_then(Value::as_str) == Some("blocked_no_operated_service")
    }) {
        failures.push("w040_diversity.operated_service_authority_blocker_missing".to_string());
    }
    if blockers.len() < 5 {
        failures.push("w040_diversity.exact_blocker_count_below_gate".to_string());
    }
    if independent_rows
        .iter()
        .chain(cross_engine_rows.iter())
        .chain(authority_rows.iter())
        .any(|row| row.get("disposition_kind").and_then(Value::as_str) == Some("promoted"))
    {
        failures.push("w040_diversity.unexpected_promotion_row".to_string());
    }
    failures
}

fn source_validation_failures(source_rows: &[Value]) -> Vec<String> {
    source_rows
        .iter()
        .flat_map(|row| {
            let row_id = text_at(row, "row_id");
            let mut failures = Vec::new();
            if number_at(row, "missing_artifact_count") > 0 {
                failures.push(format!("{row_id}.missing_artifact_count_nonzero"));
            }
            if number_at(row, "unexpected_mismatch_count") > 0 {
                failures.push(format!("{row_id}.unexpected_mismatch_count_nonzero"));
            }
            if number_at(row, "failed_row_count") > 0 {
                failures.push(format!("{row_id}.failed_row_count_nonzero"));
            }
            if !bool_at(row, "promotion_guard") {
                failures.push(format!("{row_id}.promotion_guard_failed"));
            }
            failures
        })
        .collect()
}

fn seam_validation_failures(seam_rows: &[Value]) -> Vec<String> {
    seam_rows
        .iter()
        .filter_map(|row| {
            let row_id = text_at(row, "row_id");
            let watch_state = row.get("watch_state").and_then(Value::as_str);
            (watch_state == Some("missing_blocker")).then(|| format!("{row_id}.missing_blocker"))
        })
        .collect()
}

fn row_with_field_exists(value: &Value, field: &str, expected: &str) -> bool {
    value
        .get("rows")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .any(|row| row.get(field).and_then(Value::as_str) == Some(expected))
}

fn read_json(repo_root: &Path, relative_path: &str) -> Result<Value, DiversitySeamError> {
    let path = repo_root.join(relative_path);
    let contents =
        fs::read_to_string(&path).map_err(|source| DiversitySeamError::ReadArtifact {
            path: path.display().to_string(),
            source,
        })?;
    serde_json::from_str(&contents).map_err(|source| DiversitySeamError::ParseJson {
        path: path.display().to_string(),
        source,
    })
}

fn write_json(path: &Path, value: &Value) -> Result<(), DiversitySeamError> {
    let contents =
        serde_json::to_string_pretty(value).map_err(|source| DiversitySeamError::ParseJson {
            path: path.display().to_string(),
            source,
        })?;
    fs::write(path, format!("{contents}\n")).map_err(|source| DiversitySeamError::WriteFile {
        path: path.display().to_string(),
        source,
    })
}

fn text_at(value: &Value, key: &str) -> String {
    value
        .get(key)
        .and_then(Value::as_str)
        .unwrap_or("<missing>")
        .to_string()
}

fn number_at(value: &Value, key: &str) -> u64 {
    value.get(key).and_then(Value::as_u64).unwrap_or(0)
}

fn array_len_at(value: &Value, key: &str) -> usize {
    value.get(key).and_then(Value::as_array).map_or(0, Vec::len)
}

fn array_contains_string(value: &Value, key: &str, expected: &str) -> bool {
    value
        .get(key)
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .any(|item| item.as_str() == Some(expected))
}

fn row_count_at(value: &Value) -> usize {
    array_len_at(value, "rows")
}

fn bool_at(value: &Value, key: &str) -> bool {
    value.get(key).and_then(Value::as_bool).unwrap_or(false)
}

fn bool_at_path(value: &Value, path: &[&str]) -> bool {
    path.iter()
        .try_fold(value, |current, key| current.get(*key))
        .and_then(Value::as_bool)
        .unwrap_or(false)
}

fn relative_artifact_path(parts: &[&str]) -> String {
    historical_w038_w045_artifact_path(parts).unwrap_or_else(|| parts.join("/"))
}

fn historical_w038_w045_artifact_path(parts: &[&str]) -> Option<String> {
    if parts.len() >= 5
        && parts[0] == "docs"
        && parts[1] == "test-runs"
        && parts[2] == "core-engine"
        && matches!(
            parts[4].get(..4),
            Some("w038" | "w039" | "w040" | "w041" | "w042" | "w043" | "w044" | "w045")
        )
    {
        let mut archived = vec!["archive", "test-runs-core-engine-w038-w045"];
        archived.extend_from_slice(&parts[3..]);
        Some(archived.join("/"))
    } else {
        None
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use std::path::PathBuf;

    #[test]
    fn diversity_seam_runner_classifies_w038_seams_without_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w038-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.source_evidence_row_count, 7);
        assert_eq!(summary.diversity_disposition_row_count, 5);
        assert_eq!(summary.seam_watch_row_count, 8);
        assert_eq!(summary.aligned_seam_watch_row_count, 7);
        assert_eq!(summary.accepted_boundary_count, 3);
        assert_eq!(summary.exact_blocker_count, 4);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(validation["status"], "w038_diversity_seam_packet_valid");

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(decision["oxfml_handoff_triggered"], false);

        cleanup();
    }

    #[test]
    fn diversity_seam_runner_classifies_w039_independence_without_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w039-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.schema_version, W039_RUN_SUMMARY_SCHEMA_V1);
        assert_eq!(summary.source_evidence_row_count, 10);
        assert_eq!(summary.diversity_disposition_row_count, 7);
        assert_eq!(summary.seam_watch_row_count, 7);
        assert_eq!(summary.accepted_boundary_count, 10);
        assert_eq!(summary.exact_blocker_count, 6);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(
            validation["status"],
            "w039_independent_evaluator_cross_engine_diversity_packet_valid"
        );
        assert_eq!(
            validation["operated_cross_engine_differential_service_promoted"],
            false
        );

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(
            decision["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(decision["w073_formatting_handoff_triggered"], false);

        cleanup();
    }

    #[test]
    fn diversity_seam_runner_binds_w040_bounded_independent_evaluator_without_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w040-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.schema_version, W040_RUN_SUMMARY_SCHEMA_V1);
        assert_eq!(summary.source_evidence_row_count, 10);
        assert_eq!(summary.diversity_disposition_row_count, 8);
        assert_eq!(summary.seam_watch_row_count, 8);
        assert_eq!(summary.exact_blocker_count, 5);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let implementation = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w040_independent_scalar_evaluator_implementation.json"
            ),
        )
        .unwrap();
        assert_eq!(implementation["case_count"], 5);
        assert_eq!(implementation["match_count"], 5);
        assert_eq!(
            implementation["fully_independent_evaluator_promoted"],
            false
        );

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(
            validation["status"],
            "w040_bounded_independent_evaluator_diversity_packet_valid"
        );
        assert_eq!(
            validation["operated_cross_engine_differential_service_promoted"],
            false
        );

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(
            decision["bounded_independent_scalar_evaluator_present"],
            true
        );
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(
            decision["operated_cross_engine_differential_service_promoted"],
            false
        );

        cleanup();
    }

    #[test]
    fn diversity_seam_runner_binds_w041_independent_evaluator_breadth_without_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w041-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.schema_version, W041_RUN_SUMMARY_SCHEMA_V1);
        assert_eq!(summary.source_evidence_row_count, 12);
        assert_eq!(summary.diversity_disposition_row_count, 10);
        assert_eq!(summary.seam_watch_row_count, 9);
        assert_eq!(summary.exact_blocker_count, 6);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let implementation = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w041_independent_formula_evaluator_implementation.json"
            ),
        )
        .unwrap();
        assert_eq!(implementation["case_count"], 8);
        assert_eq!(implementation["match_count"], 8);
        assert_eq!(
            implementation["fully_independent_evaluator_promoted"],
            false
        );

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(
            validation["status"],
            "w041_broadened_independent_evaluator_diversity_packet_valid"
        );
        assert_eq!(
            validation["operated_cross_engine_differential_service_promoted"],
            false
        );

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(
            decision["broadened_independent_formula_evaluator_present"],
            true
        );
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(
            decision["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(decision["exact_blocker_count"], 6);

        cleanup();
    }

    #[test]
    fn diversity_seam_runner_binds_w042_reference_model_without_service_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w042-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.schema_version, W042_RUN_SUMMARY_SCHEMA_V1);
        assert_eq!(summary.source_evidence_row_count, 15);
        assert_eq!(summary.diversity_disposition_row_count, 11);
        assert_eq!(summary.seam_watch_row_count, 10);
        assert_eq!(summary.aligned_seam_watch_row_count, 10);
        assert_eq!(summary.accepted_boundary_count, 17);
        assert_eq!(summary.exact_blocker_count, 7);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let implementation = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w042_independent_reference_model_implementation.json"
            ),
        )
        .unwrap();
        assert_eq!(implementation["case_count"], 4);
        assert_eq!(implementation["match_count"], 4);
        assert_eq!(
            implementation["fully_independent_evaluator_promoted"],
            false
        );

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(
            validation["status"],
            "w042_independent_reference_model_diversity_packet_valid"
        );
        assert_eq!(
            validation["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(validation["mismatch_quarantine_service_promoted"], false);

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(
            decision["independent_reference_model_evaluator_present"],
            true
        );
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(
            decision["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(decision["mismatch_quarantine_service_promoted"], false);
        assert_eq!(decision["w073_formatting_handoff_triggered"], false);
        assert_eq!(decision["exact_blocker_count"], 7);

        cleanup();
    }

    #[test]
    fn diversity_seam_runner_binds_w043_reference_model_without_service_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w043-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.schema_version, W043_RUN_SUMMARY_SCHEMA_V1);
        assert_eq!(summary.source_evidence_row_count, 17);
        assert_eq!(summary.diversity_disposition_row_count, 13);
        assert_eq!(summary.seam_watch_row_count, 12);
        assert_eq!(summary.aligned_seam_watch_row_count, 12);
        assert_eq!(summary.accepted_boundary_count, 21);
        assert_eq!(summary.exact_blocker_count, 8);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let implementation = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w043_independent_reference_model_implementation.json"
            ),
        )
        .unwrap();
        assert_eq!(implementation["case_count"], 6);
        assert_eq!(implementation["match_count"], 6);
        assert_eq!(
            implementation["fully_independent_evaluator_promoted"],
            false
        );

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(
            validation["status"],
            "w043_independent_reference_model_diversity_packet_valid"
        );
        assert_eq!(
            validation["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(validation["mismatch_quarantine_service_promoted"], false);

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(
            decision["independent_reference_model_evaluator_present"],
            true
        );
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(
            decision["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(decision["mismatch_quarantine_service_promoted"], false);
        assert_eq!(decision["w073_formatting_handoff_triggered"], false);
        assert_eq!(decision["exact_blocker_count"], 8);

        cleanup();
    }

    #[test]
    fn diversity_seam_runner_binds_w044_reference_model_without_service_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w044-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.schema_version, W044_RUN_SUMMARY_SCHEMA_V1);
        assert_eq!(summary.source_evidence_row_count, 21);
        assert_eq!(summary.diversity_disposition_row_count, 15);
        assert_eq!(summary.seam_watch_row_count, 16);
        assert_eq!(summary.aligned_seam_watch_row_count, 15);
        assert_eq!(summary.accepted_boundary_count, 25);
        assert_eq!(summary.exact_blocker_count, 9);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let implementation = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w044_independent_reference_model_implementation.json"
            ),
        )
        .unwrap();
        assert_eq!(implementation["case_count"], 8);
        assert_eq!(implementation["match_count"], 8);
        assert_eq!(
            implementation["fully_independent_evaluator_promoted"],
            false
        );

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(
            validation["status"],
            "w044_independent_reference_model_diversity_packet_valid"
        );
        assert_eq!(
            validation["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(validation["mismatch_quarantine_service_promoted"], false);
        assert_eq!(
            validation["retained_witness_attachment_service_promoted"],
            false
        );

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(
            decision["independent_reference_model_evaluator_present"],
            true
        );
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(
            decision["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(decision["mismatch_quarantine_service_promoted"], false);
        assert_eq!(
            decision["retained_witness_attachment_service_promoted"],
            false
        );
        assert_eq!(decision["w073_formatting_handoff_triggered"], false);
        assert_eq!(
            decision["w073_downstream_typed_rule_request_construction_verified"],
            false
        );
        assert_eq!(decision["exact_blocker_count"], 9);

        cleanup();
    }

    #[test]
    fn diversity_seam_runner_binds_w045_harness_and_witness_without_service_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w045-diversity-seam-{}", std::process::id());
        let artifact_root = repo_root.join(format!(
            "docs/test-runs/core-engine/diversity-seam/{run_id}"
        ));
        let cleanup = || {
            if artifact_root.exists() {
                let _ = fs::remove_dir_all(&artifact_root);
            }
        };

        cleanup();
        let summary = DiversitySeamRunner::new()
            .execute(&repo_root, &run_id)
            .unwrap();

        assert_eq!(summary.schema_version, W045_RUN_SUMMARY_SCHEMA_V1);
        assert_eq!(summary.source_evidence_row_count, 29);
        assert_eq!(summary.diversity_disposition_row_count, 18);
        assert_eq!(summary.seam_watch_row_count, 20);
        assert_eq!(summary.aligned_seam_watch_row_count, 19);
        assert_eq!(summary.accepted_boundary_count, 46);
        assert_eq!(summary.exact_blocker_count, 10);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.fully_independent_evaluator_promoted);

        let implementation = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w045_independent_reference_model_implementation.json"
            ),
        )
        .unwrap();
        assert_eq!(implementation["case_count"], 10);
        assert_eq!(implementation["match_count"], 10);
        assert_eq!(
            implementation["fully_independent_evaluator_promoted"],
            false
        );

        let witness = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w045_retained_witness_attachment_register.json"
            ),
        )
        .unwrap();
        assert_eq!(witness["row_count"], 8);
        assert_eq!(
            witness["retained_witness_attachment_service_promoted"],
            false
        );

        let harness = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/diversity-seam/{run_id}/w045_operated_differential_service_harness_register.json"
            ),
        )
        .unwrap();
        assert_eq!(harness["row_count"], 8);
        assert_eq!(harness["local_harness_runnable"], true);
        assert_eq!(harness["service_endpoint_present"], false);

        let validation = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/validation.json"),
        )
        .unwrap();
        assert_eq!(
            validation["status"],
            "w045_independent_reference_model_diversity_packet_valid"
        );
        assert_eq!(
            validation["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(validation["mismatch_quarantine_service_promoted"], false);
        assert_eq!(
            validation["retained_witness_attachment_service_promoted"],
            false
        );
        assert_eq!(validation["service_blocked_count"], 20);

        let decision = read_json(
            &repo_root,
            &format!("docs/test-runs/core-engine/diversity-seam/{run_id}/promotion_decision.json"),
        )
        .unwrap();
        assert_eq!(
            decision["independent_reference_model_evaluator_present"],
            true
        );
        assert_eq!(decision["fully_independent_evaluator_promoted"], false);
        assert_eq!(
            decision["operated_cross_engine_differential_service_promoted"],
            false
        );
        assert_eq!(decision["mismatch_quarantine_service_promoted"], false);
        assert_eq!(
            decision["retained_witness_attachment_service_promoted"],
            false
        );
        assert_eq!(decision["local_harness_promoted_as_service"], false);
        assert_eq!(decision["w073_formatting_handoff_triggered"], false);
        assert_eq!(
            decision["w073_downstream_typed_rule_request_construction_verified"],
            false
        );
        assert_eq!(decision["exact_blocker_count"], 10);

        cleanup();
    }
}
