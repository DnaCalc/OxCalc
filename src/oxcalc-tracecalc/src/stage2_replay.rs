...
//! W038/W039/W040/W041/W042/W043/W044/W045 bounded Stage 2 partition replay and policy-governance evidence.
...
const W045_RUN_SUMMARY_SCHEMA_V1: &str = "oxcalc.stage2_replay.w045.run_summary.v1";
const W045_SOURCE_INDEX_SCHEMA_V1: &str = "oxcalc.stage2_replay.w045.source_evidence_index.v1";
const W045_POLICY_GATE_SCHEMA_V1: &str = "oxcalc.stage2_replay.w045.stage2_policy_gate_register.v1";
const W045_PRODUCTION_PARTITION_ANALYZER_SCHEMA_V1: &str =
    "oxcalc.stage2_replay.w045.production_partition_analyzer_register.v1";
const W045_SCHEDULER_EQUIVALENCE_SCHEMA_V1: &str =
    "oxcalc.stage2_replay.w045.scheduler_equivalence_register.v1";
const W045_PACK_GRADE_EQUIVALENCE_SCHEMA_V1: &str =
    "oxcalc.stage2_replay.w045.pack_grade_equivalence_register.v1";
const W045_SERVICE_GATE_SCHEMA_V1: &str = "oxcalc.stage2_replay.w045.service_gate_register.v1";
const W045_BLOCKER_REGISTER_SCHEMA_V1: &str =
    "oxcalc.stage2_replay.w045.stage2_exact_blocker_register.v1";
const W045_PROMOTION_DECISION_SCHEMA_V1: &str = "oxcalc.stage2_replay.w045.promotion_decision.v1";
const W045_VALIDATION_SCHEMA_V1: &str = "oxcalc.stage2_replay.w045.validation.v1";
...
impl Stage2ReplayRunner {
    ...
    fn execute_w045(
        ...
        let w045_residual_obligation_map = read_json(repo_root, W045_RESIDUAL_OBLIGATION_MAP)?;
        let w045_residual_promotion_contract_map =
            read_json(repo_root, W045_RESIDUAL_PROMOTION_CONTRACT_MAP)?;
        let w045_optimized_summary = read_json(repo_root, W045_OPTIMIZED_CORE_SUMMARY)?;
        let w045_dynamic_coverage = read_json(repo_root, W045_OPTIMIZED_CORE_DYNAMIC_COVERAGE)?;
        let w045_counterpart_coverage =
            read_json(repo_root, W045_OPTIMIZED_CORE_COUNTERPART_COVERAGE)?;
        let w045_optimized_blockers = read_json(repo_root, W045_OPTIMIZED_CORE_BLOCKERS)?;
        let w045_match_guard = read_json(repo_root, W045_OPTIMIZED_CORE_MATCH_GUARD)?;
        ...
        let w045_tla_model_register = read_json(repo_root, W045_TLA_MODEL_REGISTER)?;
        let w045_lean_tla_blockers = read_json(repo_root, W045_LEAN_TLA_BLOCKERS)?;
        let w044_stage2_summary = read_json(repo_root, W044_STAGE2_RUN_SUMMARY_FOR_W045)?;
        let w044_stage2_validation = read_json(repo_root, W044_STAGE2_VALIDATION_FOR_W045)?;
        let w044_stage2_policy_gate = read_json(repo_root, W044_STAGE2_POLICY_GATE_FOR_W045)?;
        let w044_stage2_production_analyzer =
            read_json(repo_root, W044_STAGE2_PRODUCTION_ANALYZER_FOR_W045)?;
        let w044_stage2_scheduler =
            read_json(repo_root, W044_STAGE2_SCHEDULER_EQUIVALENCE_FOR_W045)?;
        let w044_stage2_pack = read_json(repo_root, W044_STAGE2_PACK_GRADE_EQUIVALENCE_FOR_W045)?;
        let w044_stage2_blockers = read_json(repo_root, W044_STAGE2_BLOCKERS_FOR_W045)?;
        let w044_stage2_promotion = read_json(repo_root, W044_STAGE2_PROMOTION_DECISION_FOR_W045)?;
        let lean_file_present = repo_root.join(W045_STAGE2_LEAN_FILE).exists();

        let w045_residual_valid = number_at(&w045_residual_summary, "successor_obligation_count")
            == 36
            ...
            && string_at(&w045_residual_summary, "w073_formatting_intake")
                == "typed_rule_only_direct_replacement_guard_retained"
            && !bool_at(
                &w045_residual_summary,
                "w073_threshold_fallback_allowed_for_typed_families",
            )
            && !bool_at(
                &w045_residual_summary,
            ...
            && row_with_field_exists(
                &w045_residual_obligation_map,
                "source_lane",
                "w044_residual.stage2_production_policy_and_pack_equivalence",
            )
            && number_at(
                &w045_residual_promotion_contract_map,
                "promotion_contract_count",
            ) == 18
            ...
            && array_field_contains_string(
                &w045_residual_summary,
                "no_promotion_claims",
                "w073_downstream_typed_rule_request_construction",
            );
        let w044_stage2_valid = string_at(&w044_stage2_validation, "status")
            == "w044_stage2_scheduler_equivalence_valid"
            && number_at(&w044_stage2_summary, "policy_row_count") == 25
            && number_at(&w044_stage2_summary, "satisfied_policy_row_count") == 17
            && number_at(&w044_stage2_summary, "exact_remaining_blocker_count") == 8
            && number_at(&w044_stage2_policy_gate, "policy_row_count") == 25
            && number_at(&w044_stage2_production_analyzer, "row_count") == 18
            && number_at(&w044_stage2_scheduler, "row_count") == 9
            && number_at(&w044_stage2_pack, "row_count") == 9
            && bool_at(
                &w044_stage2_summary,
            ...
            && !bool_at(&w044_stage2_summary, "stage2_policy_promoted")
            && !bool_at(&w044_stage2_summary, "pack_grade_replay_promoted")
            && !bool_at(&w044_stage2_promotion, "stage2_policy_promoted")
            && !bool_at(&w044_stage2_promotion, "pack_grade_replay_promoted");
        let w045_optimized_valid = number_at(&w045_optimized_summary, "failed_row_count") == 0
            && number_at(&w045_optimized_summary, "w045_disposition_row_count") == 7
            && number_at(&w045_optimized_summary, "w045_direct_evidence_bound_count") == 2
            && number_at(
                &w045_optimized_summary,
                "w045_exact_remaining_blocker_count",
            ) == 5
            && number_at(&w045_optimized_summary, "w045_match_promoted_count") == 0
            && !bool_at(&w045_match_guard, "match_promoted");
        let mixed_dynamic_valid = w045_optimized_valid
            && row_with_field_exists(
                &w045_dynamic_coverage,
                "row_id",
                "w045_dynamic_mixed_transition_carried_direct_evidence",
            )
            && number_at(&w045_dynamic_coverage, "direct_evidence_bound_count") == 2;
        let w045_rust_valid = string_at(&w045_rust_validation, "status")
            == "formal_assurance_w045_rust_totality_refinement_valid"
            && number_at(&w045_rust_summary, "automatic_dynamic_transition_row_count") == 1
            && number_at(&w045_rust_summary, "exact_remaining_blocker_count") == 7
            && number_at(&w045_rust_summary, "failed_row_count") == 0
            && !bool_at(
                &w045_rust_summary["promotion_claims"],
                "stage2_policy_promoted",
            );
        let rust_refinement_valid = w045_rust_valid
            && row_with_field_exists(
        ...
        let w045_lean_tla_valid = string_at(&w045_lean_tla_validation, "status")
            == "formal_assurance_w045_lean_tla_fairness_totality_valid"
            && number_at(&w045_lean_tla_summary, "bounded_model_row_count") == 4
            && number_at(&w045_lean_tla_summary, "stage2_dependency_blocker_count") == 1
            && number_at(&w045_lean_tla_summary, "exact_remaining_blocker_count") == 6
            && number_at(&w045_lean_tla_summary, "failed_row_count") == 0
            && !bool_at(
                &w045_lean_tla_summary["promotion_claims"],
                "stage2_policy_promoted",
            );
        let w045_model_bound_valid = w045_lean_tla_valid
            && row_with_field_exists(
                &w045_tla_model_register,
                "row_id",
                "w045_tla_stage2_partition_bounded_model_evidence",
            )
            && row_with_field_exists(
                &w045_tla_model_register,
                "row_id",
                "w045_stage2_equivalence_bounded_model_input",
            );
        let w073_guard_valid = w045_residual_valid
            && bool_at(
                &w045_residual_summary,
        ...
        let bounded_replay_valid =
            w044_stage2_valid && number_at(&w044_stage2_summary, "partition_replay_row_count") == 5;
        let permutation_replay_valid = w044_stage2_valid
            && number_at(&w044_stage2_summary, "permutation_replay_row_count") == 6
            && number_at(&w044_stage2_summary, "nontrivial_permutation_row_count") == 1;
        let observable_invariance_valid = w044_stage2_valid
            && number_at(&w044_stage2_summary, "observable_invariance_row_count") == 5;
        let declared_scheduler_valid = w044_stage2_valid
            && bool_at(
                &w044_stage2_summary,
                "declared_scheduler_equivalence_evidenced",
            )
            && w045_model_bound_valid;
        let declared_pack_valid = w044_stage2_valid
            && bool_at(&w044_stage2_summary, "declared_pack_equivalence_evidenced")
            && observable_invariance_valid
            && w073_guard_valid;
        let predecessor_production_inputs_valid = w044_stage2_valid
            && bool_at(
                &w044_stage2_summary,
                "production_relevant_analyzer_inputs_evidenced",
            );
        let no_proxy_guard_valid = w045_residual_valid
            && w045_optimized_valid
            && !bool_at(&w044_stage2_promotion, "stage2_promotion_candidate")
            && !bool_at(&w044_stage2_promotion, "pack_grade_replay_candidate");
        ...
        let production_analyzer_blocker = row_with_field_exists(
            ...
            "row_id",
            "w044_stage2_full_production_partition_analyzer_soundness_blocker",
        ) && row_with_field_exists(
            &w045_lean_tla_blockers,
            "row_id",
            "w045_stage2_production_policy_dependency_exact_blocker",
        );
        let fairness_blocker = row_with_field_exists(
            &w045_lean_tla_blockers,
            "row_id",
            "w045_tla_fairness_scheduler_unbounded_boundary",
        );
        let rust_dependency_blocker = row_with_field_exists(
            &w045_lean_tla_blockers,
            "row_id",
            "w045_rust_totality_dependency_exact_blocker",
        ) && row_with_field_exists(
            &w045_rust_blockers,
            "row_id",
            "w045_full_optimized_core_release_grade_conformance_boundary",
        );
        ...
        let production_relevant_inputs_valid = bounded_replay_valid
            ...
            && observable_invariance_valid
            && predecessor_production_inputs_valid
            && mixed_dynamic_valid
            && rust_refinement_valid
            && w045_model_bound_valid
            && w073_guard_valid
            && no_proxy_guard_valid;
        let service_gate_classification_valid = w045_residual_valid
            && w044_stage2_valid
            && w045_lean_tla_valid
            && operated_service_blocker
            && retained_witness_blocker
            && pack_governance_blocker
            && !bool_at(&w044_stage2_promotion, "stage2_policy_promoted");

        let policy_rows = vec![
            json!({"row_id":"w045_stage2_w044_predecessor_policy_carried","policy_area":"w044_stage2_predecessor","registers":["production","scheduler"],"source_artifacts":[W044_STAGE2_RUN_SUMMARY_FOR_W045,W044_STAGE2_VALIDATION_FOR_W045,W044_STAGE2_POLICY_GATE_FOR_W045],"satisfied_for_declared_profile":w044_stage2_valid,"exact_remaining_blocker":false,"promotion_consequence":"W044 Stage 2 evidence is predecessor input only; W045 still requires direct production/service gates","failures": if w044_stage2_valid { Vec::<String>::new() } else { vec!["w045_w044_stage2_predecessor_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_bounded_replay_carried","policy_area":"bounded_baseline_vs_stage2_replay","registers":["production","scheduler"],"source_artifacts":[W044_STAGE2_RUN_SUMMARY_FOR_W045],"satisfied_for_declared_profile":bounded_replay_valid,"exact_remaining_blocker":false,"promotion_consequence":"bounded replay remains declared-profile evidence","failures": if bounded_replay_valid { Vec::<String>::new() } else { vec!["w045_bounded_replay_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_partition_order_permutation_carried","policy_area":"partition_order_permutation_replay","registers":["production","scheduler"],"source_artifacts":[W044_STAGE2_RUN_SUMMARY_FOR_W045],"satisfied_for_declared_profile":permutation_replay_valid,"exact_remaining_blocker":false,"promotion_consequence":"bounded permutation replay does not discharge unbounded fairness","failures": if permutation_replay_valid { Vec::<String>::new() } else { vec!["w045_partition_permutation_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_observable_invariance_carried","policy_area":"observable_result_invariance","registers":["production","scheduler","pack"],"source_artifacts":[W044_STAGE2_RUN_SUMMARY_FOR_W045,W044_STAGE2_PACK_GRADE_EQUIVALENCE_FOR_W045],"satisfied_for_declared_profile":observable_invariance_valid,"exact_remaining_blocker":false,"promotion_consequence":"observable invariance is evidenced for declared bounded profiles only","failures": if observable_invariance_valid { Vec::<String>::new() } else { vec!["w045_observable_invariance_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_w044_production_input_bundle_carried","policy_area":"w044_production_relevant_analyzer_inputs","registers":["production"],"source_artifacts":[W044_STAGE2_PRODUCTION_ANALYZER_FOR_W045],"satisfied_for_declared_profile":predecessor_production_inputs_valid,"exact_remaining_blocker":false,"promotion_consequence":"predecessor production inputs are carried without promoting full analyzer soundness","failures": if predecessor_production_inputs_valid { Vec::<String>::new() } else { vec!["w045_w044_production_input_bundle_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_declared_scheduler_equivalence_carried","policy_area":"declared_profile_scheduler_equivalence","registers":["scheduler"],"source_artifacts":[W044_STAGE2_SCHEDULER_EQUIVALENCE_FOR_W045,W045_TLA_MODEL_REGISTER],"satisfied_for_declared_profile":declared_scheduler_valid,"exact_remaining_blocker":false,"promotion_consequence":"declared-profile scheduler equivalence remains bounded and non-promoting","failures": if declared_scheduler_valid { Vec::<String>::new() } else { vec!["w045_declared_scheduler_equivalence_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_declared_pack_equivalence_carried","policy_area":"declared_profile_pack_equivalence","registers":["pack"],"source_artifacts":[W044_STAGE2_PACK_GRADE_EQUIVALENCE_FOR_W045,W045_RESIDUAL_SUMMARY],"satisfied_for_declared_profile":declared_pack_valid,"exact_remaining_blocker":false,"promotion_consequence":"declared-profile pack evidence does not promote pack-grade replay","failures": if declared_pack_valid { Vec::<String>::new() } else { vec!["w045_declared_pack_equivalence_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_w045_mixed_dynamic_direct_evidence","policy_area":"mixed_dynamic_add_remove_reclassify_transition","registers":["production"],"source_artifacts":[W045_OPTIMIZED_CORE_DYNAMIC_COVERAGE,W045_OPTIMIZED_CORE_SUMMARY],"satisfied_for_declared_profile":mixed_dynamic_valid,"exact_remaining_blocker":false,"promotion_consequence":"mixed dynamic transition evidence narrows Stage 2 inputs but leaves broader dynamic coverage blocked","failures": if mixed_dynamic_valid { Vec::<String>::new() } else { vec!["w045_mixed_dynamic_transition_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_w045_dynamic_refinement_bridge","policy_area":"rust_dynamic_transition_refinement_bridge","registers":["production","scheduler"],"source_artifacts":[W045_RUST_SUMMARY,W045_RUST_REFINEMENT_REGISTER],"satisfied_for_declared_profile":rust_refinement_valid,"exact_remaining_blocker":false,"promotion_consequence":"Rust dynamic refinement is input evidence only and leaves Rust totality/refinement unpromoted","failures": if rust_refinement_valid { Vec::<String>::new() } else { vec!["w045_dynamic_refinement_bridge_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_w045_publication_fence_refinement_bridge","policy_area":"publication_fence_no_publish_refinement_bridge","registers":["production","pack"],"source_artifacts":[W045_RUST_REFINEMENT_REGISTER],"satisfied_for_declared_profile":rust_refinement_valid,"exact_remaining_blocker":false,"promotion_consequence":"no-publication behavior remains exercised-path evidence only","failures": if rust_refinement_valid { Vec::<String>::new() } else { vec!["w045_publication_fence_refinement_bridge_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_w045_lean_tla_model_bound_bridge","policy_area":"lean_tla_stage2_bounded_model_input","registers":["production","scheduler"],"source_artifacts":[W045_LEAN_TLA_SUMMARY,W045_TLA_MODEL_REGISTER],"satisfied_for_declared_profile":w045_model_bound_valid,"exact_remaining_blocker":false,"promotion_consequence":"bounded model evidence remains non-promoting","failures": if w045_model_bound_valid { Vec::<String>::new() } else { vec!["w045_lean_tla_model_bound_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_w045_rust_dependency_classified","policy_area":"rust_totality_refinement_dependency_classification","registers":["production","service"],"source_artifacts":[W045_RUST_SUMMARY,W045_RUST_BLOCKERS,W045_LEAN_TLA_BLOCKERS],"satisfied_for_declared_profile":w045_rust_valid && rust_dependency_blocker,"exact_remaining_blocker":false,"promotion_consequence":"Rust dependency is classified and remains separately blocked for promotion","failures": if w045_rust_valid && rust_dependency_blocker { Vec::<String>::new() } else { vec!["w045_rust_dependency_classification_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_w073_typed_rule_guard_current","policy_area":"w073_typed_only_formatting_guard","registers":["production","pack"],"source_artifacts":[W045_RESIDUAL_SUMMARY],"satisfied_for_declared_profile":w073_guard_valid,"exact_remaining_blocker":false,"promotion_consequence":"W073 typed-rule guard is carried as current input; downstream request-construction uptake remains outside this Stage 2 packet","failures": if w073_guard_valid { Vec::<String>::new() } else { vec!["w045_w073_guard_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_no_proxy_promotion_guard","policy_area":"no_proxy_promotion_guard","registers":["production","scheduler","pack","service"],"source_artifacts":[W045_RESIDUAL_SUMMARY,W045_OPTIMIZED_CORE_MATCH_GUARD,W044_STAGE2_PROMOTION_DECISION_FOR_W045],"satisfied_for_declared_profile":no_proxy_guard_valid,"exact_remaining_blocker":false,"promotion_consequence":"proxy evidence cannot promote Stage 2 policy, service gates, or pack-grade replay","failures": if no_proxy_guard_valid { Vec::<String>::new() } else { vec!["w045_no_proxy_guard_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_semantic_equivalence_statement_bound","policy_area":"semantic_equivalence_statement","registers":["scheduler"],"source_artifacts":[W045_STAGE2_LEAN_FILE],"satisfied_for_declared_profile":lean_file_present && declared_scheduler_valid && no_proxy_guard_valid,"exact_remaining_blocker":false,"promotion_consequence":"observable results remain invariant because this packet changes evidence classification only and does not change runtime scheduler policy","failures": if lean_file_present && declared_scheduler_valid && no_proxy_guard_valid { Vec::<String>::new() } else { vec!["w045_semantic_equivalence_statement_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_service_gate_classifier_carried","policy_area":"service_gate_classification","registers":["service"],"source_artifacts":[W045_RESIDUAL_SUMMARY,W044_STAGE2_BLOCKERS_FOR_W045,W045_LEAN_TLA_BLOCKERS],"satisfied_for_declared_profile":service_gate_classification_valid,"exact_remaining_blocker":false,"promotion_consequence":"service gates are classified but operated service evidence remains successor scope","failures": if service_gate_classification_valid { Vec::<String>::new() } else { vec!["w045_service_gate_classifier_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_pack_governance_classifier_carried","policy_area":"pack_governance_classification","registers":["pack","service"],"source_artifacts":[W044_STAGE2_BLOCKERS_FOR_W045,W045_RESIDUAL_PROMOTION_CONTRACT_MAP],"satisfied_for_declared_profile":pack_governance_blocker && service_gate_classification_valid,"exact_remaining_blocker":false,"promotion_consequence":"pack governance classification is carried to W045.10 without pack-grade promotion","failures": if pack_governance_blocker && service_gate_classification_valid { Vec::<String>::new() } else { vec!["w045_pack_governance_classifier_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_successor_obligation_alignment","policy_area":"w045_successor_obligation_alignment","registers":["production","service"],"source_artifacts":[W045_RESIDUAL_OBLIGATION_MAP],"satisfied_for_declared_profile":w045_residual_valid,"exact_remaining_blocker":false,"promotion_consequence":"W045 Stage 2 obligations are aligned to calc-zkio.5 and successor pack/service beads","failures": if w045_residual_valid { Vec::<String>::new() } else { vec!["w045_successor_obligation_alignment_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_promotion_contract_no_promotion_alignment","policy_area":"w045_promotion_contract_no_promotion_alignment","registers":["service"],"source_artifacts":[W045_RESIDUAL_PROMOTION_CONTRACT_MAP],"satisfied_for_declared_profile":w045_residual_valid && no_proxy_guard_valid,"exact_remaining_blocker":false,"promotion_consequence":"promotion contracts remain no-promotion contracts until direct service and pack evidence exists","failures": if w045_residual_valid && no_proxy_guard_valid { Vec::<String>::new() } else { vec!["w045_promotion_contract_alignment_not_valid".to_string()] }}),
            json!({"row_id":"w045_stage2_broader_dynamic_transition_coverage_blocker","policy_area":"broader_dynamic_transition_coverage","registers":["production"],"source_artifacts":[W045_OPTIMIZED_CORE_BLOCKERS,W045_OPTIMIZED_CORE_DYNAMIC_COVERAGE],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"production partition-analyzer soundness remains blocked","failures": if broader_dynamic_blocker { Vec::<String>::new() } else { vec!["w045_broader_dynamic_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_soft_reference_indirect_resolution_blocker","policy_area":"soft_reference_indirect_late_resolution_breadth","registers":["production"],"source_artifacts":[W045_OPTIMIZED_CORE_BLOCKERS,W045_OPTIMIZED_CORE_DYNAMIC_COVERAGE],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"soft-reference and INDIRECT late-resolution breadth remains blocked","failures": if soft_reference_blocker { Vec::<String>::new() } else { vec!["w045_soft_reference_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_snapshot_fence_breadth_blocker","policy_area":"snapshot_fence_breadth","registers":["production"],"source_artifacts":[W045_OPTIMIZED_CORE_COUNTERPART_COVERAGE,W045_OPTIMIZED_CORE_BLOCKERS],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"Stage 2 production policy remains blocked until snapshot-fence breadth exists","failures": if snapshot_breadth_blocker { Vec::<String>::new() } else { vec!["w045_snapshot_breadth_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_capability_view_breadth_blocker","policy_area":"capability_view_breadth","registers":["production"],"source_artifacts":[W045_OPTIMIZED_CORE_COUNTERPART_COVERAGE,W045_OPTIMIZED_CORE_BLOCKERS],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"Stage 2 production policy remains blocked until capability-view breadth exists","failures": if capability_breadth_blocker { Vec::<String>::new() } else { vec!["w045_capability_breadth_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_full_production_partition_analyzer_soundness_blocker","policy_area":"full_production_partition_analyzer_soundness","registers":["production"],"source_artifacts":[W044_STAGE2_BLOCKERS_FOR_W045,W045_LEAN_TLA_BLOCKERS,W045_STAGE2_LEAN_FILE],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"Stage 2 production policy remains unpromoted","failures": if production_analyzer_blocker { Vec::<String>::new() } else { vec!["w045_production_analyzer_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_scheduler_fairness_unbounded_coverage_blocker","policy_area":"fairness_scheduler_unbounded_coverage","registers":["scheduler"],"source_artifacts":[W045_LEAN_TLA_BLOCKERS,W044_STAGE2_BLOCKERS_FOR_W045],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"full TLA verification and Stage 2 production policy remain unpromoted","failures": if fairness_blocker { Vec::<String>::new() } else { vec!["w045_fairness_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_rust_totality_refinement_dependency_blocker","policy_area":"rust_totality_refinement_dependency","registers":["production","service"],"source_artifacts":[W045_RUST_BLOCKERS,W045_LEAN_TLA_BLOCKERS],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"Stage 2 policy remains blocked until Rust totality/refinement dependency is discharged","failures": if rust_dependency_blocker { Vec::<String>::new() } else { vec!["w045_rust_dependency_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_operated_cross_engine_service_dependency_blocker","policy_area":"operated_cross_engine_stage2_differential_service","registers":["service"],"source_artifacts":[W044_STAGE2_BLOCKERS_FOR_W045,W045_RESIDUAL_OBLIGATION_MAP],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"operated Stage 2 differential evidence remains required before policy promotion","failures": if operated_service_blocker { Vec::<String>::new() } else { vec!["w045_operated_service_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_retained_witness_lifecycle_pack_dependency_blocker","policy_area":"retained_witness_lifecycle_pack_dependency","registers":["pack","service"],"source_artifacts":[W044_STAGE2_BLOCKERS_FOR_W045,W045_RESIDUAL_OBLIGATION_MAP],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"pack-grade replay remains blocked until retained-witness lifecycle evidence exists","failures": if retained_witness_blocker { Vec::<String>::new() } else { vec!["w045_retained_witness_blocker_missing".to_string()] }}),
            json!({"row_id":"w045_stage2_pack_grade_replay_governance_blocker","policy_area":"pack_grade_replay_governance","registers":["pack","service"],"source_artifacts":[W044_STAGE2_BLOCKERS_FOR_W045,W045_RESIDUAL_PROMOTION_CONTRACT_MAP],"satisfied_for_declared_profile":false,"exact_remaining_blocker":true,"promotion_consequence":"pack-grade replay, C5, and release-grade verification remain blocked","failures": if pack_governance_blocker { Vec::<String>::new() } else { vec!["w045_pack_governance_blocker_missing".to_string()] }}),
        ];
        ...
        let service_gate_rows = rows_for_register("service");
        ...
        if !w045_residual_valid {
            validation_failures.push("w045_residual_map_not_valid".to_string());
        }
        if !w044_stage2_valid {
            validation_failures.push("w044_stage2_predecessor_not_valid".to_string());
        }
        if !w045_optimized_valid || !mixed_dynamic_valid {
            validation_failures.push("w045_optimized_stage2_inputs_not_valid".to_string());
        }
        if !rust_refinement_valid {
            validation_failures.push("w045_rust_refinement_bridge_not_valid".to_string());
        }
        if !w045_model_bound_valid {
            validation_failures.push("w045_lean_tla_model_bound_not_valid".to_string());
        }
        if !declared_scheduler_valid {
            validation_failures.push("w045_declared_scheduler_equivalence_not_valid".to_string());
        }
        if !declared_pack_valid {
            validation_failures.push("w045_declared_pack_equivalence_not_valid".to_string());
        }
        if !w073_guard_valid {
            validation_failures.push("w045_w073_typed_rule_guard_not_valid".to_string());
        }
        if !service_gate_classification_valid {
            validation_failures.push("w045_service_gate_classification_not_valid".to_string());
        }
        if !no_proxy_guard_valid {
            validation_failures.push("w045_no_proxy_guard_not_valid".to_string());
        }
        if !lean_file_present {
            validation_failures.push("w045_stage2_lean_file_missing".to_string());
        }
        if policy_failed_row_count != 0 {
            validation_failures.push("w045_stage2_policy_row_failures_present".to_string());
        }
        if policy_row_count != 29 {
            validation_failures.push("w045_stage2_expected_twenty_nine_policy_rows".to_string());
        }
        if satisfied_policy_row_count != 19 {
            validation_failures.push("w045_stage2_expected_nineteen_satisfied_rows".to_string());
        }
        if exact_remaining_blocker_count != 10 {
            validation_failures.push("w045_stage2_expected_ten_exact_blockers".to_string());
        }

        let source_evidence_index_path =
            format!("{relative_artifact_root}/source_evidence_index.json");
        let policy_gate_register_path =
            format!("{relative_artifact_root}/w045_stage2_policy_gate_register.json");
        let production_analyzer_register_path =
            format!("{relative_artifact_root}/w045_production_partition_analyzer_register.json");
        let scheduler_equivalence_register_path =
            format!("{relative_artifact_root}/w045_scheduler_equivalence_register.json");
        let pack_grade_equivalence_register_path =
            format!("{relative_artifact_root}/w045_pack_grade_equivalence_register.json");
        let service_gate_register_path =
            format!("{relative_artifact_root}/w045_stage2_service_gate_register.json");
        let blocker_register_path =
            format!("{relative_artifact_root}/w045_stage2_exact_blocker_register.json");
        let promotion_decision_path = format!("{relative_artifact_root}/promotion_decision.json");
        let validation_path = format!("{relative_artifact_root}/validation.json");
        ...
        write_json(
            ...
            &json!({
                ...
                "source_artifacts": {
                    ...
                    "w045_residual_obligation_map": W045_RESIDUAL_OBLIGATION_MAP,
                    "w045_residual_promotion_contract_map": W045_RESIDUAL_PROMOTION_CONTRACT_MAP,
                    "w045_optimized_core_summary": W045_OPTIMIZED_CORE_SUMMARY,
                    "w045_dynamic_transition_coverage": W045_OPTIMIZED_CORE_DYNAMIC_COVERAGE,
                    "w045_counterpart_coverage": W045_OPTIMIZED_CORE_COUNTERPART_COVERAGE,
                    "w045_optimized_core_blockers": W045_OPTIMIZED_CORE_BLOCKERS,
                    "w045_match_promotion_guard": W045_OPTIMIZED_CORE_MATCH_GUARD,
                    ...
                    "w045_tla_model_register": W045_TLA_MODEL_REGISTER,
                    "w045_lean_tla_blockers": W045_LEAN_TLA_BLOCKERS,
                    "w044_stage2_summary": W044_STAGE2_RUN_SUMMARY_FOR_W045,
                    "w044_stage2_validation": W044_STAGE2_VALIDATION_FOR_W045,
                    "w044_stage2_policy_gate": W044_STAGE2_POLICY_GATE_FOR_W045,
                    "w044_stage2_production_analyzer": W044_STAGE2_PRODUCTION_ANALYZER_FOR_W045,
                    "w044_stage2_scheduler_equivalence": W044_STAGE2_SCHEDULER_EQUIVALENCE_FOR_W045,
                    "w044_stage2_pack_grade_equivalence": W044_STAGE2_PACK_GRADE_EQUIVALENCE_FOR_W045,
                    "w044_stage2_blockers": W044_STAGE2_BLOCKERS_FOR_W045,
                    "w044_stage2_promotion_decision": W044_STAGE2_PROMOTION_DECISION_FOR_W045,
                    "w045_stage2_lean_file": W045_STAGE2_LEAN_FILE
                },
                "source_counts": {
                    "w045_successor_obligation_count": number_at(&w045_residual_summary, "successor_obligation_count"),
                    "w045_promotion_contract_count": number_at(&w045_residual_summary, "promotion_contract_count"),
                    "w045_optimized_exact_remaining_blocker_count": number_at(&w045_optimized_summary, "w045_exact_remaining_blocker_count"),
                    "w045_rust_exact_remaining_blocker_count": number_at(&w045_rust_summary, "exact_remaining_blocker_count"),
                    "w045_lean_tla_exact_remaining_blocker_count": number_at(&w045_lean_tla_summary, "exact_remaining_blocker_count"),
                    "w044_stage2_policy_row_count": number_at(&w044_stage2_summary, "policy_row_count"),
                    "w044_stage2_exact_remaining_blocker_count": number_at(&w044_stage2_summary, "exact_remaining_blocker_count")
                }
            }),
        )?;
        write_json(
            &artifact_root.join("w045_stage2_policy_gate_register.json"),
            &json!({
                "schema_version": W045_POLICY_GATE_SCHEMA_V1,
                "run_id": run_id,
                ...
                "service_gate_promoted": false,
                "rows": policy_rows
            }),
        )?;
        write_json(
            &artifact_root.join("w045_production_partition_analyzer_register.json"),
            &json!({
                "schema_version": W045_PRODUCTION_PARTITION_ANALYZER_SCHEMA_V1,
                "run_id": run_id,
                ...
                "exact_remaining_blocker_count": production_analyzer_rows
                    ...
                    .filter(|row| bool_at(row, "exact_remaining_blocker"))
                    .count(),
                "semantic_statement": "W045 binds W044 Stage 2 declared-profile evidence, W045 optimized/core mixed dynamic evidence, W045 Rust refinement bridges, W045 Lean/TLA bounded model rows, the current W073 typed-rule-only input guard, and no-proxy promotion guards as production-relevant analyzer inputs. Full production partition-analyzer soundness remains blocked.",
                "rows": production_analyzer_rows
            }),
        )?;
        write_json(
            &artifact_root.join("w045_scheduler_equivalence_register.json"),
            &json!({
                "schema_version": W045_SCHEDULER_EQUIVALENCE_SCHEMA_V1,
                "run_id": run_id,
                ...
                "exact_remaining_blocker_count": scheduler_equivalence_rows
                    ...
                    .filter(|row| bool_at(row, "exact_remaining_blocker"))
                    .count(),
                "semantic_equivalence_statement": "Observable engine results are invariant under W045.5 because this packet adds classification evidence only and does not change runtime scheduler policy. Declared-profile scheduler equivalence remains bounded and does not discharge fairness or unbounded coverage.",
                "rows": scheduler_equivalence_rows
            }),
        )?;
        write_json(
            &artifact_root.join("w045_pack_grade_equivalence_register.json"),
            &json!({
                "schema_version": W045_PACK_GRADE_EQUIVALENCE_SCHEMA_V1,
                "run_id": run_id,
                ...
                "exact_remaining_blocker_count": pack_grade_equivalence_rows
                    ...
                    .filter(|row| bool_at(row, "exact_remaining_blocker"))
                    .count(),
                "semantic_equivalence_statement": "Declared-profile pack equivalence inputs are carried with W045 no-publication, typed-formatting, and no-proxy guards, but pack-grade replay remains blocked by retained-witness lifecycle and pack governance.",
                "rows": pack_grade_equivalence_rows
            }),
        )?;
        write_json(
            &artifact_root.join("w045_stage2_service_gate_register.json"),
            &json!({
                "schema_version": W045_SERVICE_GATE_SCHEMA_V1,
                "run_id": run_id,
                "row_count": service_gate_rows.len(),
                "satisfied_policy_row_count": service_gate_rows
                    .iter()
                    .filter(|row| bool_at(row, "satisfied_for_declared_profile"))
                    .count(),
                "exact_remaining_blocker_count": service_gate_rows
                    .iter()
                    .filter(|row| bool_at(row, "exact_remaining_blocker"))
                    .count(),
                "service_gate_classification_evidenced": service_gate_classification_valid,
                "service_gate_promoted": false,
                "semantic_statement": "W045.5 classifies service-grade Stage 2 and pack dependencies and carries their exact blockers forward; operated service evidence is successor scope, not a promotion input here.",
                "rows": service_gate_rows
            }),
        )?;
        write_json(
            &artifact_root.join("w045_stage2_exact_blocker_register.json"),
            &json!({
        ...
        write_json(
            ...
            &json!({
                ...
                "run_id": run_id,
                "decision_state": "w045_stage2_production_partition_pack_service_evidence_validated_policy_unpromoted",
                "stage2_policy_promoted": false,
                "stage2_promotion_candidate": false,
                "pack_grade_replay_promoted": false,
                "pack_grade_replay_candidate": false,
                "service_gate_promoted": false,
                "service_gate_candidate": false,
                ...
                "rust_refinement_bridge_evidenced": rust_refinement_valid,
                "lean_tla_model_bound_evidenced": w045_model_bound_valid,
                "service_gate_classification_evidenced": service_gate_classification_valid,
                "w073_typed_rule_only_direct_replacement_guard_carried": w073_guard_valid,
                ...
                "rust_totality_refinement_dependency_promoted": false,
                ...
                "blockers": [
                    ...
                    "stage2.soft_reference_indirect_late_resolution_breadth",
                    ...
                    "stage2.rust_totality_refinement_dependency",
        ...
        let validation_status = if validation_failures.is_empty() {
            "w045_stage2_pack_service_equivalence_valid"
        } else {
            "w045_stage2_pack_service_equivalence_invalid"
        };
        write_json(
            ...
            &json!({
                ...
                "policy_row_count": policy_row_count,
                "satisfied_policy_row_count": satisfied_policy_row_count,
                "partition_replay_row_count": number_at(&w044_stage2_summary, "partition_replay_row_count"),
                "permutation_replay_row_count": number_at(&w044_stage2_summary, "permutation_replay_row_count"),
                "nontrivial_permutation_row_count": number_at(&w044_stage2_summary, "nontrivial_permutation_row_count"),
                "observable_invariance_row_count": number_at(&w044_stage2_summary, "observable_invariance_row_count"),
                "formatting_watch_row_count": number_at(&w044_stage2_summary, "formatting_watch_row_count"),
                "mixed_dynamic_transition_evidenced": mixed_dynamic_valid,
                "rust_refinement_bridge_evidenced": rust_refinement_valid,
                "lean_tla_model_bound_evidenced": w045_model_bound_valid,
        ...
        write_json(
            ...
            &json!({
                ...
                "artifact_root": relative_artifact_root,
                "source_evidence_index_path": source_evidence_index_path,
                "w045_stage2_policy_gate_register_path": policy_gate_register_path,
                "w045_production_partition_analyzer_register_path": production_analyzer_register_path,
                "w045_scheduler_equivalence_register_path": scheduler_equivalence_register_path,
                "w045_pack_grade_equivalence_register_path": pack_grade_equivalence_register_path,
                "w045_stage2_service_gate_register_path": service_gate_register_path,
                "w045_stage2_exact_blocker_register_path": blocker_register_path,
                "promotion_decision_path": promotion_decision_path,
                "validation_path": validation_path,
                "policy_row_count": policy_row_count,
                "satisfied_policy_row_count": satisfied_policy_row_count,
                "production_partition_analyzer_row_count": production_analyzer_rows.len(),
                "scheduler_equivalence_row_count": scheduler_equivalence_rows.len(),
                "pack_grade_equivalence_row_count": pack_grade_equivalence_rows.len(),
                "service_gate_row_count": service_gate_rows.len(),
                "partition_replay_row_count": number_at(&w044_stage2_summary, "partition_replay_row_count"),
                "permutation_replay_row_count": number_at(&w044_stage2_summary, "permutation_replay_row_count"),
                "nontrivial_permutation_row_count": number_at(&w044_stage2_summary, "nontrivial_permutation_row_count"),
                "observable_invariance_row_count": number_at(&w044_stage2_summary, "observable_invariance_row_count"),
...
mod tests {
    ...
    #[test]
    fn stage2_replay_runner_writes_w045_service_equivalence_without_promotion() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let run_id = format!("test-w045-stage2-replay-{}", std::process::id());
        let artifact_root =
            repo_root.join(format!("docs/test-runs/core-engine/stage2-replay/{run_id}"));
        let cleanup = || {
            if artifact_root.exists() {
        ...
        assert_eq!(summary.observable_invariance_row_count, 5);
        assert_eq!(summary.formatting_watch_row_count, 1);
        assert_eq!(summary.exact_remaining_blocker_count, 10);
        assert_eq!(summary.failed_row_count, 0);
        assert!(!summary.stage2_policy_promoted);
        ...
        assert_eq!(validation["policy_row_count"], 29);
        assert_eq!(validation["satisfied_policy_row_count"], 19);
        assert_eq!(validation["mixed_dynamic_transition_evidenced"], true);
        assert_eq!(validation["rust_refinement_bridge_evidenced"], true);
        assert_eq!(validation["lean_tla_model_bound_evidenced"], true);
        assert_eq!(
        ...
        assert_eq!(validation["service_gate_classification_evidenced"], true);
        assert_eq!(
            validation["w073_typed_rule_only_direct_replacement_guard_carried"],
            true
        );
        assert_eq!(validation["stage2_policy_promoted"], false);
        assert_eq!(validation["pack_grade_replay_promoted"], false);
        assert_eq!(validation["service_gate_promoted"], false);
        ...
        assert_eq!(promotion["stage2_policy_promoted"], false);
        assert_eq!(promotion["pack_grade_replay_promoted"], false);
        assert_eq!(promotion["service_gate_promoted"], false);
        assert_eq!(
            promotion["production_relevant_analyzer_inputs_evidenced"],
            true
        );
        assert_eq!(promotion["service_gate_classification_evidenced"], true);
        assert_eq!(
            promotion["production_partition_analyzer_soundness_promoted"],
            false
        );
        ...
        let blocker_register = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/stage2-replay/{run_id}/w045_stage2_exact_blocker_register.json"
            ),
        )
        .unwrap();
        assert_eq!(blocker_register["exact_remaining_blocker_count"], 10);

        let service_gate = read_json(
            &repo_root,
            &format!(
                "docs/test-runs/core-engine/stage2-replay/{run_id}/w045_stage2_service_gate_register.json"
            ),
        )
        .unwrap();
        assert_eq!(service_gate["service_gate_promoted"], false);
        assert_eq!(service_gate["exact_remaining_blocker_count"], 4);
